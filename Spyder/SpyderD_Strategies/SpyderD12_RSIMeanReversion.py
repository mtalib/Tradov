...
class RSIMeanReversionStrategy(BaseStrategy):
    ...
    def _detect_divergence(self, prices: pd.Series, rsi: pd.Series) -> RSIDivergence | None:
    ...
    def _create_oversold_signal(self, market_data: pd.DataFrame,
                               divergence: RSIDivergence | None) -> TradingSignal | None:
    ...
    def add_position(self, signal: TradingSignal) -> str:
        ...
        position_id = f"RSI_{datetime.now(UTC).strftime('%Y%m%d_%H%M%S')}"
...
def test_rsi_mean_reversion():
    ...
    dates = pd.date_range(start=datetime.now(UTC).replace(hour=11, minute=0), periods=100, freq='5min')
