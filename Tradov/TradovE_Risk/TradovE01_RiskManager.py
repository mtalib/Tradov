...
PAIR_COMPOSITE_SEPARATOR = "/"

STRATEGY_BUCKET_DEFAULTS: dict[str, dict[str, float]] = {
    "pair_trading": {
        "max_exposure_fraction": 0.20,
        "max_daily_loss_fraction": 0.30,
    },
}


def _decompose_pair_symbol(symbol: str) -> list[str]:
    """Return constituent legs for a pair composite like 'AAPL/MSFT'.

    If *symbol* contains the pair separator, both legs are returned.
    Otherwise a single-element list ``[symbol]`` is returned.
    """
    if PAIR_COMPOSITE_SEPARATOR in symbol:
        parts = [p.strip() for p in symbol.split(PAIR_COMPOSITE_SEPARATOR) if p.strip()]
        return parts if len(parts) == 2 else [symbol]
    return [symbol]
...
class RiskManager:
    ...
    def __init__(
        ...
        # Per-strategy risk buckets — keyed by strategy_type (e.g. "pair_trading",
        # "directional_trade").  Each bucket tracks cumulative notional exposure
        # and daily P&L so that one strategy cannot silently consume the entire
        # risk budget.
        self._strategy_buckets: dict[str, dict[str, float]] = {}

        # Net-position map keyed by underlying symbol.  Values are dicts:
        #   {"quantity": float, "strategy_type": str}
        # Used by the cross-strategy conflict detector to spot opposing
        # positions on the same underlying from different strategies.
        self._net_positions_by_symbol: dict[str, dict[str, Any]] = {}
    ...
    def _on_position_updated(self, event: Any) -> None:
        ...
        with self._risk_lock:
            ...
            # --- Maintain net-position-by-symbol map for cross-strategy conflict detection ---
            strategy_type = str((data.get("metadata") or {}).get("strategy_type", ""))
            position_value = abs(int(qty)) * (float(fill_price) if fill_price else 0.0)
            for leg in _decompose_pair_symbol(symbol):
                if qty == 0:
                    self._net_positions_by_symbol.pop(leg, None)
                else:
                    self._net_positions_by_symbol[leg] = {
                        "quantity": int(qty),
                        "strategy_type": strategy_type,
                    }

            # --- Maintain per-strategy risk bucket ---
            if strategy_type:
                bucket = self._strategy_buckets.get(strategy_type, {"exposure": 0.0, "daily_pnl": 0.0})
                if qty == 0:
                    pass
                else:
                    bucket["exposure"] = bucket.get("exposure", 0.0) + position_value
                self._strategy_buckets[strategy_type] = bucket
    ...
    def validate_signal(self, request: RiskValidationRequest) -> RiskValidationResult:
        ...
        try:
            with self._risk_lock:
                ...
                # --- Per-strategy risk bucket ---
                # Each strategy_type has a configurable fraction of the global
                # exposure / daily-loss limits.  This prevents one strategy from
                # silently consuming the entire risk budget.
                bucket_cfg = STRATEGY_BUCKET_DEFAULTS.get(strategy_type)
                if bucket_cfg:
                    bucket = self._strategy_buckets.get(strategy_type, {"exposure": 0.0, "daily_pnl": 0.0})
                    max_exp_frac = bucket_cfg.get("max_exposure_fraction", 1.0)
                    max_dl_frac = bucket_cfg.get("max_daily_loss_fraction", 1.0)
                    global_max_exp = self.config.risk_limits["max_total_exposure"]
                    global_max_dl = self.config.risk_limits["max_daily_loss"]
                    bucket_max_exp = global_max_exp * max_exp_frac
                    bucket_max_dl = global_max_dl * max_dl_frac

                    new_bucket_exp = bucket["exposure"] + order_value
                    if new_bucket_exp > bucket_max_exp:
                        return RiskValidationResult(
                            approved=False,
                            rejection_reason=(
                                f"Strategy bucket '{strategy_type}' exposure "
                                f"{new_bucket_exp:.0f} exceeds per-strategy limit "
                                f"{bucket_max_exp:.0f} ({max_exp_frac:.0%} of global)"
                            ),
                            risk_score=1.0,
                            violations=["STRATEGY_BUCKET_EXPOSURE"],
                        )

                    if bucket["daily_pnl"] < -bucket_max_dl:
                        return RiskValidationResult(
                            approved=False,
                            rejection_reason=(
                                f"Strategy bucket '{strategy_type}' daily loss "
                                f"{bucket['daily_pnl']:.0f} exceeds per-strategy limit "
                                f"{bucket_max_dl:.0f} ({max_dl_frac:.0%} of global)"
                            ),
                            risk_score=1.0,
                            violations=["STRATEGY_BUCKET_DAILY_LOSS"],
                        )

                # --- Cross-strategy conflict detection ---
                # Decompose pair composite symbols (e.g. "AAPL/MSFT") into their
                # legs and check whether another strategy already holds an
                # opposing position on any constituent.
                _conflict_violations: list[str] = []
                legs = _decompose_pair_symbol(symbol)
                signal_direction = 1 if qty > 0 else -1
                for leg in legs:
                    existing = self._net_positions_by_symbol.get(leg)
                    if existing and existing["strategy_type"] != strategy_type:
                        net_dir = 1 if existing["quantity"] > 0 else -1
                        if net_dir != 0 and signal_direction != 0 and net_dir != signal_direction:
                            self.logger.warning(
                                "Cross-strategy conflict: strategy '%s' wants %s %d %s "
                                "but strategy '%s' already holds net %d — flagging",
                                strategy_type, "BUY" if qty > 0 else "SELL", abs(qty), leg,
                                existing["strategy_type"], existing["quantity"],
                            )
                            _conflict_violations.append(
                                f"CROSS_STRATEGY_CONFLICT:{leg}:{strategy_type}:vs:{existing['strategy_type']}"
                            )
                ...
                # --- Total exposure ---
                ref_price = ref_price or (current_pos.market_price if current_pos else 0.0)
                order_value = qty * ref_price
                ...
                violations: list[str] = list(_conflict_violations)
