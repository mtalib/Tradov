...
from SpyderI_Integration.SpyderI04_DiagnosticsEngine_Types import (
    DiagnosticReport, DiagnosticIssue, HealthStatus, ProblemSeverity
)
from SpyderI_Integration.SpyderI04_DiagnosticsEngine_HealthChecks import HealthCheckManager
from SpyderI_Integration.SpyderI04_DiagnosticsEngine_DataCollector import DataCollector
from SpyderI_Integration.SpyderI04_DiagnosticsEngine_Analyzers import AnalysisManager
from SpyderI_Integration.SpyderI04_DiagnosticsEngine_Utils import DiagnosticUtils
    ...
    def get_health_summary(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if HUB_AVAILABLE:
                ...
                healthy_modules = len([
                    m for m in self.data_collector.collect_module_health() 
            ...
            return {
                ...
                'modules': {
                    ...
                    'health_rate': f"{(healthy_modules/module_count*100):.1f}%" if module_count > 0 else "0%"
                },
                'issues': {
    ...
    def _performance_monitoring_loop(self) -> None:
        ...
        while self.is_monitoring:
            try:
                ...
                performance_issues = self.health_check_manager.check_performance_health()
    ...
    def _find_similar_issue(self, issue: DiagnosticIssue) -> Optional[DiagnosticIssue]:
        ...
        for existing_issue in self.active_issues.values():
            if (existing_issue.category == issue.category and
                ...
                set(existing_issue.affected_components) == set(issue.affected_components)):
...
def get_diagnostics_engine(config: Optional[Dict[str, Any]] = None) -> DiagnosticsEngine:
    """
