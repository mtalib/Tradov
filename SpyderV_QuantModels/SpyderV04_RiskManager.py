...
class RiskParameters:
    ...
    def validate(self) -> bool:
        """Validate parameters."""
        return (0.5 <= self.confidence_level <= 0.999 and
                self.time_horizon > 0 and
...
class SpyderRiskManager:
    ...
    async def calculate_portfolio_risk(self,
                                     ...
                                     parameters: RiskParameters = None) -> RiskMetrics:
        """
        ...
        try:
            ...
            risk_metrics = RiskMetrics(
                ...
                portfolio_value=sum(pos.get('market_value', 0) for pos in self.positions.values()),
                ...
                risk_utilization=var_result['var'] / (sum(pos.get('market_value', 0) for pos in self.positions.values()) * self.risk_limits['max_var_95']),
            ...
            self.error_counts['portfolio_risk'] = self.error_counts.get('portfolio_risk', 0) + 1
            raise
    ...
    async def _historical_var(self, params: RiskParameters) -> Dict[str, float]:
        ...
        if len(self.portfolio_returns) < params.min_observations:
            raise ValueError(f"Insufficient data: {len(self.portfolio_returns)} < {params.min_observations}")
    ...
    async def _monte_carlo_var(self, params: RiskParameters) -> Dict[str, float]:
        ...
        if abs(return_skewness) > 0.5 or abs(return_kurtosis) > 1.0:
            ...
            # Standard normal
            mc_returns = np.random.normal(mean_return, std_return, params.monte_carlo_sims)
    ...
    async def _calculate_cvar(self, params: RiskParameters, var_result: Dict[str, float]) -> Dict[str, float]:
        ...
        if len(tail_returns) == 0:
            ...
            tail_mean = expected_shortfall
            tail_volatility = np.std(tail_returns, ddof=1) if len(tail_returns) > 1 else 0
    ...
    async def _calculate_additional_metrics(self, params: RiskParameters) -> Dict[str, float]:
        """Calculate additional risk metrics."""
        if len(self.portfolio_returns) < params.min_observations:
        ...
        downside_deviation = np.std(negative_returns, ddof=1) if len(negative_returns) > 1 else std_return
        ...
        position_values = [pos.get('market_value', 0) for pos in self.positions.values()]
    ...
    async def run_stress_tests(self, 
                             custom_scenarios: List[StressTestScenario] = None) -> List[StressTestResult]:
        """
    ...
    async def _run_single_stress_test(self, scenario: StressTestScenario) -> StressTestResult:
        ...
        for pos_id, position in self.positions.items():
            ...
            if abs(pos_impact) > position.get('market_value', 0) * 0.05:  # 5% impact threshold
        ...
        hedge_recommendations = self._generate_hedge_recommendations(scenario, position_impacts)
    ...
    def _calculate_position_stress_impact(self, position: Dict[str, Any], scenario: StressTestScenario) -> float:
        ...
        if pos_type == 'option':
            ...
            underlying_impact = (delta * scenario.underlying_shock + 
                               0.5 * gamma * scenario.underlying_shock**2) * market_value
    ...
    def _generate_hedge_recommendations(self, scenario: StressTestScenario, impacts: Dict[str, float]) -> List[str]:
        ...
        if abs(total_loss) > sum(pos.get('market_value', 0) for pos in self.positions.values()) * 0.10:
            recommendations.append("Consider portfolio-level hedging with SPY puts")
        
        if scenario.volatility_shock > 2.5:
            recommendations.append("Reduce vega exposure or implement volatility hedges")
        
        if scenario.correlation_shock > 0.9:
            recommendations.append("Increase diversification across uncorrelated strategies")
    ...
    def _estimate_recovery_time(self, scenario: StressTestScenario, loss_percentage: float) -> int:
    ...
    async def _validate_risk_model(self, params: RiskParameters) -> Dict[str, float]:
        ...
        if expected_breaches > 0 and breaches > 0:
            ...
            lr_stat = 2 * (breaches * np.log(breach_rate / (1 - params.confidence_level)) +
                          (len(returns) - breaches) * np.log((1 - breach_rate) / params.confidence_level))
    ...
    def export_risk_report(self, format_type: str = 'json') -> Union[str, Dict[str, Any]]:
        ...
        report_data = {
            ...
            'model_validation': {
                method.value: result.__dict__ if hasattr(result, '__dict__') else str(result)
...
def create_risk_manager(config: Dict[str, Any] = None, 
                       data_manager: MultiClientDataManager = None) -> SpyderRiskManager:
...
async def main():
    ...
    print(f"   Default confidence level: {risk_manager.default_params.confidence_level}")
    ...
    print(f"   Total Value: ${sum(pos['market_value'] for pos in sample_portfolio):,.2f}")
    ...
    methods_to_test = [RiskMethod.HISTORICAL, RiskMethod.PARAMETRIC, RiskMethod.MONTE_CARLO]
    ...
    for method in methods_to_test:
        try:
            ...
            print(f"   {method.value.title():<12}: VaR=${metrics.var:>8,.0f}  CVaR=${metrics.cvar:>8,.0f}")
        except Exception as e:
    ...
    try:
        ...
        for result in stress_results[:5]:  # Show first 5
            print(f"   {result.scenario.name:<20} "
        ...
        print(f"      Loss: ${worst_scenario.portfolio_loss:,.2f} ({worst_scenario.loss_percentage:.1%})")
        print(f"      Recovery Estimate: {worst_scenario.recovery_time_estimate} days")
        print(f"      Recommendations: {', '.join(worst_scenario.hedge_recommendations[:2])}")
