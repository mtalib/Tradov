...
from SpyderI_Integration.SpyderI06_AgentMessageBus import AgentMessageBus, Message, MessagePriority
...
class MultiStrategyAllocator:
    ...
    def _risk_parity_allocation(self, strategies: List[str]) -> AllocationResult:
        ...
        if result.success:
            allocations = dict(zip(strategies, result.x))
        else:
...
# FACTORY FUNCTION
# ==============================================================================
def create_multi_strategy_allocator(config: Optional[Dict[str, Any]] = None) -> MultiStrategyAllocator:
...
if __name__ == "__main__":
    ...
    print("\n" + "="*60)
    print("MULTI-STRATEGY ALLOCATOR TEST")
    print("="*60)
    ...
    for method in methods:
        ...
        print(f"  Active Strategies: {result.effective_strategies}")
        
        if result.allocations:
            print("  Allocations:")
            for strategy, weight in sorted(result.allocations.items(), 
    ...
    print(f"Rebalance completed at {event.timestamp}")
    print(f"Trades required: {len(event.trades_required)}")
    print(f"Expected impact: {event.expected_impact:.3%}")
    
    # Get portfolio state
    state = allocator.get_portfolio_state()
    print(f"\nPortfolio State:")
    print(f"  Total Capital: ${state.total_capital:,.0f}")
    print(f"  Allocated: ${state.allocated_capital:,.0f}")
    print(f"  Free: ${state.free_capital:,.0f}")
    print(f"  Active Strategies: {len(state.active_strategies)}")
    print(f"  Portfolio Sharpe: {state.portfolio_sharpe:.3f}")
    
    # Get full report
    report = allocator.get_allocation_report()
    print(f"\nAllocation Report:")
    print(f"  Total Strategies: {report['metrics']['total_strategies']}")
    print(f"  Active Strategies: {report['metrics']['active_strategies']}")
    print(f"  Portfolio Return: {report['metrics']['portfolio_return']:.2%}")
    print(f"  Portfolio Volatility: {report['metrics']['portfolio_volatility']:.2%}")
