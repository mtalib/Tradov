...
class PaperTradingQtWorker(QObject):
    ...
    def __init__(self, initial_capital: float = 100_000.0):
        ...
        #   SPYDER_LOSS_COOLDOWN_COUNT   (default 3)  0 disables feature
        #   SPYDER_LOSS_COOLDOWN_SECONDS (default 1800 = 30 min)
        ...
        # ``SPYDER_KELLY_SIZING=1``. Capped at KELLY_HISTORY_MAX entries to
    ...
    def set_risk_manager(self, risk_manager) -> None:
        """Inject a SpyderE01_RiskManager instance for pre-trade validation.
    ...
    def _regime_preferred_direction(self) -> str | None:
        ...
        Heuristic (only fires when ``SPYDER_REGIME_STRUCTURE=1``):
        ...
        """
        if os.environ.get("SPYDER_REGIME_STRUCTURE", "1") != "1":
            return None
    ...
    def _pivot_preferred_direction(self, spy_price: float) -> str | None:
        ...
        Gated by ``SPYDER_PIVOT_MR_ENABLED=1`` so the producer is opt-in for
        the first paper-trading sessions. Defaults off for safety.
        """
        if os.environ.get("SPYDER_PIVOT_MR_ENABLED", "0") != "1":
            return None
    ...
    def _emit_pivot_signal_state(self, spy_price: float) -> None:
        ...
        """
        enabled = os.environ.get("SPYDER_PIVOT_MR_ENABLED", "0") == "1"
        payload: dict = {
            "enabled": enabled,
    ...
    def _validate_with_risk_manager(
        ...
        """Route a proposed trade through SpyderE01_RiskManager.validate_signal.
        ...
        try:
            ...
            request = RiskValidationRequest(
                ...
                confidence=0.5,
                metadata={"source": "SpyderR08", "instrument": "stock"},
            )
    ...
    def _poll_and_trade(self):
        ...
        _dec: dict = {
            ...
            "bid": round(bid, 4),
            "ask": round(ask, 4),
            "options_mode": os.environ.get("SPYDER_OPTIONS_LIVE_PAPER", "0") == "1",
            ...
            "dix": self._regime_scalar(self._regime_metrics.get("DIX")),
            "gex": self._regime_scalar(self._regime_metrics.get("GEX")),
            "s08_enabled": os.environ.get("SPYDER_PIVOT_MR_ENABLED", "0") == "1",
            "s08_score": None,
            "s08_fired": False,
            "s08_direction": None,
            "action": "NO_HISTORY",
        ...
        if len(self._price_history) >= self.LONG_MA_WINDOW:
            ...
            if signal == "BUY" and self._position_qty == 0:
                ...
                if (
                    signal is None
                    and os.environ.get("SPYDER_OPTIONS_LIVE_PAPER", "0") == "1"
                    ...
                    if not regime_ok:
                        ...
                        # neutral-path producer. Only fires when SPYDER_PIVOT_MR_ENABLED=1
                        ...
                        if pivot_bias is not None:
                            ...
                            if bias == "bullish":
                                ...
                                os.environ.get("SPYDER_IRON_CONDOR_ENABLED", "0")
                                == "1"
                            ):
    ...
    def _greeks_gate_allows_entry(
        ...
        ``SPYDER_GREEKS_GATE_DISABLED=1``.
        """
        if os.environ.get("SPYDER_GREEKS_GATE_DISABLED", "0") == "1":
            return True, ""
    ...
    def _iv_gate_allows_entry(self, expiration: str, spy_price: float) -> tuple[bool, str]:
        ...
        Controlled by env:
          - ``SPYDER_IV_GATE_DISABLED=1`` → always allow (useful in tests)
          - ``SPYDER_MIN_IV_ABS`` (float, default 0.12) → absolute IV floor
          - ``SPYDER_MIN_IV_RANK`` (float, default 30.0) → percentile floor once
            we have enough samples

        Returns (allowed, reason). Never raises.
        """
        if os.environ.get("SPYDER_IV_GATE_DISABLED", "0") == "1":
            return True, ""
        ...
        try:
            min_iv_abs = float(os.environ.get("SPYDER_MIN_IV_ABS", "0.12"))
        except ValueError:
            min_iv_abs = 0.12
        try:
            min_iv_rank = float(os.environ.get("SPYDER_MIN_IV_RANK", "30.0"))
        except ValueError:
            min_iv_rank = 30.0
    ...
    def _select_wing_width(self) -> float:
        ...
        Default: ``SPYDER_SPREAD_WING_WIDTH`` (env, default 5.0). When
        ``SPYDER_DYNAMIC_WING=1`` and IV rank is available, the wing scales
        ...
        """
        try:
            base = float(os.environ.get("SPYDER_SPREAD_WING_WIDTH", "5"))
        except (TypeError, ValueError):
            base = 5.0
        if os.environ.get("SPYDER_DYNAMIC_WING", "0") != "1":
            return base
        rank = self._current_iv_rank()
        if rank is None:
            return base
    ...
    def _select_credit_spread(
        ...
        *direction* chooses the structure:
        ...
        # When SPYDER_TARGET_SHORT_DELTA is set (e.g., 0.20), pick the short
        ...
        target_delta_env = os.environ.get("SPYDER_TARGET_SHORT_DELTA", "")
    ...
    def _kelly_fraction(self) -> float | None:
        ...
        The raw Kelly is multiplied by ``SPYDER_KELLY_FRACTION`` (default
        0.25 = quarter-Kelly) for safety margin against estimation error,
        then clamped to ``[0, SPYDER_KELLY_MAX]`` (default 0.05 = 5% of
        capital per trade).
        """
        ...
        try:
            scaling = float(os.environ.get("SPYDER_KELLY_FRACTION", "0.25"))
        except (TypeError, ValueError):
            scaling = 0.25
        try:
            cap = float(os.environ.get("SPYDER_KELLY_MAX", "0.05"))
        except (TypeError, ValueError):
            cap = 0.05
    ...
    def _size_spread_qty(self, spread: dict) -> int:
        ...
        When ``SPYDER_KELLY_SIZING=1`` and enough trade history exists
        ...
        if os.environ.get("SPYDER_KELLY_SIZING", "0") == "1":
    ...
    def _spread_commission(self, qty: int) -> float:
        ...
        try:
            per_contract = float(
                os.environ.get(
                    "SPYDER_COMMISSION_PER_CONTRACT",
    ...
    def _close_paper_credit_spread(
        ...
        if realized > 0:
            ...
            try:
                threshold = int(
                    os.environ.get("SPYDER_LOSS_COOLDOWN_COUNT", "3")
                )
                cooldown_secs = float(
                    os.environ.get("SPYDER_LOSS_COOLDOWN_SECONDS", "1800")
                )
            except (TypeError, ValueError):
    ...
    def _mark_spreads_mtm(self) -> None:
        ...
        Exit rules (first to trigger wins):
          ...
          2. Stop loss when current debit >= ``SPYDER_SPREAD_STOP_LOSS_MULT``
             times the received credit (default 2.0 = -1x credit realized).
             Disable by setting the env var to 0.
          3. Defensive roll: when ``SPYDER_SPREAD_ROLL_ENABLED=1`` and the
        ...
        try:
            stop_mult = float(
                os.environ.get("SPYDER_SPREAD_STOP_LOSS_MULT", "2.0")
            )
        except (TypeError, ValueError):
            stop_mult = 2.0
        roll_enabled = (
            os.environ.get("SPYDER_SPREAD_ROLL_ENABLED", "0") == "1"
        )
        try:
            roll_dte = int(os.environ.get("SPYDER_SPREAD_ROLL_DTE", "2"))
        except (TypeError, ValueError):
            roll_dte = 2
        from datetime import date as _date
        today = _date.today()
    ...
    def _shadow_log_credit_spread(self, spy_price: float) -> None:
        ...
        Controlled by ``SPYDER_OPTIONS_SHADOW=1`` (default off). Disabled
        automatically when ``SPYDER_OPTIONS_LIVE_PAPER=1`` (redundant then).
        """
        if os.environ.get("SPYDER_OPTIONS_SHADOW", "0") != "1":
            return
        if os.environ.get("SPYDER_OPTIONS_LIVE_PAPER", "0") == "1":
            return
