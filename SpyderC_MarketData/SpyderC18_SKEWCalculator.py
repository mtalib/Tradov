...
Spyder Version: 1.0
Module: SpyderC18_SKEWCalculator.py
Group: C (Market Data)
...
class SpyderC18_SKEWCalculator:
    """
    ...
    def _create_option_data(self, row: pd.Series, option_type: str, expiry: datetime) -> Optional[OptionData]:
    ...
    def _is_valid_option(self, option: OptionData) -> bool:
        ...
        if option.option_type == 'put' and option.moneyness < 1.0:
            if abs(option.delta) < self.config['delta_cutoff']:
                return False
        elif option.option_type == 'call' and option.moneyness > 1.0:
            if abs(option.delta) < self.config['delta_cutoff']:
                return False
    ...
    def _calculate_skew_components(self, options: List[OptionData], dte: float) -> Optional[SKEWComponents]:
        """
    ...
    def _fetch_option_chain(self) -> Optional[Dict[str, pd.DataFrame]]:
    ...
    def _get_cached_calculation(self) -> Optional[SKEWCalculation]:
    ...
    def get_current_skew(self) -> Optional[float]:
        """Get current SKEW value"""
        return self.current_skew
    
    def get_last_calculation(self) -> Optional[SKEWCalculation]:
        """Get last calculation details"""
        return self.last_calculation
    
    def get_components(self) -> Optional[SKEWComponents]:
...
def create_skew_calculator(config: Optional[Dict[str, Any]] = None) -> SpyderC18_SKEWCalculator:
    """
    ...
    Returns:
        SpyderC18_SKEWCalculator instance
    """
    return SpyderC18_SKEWCalculator(config)

# Singleton instance
_module_instance = None

def get_skew_calculator() -> SpyderC18_SKEWCalculator:
...
if __name__ == "__main__":
    ...
    print("="*60)
    print("SPYDER C18 - SKEW CALCULATOR TEST")
    print("="*60)
    ...
    if calculation:
        print(f"\n✅ SKEW Calculation Successful!")
        print(f"  SKEW Index: {calculation.skew_index:.2f}")
        print(f"  Spot Price: ${calculation.spot_price:.2f}")
        print(f"  Confidence: {calculation.confidence:.2%}")
        print(f"  Strikes Used: {calculation.strikes_used}")
        print(f"  Calculation Time: {calculation.calculation_time:.1f}ms")
        ...
        if stats:
            ...
            print(f"  Percentile: {stats.get('percentile', 50):.0f}%")
        ...
        if components:
            print(f"\n🔍 SKEW Components:")
            print(f"  Forward Price: ${components.forward:.2f}")
            print(f"  ATM Volatility: {components.atm_volatility:.2%}")
            print(f"  Risk-Neutral Skew: {components.risk_neutral_skew:.4f}")
            print(f"  Risk-Neutral Kurtosis: {components.risk_neutral_kurtosis:.4f}")
            print(f"  Put Wing Points: {len(components.put_wing)}")
            print(f"  Call Wing Points: {len(components.call_wing)}")
        
        # Performance metrics
        metrics = calculator.get_performance_metrics()
        print(f"\n⚡ Performance Metrics:")
        print(f"  Calculations: {metrics.get('calculations', 0)}")
        print(f"  Cache Hits: {metrics.get('cache_hits', 0)}")
        print(f"  Avg Calc Time: {metrics.get('avg_calc_time', 0):.1f}ms")
        
    else:
        print("\n❌ SKEW calculation failed")
        print("  This might be due to market being closed or data unavailable")
    
    print("\n✅ SKEW Calculator test completed!")
