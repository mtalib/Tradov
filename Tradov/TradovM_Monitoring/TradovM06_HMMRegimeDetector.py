...
Tradov Version: 1.0
Module: TradovM06_HMMRegimeDetector.py
...
Description:
    ...
    regimes in TRAD options trading. It identifies three distinct market states:
...
# avoid loading PyTorch (via TradovL13_LSTMPricer) at module-import time.
...
# ==============================================================================
try:
    from Tradov.TradovU_Utilities.TradovU01_Logger import TradovLogger as _TradovLogger
    logger = _TradovLogger.get_logger(__name__)
except ImportError:
...
class TradovM06_HMMRegimeDetector:
    """
    Hidden Markov Model for Market Regime Detection.

    This class implements a sophisticated HMM-based regime detection system
    specifically optimized for TRAD options trading. It identifies market
    ...
    def _should_retrain(self) -> bool:
        ...
        if last_retrain is not None:
            hours_since_retrain = (datetime.now(UTC) - last_retrain).total_seconds() / 3600
    ...
    def _map_state_to_regime(self, state: int, features: np.ndarray) -> MarketRegime:
        ...
        try:
            ...
            np.mean(features[:, mom_idx]) if features.shape[1] > mom_idx else 0
        ...
        if cls._L09_TO_M06_REGIME_MAP is None:
            if L09_AVAILABLE is None:
                ...
                try:
                    ...
                    try:
                        from TradovL_ML.TradovL09_UnifiedRegimeEngine import (
                            MarketRegime as L09MarketRegime,
                        )
                        L09_AVAILABLE = True
                    except ImportError:
...
def create_hmm_detector(config: dict[str, Any] | None = None) -> TradovM06_HMMRegimeDetector:
    """
    ...
    Returns:
        TradovM06_HMMRegimeDetector instance
    """
    return TradovM06_HMMRegimeDetector(config)

# Singleton instance
_module_instance = None

def get_module_instance() -> TradovM06_HMMRegimeDetector:
...
if __name__ == "__main__":
    ...
    dates = pd.date_range(end=datetime.now(UTC), periods=500, freq='D')
    np.random.seed(42)
