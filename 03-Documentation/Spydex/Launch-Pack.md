...
This document captures the minimum practical setup for spinning up Spydex on a separate machine after Tradov has cleared the clone-readiness gate.
...
2. Install the same base dependencies used by Tradov.
3. Keep the host OS, Python environment, and application paths isolated from the Tradov machine.
...
2. Rotate and store the new credentials separately from Tradov.
...
3. Do not share runtime state between Tradov and Spydex.
...
2. Confirm the dashboard loads without sharing Tradov state.
...
1. Start from a known-good Tradov baseline commit.
...
If Tradov is unstable again, stop the fork work and return to stabilization first. Spydex should only move forward when the baseline is boring and reliable.