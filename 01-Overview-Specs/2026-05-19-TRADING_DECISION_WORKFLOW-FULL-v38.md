# Trading Decision Workflow (Full) — v38
...
Source note: this saved `v38` snapshot records the first post-v37 overlay implementation slice plus a read-only post-open runtime audit. Current code no longer matches the fully planned-only posture captured in v37: E00/E01 now expose a typed `OverlayPretradeVerdict` / `validate_overlay_slot()` API, and D31 now attaches overlay metadata plus a narrow signal-path gate for baseline-full PMR candidates. The baseline two-slot admission model remains authoritative: no live third-slot allocation/admission exception exists yet, the existing D31 warning still reflects that the full overlay contract is incomplete, and overlay audit-schema/runtime-disable/paper-verification work is still outstanding. The post-open runtime audit for today's active GUI run found no armed paper session, no `2026-05-19` decision log, and no paper DB activity, so there was no live D31 session evidence to audit beyond confirming dashboard-only operation.
...
- **No live third-slot overlay admission exists in current code.** E00/E01 now expose a typed overlay verdict API and D31 now performs a narrow overlay pre-dispatch check for baseline-full PMR candidates, but D31 still enforces the baseline two-slot cap and does not yet implement third-slot allocation/admission.
...
- The optional ODTE Pivot third-slot concept is only partially implemented. In the current code, D31 still enforces the baseline two-slot model even if an operator sets the documented overlay flag; the landed overlay slice adds only a narrow D31/E01 signal-path gate, not a third-slot admission exception.
...
>
> **Concurrency limit:** Max 2 strategies open (one long-term/swing + one intraday/0DTE). v38 adds a narrow E00/E01/D31 overlay gate on the signal path, but no live third-slot allocation/admission exception exists in the current code path yet.
...
As-implemented status (v38 audit): current code partially honors this contract. E00/E01 now expose a typed `OverlayPretradeVerdict` / `validate_overlay_slot()` API, flat overlay risk-limit keys exist in E01, and D31 now normalizes PMR aliases plus calls the overlay gate on the narrow `_on_strategy_signal()` path when the flag is on, the candidate normalizes to `pivot_mean_reversion`, and the baseline cap is already full. However, D31 still enforces `MAX_CONCURRENT_STRATEGIES = 2` and `MAX_ACTIVE_HORIZON_BUCKETS = 2` as the authoritative allocation contract, the third-slot exception is not implemented in `add_strategy()`, and the overlay audit-schema/runtime-disable/test-migration work remains incomplete. The items below are retained as design intent beyond the landed signal-path slice.
...
- Design-target overlay state: up to 3 active strategies only when the 3rd is an ODTE Pivot overlay entry that passes Section 6.2 gates. v38 only lands the narrow pre-dispatch overlay risk gate; full third-slot allocation/admission is still not implemented.
...
9. Enforce baseline max 2 concurrent strategies (one long-term/swing + one intraday/0DTE, enforced per horizon bucket). v38 adds only a narrow overlay pre-dispatch risk gate; the ODTE Pivot third-slot allocation/admission design in Section 6.2 is not currently active.
...
- ODTE Pivot third-slot logic is only partially implemented. If `SPYDER_ENABLE_ODTE_PIVOT_OVERLAY_SLOT` is set, the current code must still be treated as baseline two-slot allocation only; v38 adds a narrow D31/E01 signal-path gate, but the live third-slot admission path is not implemented and the D31 warning remains conservative.
...
Verification boundary (v38): read/validated audit of D31, E00, E01, focused tests, and a read-only runtime check found a partial overlay implementation: E00/E01 now expose a typed `OverlayPretradeVerdict` / `validate_overlay_slot()` API, flat overlay risk-limit keys exist in E01, and D31 now adds overlay metadata plus a narrow `_on_strategy_signal()` call when the flag is on and a baseline-full PMR candidate is evaluated. The third-slot allocation exception in `add_strategy()`, expanded decision-audit schema, runtime disable loop, scaffold migration, and paper-session verification are still outstanding.
...
### 9.4) Implementation Status and Sign-Off Matrix (v34-v38)
...
>
> **Current audit status (v38):** Overlay work is no longer fully **Planned**. E00/E01 overlay verdict surfaces and the narrow D31 signal-path gate are landed and focused-validated, but the authoritative third-slot allocation exception, expanded audit schema, runtime disable loop, scaffold migration, and end-to-end paper verification remain incomplete.

| Workstream | Owner Module(s) | Status | Sign-Off Criteria | Target Test File(s) |
|---|---|---|---|---|
| D31 overlay admission ordering | D31 | Partial | Narrow `_on_strategy_signal()` overlay gate is live for baseline-full PMR candidates; third-slot allocation/admission in `add_strategy()` is still missing | `Spyder/SpyderT_Testing/SpyderT141_D31_EntryTrustGate.py` (extended and passing), `Spyder/SpyderT_Testing/SpyderT219_D31_ODTEOverlaySlot.py` (scaffold remains) |
| D31 normalization and duplicate overlay guard | D31 | Partial | PMR aliases normalize to `pivot_mean_reversion`; existing duplicate-entry protections remain reused, but a dedicated third-slot allocation guard is still pending | `Spyder/SpyderT_Testing/SpyderT141_D31_EntryTrustGate.py` (extended and passing), `Spyder/SpyderT_Testing/SpyderT193_D31_DispatchResultHardening.py` (future regression) |
...
### 9.5) Concrete Implementation Plan Anchored to Current Code (v38)

This section now records both the lowest-churn implementation plan and the landed slice that matches the live codebase as of v38.

Landing state at v38:
...
- Completed: D31 PMR alias normalization, overlay metadata builder, and narrow `_on_strategy_signal()` gate for baseline-full PMR candidates.
...
- Pending: `add_strategy()` third-slot allocation exception, expanded decision-log schema, runtime-disable loop, scaffold migration, and a clean paper-session replay.
...
3. `_on_strategy_signal()` is the owning pre-dispatch risk handoff surface.
  ...
  - The remaining gap is that `add_strategy()` still does not create a live third-slot allocation exception.
...
1. Implement the `add_strategy()` third-slot allocation exception and replace the still-conservative D31 warning with final live feature logging.
2. Extend decision-audit schema and scaffold-based telemetry tests.
3. Add the runtime disable/recovery loop and its focused coverage.
