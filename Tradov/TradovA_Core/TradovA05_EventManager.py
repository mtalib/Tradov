...
class EventManager:
    """
    Centralized event management system for Tradov.
    ...
    Example:
        ...
        >>> manager.emit(EventType.TRADE, {"symbol": "TRAD", "action": "BUY"})
        >>> manager.stop()
    """
    ...
    def _allow_multiple() -> bool:
        ...
        Set the environment variable ``TRADOV_ALLOW_MULTIPLE_EM=1`` in unit
        tests that deliberately construct isolated EventManager instances.
        """
        return os.environ.get("TRADOV_ALLOW_MULTIPLE_EM", "0") == "1"
    ...
    def __init__(self, persist_events: bool = True,
        ...
        with EventManager._construction_lock:
            ...
            if EventManager._constructed_count > 1 and not EventManager._allow_multiple():
                ...
                _logging.getLogger(__name__).warning(
                    ...
                    "Set TRADOV_ALLOW_MULTIPLE_EM=1 to suppress this warning in tests.",
        ...
        # Persistence
        self.db_path = db_path or Path.home() / ".tradov" / "events.db"
