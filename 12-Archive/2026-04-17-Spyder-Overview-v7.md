# Spyder Trading System — Codebase Review v17 (v7)
**Date:** April 17, 2026
**Scope:** Current-state inventory of the Spyder codebase
**Prepared by:** Claude Sonnet 4.6
**Supersedes:** 2026-04-17-Spyder-Overview-v6.md

---

## Change Summary (v6 → v7)

This revision incorporates the **Pivot Mean-Reversion (PMR) signal end-to-end integration** delivered on April 17, 2026 on the `refactor/g05-widget-extraction` branch. The PMR signal is now wired from producer through audit display:

- **New `SpyderS08_PivotMeanReversionSignal.py`** (329 LOC): Pure dataclass-driven pivot mean-reversion signal engine. Computes classic pivot levels (P/R1–R3/S1–S3) from prior-session OHLC, scores fade-resistance / fade-support setups by combining ATR-normalised distance, RSI extremity, regime context (RANGE/NORMAL/SWAN), GEX magnitude, VWAP slope, and max-pain alignment, and applies penalties for news, edge-of-day, VIX spikes, and backwardation. Returns a typed `PivotMRSignal` with `direction`, `score` (0–100), `confidence`, `level_name`, `level_price`, `atr_distance`, plus full `reasons` and `penalties` audit lists. Stateless, fully dependency-injected, no Qt or broker dependencies. The existing `SpyderS08_ShortSqueezeDetector.py` is retained at the same numeric slot pending a future S-series renumbering.
- **`SpyderD25_UnifiedCreditSpreadEngine.py`** (1,454 → 1,525 LOC): Integrated S08 PMR override into both directional-bias decision paths. When the PMR engine fires, it can flip `BULL_PUT` ↔ `BEAR_CALL` selection ahead of the legacy regime bias. Pivot signal context is preserved on the spread metadata for downstream auditing.
- **`SpyderR08_PaperTradingQtWorker.py`** (NEW to v7 inventory at 2,041 LOC): Full producer wiring for the S08 PMR signal. Adds optional S08 import with `PIVOT_MR_SIGNAL_AVAILABLE` flag, four new caches (`_pivots_cache`, `_pivots_cache_date`, `_pivot_signal_engine`, `_last_pivot_signal`) initialised in `__init__`, and seven new helper methods (`_get_session_pivots`, `_rsi_from_prices`, `_atr_proxy_from_prices`, `_pivot_regime_label`, `_compute_pivot_signal`, `_pivot_preferred_direction`, `_snapshot_pivot_signal`). Pivot bias is consulted before legacy regime bias in the neutral-signal decision branch when the new `SPYDER_PIVOT_MR_ENABLED=1` environment flag is set (defaults off). The fired pivot signal is stamped onto both `_open_spreads` entries and `_closed_trades` audit rows for ML training.
- **`SpyderG22_TradeAuditDialog.py`** (NEW to v7 inventory at 348 LOC): Closed-trade audit dialog now displays three new pivot columns (Pivot Dir / Pivot Score / Pivot Level) plus a tooltip on the Pivot Dir cell showing the full `reasons` and `penalties` breakdown when the signal fired. CSV export uses a new `_flatten_pivot()` helper to expand the nested `pivot_signal` dict into eight plain columns (`pivot_direction`, `pivot_score`, `pivot_confidence`, `pivot_level_name`, `pivot_level_price`, `pivot_atr_distance`, `pivot_reasons`, `pivot_penalties`) so the audit log remains a clean ML-ready CSV.
- **New `SpyderT131_PivotMRSignalTests.py`** (301 LOC, 24 tests — all passing): Comprehensive pytest suite for the S08 engine across six test classes covering no-fire conditions, fade-resistance and fade-support fire paths, bonuses (regime/GEX/ATR/RSI/VWAP/max-pain), penalties (news, edge-of-day, VIX, backwardation), fire-gate threshold, level selection (deepest-breach semantics), and bidirectional choice arbitration.
- **New environment flag:** `SPYDER_PIVOT_MR_ENABLED=1` — opt-in gate for the R08 producer (defaults off for safety).
- **D-Series, G-Series, R-Series, S-Series, T-Series, and codebase totals updated** to reflect the new modules and LOC deltas.
...
| Production files (A–Z, excluding T) | **360** |
| Test files (T-series) | **124** |
| Total lines of code | **428,144** |
| Production LOC | **333,703** |
| Test LOC | **94,441** |
| Series (packages) | **25** |
...
| D | `SpyderD_Strategies` | 32 | 35,384 | Strategy implementations (Iron Condor, Credit Spread, Zero-DTE, etc.) |
...
| G | `SpyderG_GUI` | 26 | 19,034 | PySide6 interface, dashboards, charts, widgets |
...
| R | `SpyderR_Runtime` | 8 | 8,528 | Paper engine, live engine, paper Qt worker, backtest harness, production deployment |
| S | `SpyderS_Signals` | 13 | 9,853 | Custom signals: DIX, GEX, SKEW, Black Swan, pivot mean-reversion, short squeeze, breadth, macros |
| T | `SpyderT_Testing` | 124 | 94,441 | Full pytest test suite covering all series |
...
| `SpyderA02_TradingEngine.py` | 1,817 | Core trading loop. Drives the main cycle: polls market data, runs analysis, dispatches signals to strategies, validates through risk, submits orders, and updates portfolio state. Risk validation calls `validate_signal(RiskValidationRequest)` directly — the typed E00 Protocol boundary — rather than the legacy `check_trade` dict adapter. |
...
| `SpyderG05_TradingDashboard.py` | 3,608 | Main trading dashboard. Integrates positions, P&L, Greeks exposure, open orders, market internals, agent status, and signal feeds in a unified tabbed view with live data refresh. Progressively decomposed: data-fetch logic in G18, panel builders in G20, signal wiring in G21, indicator computation in G19, and widget classes in G13. **Mode-switch state preservation (v6):** switching PAPER ↔ LIVE now saves the outgoing mode's "ORDERS & POSITIONS" `QTreeWidget` rows (`_snapshot_positions_table()` / `_restore_positions_snapshot()`) and the outgoing mode's P&L stats dict (`_pnl_stats_by_mode`) so both containers restore immediately when switching back. Two per-mode snapshot dictionaries (`_pnl_stats_by_mode`, `_positions_snapshot_by_mode`) are initialised at construction. |
...
| `SpyderG17_MarketInternalsWidget.py` | 686 | Live market internals monitor dialog. Embeds 1-day mini-charts and configurable alert sliders for NYSE $TICK, $ADD, and $TRIN. Receives data via `S07.breadth_updated` signal; auto-refreshes at a configurable interval (default 5 s). |
| `SpyderG18_MarketDataWorker.py` | 873 | QObject-based market data worker extracted from G05. Runs on its own `QThread` and fans out Tradier quotes, account balances, option-chain CPC computation, heartbeat status, and 5-minute SPY chart bars into Qt signals consumed by G05. Also the canonical home for quote-freshness helpers and `REALTIME_SENTINEL_SYMBOLS`. |
...
| `SpyderG20_DashboardBuilder.py` | 922 | UI builder helpers extracted from G05. Contains `build_left_panel()`, `build_center_panel()`, `build_right_panel()`, and widget-factory functions (`create_chart_widget`, `create_positions_table`, `create_pnl_table`, `create_unified_prometheus_metrics`) that assemble the dashboard layout. |
...
**Package:** `SpyderS_Signals` | **13 files** | **9,853 LOC**

The S-Series generates custom proprietary signals beyond standard technical analysis: dark pool indices, gamma/delta exposure, SKEW, Black Swan risk indicators, pivot mean-reversion, short squeeze detection, NYSE breadth internals (via TradingView scraping), and macro economic data integration.
...
**Package:** `SpyderT_Testing` | **124 files** | **94,441 LOC**
...
*End of Spyder Codebase Review v17 (v6) — April 17, 2026*
