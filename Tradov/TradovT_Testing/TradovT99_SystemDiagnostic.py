...
TRADOV_ROOT = Path(__file__).parent.parent if Path(__file__).parent.name.startswith('TradovT') else Path(__file__).parent
sys.path.insert(0, str(TRADOV_ROOT))
...
class TradovDiagnostic:
    """Comprehensive Tradov system diagnostic tool"""

    def __init__(self):
        self.results: list[DiagnosticResult] = []
        self.tradov_root = TRADOV_ROOT
        self.errors_found = []
        self.warnings_found = []
        ...
        print("=" * 80)
        print("TRADOV SYSTEM DIAGNOSTIC & DEBUGGING TOOL")
        print("=" * 80)
        print(f"{Colors.NC}")
        print(f"Tradov Root: {self.tradov_root}")
    ...
    def test_directory_structure(self):
        """Test Tradov directory structure"""
        print(f"\n{Colors.BOLD}3. DIRECTORY STRUCTURE CHECKS{Colors.NC}")
        print("-" * 40)

        expected_dirs = [
            ('TradovA_Core', 'Core system modules'),
            ('TradovB_Broker', 'Broker integration'),
            ('TradovG_GUI', 'GUI components'),
            ('TradovH_Storage', 'Data storage'),
            ('Tradov.TradovU_Utilities', 'Utility modules'),
            ('TradovE_Risk', 'Risk management'),
            ('logs', 'Log files'),
            ('.tradov', 'Configuration directory (in home)')
        ]
        ...
        for dir_name, description in expected_dirs:
            if dir_name == '.tradov':
                ...
                dir_path = self.tradov_root / dir_name
    ...
    def test_configuration_files(self):
        ...
        config_files = [
            (Path.home() / '.tradov' / 'config.yaml', 'Main configuration'),
            (Path.home() / '.tradov' / 'data', 'Data directory'),
            (self.tradov_root / 'launch_tradov_wayland.sh', 'Launch script')
        ]
    ...
    def test_core_modules(self):
        """Test core Tradov modules"""
        print(f"\n{Colors.BOLD}5. CORE MODULE IMPORT CHECKS{Colors.NC}")
        print("-" * 40)

        core_modules = [
            ('TradovA_Core.TradovA01_Main', 'Main application entry point'),
            ('TradovA_Core.TradovA03_Configuration', 'Configuration manager'),
            ('TradovA_Core.TradovA05_EventManager', 'Event management system'),
            ('Tradov.TradovU_Utilities.TradovU01_Logger', 'Logging system'),
            ('Tradov.TradovU_Utilities.TradovU02_ErrorHandler', 'Error handling'),
            ('Tradov.TradovU_Utilities.TradovU10_TradingCalendar', 'Trading calendar'),
            ('TradovH_Storage.TradovH01_DataAccessLayer', 'Database access'),
            ('TradovE_Risk.TradovE01_RiskManager', 'Risk management'),
            ('TradovE_Risk.TradovE02_PositionSizer', 'Position sizing')
        ]
    ...
    def test_gui_modules(self):
        ...
        gui_modules = [
            ('TradovG_GUI.TradovG01_MainWindow', 'Main window'),
            ('TradovG_GUI.TradovG05_TradingDashboard', 'Trading dashboard'),
            ('TradovG_GUI.TradovG06_SignalInfoDialog', 'Signal info dialog'),
            ('TradovG_GUI.TradovG07_RiskParametersDialog', 'Risk parameters dialog')
        ]
    ...
    def test_broker_modules(self):
        ...
        broker_modules = [
            ('TradovB_Broker.TradovB01_BrokerClient', 'Broker client'),
            ('TradovB_Broker.TradovB02_MarketData', 'Market data'),
            ('TradovB_Broker.TradovB03_OrderManagement', 'Order management'),
            ('TradovB_Broker.TradovB04_AccountManager', 'Account manager'),
            ('TradovB_Broker.TradovB05_PositionManager', 'Position manager')
        ]
    ...
    def test_event_manager_initialization(self):
        ...
        try:
            from TradovA_Core.TradovA05_EventManager import EventManager, EventType
    ...
    def test_database_initialization(self):
        ...
        try:
            from TradovH_Storage.TradovH01_DataAccessLayer import DataAccessLayer

            # Test database connection
            db_path = Path.home() / '.tradov' / 'data' / 'test_tradov.db'
            ...
            self.add_result("Database Initialization", "FAIL", f"Error: {str(e)[:50]}",
                          details=traceback.format_exc(),
                          fix_suggestion="Check TradovH01_DataAccessLayer implementation")
    ...
    def test_configuration_initialization(self):
        ...
        try:
            from TradovA_Core.TradovA03_Configuration import ConfigManager
    ...
    def test_gui_component_creation(self):
        ...
        try:
            ...
            try:
                from TradovG_GUI.TradovG01_MainWindow import MainWindow
            ...
            try:
                ...
                try:
                    ...
                    import inspect
                    sig = inspect.signature(TradovTradingDashboard.__init__)
                    ...
                    self.add_result("TradingDashboard Creation", "FAIL",
                                  ...
                                  details=str(te),
                                  fix_suggestion="Fix TradovTradingDashboard.__init__() to accept correct parameters")
                except Exception as e:
                    self.add_result("TradingDashboard Creation", "FAIL",
    ...
    def generate_report(self):
        ...
        if failed_tests == 0:
            ...
            print("4. Check missing modules in TradovH_Storage and TradovB_Broker series")
    ...
    def save_report_to_file(self):
        ...
        try:
            report_file = self.tradov_root / "logs" / f"diagnostic_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
            report_file.parent.mkdir(exist_ok=True)

            report_data = {
                'timestamp': datetime.now().isoformat(),
                'tradov_root': str(self.tradov_root),
...
if __name__ == "__main__":
    print("Starting Tradov System Diagnostic...")
    success = main()
