...
def nuclear_restart_gateway():
    ...
    try:
        subprocess.run(
            ["pkill", "-9", "-f", "ibgateway"], capture_output=True, check=False
        )
        time.sleep(3)
    ...
    try:
        ...
        try:
            await asyncio.wait_for(
                ib.connectAsync("127.0.0.1", IB_PORT, clientId=0), timeout=timeout
            )
...
def launch_gateway_with_ibc():
    ...
    env = os.environ.copy()
    env["TWSUSERID"] = IB_USERNAME
    env["TWSPASSWORD"] = IB_PASSWORD
    env["TRADING_MODE"] = IB_TRADING_MODE.upper()
    # Set other required env vars
    env["IBC_INI"] = str(IBC_PATH / "config.ini")
    env["TWS_MAJOR_VRSN"] = "1039"  # Gateway version
    env["IBC_PATH"] = str(IBC_PATH)
    env["TWS_PATH"] = str(Path.home() / "Jts" / "ibgateway" / "1039")
    env["TWS_SETTINGS_PATH"] = str(Path.home() / "Jts")
    env["LOG_PATH"] = str(IBC_PATH / "logs")
...
async def main():
    ...
    parser.add_argument(
        "--clean-start", action="store_true", help="Force nuclear restart before launch"
    )
    parser.add_argument(
        "--skip-gateway", action="store_true", help="Skip Gateway launch (use existing)"
    )
