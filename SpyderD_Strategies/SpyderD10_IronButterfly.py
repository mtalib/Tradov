...
class IronButterflyStrategy(BaseStrategy):
    ...
    def _calculate_butterfly_structure(self, atm_strike: float, 
                                     current_price: float,
    ...
    def _create_butterfly_signal(self, setup: Dict, market_data: pd.DataFrame) -> Optional[ButterflySignal]:
    ...
    def _check_exit_conditions(self, position: ButterflyPosition, 
    ...
    def _calculate_current_pnl(self, position: ButterflyPosition, current_price: float) -> float:
        ...
        if distance_from_atm < 0.5:
            ...
            excess = max(0, current_price - position.breakeven_upper) + \
                    max(0, position.breakeven_lower - current_price)
    ...
    def _should_adjust_position(self, position: ButterflyPosition, 
    ...
    def _create_adjustment_signal(self, position: ButterflyPosition,
                                 market_data: pd.DataFrame) -> Optional[TradingSignal]:
    ...
    def calculate_butterfly_metrics(self, position: ButterflyPosition) -> Dict[str, float]:
        ...
        metrics = {
            ...
            'profit_loss_ratio': position.max_profit / position.max_loss if position.max_loss > 0 else 0,
            'current_pnl': position.current_pnl,
            'pnl_percentage': position.current_pnl / position.max_profit if position.max_profit > 0 else 0,
            'days_held': (datetime.now() - position.entry_time).days,
            'days_to_expiry': (position.expiry - datetime.now()).days,
