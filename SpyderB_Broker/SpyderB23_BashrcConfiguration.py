...
# IB Gateway 10.37 Environment Variables
IB_ENV_VARS = {
    # Version Configuration
    "TWS_MAJOR_VRSN": "1037",
    "IB_GATEWAY_VERSION": "10.37",
    "IB_GATEWAY_BUILD": "10.37.1l",
    # Directory Paths
    "IB_GATEWAY_HOME": "$HOME/Jts",
    "IB_GATEWAY_DIR": "$HOME/Jts/ibgateway/1037",
    "IBC_PATH": "$HOME/ibc",
        ...
        for var, expected_value in IB_ENV_VARS.items():
            ...
            if not match:
                ...
                if not ("$" in expected_value and "$" in match.group(1)):
                    result.incorrect_vars[var] = (
                        f"Found: {match.group(1)}, Expected: {expected_value}"
                    )
    ...
    def apply_configuration(self) -> bool:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                self.logger.info(
                    "Note: Restart shell or run 'source ~/.bashrc' for full effect"
                )
                return True
            else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--test", action="store_true", help="Run in test mode (no actual changes)"
    )
    ...
    # Create manager
    config = BashrcConfig(
        test_mode=args.test, force_update=args.force, backup_enabled=not args.no_backup
    )
