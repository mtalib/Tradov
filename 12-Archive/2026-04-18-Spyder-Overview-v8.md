# Spyder Trading System — Codebase Review v18 (v8)
**Date:** April 18, 2026
**Scope:** Current-state inventory of the Spyder codebase
**Prepared by:** Claude Sonnet 4.6
**Supersedes:** 2026-04-17-Spyder-Overview-v7.md

---

## Change Summary (v7 → v8)

This revision incorporates the **S07 Options Analytics wiring and dashboard integration** delivered on April 18, 2026 on the `refactor/g05-widget-extraction` branch. Three phases extended the dashboard and signals layer, one task hardened the S07 TradierClient instantiation, and a second batch of work added NYMO breadth display end-to-end and extracted the toolbar builder:

- **Phase 1 — Market Overview panel update (`SpyderG05_TradingDashboard.py`):** Removed the NAAIM and AABULL sentiment rows from the Market Overview panel (data sources retired from the live display). Added two new rows — **VIX9D** (short-term VIX term structure) and **HYG** (high-yield credit ETF as a risk-sentiment proxy) — giving the panel a tighter, more actionable set of cross-asset signals.
- **Phase 2 — G18 data wiring (`SpyderG18_MarketDataWorker.py`):** Extended the market-data worker to fetch and emit VIX9D and HYG quotes on every poll cycle. The worker now resolves both symbols via `REALTIME_SENTINEL_SYMBOLS`, validates freshness, and fans the values out as Qt signals consumed by the G05 Market Overview panel. LOC 873 → 879.
- **Phase 3 — OPTIONS ANALYTICS panel (`SpyderG05_TradingDashboard.py`, `SpyderG20_DashboardBuilder.py`):** Added a new **OPTIONS ANALYTICS** group to the left panel of the trading dashboard, displaying three live metrics: **IVR** (Implied Volatility Rank, 0–100 scale), **ATM IV** (at-the-money implied volatility in percent), and **VRP** (Volatility Risk Premium = ATM IV − HV20). Values are populated from the S07 options analytics metrics dict. G05 LOC 3,608 → 4,144; G20 LOC 922 → 996.
- **Task A — TradierClient caching in S07 (`SpyderS07_CustomMetricsOrchestrator.py`):** The `_update_options_analytics_metrics` method previously instantiated a fresh `TradierClient` on every call. Two new instance attributes — `self._options_tradier_client: Any | None = None` and `self._options_tradier_env: str | None = None` — are initialised in `__init__`. The method now reuses the cached client and only recreates it when the active Tradier environment string changes. Methods added: `_compute_ivr(current_iv)` (reads/writes `market_data/iv_history.json`, returns 0–100 float or `nan` with <5 data points), `_compute_atm_iv(contracts, spot)` (averages `c.iv * 100` for the 6 nearest-ATM contracts where `c.iv > 0`), `_compute_hv20(client)` (20-day annualised historical volatility from Tradier daily bars, returns `None` if <22 bars). VRP is computed as ATM_IV − HV20 (or `nan` when HV20 is `None`). IVR, ATM_IV, and VRP are now present in `current_metrics`, `metric_quality`, and `_format_metrics` output. S07 LOC 1,148 → 1,376.
- **Task B — New `SpyderT120_S07OptionsAnalytics.py`** (516 LOC, 30 tests — all passing): Comprehensive pytest suite for the S07 options analytics additions, across six test classes: `TestComputeAtmIv` (ATM IV extraction, zero-IV filtering, annualised percent), `TestComputeIvr` (52-week IVR rank at high/low/midpoint, history trimming to 252 entries), `TestComputeHv20` (HV20 calculation, insufficient-bar guard), `TestUpdateOptionsAnalyticsMetrics` (no-credentials guard, full API success path, chain-fetch error, client caching, env-change client recreation), `TestFormatMetricsOptionsAnalytics` (IVR/ATM_IV/VRP formatting), `TestMetricDataStructures` (keys in `current_metrics`, `metric_snapshot`, `metric_quality`, cache attrs initialised to `None`). Tests use a clean four-patch approach for filesystem isolation (`exists`, `read_text`, `write_text`, `mkdir`) and pre-inject the mock client directly to avoid import-path issues with lazy broker imports.
- **Task C — SPY options chain fetch deduplication (`SpyderG18_MarketDataWorker.py`):** The CPC computation block previously issued fresh `get_option_expirations` + `get_option_chain` API calls on every ~60 s poll cycle regardless of concurrent callers. A new module-level `_get_cached_chain(client)` function (with `_CHAIN_LOCK: threading.Lock`, `_CHAIN_TTL = 30.0 s`, `_CHAIN_CACHE: dict`) returns the cached result when less than 30 s old, blocking concurrent callers with a `threading.Lock` until the in-flight fetch completes. The original CPC block is replaced with a single call to `_get_cached_chain(client)`. G18 LOC 879 → ~955.
- **Task D — NYMO breadth signal end-to-end (`SpyderS11_TradingViewInternals.py` → `SpyderS07_CustomMetricsOrchestrator.py` → `SpyderG05_TradingDashboard.py` → `SpyderG17_MarketInternalsWidget.py`):** Added the NYSE McClellan Oscillator ($NYMO) as a fourth live breadth indicator, delivered across four modules:
  - *S11:* `"nymo": "https://www.tradingview.com/symbols/USI-NYMO/"` added to `_SYMBOLS`; four `_NYMO_*` threshold constants defined; `_classify_breadth()` now casts a four-way majority vote (TICK, TRIN, ADD, NYMO); `_stub()` returns `"nymo": float("nan")`; module and `get_snapshot` docstrings updated. S11 LOC 369 → ~395.
  - *S07:* `_update_tv_breadth_metrics` now relays `updated_metrics["NYMO"] = snap.get("nymo", float("nan"))` alongside TICK, ADD, and TRIN. S07 already held NYMO in `MetricSnapshot`, default metrics, and `_format_metrics`; this single line closes the data path.
  - *G05:* `_on_custom_metrics_updated` now extracts `nymo_entry = metrics.get("NYMO", {})` and includes `"nymo": nymo` in the dict forwarded to `dlg.on_breadth_updated()`. Comment updated to note TICK/ADD/TRIN/NYMO forwarding.
  - *G17:* `_DEFAULTS` gains `"NYMO": {"high": 40, "low": -40, "unit": "", "scale": 1}`; window widened from 820 px to 1060 px; title updated to `TICK | ADD | TRIN | NYMO`; panel loop extended to four symbols; `on_breadth_updated` handles the `"nymo"` key; `_push_current_metrics` includes `nymo`; `_FetchWorker.run` and `_HistoryWorker.run` updated to emit four-symbol dicts. G17 LOC 686 → ~720.
- **Task E — G05 toolbar extraction to G20 (`SpyderG05_TradingDashboard.py`, `SpyderG20_DashboardBuilder.py`):** The 244-line `create_toolbar` method was extracted from G05 into a new `build_toolbar(dashboard: Any) -> QWidget` function in G20. The function replicates the full toolbar — SPYDER logo, market index centre section (DJI/COMP/SPX/RUT), TRADIER EXEC / MASSIVE DATA / SIMULATED status containers, and ET clock — setting all runtime attributes on the `dashboard` argument. G05's `create_toolbar` is reduced to a 2-line delegate (`return build_toolbar(self)`). `QFont` added to G20 imports; `build_toolbar` added to the G05 import from G20. G05 LOC 4,144 → ~3,904; G20 LOC 996 → ~1,255.
- **Live Trading Pipeline — Seven Readiness Gap Fixes (April 18, 2026):** Audit identified seven gaps preventing live orders from reaching Tradier. All seven were closed plus one bonus broker-routing fix:
  - **GAP 1 — A02: `_order_processor_loop` submission stub replaced:** The loop previously logged orders but never called any broker. New `_submit_order_to_broker(order: OrderInfo)` helper (~110 lines): Path 1 builds a `B02Order` and calls `OrderManager.submit_order()`; Path 2 maps `OrderAction`/`OrderType` enums to `B40OrderSide`/`B40OrderType` and calls `TradierClient.place_order()`; no broker sets `OrderState.ERROR`. A02 LOC 1,817 → 1,958.
  - **GAP 2 — D01: Dual-emit `STRATEGY_SIGNAL`:** `BaseStrategy._process_signal()` now lazily imports A05's `EventType` and emits a second `STRATEGY_SIGNAL` event (the type D31 subscribes to) alongside the existing local `SIGNAL_GENERATED` emit. Non-fatal (wrapped in broad `except`). Closes the enum-class mismatch that silently blocked strategy signals from reaching D31. D01 LOC 1,064 → 1,086.
  - **GAP 3 — E01: `check_daily_limits()` added:** New public method acquires `_risk_lock`, returns `False` when data is stale, calls `_calculate_risk_metrics()`, and rejects when `daily_pnl < -abs(max_daily_loss)`. Fails safe on any exception. E01 LOC 1,280 → 1,330.
  - **GAP 4 — Q14: `live_engine.start()` → `live_engine.start_trading()`:** One-line rename matching the R04 `LiveEngine` public API.
  - **GAP 5 — D31: Approved signals dispatched to `LiveEngine`:** Added `self._live_engine = None` in `__init__`, `set_live_engine(engine)` setter, and `_dispatch_approved_signal(signal)` helper (~65 lines). After E01 approves a signal, the helper normalises it to a dict, validates symbol and quantity, calls `self._live_engine.execute_order(order)`, and logs the result. D31 LOC 2,080 → 2,203.
  - **GAP A — D31: Three sets of missing loop-body methods added (~305 lines):** `start_orchestration()` called `_configure_strategies_for_regime()` and `_perform_initial_allocation()` which did not exist — causing an `AttributeError` that Q14 caught silently, leaving the orchestrator permanently idle. `_orchestration_loop` also called `_resolve_strategy_conflicts()` and `_adaptive_strategy_management()`; `_monitoring_loop` called `_monitor_strategy_health()`, `_check_risk_limits()`, and `_update_performance_attribution()`; `stop_orchestration` called `_generate_final_report()`. All eight methods implemented. D31 LOC 2,203 → 2,508.
  - **GAP B — D31: `_configure_strategies_for_regime()` instantiates strategies:** Previously `active_strategies` was always empty so no signals were ever generated. Now reads the regime-weight map, selects matching entries from `available_strategies` (IronCondor, CreditSpread, ZeroDTE, Straddle, SpecializedZeroDTE), normalises allocations, and calls `add_strategy()` for each missing type.
  - **GAP C — D31: Market data fan-out to active strategies:** `_on_market_data_event()` previously cached the event data but never called `strategy.process_market_data()` on any instance. `BaseStrategy.process_market_data()` is the only entry point that calls `generate_signals()`. The handler now converts the event payload to a `pd.DataFrame` and iterates all `active_strategies`, calling `process_market_data(market_df)` on each — completing the C-series → D01 → D31 → E01 → R04 → B40 autonomous signal chain.
  - **GAP D — A05: `MARKET_DATA_TICK` and `CUSTOM_METRIC_UPDATE` added to `EventType`:** C01's `_setup_event_subscriptions()` subscribed to `EventType.CUSTOM_METRIC_UPDATE` and published `EventType.MARKET_DATA_TICK`, neither of which existed in the `EventType` enum — causing `AttributeError` at feed startup. Both values added to A05 `EventType`. A05 LOC unchanged (enum extension only).
  - **GAP E — C01: `MARKET_DATA_TICK` → `MARKET_DATA` in `_on_provider_data()`:** Even after adding the enum value, D31 subscribed to `EventType.MARKET_DATA` while C01 published `EventType.MARKET_DATA_TICK`, so market-data events never reached strategies. C01 now publishes `EventType.MARKET_DATA`, aligning publisher and subscriber.
  - **GAP F — D31: Five missing helper methods added (~81 lines):** `add_strategy()` called `_get_risk_profile_for_strategy()`, `_get_strategy_type()`, and `_calculate_optimal_allocation()` (singular, distinct from the plural `_calculate_optimal_allocations()`); `_update_market_regime()` called `_calculate_vix_percentile()` and `_calculate_trend_strength()` — all five were absent, causing `AttributeError` on every strategy-add and every regime-update cycle. Implementations: `_get_risk_profile_for_strategy()` builds a `RiskProfile` sized to a per-strategy capital slice; `_get_strategy_type()` strips common class-name suffixes for a clean label; `_calculate_optimal_allocation()` uses the regime-weight map with remaining-capital normalisation; `_calculate_vix_percentile()` maps VIX levels to approximate historical percentile bands; `_calculate_trend_strength()` derives a −1…+1 momentum score from cached SPY tick prices. D31 LOC 2,508 → 2,589.
  - **GAP 6 — `SpyderB20_IntegratedConnectivityManager.py` created:** New 191-line module providing `ConnectivityState` enum (DISCONNECTED, CONNECTING, CONNECTED, AUTHENTICATED, DEGRADED, RECONNECTING, ERROR) and `IntegratedConnectivityManager` class with `connect()`, `disconnect()`, `is_connected()`, `heartbeat()`, `get_status()`, and `state` property. B-Series 8 → 9 files, 10,247 → 10,438 LOC.
  - **GAP 7 — Q14: `StrategyOrchestrator` wired to live engine:** After `live_engine.start_trading()`, Q14 now imports D31's `StrategyOrchestrator`, creates an `EventManager`, reads `BASE_CAPITAL` from env (default 100,000), instantiates the orchestrator with `ADAPTIVE` mode and `RISK_PARITY` allocation, calls `orchestrator.set_live_engine(live_engine)`, and starts orchestration. Failure is non-fatal. Q14 LOC 499 → 526.
  - **BONUS — R04: `_broker_submit()` adapter fixes broker routing:** `_execute_order_internal` previously called `self.broker.submit_order(dict)` — B40 `TradierClient` has no `submit_order()`, so every live order silently failed. Now calls `self._broker_submit(order)`. New helper tries B02's `submit_order(B02Order)` first, falls back to B40's `place_order(**kwargs)` with full enum mapping, and returns a standardised `{"status", "order_id", "tradier_order_id"}` dict. R04 LOC 1,385 → 1,517.
- **A-Series, B-Series, C-Series, D-Series, E-Series, Q-Series, R-Series, G-Series, S-Series, T-Series, and codebase totals updated** to reflect all gap-fix modules and LOC deltas (including six additional gap fixes across A05, C01, and D31 — bringing D31 from 2,080 to 2,589 LOC across three sessions).
...
| Production files (A–Z, excluding T) | **361** |
| Test files (T-series) | **125** |
| Total lines of code | **430,576** |
| Production LOC | **335,619** |
| Test LOC | **94,957** |
| Series (packages) | **25** |
...
- **Canonical regime engine:** `SpyderL09_UnifiedRegimeEngine` is the single source of truth for market regime classification. E21, F10, M06, and V02's embedded regime detector are retained as internal/legacy components. `SpyderY01_MarketSenseAgent` now resolves regime through L09 first, with E21 HMM as a named fallback, completing canonical regime wiring for all production callers. T129 asserts this programmatically.
...
- **Risk gate:** E01 `validate_signal(RiskValidationRequest)` is the pre-submit risk entry point, wired into D31 `StrategyOrchestrator` for every strategy signal. A02 `TradingEngine` calls `validate_signal()` directly with a typed `RiskValidationRequest` object, using the declared E00 Protocol boundary throughout the primary trading path. D31 now dispatches approved signals to R04 `LiveEngine.execute_order()` via `_dispatch_approved_signal()`, and R04 routes them to Tradier via `_broker_submit()` — completing the full D01 → D31 (risk gate) → R04 → B40 live order chain.
- **Q10 CI gate:** Four gates are active in `SpyderQ10_ProtocolComplianceGate`: (1) no `random.*` / `np.random.*` in non-allowed production paths, (2) T129 protocol-compliance suite passes, (3) no `datetime.utcnow()` in production code, (4) no bare `datetime.now()` (without `timezone.utc`) in production code.
...
|--------|---------|------:|----:|----------------|
| A | `SpyderA_Core` | 8 | 9,853 | System orchestration, main entry point, configuration |
| B | `SpyderB_Broker` | 9 | 10,438 | Tradier API integration, order management, execution |
| C | `SpyderC_MarketData` | 26 | 28,172 | Real-time and historical market data, validation, routing |
| D | `SpyderD_Strategies` | 32 | 35,915 | Strategy implementations (Iron Condor, Credit Spread, Zero-DTE, etc.) |
| E | `SpyderE_Risk` | 25 | 28,325 | Risk management, position sizing, circuit breakers, drawdown |
| F | `SpyderF_Analysis` | 22 | 22,320 | Technical indicators, price action, volatility regime, ML prediction |
| G | `SpyderG_GUI` | 26 | 19,650 | PySide6 interface, dashboards, charts, widgets |
...
| Q | `SpyderQ_Scripts` | 22 | 9,387 | Launchers, validators, utilities, fine-tuning scripts, CI gates |
| R | `SpyderR_Runtime` | 8 | 8,660 | Paper engine, live engine, paper Qt worker, backtest harness, production deployment |
| S | `SpyderS_Signals` | 13 | 10,081 | Custom signals: DIX, GEX, SKEW, Black Swan, pivot mean-reversion, short squeeze, breadth, options analytics, macros |
| T | `SpyderT_Testing` | 125 | 94,957 | Full pytest test suite covering all series |
...
**Package:** `SpyderA_Core` | **8 files** | **9,712 LOC**
...
| `SpyderA02_TradingEngine.py` | 1,958 | Core trading loop. Drives the main cycle: polls market data, runs analysis, dispatches signals to strategies, validates through risk, submits orders, and updates portfolio state. Risk validation calls `validate_signal(RiskValidationRequest)` directly — the typed E00 Protocol boundary — rather than the legacy `check_trade` dict adapter. **Order submission (gap fix):** `_order_processor_loop` now calls `_submit_order_to_broker(order)`, which routes to B02 `OrderManager.submit_order()` (Path 1) or B40 `TradierClient.place_order()` with enum mapping (Path 2), setting `OrderState.SUBMITTED/REJECTED/ERROR` accordingly. |
...
| `SpyderA05_EventManager.py` | 1,205 | Central synchronous event bus. Publishes and dispatches typed system-wide events (market open, signal triggered, risk breach, order fill) to registered handlers. **Gap fix:** added `MARKET_DATA_TICK` and `CUSTOM_METRIC_UPDATE` to the `EventType` enum; both were referenced in C01 but missing, causing `AttributeError` at feed startup. |
...
**Package:** `SpyderB_Broker` | **9 files** | **10,438 LOC**
...
| `SpyderB20_IntegratedConnectivityManager.py` | 191 | Integrated connectivity state machine for broker and data-feed connections. Provides `ConnectivityState` enum (DISCONNECTED, CONNECTING, CONNECTED, AUTHENTICATED, DEGRADED, RECONNECTING, ERROR) and `IntegratedConnectivityManager` with `connect()`, `disconnect()`, `is_connected()`, `heartbeat()`, `get_status()`, and `state` property. |
...
| `SpyderC01_DataFeed.py` | 1,068 | Primary real-time tick data feed. Normalises incoming raw ticks, validates completeness, applies subscriptions, and fans out to all registered consumers. **Gap fix:** `_on_provider_data()` was publishing `EventType.MARKET_DATA_TICK` while D31 subscribed to `EventType.MARKET_DATA`; publisher changed to `EventType.MARKET_DATA` so market data now reaches strategy instances. |
...
**Package:** `SpyderD_Strategies` | **32 files** | **35,384 LOC**
...
| `SpyderD01_BaseStrategy.py` | 1,086 | Abstract base class for all strategies. Defines the interface: `generate_signal()`, `enter_position()`, `exit_position()`, `size_position()`, and lifecycle hooks. **Dual-emit (gap fix):** `_process_signal()` now also emits A05's `EventType.STRATEGY_SIGNAL` (in addition to the local `SIGNAL_GENERATED`) so signals reliably reach D31's subscriber, closing the enum-class mismatch that previously silenced the signal path. |
...
| `SpyderD25_UnifiedCreditSpreadEngine.py` | 1,525 | Unified engine consolidating all credit spread variants under a single scoring and selection framework for consistent risk-adjusted strike choice. **PMR-aware (v7):** integrates the S08 Pivot Mean-Reversion signal as a directional override; when fired, it can flip `BULL_PUT` ↔ `BEAR_CALL` selection ahead of the legacy regime bias and preserves pivot context on the spread metadata. |
...
| `SpyderD31_StrategyOrchestrator.py` | 2,589 | Strategy orchestrator. Coordinates concurrent strategy execution, manages capital budgeting between active strategies, and arbitrates signal conflicts. All emitted signals are routed through E01 `validate_signal()` and rejected signals publish a `RISK_ALERT` event. **Live engine dispatch (gap fix):** `set_live_engine(engine)` stores a reference to R04; approved signals are forwarded via `_dispatch_approved_signal()` which normalises the signal dict, validates required fields, calls `live_engine.execute_order(order)`, and logs the result, completing the D01 → D31 → R04 → B40 signal chain. **Strategy instantiation (gap fix):** `_configure_strategies_for_regime()` reads the regime-weight map and calls `add_strategy()` for each regime-appropriate strategy class. **Market data fan-out (gap fix):** `_on_market_data_event()` now converts the event payload to a DataFrame and calls `process_market_data()` on every active strategy instance, triggering `generate_signals()`. **Missing loop methods (gap fix):** added `_perform_initial_allocation()`, `_resolve_strategy_conflicts()`, `_adaptive_strategy_management()`, `_monitor_strategy_health()`, `_check_risk_limits()`, `_update_performance_attribution()`, and `_generate_final_report()`. **Missing strategy helpers (gap fix):** added `_get_risk_profile_for_strategy()`, `_get_strategy_type()`, `_calculate_optimal_allocation()`, `_calculate_vix_percentile()`, and `_calculate_trend_strength()`; all five were called by existing methods but never defined, causing `AttributeError` on every strategy-add and regime-update cycle. |
...
| `SpyderE01_RiskManager.py` | 1,330 | Core risk manager entry point. Routes all pre-trade and post-trade risk checks to the appropriate E-Series sub-modules and aggregates verdicts. Consumes live positions and account balances from the Tradier client via `_request_positions()` and `_request_account_summary()`. Exposes `validate_signal()` as the authoritative pre-submit gate. **`check_daily_limits()` (gap fix):** new public method acquires `_risk_lock`, returns `False` when data is stale, checks `daily_pnl < -abs(max_daily_loss)`, and fails safe on any exception. |
...
| `SpyderE13_DayProfitTarget.py` | 2,239 | Daily profit target management. Scales back risk exposure once the daily target is met; implements graduated risk reduction tiers. (Legacy Interactive Brokers `IB = None` stubs removed.) |
...
**Package:** `SpyderG_GUI` | **26 files** | **19,650 LOC**

The G-Series implements the PySide6 (Qt6) graphical interface. G05 (`SpyderG05_TradingDashboard.py`) is the central dashboard; it has been progressively decomposed — G13 holds extracted widget classes, G18 holds the QThread market-data worker, G19 holds pure chart-indicator functions, G20 holds panel builder helpers (including the toolbar since v8), and G21 holds signal-connection helpers. G17 provides the standalone market internals monitor dialog.
...
| `SpyderG05_TradingDashboard.py` | ~3,904 | Main trading dashboard. Integrates positions, P&L, Greeks exposure, open orders, market internals, agent status, and signal feeds in a unified tabbed view with live data refresh. Progressively decomposed: data-fetch logic in G18, panel builders in G20, signal wiring in G21, indicator computation in G19, and widget classes in G13. **Mode-switch state preservation (v6):** switching PAPER ↔ LIVE now saves and restores per-mode "ORDERS & POSITIONS" snapshot and P&L stats. **Market Overview panel (v8):** panel updated to show VIX9D (short-term VIX term structure) and HYG (high-yield credit risk proxy) in place of the retired NAAIM and AABULL sentiment rows. **OPTIONS ANALYTICS panel (v8):** new left-panel group displaying live IVR (Implied Volatility Rank, 0–100), ATM IV (at-the-money implied volatility in percent), and VRP (Volatility Risk Premium = ATM IV − HV20) sourced from S07 `current_metrics`. **NYMO forwarding (v8):** `_on_custom_metrics_updated` now extracts `NYMO` from the S07 metrics dict and includes it in the snapshot forwarded to G17. **Toolbar extraction (v8):** `create_toolbar` reduced to `return build_toolbar(self)`; all 244 lines now live in G20. |
...
| `SpyderG09_RiskParametersDialog.py` | 1,200 | Risk parameters configuration dialog. Allows operator adjustment of delta limits, VaR caps, daily loss thresholds, and Greeks limits at runtime. |
...
| `SpyderG12_SignalInfoDialog.py` | 646 | Signal information popup. Displays signal source, confidence score, Greeks impact, recommended action, and contributing indicators. |
| `SpyderG13_EnhancedWidgets.py` | 1,676 | Enhanced UI widget collection extracted from G05. Houses `TrafficLightButton`, `SignalMonitorPanel`, `MarketSymbolWidget`, `GreekBar`, shared colour constants, and typed dataclasses used across the dashboard. |
| `SpyderG14_Dashboard.py` | 128 | Lightweight legacy stub dashboard for backward compatibility with older launch scripts. No longer re-exported from the package `__init__.py`. |
...
| `SpyderG17_MarketInternalsWidget.py` | ~720 | Live market internals monitor dialog. Embeds 1-day mini-charts and configurable alert sliders for NYSE $TICK, $ADD, $TRIN, and **NYMO** (NYSE McClellan Oscillator). Receives data via `S07.breadth_updated` signal; auto-refreshes at a configurable interval (default 5 s). **NYMO panel (v8):** 4th `_InternalPanel` added; window widened to 1060 px; `on_breadth_updated` and `_push_current_metrics` handle the `"nymo"` key; `_DEFAULTS` gains `"NYMO": {"high": 40, "low": -40}`. |
| `SpyderG18_MarketDataWorker.py` | ~955 | QObject-based market data worker extracted from G05. Runs on its own `QThread` and fans out Tradier quotes, account balances, option-chain CPC computation, heartbeat status, and 5-minute SPY chart bars into Qt signals consumed by G05. **VIX9D and HYG wiring (v8):** the worker now fetches and emits VIX9D and HYG quotes on every poll cycle via `REALTIME_SENTINEL_SYMBOLS`. **Chain dedup (v8):** module-level `_get_cached_chain(client)` with `_CHAIN_LOCK` / `_CHAIN_TTL = 30 s` / `_CHAIN_CACHE` replaces the per-cycle chain API calls, preventing redundant requests and blocking concurrent callers safely. |
| `SpyderG19_ChartIndicators.py` | 166 | Pure-function chart indicator computation extracted from G05. Calculates OHLCV overlay values (EMA, VWAP, Bollinger) and returns a `ChartIndicators` dataclass ready for rendering. No Qt dependency; fully unit-testable without a display. |
| `SpyderG20_DashboardBuilder.py` | ~1,255 | UI builder helpers extracted from G05. Contains `build_left_panel()`, `build_center_panel()`, `build_right_panel()`, and widget-factory functions. **OPTIONS ANALYTICS group (v8):** `build_left_panel()` appends an OPTIONS ANALYTICS `QGroupBox` with IVR, ATM IV, and VRP rows. **Toolbar extraction (v8):** new `build_toolbar(dashboard)` function contains the full 244-line toolbar (logo, DJI/COMP/SPX/RUT indices, status containers, ET clock), making G05's `create_toolbar` a 2-line delegate. |
| `SpyderG21_DashboardSignalHandlers.py` | 155 | Qt signal connection helpers extracted from G05. Wires dashboard Qt signals to their handler slots, keeping signal-setup code separate from the dashboard class body. |
| `SpyderG22_TradeAuditDialog.py` | 348 | Closed-trade audit dialog for paper-engine spreads. Renders the worker's full lifecycle log with sortable columns, per-row P&L colouring, and CSV export. **PMR-aware (v7):** displays three pivot columns (Pivot Dir / Pivot Score / Pivot Level) with a tooltip on Pivot Dir showing the full `reasons` and `penalties` audit. CSV export uses `_flatten_pivot()` to expand the nested `pivot_signal` dict into eight plain columns for ML-ready output. |
...
| `__init__.py` | 329 | Package init exporting the application manager, main window, and trading dashboard. (G14 legacy stub no longer re-exported.) |
...
**Package:** `SpyderI_Integration` | **13 files** | **9,164 LOC**
...
| `SpyderJ02_EmailNotifier.py` | 825 | Email notification via SMTP with TLS. Sends HTML-formatted trade alerts, daily P&L summaries, and risk breach notifications with attachment support. SMTP password field in demo block uses `'<REPLACE_ME>'` placeholder (secrets-scanner safe). |
...
| `SpyderK02_DailyTradingReport.py` | 1,568 | Comprehensive daily P&L report. Includes trade log, strategy breakdown, Greeks exposure at close, commissions, slippage, and next-day outlook. All timestamps are timezone-aware (`datetime.now(timezone.utc)`). |
...
| Module | LOC | Description |
|--------|----:|-------------|
| `SpyderM01_SystemMonitor.py` | 962 | System health monitor. Tracks CPU, memory, disk, network, and process metrics via psutil; alerts on threshold breaches. All timestamps are timezone-aware. |
...
| `SpyderM04_TradingMetrics.py` | 1,125 | Trading metrics collector. Captures real-time P&L, fill count, order rejection rate, execution latency histograms, and Prometheus metric exports. Includes `spyder_risk_rejections_total{strategy, rejection_reason}` counter incremented by D31 on every risk-gate rejection. |
...
| `SpyderM06_HMMRegimeDetector.py` | 1,490 | Legacy standalone HMM regime detector. Retained as an internal component with no production callers outside its own tests; the canonical regime signal is supplied by `SpyderL09_UnifiedRegimeEngine`. |
...
| `SpyderP04_CapitalAllocator.py` | 1,582 | Dynamic capital allocation engine. Adjusts per-strategy capital budgets based on recent performance, Sharpe, and drawdown metrics. Regime is sourced from `SpyderL09_UnifiedRegimeEngine`; tactical-rebalance trigger is rule-based (regime transition or allocator drift), not random. |
...
| `SpyderQ10_ProtocolComplianceGate.py` | 61 | CI gate runner for the T129 protocol-compliance suite. Four gates enforced: (1) no `random.*` / `np.random.*` in non-allowed production paths, (2) T129 protocol-compliance suite passes, (3) no `datetime.utcnow()` in production code, (4) no bare `datetime.now()` (without `timezone.utc`) in production code. Exits 0 on all-pass, 1 on failure. Run with `python -m Spyder.SpyderQ_Scripts.SpyderQ10_ProtocolComplianceGate`. |
| `SpyderQ14_MainLauncher.py` | 526 | Main system launcher. Presents mode selection (live / paper / backtest) and bootstraps the appropriate engine and dashboard. **Live pipeline wiring (gap fixes):** renamed `live_engine.start()` call to `live_engine.start_trading()` (GAP 4); after startup, instantiates `StrategyOrchestrator` with `ADAPTIVE` mode and `RISK_PARITY` allocation, calls `orchestrator.set_live_engine(live_engine)` and `orchestrator.start_orchestration()` (GAP 7). |
...
**Package:** `SpyderR_Runtime` | **8 files** | **8,528 LOC**

The R-Series contains the execution engines for paper trading, live trading, and production deployment management. Each engine mode runs independently and shares the same strategy/risk interfaces. R04 API Panic Mode resets its counter on any successful broker round-trip (quote, account, chain, or fill). R08 is the Qt-threaded paper-trading worker that drives the dashboard's live paper session.
...
| `SpyderR04_LiveEngine.py` | 1,517 | Live trading engine. Routes validated strategy signals to B40 Tradier client, manages real order flow, confirmations, and error recovery. `_wait_for_execution` uses a `concurrent.futures.Future` set by the fill callback — zero blocking until the confirmation arrives — rather than a poll/sleep loop. `CLOSE_POSITIONS_ON_EMERGENCY` defaults to `false` (opt-in); API Panic Mode counter resets on any successful broker round-trip. **`_broker_submit()` adapter (bonus gap fix):** `_execute_order_internal` now calls `_broker_submit(order)` instead of the defunct `self.broker.submit_order(dict)`. The helper tries B02's `submit_order(B02Order)` first, falls back to B40's `place_order(**kwargs)` with full `OrderSide`/`OrderType` enum mapping, and returns a standardised `{"status", "order_id", "tradier_order_id"}` dict; logs an error and returns `{"status": "error"}` when no matching interface is found. |
...
| `SpyderR08_PaperTradingQtWorker.py` | 2,041 | QObject paper-trading worker driving the G05 dashboard's live paper session. Polls Tradier quotes, manages SPY price history, opens and closes credit spreads via D25, and emits status / position / P&L Qt signals. **PMR producer (v7):** when `SPYDER_PIVOT_MR_ENABLED=1`, the worker computes the S08 Pivot Mean-Reversion signal each poll using cached prior-session pivots, in-process Wilder RSI, and a max-min ATR proxy from the closes-only `_price_history`. Fired pivot bias takes precedence over the legacy regime bias in the neutral-signal branch and the full signal snapshot is stamped onto both `_open_spreads` entries and `_closed_trades` audit rows. |
...
**Package:** `SpyderS_Signals` | **13 files** | **10,081 LOC**

The S-Series generates custom proprietary signals beyond standard technical analysis: dark pool indices, gamma/delta exposure, SKEW, Black Swan risk indicators, pivot mean-reversion, short squeeze detection, NYSE breadth internals including NYMO (via TradingView scraping), macro economic data integration, and live options analytics (IVR, ATM IV, VRP).
...
| `SpyderS07_CustomMetricsOrchestrator.py` | 1,376 | Orchestrates all custom metrics (DIX, GEX, SKEW, Black Swan, TradingView breadth, options analytics) into a unified real-time signal feed consumed by strategies and risk modules. Emits `breadth_updated` signal with TICK/TRIN/ADD/NYMO snapshot. **Options analytics (v8):** caches a single `TradierClient` instance (`_options_tradier_client` / `_options_tradier_env`) and only recreates it when the active environment changes. Computes and publishes three options analytics metrics each cycle: **IVR** (Implied Volatility Rank 0–100 via `_compute_ivr`, persisted to `market_data/iv_history.json` with 252-entry rolling window; returns `nan` with fewer than 5 history points), **ATM IV** (average `c.iv × 100` for the 6 nearest-ATM contracts with `c.iv > 0` via `_compute_atm_iv`), and **HV20** / **VRP** (20-day annualised historical volatility via `_compute_hv20` from Tradier daily bars; VRP = ATM_IV − HV20, `nan` when HV20 is unavailable). All three metrics appear in `current_metrics`, `metric_quality`, and `_format_metrics` output. **NYMO relay (v8):** `_update_tv_breadth_metrics` now sets `updated_metrics["NYMO"] = snap.get("nymo", float("nan"))` so NYMO flows from S11 through to strategies, risk, and the G17 dialog. |
| `SpyderS08_PivotMeanReversionSignal.py` | 329 | Pivot Mean-Reversion (PMR) signal engine. Computes classic pivot levels (P/R1–R3/S1–S3) from prior-session OHLC and scores fade-resistance / fade-support setups via ATR-normalised distance, RSI extremity, regime context (RANGE/NORMAL/SWAN), GEX magnitude, VWAP slope, and max-pain alignment; applies penalties for news, edge-of-day, VIX spikes, and backwardation. Returns a typed `PivotMRSignal` with `direction`, `score` (0–100), `confidence`, `level_name`, `level_price`, `atr_distance`, and full `reasons` / `penalties` audit lists. Stateless, fully dependency-injected, no Qt or broker dependencies. Consumed by R08 (producer) and D25 (override). |
| `SpyderS08_ShortSqueezeDetector.py` | 1,330 | Short squeeze detector. Identifies rapid market spike patterns driven by short-covering dynamics using volume, price velocity, and short interest data. (Shares the S08 numeric slot with the PMR signal pending a future S-series renumbering.) |
...
| `SpyderS10_SentimentScraper.py` | 413 | Weekly sentiment data scrapers. Collects AAII individual investor survey data for contrarian positioning signals. (NAAIM scraping retired from live display in v8; data remains accessible via this module for research use.) |
| `SpyderS11_TradingViewInternals.py` | ~395 | NYSE market breadth internals via Playwright headless Chromium scraping of public TradingView symbol pages. Retrieves $TICK (USI:TICK), $ADD (USI:ADD), $TRIN (USI:TRIN.NY), and **$NYMO** (USI-NYMO) with majority-vote breadth regime classification. Browser tabs persist across polling cycles; ~2 s refresh for all 4 symbols during market hours, cached off-hours. No API key or authentication required. **NYMO (v8):** 4th symbol added to `_SYMBOLS`; `_NYMO_STRONG_BULL/BULL/BEAR/STRONG_BEAR` thresholds defined; `_classify_breadth()` now casts a four-way majority vote; `_stub()` returns `"nymo": float("nan")`. |
...
**Package:** `SpyderT_Testing` | **125 files** | **94,957 LOC**
...
| `SpyderT09_TestDashboard.py` | 3,370 | Interactive manual QA dashboard for visual testing of all G-Series GUI components with live interaction. Tagged `@pytest.mark.manual` to prevent CI import. |
...
### Series Integration Tests (T100–T131 range)
...
| `SpyderT117_PSeries.py` | 999 | P-Series (Portfolio) test suite. Stale `skipif` guard (predicated on a missing `get_risk_manager` symbol that has since been present in E01) removed; P01 tests now import unconditionally. |
...
| `SpyderT120_S07OptionsAnalytics.py` | 516 | **S07 options analytics test suite (30 tests — all passing). Six test classes: `TestComputeAtmIv`** (ATM IV extraction from option contracts, zero-IV filtering, annualised percent output); **`TestComputeIvr`** (52-week IVR rank at high/low/midpoint, history trimming to 252 entries, `nan` guard for <5 data points); **`TestComputeHv20`** (HV20 annualised calculation, insufficient-bar guard returning `None`); **`TestUpdateOptionsAnalyticsMetrics`** (no-credentials early-exit guard, full API success path with IVR/ATM_IV/VRP in metrics dict, chain-fetch error handling, client caching verification — same client reused on second call, env-change client recreation); **`TestFormatMetricsOptionsAnalytics`** (IVR/ATM_IV/VRP appear in `_format_metrics` output with correct formatting); **`TestMetricDataStructures`** (IVR/ATM_IV/VRP keys present in `current_metrics` and `metric_snapshot`, quality flags in `metric_quality`, `_options_tradier_client` and `_options_tradier_env` initialised to `None`). Uses four-patch filesystem isolation (`exists`, `read_text`, `write_text`, `mkdir`); mock client pre-injected to avoid lazy broker import path issues. |
...
| `SpyderT129_ProtocolCompliance.py` | 92 | Runtime protocol-compliance suite. Verifies E01 exposes `validate_signal()` and accepts `tradier_client`; F10 returns `None` from `calculate_all_indicators`; C04 `get_current_condition()` returns a valid `MarketCondition` member; F00 `AnalyticsProviderProtocol` exports `calculate_all_indicators`. Also includes `RegimeCanonicalWiringTest`: asserts D30 imports `UnifiedRegimeEngine` with L09 preceding E21, and Y01 declares `L09_AVAILABLE` with L09 checked before E21. Executed by the Q10 CI gate. |
| `SpyderT131_PivotMRSignalTests.py` | 301 | S08 Pivot Mean-Reversion signal test suite (24 tests, all passing). Six test classes cover: no-fire conditions, fade-resistance fire paths, fade-support fire paths, bonuses (regime/GEX/ATR/RSI/VWAP/max-pain), penalties (news / edge-of-day / VIX / backwardation), fire-gate threshold, level selection (deepest-breach semantics), and bidirectional choice arbitration. Standard pivot fixtures: P=587.67, R1=592.33, R2=596.67, R3=601.33, S1=583.33, S2=578.67, S3=574.33. |
...
| `test_g13_signal_monitor_styles.py` | ~100 | Regression guard for G13 widget stylesheets. Asserts `TrafficLightButton` uses valid single-brace QSS syntax and that `SignalMonitorPanel` buttons carry non-empty stylesheets with expected CSS properties. Headless offscreen, no display required. |
...
| `SpyderU03_DateTimeUtils.py` | 1,841 | Comprehensive date/time utilities. Market hours detection, session classification (pre/regular/post), timezone conversion, and trading day arithmetic. Provides `now_utc()` and `now_et()` helpers for timezone-aware timestamps throughout the system. |
...
| `SpyderU05_NetworkUtils.py` | 491 | Network utilities. Connection health checks, retry-with-backoff for HTTP calls, SSL certificate validation helpers, and timeout wrappers. (Legacy Interactive Brokers gateway/TWS endpoint config removed.) |
...
| `SpyderY01_MarketSenseAgent.py` | 524 | Continuously monitors market conditions. Emits typed regime-change and anomaly events when the detected market state shifts. Resolves regime through `SpyderL09_UnifiedRegimeEngine` (canonical), falling back to E21 HMM when L09 is unavailable. |
...
*End of Spyder Codebase Review v18 (v8) — April 18, 2026*
