...
DEFAULT_SYMBOL_CONFIG = {
    ...
    "hidden_symbols": {
        ...
        "ADDITIONAL_INTERNALS": {
            "symbols": ["ADVN-NYSE", "DECN-NYSE", "UVOL-NYSE", "DVOL-NYSE", "VOLD-NYSE", "NYHL-NYSE"],
            "update_frequency": 5,
            "priority": "MEDIUM"
        },
        "SECTOR_ETFS": {
...
class MarketConfigManager:
    ...
    def _load_schemas(self):
        ...
        self.schemas['symbols'] = {
            ...
            "properties": {
                "visible_symbols": {
                    "type": "object",
                    "additionalProperties": SYMBOL_SCHEMA
                },
                "hidden_symbols": {
                    "type": "object",
                    "additionalProperties": SYMBOL_SCHEMA
    ...
    def get_change_history(self, category: Optional[str] = None,
                    ...
                    if path.exists():
                        ...
                        if current_modified > last_modified:
                            ...
                            if self.environment in path.parts:
                                self.logger.info(f"Detected change in environment config: {category}")
                            else:
        ...
        if category == 'symbols':
            ...
            if 'max_daily_loss' in config and 'max_position_size' in config:
                if config['max_daily_loss'] < config['max_position_size'] * 0.05:
                    result.warnings.append("Max daily loss may be too low relative to position size")
...
if __name__ == "__main__":
    ...
    for change in changes:
        print(f"  {change.timestamp}: {change.category}.{change.key} "
