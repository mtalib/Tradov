...
class TransactionCostAnalyzer:
    ...
    def analyze_venue_performance(self, 
        ...
        for order_id, execs in self.executions.items():
    ...
    def generate_tca_report(self,
                           start_time: datetime,
                           end_time: datetime,
                           group_by: Optional[str] = None) -> TCAReport:
        """
...
if __name__ == "__main__":
    ...
    print(f"  Total Cost: {breakdown.total_cost_bps:.1f} bps (${breakdown.total_cost_dollars:,.2f})")
    print(f"  Spread Cost: ${breakdown.spread_cost:,.2f}")
    print(f"  Impact Cost: ${breakdown.impact_cost:,.2f}")
    print(f"  Commission: ${breakdown.commission_cost:,.2f}")
    print(f"  Slippage: {breakdown.slippage:.1f} bps")
    
    # Benchmark comparison
    print("\n📈 Benchmark Comparison:")
    comparison = tca.compare_to_benchmarks(test_order.order_id)
    
    print(f"  Arrival Shortfall: {comparison.arrival_shortfall_bps:.1f} bps")
    print(f"  VWAP Shortfall: {comparison.vwap_shortfall_bps:.1f} bps")
    print(f"  TWAP Shortfall: {comparison.twap_shortfall_bps:.1f} bps")
    print(f"  Best Benchmark: {comparison.best_benchmark}")
    print(f"  Quality Rating: {comparison.quality_rating.value}")
    ...
    if report:
        ...
        print(f"  Avg Cost: {report.avg_cost_bps:.1f} bps")
        print(f"  Recommendations: {report.recommendations}")
    
    # Shutdown
    tca.shutdown()
    print("\n✅ Transaction cost analysis test completed")
