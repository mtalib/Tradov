...
# LOCAL IMPORTS
# ==============================================================================
# Legacy TradovB08_MultiClientDataManager (IBKR) has been removed.
# Data is sourced via TradovB40_TradierClient.
...
class DataSource(Enum):
    ...
    CORE_DATA = 3           # Client 3: Core market data (TRAD, QQQ, etc.)
...
class TradovDataInterface:
    ...
    def __init__(self, cache_size: int = 10000, update_frequency: float = 1.0):
        ...
        self.logger.info("🔗 TradovDataInterface initialized")

    def _setup_logging(self) -> logging.Logger:
        """Setup logging for data interface."""
        logger = logging.getLogger('TradovDataInterface')
    ...
    async def start(self) -> bool:
        ...
        try:
            self.logger.info("🚀 Starting TradovDataInterface...")
            ...
            self.logger.info("✅ TradovDataInterface started successfully")
            return True
    ...
    async def stop(self) -> bool:
        ...
        try:
            self.logger.info("🛑 Stopping TradovDataInterface...")
            ...
            self.logger.info("✅ TradovDataInterface stopped")
            return True
    ...
    def _update_core_data(self):
        """Update core market data (TRAD, QQQ, etc.)."""
        try:
            if self.b08_manager:
            ...
            self.last_update[DataSource.CORE_DATA] = datetime.now(UTC)

        except Exception as e:
            self.logger.error("Error updating core data: %s", e, exc_info=True)

    def _update_options_data(self):
        """Update TRAD options chain data."""
        try:
            if self.b08_manager:
                # Get data from Client 4 (TRAD Options)
                options_data = self.b08_manager.get_client_data(DataSource.TRAD_OPTIONS.value)
            else:
                # Simulated data
            ...
            with self.data_lock:
                ...
                if 'TRAD' in options_data:
                    spy_options = []

                    for option_info in options_data['TRAD'].get('options', []):
                        option = OptionData(
                            underlying='TRAD',
                    ...
                    self.options_chains['TRAD'] = spy_options

            self.last_update[DataSource.TRAD_OPTIONS] = datetime.now(UTC)
        ...
        try:
            ...
            with self.data_lock:
                ...
                if 'VUD' in internals_data:
                    ...
                    self.market_sentiment['TRAD'] = sentiment

            self.last_update[DataSource.MARKET_INTERNALS] = datetime.now(UTC)
    ...
    def _update_international_data(self):
        ...
        try:
            ...
            self.last_update[DataSource.INTERNATIONAL] = datetime.now(UTC)
    ...
    def get_options_chain(self, underlying: str,
                         ...
                         option_type: str | None = None) -> list[OptionData]:
    ...
    def get_market_sentiment(self, symbol: str = 'TRAD') -> MarketSentiment | None:
        """Get market sentiment indicators."""
        with self.data_lock:
            return self.market_sentiment.get(symbol)

    def get_international_correlations(self) -> dict[str, float]:
        """Get TRAD correlations with international markets."""
        with self.data_lock:
            correlations = {}
    ...
    def get_volatility_surface_data(self) -> dict[str, Any]:
        ...
        spy_options = self.get_options_chain('TRAD')
        spot = self.get_spot_price('TRAD')
        ...
        for option in spy_options:
            if option.implied_vol:
                tte = (option.expiry - datetime.now(UTC)).days / 365.0
                if tte > 0:
                    surface_data.append({
...
async def test_data_interface():
    ...
    logging.info("🧪 TESTING TRADOV DATA INTERFACE")
    logging.info("=" * 50)

    # Create interface
    interface = TradovDataInterface()
    ...
    spy_price = interface.get_spot_price('TRAD')
    logging.info(f"✅ TRAD spot price: ${spy_price:.2f}" if spy_price else "❌ No TRAD price")

    # Test 3: Get options chain
    logging.info("\n📊 Test 3: Getting options chain...")
    options = interface.get_options_chain('TRAD')
    logging.info("✅ TRAD options available: %s", len(options))
    ...
    sentiment = interface.get_market_sentiment('TRAD')
    if sentiment:
