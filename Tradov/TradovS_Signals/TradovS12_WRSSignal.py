...
Caching:
    Daily OHLCV data cached to ~/.tradov/wrs_cache/ with 4-hour TTL.
    Avoids repeated API calls on every run.

Update Cadence:
    Daily close — WRS moves on weekly/monthly timescales. No intraday
    refresh needed. Integrated with TradovS07_CustomMetricsOrchestrator.
...
Integration:
    ...
    - TradovS07_CustomMetricsOrchestrator: exposes wrs to MetricSnapshot
"""
...
# Cache
_CACHE_DIR = Path("~/.tradov/wrs_cache").expanduser()
CACHE_TTL_HOURS = 4
...
class _TradierMarketClient:
    ...
    TradovB40_TradierClient covers order management endpoints only.
    """
...
class WRSSignal:
    """
    Walmart Recession Signal — macro regime overlay for Tradov.
