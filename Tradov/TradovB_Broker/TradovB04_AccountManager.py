...
Module Description:
    ...
    - Works with fixed TradovB01_TradovClient implementation
...
Dependencies Fixed:
    ...
    - TradovClient integration uses our fixed implementation
...
_DEFAULT_CACHE_PATH = Path.home() / ".tradov" / "account_cache.json"
...
# ==============================================================================
# TRADOV MODULE IMPORTS WITH SAFE FALLBACKS
# ==============================================================================
...
try:
    from TradovA_Core.TradovA05_EventManager import Event, EventManager, EventType, get_event_manager  # noqa: E501
...
# TradovClient (B01_TradovClient removed — legacy broker) — use Tradier via TradovB40_TradierClient
HAS_TRADOV_CLIENT = False
...
class AccountManager:
    ...
    def __init__(self, tradov_client: TradovClient | None = None,
        ...
        Args:
            tradov_client: TradovClient instance (creates fallback if None)
        ...
        if HAS_ERROR_HANDLER:
            self.error_handler = TradovErrorHandler(self.logger)
        else:
            self.error_handler = TradovErrorHandler(self.logger)

        # TradovClient (use provided or create fallback)
        if tradov_client:
            self.tradov_client = tradov_client
        elif HAS_TRADOV_CLIENT:
            try:
                self.tradov_client = TradovClient()
            except Exception as e:
                self.logger.warning("Could not create TradovClient: %s", e, exc_info=True)
                self.tradov_client = TradovClient()  # Use fallback
        else:
            self.tradov_client = TradovClient()  # Use fallback
        ...
        self.logger.info(f"Module availability - TradovClient: {HAS_TRADOV_CLIENT}, "
    ...
    def initialize(self) -> bool:
        ...
        try:
            ...
            if not self.tradov_client.is_connected():
                self.logger.warning("TradovClient not connected - will work in offline mode")

            # Subscribe to account events if available
            if hasattr(self.tradov_client, 'add_account_callback'):
    ...
    def _sync_accounts_with_broker(self):
        ...
        try:
            ...
            accounts = self.tradov_client.get_managed_accounts()
    ...
    def _update_account_data(self, account_id: str):
        ...
        try:
            ...
            account_values = self.tradov_client.get_account_values(account_id)
                    ...
                    try:
                        account = AccountInfo(
                            ...
                            last_updated=datetime.fromisoformat(
                                raw.get("last_updated", datetime.now(UTC).isoformat())
                            ),
                        )
    ...
    def get_comprehensive_status(self) -> dict[str, Any]:
        ...
        with self._account_lock:
            return {
                ...
                'module_availability': {
                    'tradov_client': HAS_TRADOV_CLIENT,
...
def create_account_manager(tradov_client: TradovClient | None = None,
                          event_manager: EventManager | None = None,
    ...
    return AccountManager(tradov_client, event_manager, config)
