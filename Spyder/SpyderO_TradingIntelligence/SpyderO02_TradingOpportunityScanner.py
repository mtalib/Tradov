...
Module: SpyderO02_TradingOpportunityScanner.py
...
Module Description:
    ...
    modules (SpyderN, SpyderF, SpyderS) to identify, rank, and recommend optimal trading
...
try:
    ...
    from SpyderF_Analysis.SpyderF11_GreeksAggregator import GreeksAggregator
    from SpyderF_Analysis.SpyderF04_VolatilityAnalysis import VolatilityAnalyzer
    from SpyderF_Analysis.SpyderF08_VolatilityRegime import VolatilityRegimeDetector
...
class TradingOpportunityScanner:
    ...
    def _initialize_analytics_connections(self) -> None:
        ...
        try:
            ...
            if OPTIONS_ANALYTICS_AVAILABLE:
                self.analytics_modules['greeks'] = OptionsGreeksCalculator()
                self.analytics_modules['vol_surface'] = VolatilitySurfaceBuilder()
                self.analytics_modules['flow_tracker'] = OptionsFlowTracker()
                self.analytics_modules['gamma_exposure'] = GammaExposureAnalyzer()
                self.analytics_modules['iv_engine'] = ImpliedVolatilityEngine()
                self.logger.info("Connected to options analytics modules")
    ...
    def _create_iron_condor_opportunity(self, context: OpportunityContext, 
                                      market_data: pd.DataFrame) -> Optional[TradingOpportunity]:
        ...
        try:
            ...
            breakeven_low = put_strike_short - estimated_credit
            breakeven_high = call_strike_short + estimated_credit
        ...
        with self._lock:
            return {
                ...
                'analytics_connections': {
                    ...
                    'options_analytics': OPTIONS_ANALYTICS_AVAILABLE,
...
if __name__ == "__main__":
    print("=" * 80)
    print("SPYDER O02 - TRADING OPPORTUNITY SCANNER DEMONSTRATION")
    print("=" * 80)
    ...
    print(f"\nTrading Opportunity Scanner initialized")
    status = scanner.get_scanner_status()
    print(f"Scanner Status: {'Active' if not status['is_scanning'] else 'Scanning'}")
    print(f"Analytics Connections:")
    for connection, available in status['analytics_connections'].items():
    ...
    print(f"Current VIX: {vix_data[-1]:.1f}")
    ...
    if opportunities:
        for i, opp in enumerate(opportunities[:5], 1):  # Show top 5
            print(f"\n{i}. {opp.strategy_name} ({opp.opportunity_type.value.upper()})")
            print(f"   Priority: {opp.priority.value.upper()}")
            print(f"   Confidence: {opp.confidence_score:.1%}")
            print(f"   Risk/Reward: {opp.risk_reward_ratio:.2f}")
            print(f"   Probability of Profit: {opp.probability_of_profit:.1%}")
            print(f"   Expected Profit: ${opp.expected_profit:.2f}")
            print(f"   Maximum Loss: ${opp.maximum_loss:.2f}")
            print(f"   Market Bias: {opp.market_bias.value}")
            
            if opp.strikes:
                print(f"   Strikes: {[f'${s:.0f}' for s in opp.strikes]}")
            
            print(f"   Supporting: {', '.join(opp.supporting_indicators[:3])}")
            
            if opp.risk_factors:
                print(f"   Risk Factors: {', '.join(opp.risk_factors[:2])}")
    else:
        print("No opportunities found matching criteria")
    
    # Test strategy comparison
    print(f"\nStrategy Comparison for Bullish View:")
    print("=" * 50)
    ...
    print(f"Market View: {comparison.market_view.upper()}")
    print(f"Recommended Strategy: {comparison.recommended_strategy}")
    print(f"Efficiency Score: {comparison.efficiency_score:.2f}")
    print(f"Reasoning: {comparison.reasoning}")
    
    print(f"\nStrategy Options:")
    for strategy in comparison.strategies:
        print(f"  • {strategy['name']}: {strategy.get('efficiency_score', 0):.2f}")
    ...
    if summary.get('total_opportunities', 0) > 0:
        ...
        print(f"High Confidence Count: {summary.get('high_confidence_count', 0)}")
        print(f"Total Expected Profit: ${summary.get('total_expected_profit', 0):.2f}")
        print(f"Total Risk Capital: ${summary.get('total_max_loss', 0):.2f}")
        
        if summary.get('type_breakdown'):
            print(f"Opportunity Types:")
            for opp_type, count in summary['type_breakdown'].items():
                print(f"  • {opp_type.replace('_', ' ').title()}: {count}")
    
    print(f"\n{('='*80)}")
    print("TRADING OPPORTUNITY SCANNER READY!")
    print("✅ Multi-strategy opportunity identification")
    print("✅ Cross-strategy comparison and ranking")
    print("✅ Risk-adjusted confidence scoring")
    print("✅ Integration with existing Spyder analytics")
    print("✅ Real-time market context awareness")
