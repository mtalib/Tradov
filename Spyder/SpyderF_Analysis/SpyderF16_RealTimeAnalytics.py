...
class RealTimeAnalyticsEngine:
    ...
    async def _cleanup_loop(self) -> None:
        ...
        while self.running:
            try:
                current_time = datetime.now()
                cutoff_time = current_time - timedelta(hours=METRICS_RETENTION_HOURS)
                
                # Clean old metrics from buffers
                for stream_type, buffer in self.metrics_buffer.items():
...
def get_realtime_analytics_engine(config: Optional[Dict[str, Any]] = None) -> RealTimeAnalyticsEngine:
    """
