...
Module: TradovC22_FactorDataProvider.py
...
try:
    from Tradov.TradovC_MarketData.TradovC29_DataProviderRouter import get_data_provider as _get_c29_provider  # noqa: E501
    _C29_AVAILABLE = True
except ImportError:
...
try:
    ...
    from TradovU03_DateTimeUtils import TradingTimeUtils  # noqa: F401
except ImportError:
...
# Tradov integrations
try:
    from TradovC01_DataFeed import get_data_feed_manager
    C01_AVAILABLE = True
except ImportError:
    C01_AVAILABLE = False
...
# Factor Data Sources
FACTOR_SOURCES = {
    'MKT': {'provider': 'yahoo', 'symbol': 'TRAD', 'transformation': 'excess_return'},
...
class FactorDataProvider:
    ...
    def _initialize_factor_models(self) -> None:
        ...
        for model_name, _model_config in FACTOR_MODELS.items():
    ...
    def _initialize_external_connections(self) -> None:
        """Initialize connections to other Tradov modules."""
        try:
            # Connect to C01 DataFeed
            if C01_AVAILABLE:
                self.data_feed_manager = get_data_feed_manager()
                self.logger.info("Connected to TradovC01_DataFeed")
    ...
    async def get_factor_model_data(
        ...
        try:
            ...
            if model_name in self.factor_models:
                self.factor_models[model_name].last_updated = datetime.now(UTC)
    ...
    async def _get_factor_data(
        ...
        try:
            ...
            if self.enable_caching and cache_key in self.factor_cache:
                ...
                cache_age = (datetime.now(UTC) - cached_data['timestamp']).total_seconds() / 3600
    ...
    async def _calculate_excess_return(self, factor_name: str, start_date: datetime, end_date: datetime) -> pd.Series | None:  # noqa: E501
        ...
        try:
            # Try C29 for TRAD first; fall back to yfinance
            hist_data: pd.DataFrame | None = None
            if _C29_AVAILABLE:
                try:
                    client = _get_c29_provider()
            ...
            if hist_data is None or hist_data.empty:
                if YFINANCE_AVAILABLE:
                    self.logger.debug("C29 unavailable for TRAD — using yfinance fallback")
                    spy = yf.Ticker('TRAD')
    ...
    async def _calculate_gamma_flip_level(self, factor_name: str, start_date: datetime, end_date: datetime) -> pd.Series | None:  # noqa: E501
        ...
        try:
            ...
            current_price = 400  # TRAD reference price
    ...
    def _quality_monitoring_loop(self) -> None:
        ...
        while not self._stop_event.is_set():
            try:
                ...
                for factor_name in FACTOR_SOURCES:
    ...
    def get_factor_model_info(self, model_name: str) -> dict[str, Any] | None:
...
def get_factor_data_provider(config: dict[str, Any] | None = None) -> FactorDataProvider:
    """
...
async def main():
    ...
    logging.info("🎯 TRADOV C22 - Factor Data Provider")
    logging.info("=" * 80)
    ...
    try:
        ...
        dates = pd.date_range(start=datetime.now(UTC) - timedelta(days=60), end=datetime.now(UTC), freq='D')  # noqa: E501
        ...
        if exposure:
            ...
            logging.info("   ❌ Failed to calculate exposures")
