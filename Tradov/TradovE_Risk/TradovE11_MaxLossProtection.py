...
Tradov Version: 1.0
Module: TradovE11_MaxLossProtection.py
...
from Tradov.TradovA_Core.TradovA05_EventManager import EventManager, Event, EventType, get_event_manager  # noqa: E501, F401
...
class MaxLossProtection:
    ...
    def _save_emergency_state(self):
        ...
        try:
            emergency_file = Path(f"data/risk/emergency_{datetime.now(UTC):%Y%m%d_%H%M%S}.json")
    ...
    def _create_recovery_plan(self, breach_event: BreachEvent) -> RecoveryPlan:
        ...
        cooldown_end = datetime.now(UTC) + timedelta(minutes=cooldown_minutes)
    ...
    def _update_status(self):
        ...
        with self._lock:
            ...
            active_plans = [p for p in self.recovery_plans
                          if p.cooldown_end > datetime.now(UTC)]
    ...
    def reset_limits(self, limit_type: str = None):
        ...
        if limit_type:
            if limit_type in self.limits:
                ...
                self.limits[limit_type].last_reset = datetime.now(UTC)
                self.logger.info("Reset %s limit", limit_type)
        else:
            # Reset all limits
            for limit in self.limits.values():
                limit.current_loss = 0
                limit.last_reset = datetime.now(UTC)
...
if __name__ == "__main__":
    ...
    can_open, reason = protection.can_open_position("IronCondor", "TRAD")
