...
from Spyder.SpyderQ_Scripts.SpyderQ06_LaunchDashboardDirect import main as direct_launcher
...
if __name__ == "__main__":
    # Prefer the main unified dashboard; keep legacy direct dashboard fallback.
    if len(sys.argv) == 1:
        sys.argv = [sys.argv[0], "--mode", "paper", "--gui"]
    exit_code = main_launcher()
    if exit_code != 0 and not _qt_app_exists():
        raise SystemExit(direct_launcher())
