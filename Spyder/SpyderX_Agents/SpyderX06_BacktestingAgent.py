...
class SpyderX06_BacktestingAgent:
    ...
    async def backtest_strategy(
        ...
        hypothesis: BacktestHypothesis | None = None
    ) -> BacktestResults:
        """
    ...
    async def optimize_parameters(
        ...
        async def objective_function(params):
            ...
            test_config.parameters.update(
                dict(zip(bounds.keys(), params, strict=False))
            )
        ...
        optimal_params = dict(zip(bounds.keys(), result.x, strict=False))
...
Provide insights about when each strategy might be preferred."""
    ...
    def _validate_data(self, data: BacktestData) -> bool:
        ...
        return all(col in data.price_data.columns for col in required_columns)
    ...
    def get_test_history(
        ...
        test_type: BacktestType | None = None
...
# MODULE FUNCTIONS
# ==============================================================================
def create_backtesting_agent(config: dict[str, Any] | None = None) -> SpyderX06_BacktestingAgent:
    """
...
_module_instance: SpyderX06_BacktestingAgent | None = None

def get_module_instance(config: dict[str, Any] | None = None) -> SpyderX06_BacktestingAgent:
    """
...
if __name__ == "__main__":
    async def test_agent():
        ...
        for _i, _hypothesis in enumerate(hypotheses[:3]):
            pass

        # 3. Analyze results
        analysis = await agent.analyze_results(results)

        for _strength in analysis['strengths']:
            pass

        for _weakness in analysis['weaknesses']:
            pass

        for _rec in analysis['recommendations'][:3]:
            pass
        ...
        for param, _value in optimization.optimal_parameters.items():
            strategy_config.parameters.get(param, 0)
