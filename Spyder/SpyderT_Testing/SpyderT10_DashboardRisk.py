    ...
    for attempt_name, import_func in import_attempts:
        try:
            ...
            if hasattr(module, 'show_risk_parameters_dialog'):
                ...
                print("⚠️  Module found but missing show_risk_parameters_dialog function")
...
# MODULE FUNCTIONS
# ==============================================================================
def find_risk_dialog_module() -> str | None:
    """
    ...
    print("🔍 Searching for SpyderG06_RiskParametersDialog.py...")
...
def create_gui_init_file():
    ...
    if os.path.exists(gui_dir) and not os.path.exists(init_file):
        try:
            ...
            print("✅ Created __init__.py in SpyderG_GUI directory")
            return True
        except Exception as e:
            print(f"❌ Failed to create __init__.py: {e}")
            return False
    elif os.path.exists(init_file):
        print("✅ __init__.py already exists in SpyderG_GUI")
        return True
    else:
        print("❌ SpyderG_GUI directory doesn't exist, cannot create __init__.py")
        return False
...
_integrator_instance: DashboardRiskIntegrator | None = None
