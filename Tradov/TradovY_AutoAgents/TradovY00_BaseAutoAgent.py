...
Description:
    ...
    TradovY agents differ from TradovX agents:
      - TradovX agents are on-demand utilities (stateless, called when needed)
      - TradovY agents are autonomous daemons (persistent state, scheduled,
...
try:
    ...
    logger = TradovLogger.get_logger("TradovY_AutoAgents")
except ImportError:
...
try:
    from Tradov.TradovI_Integration.TradovI06_AgentMessageBus import (
        MessagePriority,
    )
...
def _build_inference_backend(ollama_config: "OllamaConfig") -> InferenceBackend:
    """Construct an InferenceBackend from TRADOV_LLM_BACKEND env var.

    TRADOV_LLM_BACKEND=ollama    → OllamaBackend  (default)
    TRADOV_LLM_BACKEND=openvino  → OpenVINOBackend (Intel CPU/GPU/NPU)
    ...
    """
    backend_name = os.getenv("TRADOV_LLM_BACKEND", "ollama").lower().strip()
...
class BaseAutoAgent(ABC):
    """Abstract base class for all TradovY autonomous agents.
