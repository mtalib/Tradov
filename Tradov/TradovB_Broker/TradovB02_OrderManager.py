...
class Order:
    """
    Order representation for the Tradov trading system.
...
class OrderManager:
    ...
    def submit_limit_with_walk(
        ...
        3. **Underlying price abort** — if ``underlying_spot_fn`` and
           ...
           ``|TRAD_now - TRAD_entry| / TRAD_entry > underlying_abort_pct``.  This
        ...
        Args:
            ...
            max_walk_ticks: Maximum number of price walks before giving up
                (default 10).
            underlying_entry_price: TRAD price at signal generation time.  Required
            ...
            underlying_spot_fn: Zero-argument callable that returns the current
                TRAD price.  Called once per tick inside the walk loop.
        ...
        walk_start_time = datetime.now(UTC)

        for tick in range(max_walk_ticks + 1):
            # ── Timeout guard ─────────────────────────────────────────────────
            elapsed = (datetime.now(UTC) - walk_start_time).total_seconds()
    ...
    def _build_execution_feed_payload(
        ...
        return {
            ...
            "session_id": self._execution_session_id,
            "published_ts": datetime.now(UTC).isoformat(),
            "data": data,
        }
