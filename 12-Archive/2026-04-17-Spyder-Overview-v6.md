# Spyder Trading System — Codebase Review v17 (v6)
**Date:** April 17, 2026
**Scope:** Current-state inventory of the Spyder codebase
**Prepared by:** Claude Sonnet 4.6
**Supersedes:** 2026-04-14-Spyder-Overview-v5.md

---

## Change Summary (v5 → v6)

This revision incorporates one targeted GUI enhancement made on April 17, 2026 on the `refactor/g05-widget-extraction` branch:

- **G05 mode-switch state preservation**: The "ORDERS & POSITIONS" and "P&L PERFORMANCE" table containers now save their contents when switching trading modes (PAPER ↔ LIVE) and restore them when switching back. Two new per-mode snapshot dictionaries (`_pnl_stats_by_mode`, `_positions_snapshot_by_mode`) are initialised at construction. Two new helper methods (`_snapshot_positions_table()`, `_restore_positions_snapshot()`) serialise and reconstruct the `QTreeWidget` rows. `_refresh_pnl_table()` persists the augmented stats dict after every update. `_apply_mode_change()` snapshots the outgoing mode before committing and restores the incoming mode's saved state immediately after. G05 LOC updated from 3,535 → 3,608.
- **G-Series and codebase metrics updated** to reflect the G05 LOC increase (~+73 lines).
...
| Production files (A–Z, excluding T) | **358** |
| Test files (T-series) | **123** |
| Total lines of code | **425,373** |
| Production LOC | **331,233** |
| Test LOC | **94,140** |
| Series (packages) | **25** |
...
| G | `SpyderG_GUI` | 25 | 18,686 | PySide6 interface, dashboards, charts, widgets |
...
| T | `SpyderT_Testing` | 123 | 94,140 | Full pytest test suite covering all series |
...
**Package:** `SpyderG_GUI` | **25 files** | **18,686 LOC**
...
**Package:** `SpyderT_Testing` | **123 files** | **94,140 LOC**
