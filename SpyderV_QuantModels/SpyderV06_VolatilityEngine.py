...
class HestonParameters:
    ...
    def validate(self) -> bool:
        ...
        return (self.v0 > 0 and self.theta > 0 and self.kappa > 0 and 
                self.sigma > 0 and -1 <= self.rho <= 1 and
                2 * self.kappa * self.theta > self.sigma**2)  # Feller condition
...
class GARCHParameters:
    ...
    def validate(self) -> bool:
        ...
        return (self.omega > 0 and self.alpha >= 0 and self.beta >= 0 and
                self.alpha + self.beta < 1)  # Stationarity condition
...
def _garch_likelihood(params, returns):
    ...
    for t in range(n):
        ...
        log_likelihood -= 0.5 * (np.log(2 * np.pi * variance) + returns[t]**2 / variance)
...
def _heston_characteristic_function(phi, S, K, T, r, q, v0, theta, kappa, sigma, rho):
    ...
    A2 = (kappa * theta) / (sigma**2) * ((xi - d) * T - 2 * np.log((1 - np.exp(-d * T) * (xi - d) / (xi + d)) / 2))
    A3 = (v0 / sigma**2) * (xi - d) * (1 - np.exp(-d * T)) / (1 - np.exp(-d * T) * (xi - d) / (xi + d))
...
class SpyderVolatilityEngine:
    ...
    def __init__(self, 
        ...
        self.max_data_points = self.config.get('max_data_points', 2520)  # ~10 years daily
        self.calibration_frequency_hours = self.config.get('calibration_frequency_hours', 24)
    ...
    async def get_volatility(self,
                           ...
                           model: VolatilityModel = VolatilityModel.AUTO) -> float:
        """
    ...
    async def forecast_volatility(self,
        ...
        try:
            ...
            if model == VolatilityModel.GARCH:
                ...
                forecast = await self._rough_vol_forecast(horizon_days, confidence_level)
            else:
                # Historical projection
                forecast = await self._historical_forecast(horizon_days, confidence_level)
            
            # Add regime analysis
            forecast.regime_probability = self._analyze_volatility_regime(forecast.volatility_forecast)
            ...
            return VolatilityForecast(
                ...
                confidence_intervals=(np.full(horizon_days, 0.15), np.full(horizon_days, 0.25)),
                model_confidence=0.5,
                regime_probability={regime: 0.25 for regime in VolatilityRegime},
                warnings=[f"Forecast failed: {str(e)}"]
            )
    
    async def generate_volatility_surface(self,
                                        spot_price: float,
                                        strikes: List[float],
                                        maturities: List[float],
                                        model: VolatilityModel = VolatilityModel.HESTON) -> VolatilitySurface:
        """
        ...
        try:
            ...
            if model == VolatilityModel.HESTON:
                surface = await self._generate_heston_surface(spot_price, strikes, maturities)
            else:
                # Default to flat surface with smile
                surface = await self._generate_default_surface(spot_price, strikes, maturities)
    ...
    def _set_default_parameters(self):
        ...
        self.heston_params = HestonParameters(v0=0.04, theta=0.04, kappa=2.0, sigma=0.3, rho=-0.7)
    ...
    async def _get_heston_volatility(self, horizon: VolatilityHorizon) -> float:
        ...
        if horizon == VolatilityHorizon.INTRADAY:
            ...
            blended_variance = (self.heston_params.v0 * mean_reversion + 
    ...
    async def _get_garch_volatility(self, horizon: VolatilityHorizon) -> float:
        ...
        current_variance = self.garch_conditional_variance[-1] if len(self.garch_conditional_variance) > 0 else self.garch_params.unconditional_variance
        ...
        if horizon == VolatilityHorizon.INTRADAY:
            ...
            forecasted_variance = self._garch_multi_step_forecast(current_variance, days)
    ...
    async def _garch_forecast(self, horizon_days: int, confidence_level: float) -> VolatilityForecast:
        ...
        for t in range(horizon_days):
            ...
            variance = (self.garch_params.omega + 
                       self.garch_params.persistence * variance)
    ...
    async def _heston_forecast(self, horizon_days: int, confidence_level: float) -> VolatilityForecast:
        ...
        for t in range(horizon_days):
            ...
            expected_variance = (self.heston_params.v0 * mean_reversion + 
                               self.heston_params.theta * (1 - mean_reversion))
    ...
    async def _rough_vol_forecast(self, horizon_days: int, confidence_level: float) -> VolatilityForecast:
        ...
        n_paths = 100
        paths = _rough_vol_simulation(hurst, self.rough_vol_params.xi, 
                                     self.rough_vol_params.theta, horizon_days, dt, n_paths)
    ...
    async def _historical_forecast(self, horizon_days: int, confidence_level: float) -> VolatilityForecast:
    ...
    async def _generate_heston_surface(self,
                                     spot_price: float,
                                     strikes: List[float],
                                     maturities: List[float]) -> VolatilitySurface:
        ...
        for i, strike in enumerate(strikes):
            for j, maturity in enumerate(maturities):
                ...
                moneyness_effect = self._calculate_smile_adjustment(moneyness[i], maturity)
    ...
    async def _generate_default_surface(self,
                                      spot_price: float,
                                      strikes: List[float],
                                      maturities: List[float]) -> VolatilitySurface:
        ...
        for i, strike in enumerate(strikes):
            for j, maturity in enumerate(maturities):
                # Simple smile model
                smile_adjustment = self._calculate_smile_adjustment(moneyness[i], maturity)
    ...
    def _update_garch_conditional_variance(self):
        ...
        for t in range(1, n):
            conditional_variance[t] = (self.garch_params.omega + 
    ...
    def _garch_multi_step_forecast(self, current_variance: float, steps: int) -> float:
        ...
        if steps == 1:
            return self.garch_params.omega + self.garch_params.persistence * current_variance
        ...
        if persistence < 0.9999:
            forecast_var = (unconditional_var + 
                          (persistence**steps) * (current_variance - unconditional_var))
        else:
            # Near unit root case
    ...
    def _analyze_volatility_regime(self, volatility: float) -> Dict[VolatilityRegime, float]:
    ...
    def _get_cached_forecast(self, cache_key: str) -> Optional[VolatilityForecast]:
        ...
        age_minutes = (datetime.now() - cached_forecast.metadata.get('cache_time', datetime.now())).total_seconds() / 60
    ...
    def _get_cached_surface(self, cache_key: str) -> Optional[VolatilitySurface]:
        ...
        # Check if cache is still valid
        age_minutes = (datetime.now() - cached_surface.generation_time).total_seconds() / 60
    ...
    def _update_model_performance(self, model: VolatilityModel, calculation_time: float):
    ...
    async def update_market_data(self, prices: List[float], timestamps: List[datetime] = None):
        ...
        if len(self.price_history) > 1:
            new_returns = []
            for i in range(len(self.price_history) - len(prices), len(self.price_history)):
                if i > 0:
    ...
    def get_volatility_metrics(self) -> VolatilityMetrics:
        ...
        mean_reversion = -np.corrcoef(vol_series[:-1], vol_changes)[0, 1] if len(vol_changes) > 1 else 0.0
        ...
        return {
            ...
            'model_parameters': {
                'garch': {
                    ...
                    'persistence': self.garch_params.persistence if self.garch_params else None
                },
                'heston': {
...
# FACTORY FUNCTIONS
# ==============================================================================
def create_volatility_engine(config: Dict[str, Any] = None,
                           data_manager: MultiClientDataManager = None) -> SpyderVolatilityEngine:
...
async def main():
    ...
    try:
        ...
        print(f"   Forecast Range: {forecast.forecast_path.min():.1%} - {forecast.forecast_path.max():.1%}")
    ...
    try:
        ...
        for model in models_to_test:
            try:
                ...
                print(f"{model.value:<20} {volatility:>8.1%} {calc_time:>8.1f}ms {quality:>8.1%}")
