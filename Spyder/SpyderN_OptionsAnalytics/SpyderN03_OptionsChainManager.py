...
class OptionContract:
    ...
    def days_to_expiry(self) -> int:
        ...
        return max(0, (self.expiry.date() - datetime.now(UTC).date()).days)
...
class OptionsChainManager:
    ...
    def get_expirations(self, symbol: str,
        ...
        for expiry in sorted(self.chains[symbol].keys()):
            dte = (expiry.date() - datetime.now(UTC).date()).days
    ...
    def analyze_volume_oi(self, symbol: str,
        ...
        self.cache_timestamp[cache_key] = datetime.now(UTC)
    ...
    def prune_expired_chains(self, as_of: datetime | None = None) -> int:
        ...
        """
        cutoff = (as_of or datetime.now(UTC)).replace(
...
if __name__ == "__main__":
    ...
    expiry1 = datetime.now(UTC) + timedelta(days=7)   # Weekly
    expiry2 = datetime.now(UTC) + timedelta(days=30)  # Monthly
