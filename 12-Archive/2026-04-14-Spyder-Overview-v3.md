# Spyder Trading System — Codebase Review v17 (v3)
**Date:** April 14, 2026
**Scope:** Full codebase inventory plus v3 audit findings and remediation targets
**Prepared by:** GitHub Copilot (v2 inventory) + Claude Opus 4.6 (v3 audit)
...
28. [v3 Audit — Anomalies, Deficiencies & Opportunities](#28-v3-audit--anomalies-deficiencies--opportunities)
...
- Removed the remaining Databento runtime and test surfaces from the Python codebase; the repository now reflects a Tradier + Massive architecture only.
- Deleted `SpyderC26_DatabentoClient.py` and removed the associated adapter, provider-selection path, and legacy tick-source wiring.
- Updated the inventory counts below to match the current repository state after the removal and follow-up cleanup.
- Revalidated the touched surfaces with focused pytest slices: `53 passed, 9 skipped` for the config/feed/integration slice and `499 passed` for the cleanup residue slice.

### v3 Update Highlights

- Carried forward the full v2 inventory unchanged (473 files / 423,948 LOC / 25 series); no structural or LOC drift detected between the v2 review and the v3 audit pass.
- Performed an independent code audit against the current working tree, cross-checked against the prior v5 audit memo (April 13, 2026) to confirm which findings have been resolved and which remain open.
- **Resolved since v5**: C04 `MarketCondition.UNKNOWN` enum bug (now returns valid members via direct attribute access), G05 `CircuitBreaker` import now correctly prefixed with `Spyder.`, and F00 `AnalyticsProviderProtocol` surface aligned with implementors.
- **Still open from v5**: E01 dead `connect_api.send_message()` calls (now at lines 777 / 801 after refactor — still crash-on-call), S01 unconditional `import fcntl` at line 29 (Linux-only), and `.env.example` vs `.env.template` divergence.
- **New in v3**: 3 CRITICAL, 4 HIGH, 5 MEDIUM, and 5 LOW/opportunity findings documented in Section 28, including an `OptionsAdjustmentRL` stub cluster, unguarded global singletons in the P-series, and a regime-detector proliferation across E/F/L/M/V that lacks a canonical owner.
- Test count clarification: T-series contains 121 files but the effective production-module coverage is uneven — several recently-touched modules (R04, E01, F10, M06, B03) have no dedicated protocol-compliance or integration tests.

### Codebase Metrics (as of April 14, 2026)

| Metric | Count |
|--------|------:|
| Total Python files | **473** |
| Production files (A–Z, excluding T) | **352** |
| Test files (T-series) | **121** |
| Total lines of code | **423,948** |
| Production LOC | **330,020** |
| Test LOC | **93,928** |
| Series (packages) | **25** |
...
| Q | `SpyderQ_Scripts` | 21 | 9,299 | Launchers, validators, utilities, fine-tuning scripts |
...
| T | `SpyderT_Testing` | 121 | 93,928 | Full pytest test suite covering all series |
| U | `SpyderU_Utilities` | 32 | 19,770 | Logger, error handler, date/time, math, encryption, resilience utils |
...
| `SpyderB03_PositionTracker.py` | 447 | Real-time position cache. Maintains an in-memory map of open positions keyed by symbol and expiry, updated on every fill event received from the broker. |
...
| `SpyderB40_TradierClient.py` | 3,236 | Primary Tradier REST and WebSocket client. Handles Bearer token auth, all API endpoints (quotes, options, orders, accounts), retry with exponential backoff, rate limiting, and sandbox/live mode switching. |
...
The C-Series provides all market data ingestion, normalisation, validation, caching, and distribution. The active architecture is now Tradier + Massive only. C27 provides the Massive client, while C29 exposes provider-agnostic routing without the previously retained Databento compatibility layer.
...
The D-Series contains all trading strategy implementations. Each strategy extends `SpyderD01_BaseStrategy` and implements signal generation, entry/exit logic, and position sizing callbacks. The orchestrator (D31) and multi-leg coordinator (D32) manage concurrent strategy execution.
...
| `SpyderD30_RegimeGatedSelector.py` | 1,308 | Regime-gated strategy selector. Maps the current market regime (from L09/F10) to the best-fit strategy set and activates accordingly. |
| `SpyderD31_StrategyOrchestrator.py` | 2,065 | Strategy orchestrator. Coordinates concurrent strategy execution, manages capital budgeting between active strategies, and arbitrates signal conflicts. |
...
The E-Series implements all risk management logic in a multi-layer framework: pre-trade validation, real-time position monitoring, portfolio-level Greeks enforcement, drawdown controls, and circuit breakers.
...
| `SpyderE01_RiskManager.py` | 1,253 | Core risk manager entry point. Routes all pre-trade and post-trade risk checks to the appropriate E-Series sub-modules and aggregates verdicts. |
...
| `SpyderE21_HMMRegimeDetector.py` | 1,041 | Hidden Markov Model regime detector embedded in the risk layer. Switches portfolio into defensive mode when probability of adverse regime exceeds threshold. |
...
The F-Series provides the analytical backbone: technical indicators, price action patterns, volatility analysis, trend detection, Greeks computation, market regime detection, and the ML prediction integration layer.

| Module | LOC | Description |
|--------|----:|-------------|
| `SpyderF00_AnalysisProtocol.py` | 267 | Typed Protocol interfaces defining the F-Series ↔ X-Series boundary for passing analysis results to AI agents. |
| `SpyderF01_Indicators.py` | 871 | Core technical indicator library: RSI, MACD, Bollinger Bands, ATR, EMA, SMA, Stochastic, and Williams %R with configurable periods. |
...
| `SpyderF09_EntryFilters.py` | 1,287 | Pre-entry signal filters: time-of-day restrictions, bid-ask spread quality, volume thresholds, regime guards, and news blackout windows. |
| `SpyderF10_MarketRegimeDetector.py` | 1,511 | Composite market regime detector combining breadth, momentum, volatility surface shape, and macro signals into a unified regime label. |
| `SpyderF11_GreeksAggregator.py` | 1,047 | Aggregates position-level Greeks to portfolio-level net exposure with P&L attribution by Greeks component. |
...
The L-Series implements all machine learning infrastructure: model management, feature engineering, inference wrappers, reinforcement learning for options adjustment, regime classification, and federated learning.
...
| `SpyderL09_UnifiedRegimeEngine.py` | 2,200 | Unified ML regime classification engine. Combines Hidden Markov Models, k-means clustering, and ensemble voting to output a consensus regime label. |
...
| `SpyderL16_OptionsAdjustmentRL.py` | 1,984 | Reinforcement learning agent for adaptive options position adjustments. Trained with PPO via stable-baselines3; manages roll, close, or hold decisions. |
...
| `SpyderM06_HMMRegimeDetector.py` | 1,490 | Standalone HMM regime detector for system-level market state monitoring. Operates independently of L09, providing a redundant regime signal. |
...
The P-Series handles portfolio-level management: tracking all open positions, optimising capital allocation across strategies, managing correlation, dynamically rotating strategy weights by regime, and Renaissance-style position sizing.

| Module | LOC | Description |
|--------|----:|-------------|
| `SpyderP01_PortfolioManager.py` | 2,183 | Top-level portfolio manager. Tracks all open positions, computes portfolio-level Greeks, P&L attribution, and provides the authoritative position state. |
...
**Package:** `SpyderQ_Scripts` | **21 files** | **9,299 LOC**

The Q-Series contains all operational scripts: system launchers, validators, diagnostic utilities, production watchdog, GUI testing tools, and AI fine-tuning scripts.
...
The R-Series contains the execution engines for paper trading, live trading, and production deployment management. Each engine mode runs independently and shares the same strategy/risk interfaces.
...
| `SpyderR04_LiveEngine.py` | 1,382 | Live trading engine. Routes validated strategy signals to B40 Tradier client, manages real order flow, confirmations, and error recovery. |
...
| Module | LOC | Description |
|--------|----:|-------------|
| `SpyderS01_DIXCalculator.py` | 756 | DIX (Dark Index) calculation. Computes the ratio of dark pool volume to total volume as a proxy for institutional accumulation or distribution. |
...
**Package:** `SpyderT_Testing` | **121 files** | **93,928 LOC**

The T-Series is the complete pytest test suite covering all 24 production series. It includes unit tests, integration tests, system tests, scenario validators, and a full interactive test dashboard.
...
### Series Integration Tests (T100–T128 range)
...
**Package:** `SpyderU_Utilities` | **32 files** | **19,770 LOC**

The U-Series provides all shared infrastructure utilities used across every other series: logging, error handling, date/time, encryption, math, validation, constants, trading calendar, feature flags, technical indicators, resilience patterns, and secrets management.
...
| `SpyderV02_ModelManager.py` | 1,046 | Quant model lifecycle manager. Handles model registration, parameter storage, calibration scheduling, and performance tracking. |
...
| `SpyderV09_IVEngine.py` | 973 | Canonical synchronous BSM pricing, IV solver, and full Greeks computation engine. Designated single source of truth for IV calculations across the system; wraps all pricer access behind a consistent interface. |
...
## 28. v3 Audit — Anomalies, Deficiencies & Opportunities

Findings below are the result of a fresh static audit of the working tree on April 14, 2026. Each item lists the file, a pointer line, severity, and a short description. "Verified" means the issue was confirmed by direct inspection; "Carry-over" means the v5 memo flagged it and it remains unresolved.

### 28.1 Critical (fix before next trading session)

| # | File | Line(s) | Status | Issue |
|---|------|--------:|--------|-------|
| CRIT-01 | `SpyderE_Risk/SpyderE01_RiskManager.py` | 777, 801 | Verified, carry-over | `_request_position_updates()` and `_request_account_summary()` call `await self.connect_api.send_message(...)` but `connect_api` is wired as `None` after the legacy Connect-API removal. Any code path that triggers a position/account refresh raises `AttributeError` and silently kills the risk refresh loop. Replace with Tradier-native `SpyderB40_TradierClient` calls or delete the stubs outright. |
| CRIT-02 | `SpyderS_Signals/SpyderS01_DIXCalculator.py` | 29 | Verified, carry-over | `import fcntl` at module top. `fcntl` is Linux/macOS only; on Windows the module import fails, which cascades to `SpyderS_Signals/__init__.py` and breaks the custom-metrics orchestrator. Wrap in `try/except ImportError` with a portable fallback (e.g. `portalocker` or `msvcrt` on Windows), or gate the file-lock code path behind `sys.platform != "win32"`. |
| CRIT-03 | `.env.template` vs `.env.example` | n/a | Verified, carry-over | `.env.template` is a 15-line minimal stub while `.env.example` is 208 lines with every documented variable. New clones initialised from the wrong file boot with missing Tradier / Massive / Ollama / FRED credentials and fail at startup validation (Q02). Consolidate into a single source of truth (prefer `.env.example`) and have `.env.template` symlink or be regenerated from it. |

### 28.2 High (fix in the same pass)

| # | File | Line(s) | Status | Issue |
|---|------|--------:|--------|-------|
| HIGH-01 | `SpyderL_ML/SpyderL16_OptionsAdjustmentRL.py` | 306–309, 459–477 | Verified, new | Eight methods raise `NotImplementedError` (e.g. `_initialize_position`, `_compute_reward`, `_step_environment`). Any caller that instantiates the RL environment at runtime will crash. Either complete the implementation, or gate the module behind a `FEATURE_RL_ENABLED` flag in U11 and skip registration when disabled. |
| HIGH-02 | `SpyderR_Runtime/SpyderR04_LiveEngine.py` | ~69, ~132 | Verified, carry-over | `CLOSE_POSITIONS_ON_EMERGENCY` env var defaults to `false`, but a sibling dataclass default and docstring still imply `true`. Combined with the "API Panic Mode" (3×5xx → `emergency_stop_all()`) this leaves open positions unmanaged during a Tradier outage. Either document the safety rationale in `.env.example` or restore the `true` default and require an explicit opt-out. |
| HIGH-03 | Regime detector proliferation | — | Verified, new | Five separate regime detectors now coexist without a documented canonical owner: `SpyderE21_HMMRegimeDetector`, `SpyderF10_MarketRegimeDetector`, `SpyderL09_UnifiedRegimeEngine`, `SpyderM06_HMMRegimeDetector`, and implicit state machines inside `SpyderV02_ModelManager`. Agents and strategies currently pick whichever they were wired to first, so regime signals can contradict each other within the same tick. Designate `SpyderL09_UnifiedRegimeEngine` as the single source of truth, wrap the others as private internals, and delete or deprecate duplicates. |
| HIGH-04 | `SpyderF_Analysis/SpyderF10_MarketRegimeDetector.py` | 321–331 | Verified, new | `F10` nominally implements `AnalyticsProviderProtocol` but several methods return empty `IndicatorSnapshot` objects and empty signal lists as protocol stubs. Because `Protocol` is structurally checked, this passes `isinstance()` but silently delivers no data to downstream F-Series consumers. Add an explicit `stub_mode: bool` flag or raise when called without full wiring. |

### 28.3 Medium

| # | File | Line(s) | Status | Issue |
|---|------|--------:|--------|-------|
| MED-01 | `SpyderB_Broker/SpyderB03_PositionTracker.py` | ~390–398 | Verified, carry-over | `_handle_orphaned_position()` captures `pos` inside the lock at L390 but re-reads `pos_qty` after the lock is released. A fill that lands between those lines can flip the cleanup side from SELL to BUY. Fix by capturing both `pos` and `pos_qty` inside the locked section. |
| MED-02 | `SpyderS_Signals/SpyderS01_DIXCalculator.py` | ~409–411 | Verified, new | Blocking `time.sleep()` calls inside the initial symbol-fetch loop (10 retries × 1 s batch delay). On cold start with no cached data this can delay A01 boot by >10 s and delay the market-open prep schedule. Move behind a thread or run inside the A04 scheduler instead of the synchronous init path. |
| MED-03 | `SpyderE_Risk/SpyderE01_RiskManager.py` | ~595–610 | Verified, carry-over (partial) | `validate_signal()` now exists (good — v5 gap closed), but no D-Series strategy actually calls it; the risk gate is defined but dormant. Wire it into `SpyderD31_StrategyOrchestrator` pre-submit path and assert via a new T112 test. |
| MED-04 | `SpyderF_Analysis/SpyderF09_EntryFilters.py` | ~913 | Verified, new | `TODO: integrate portfolio-level Greek correlation when position data available` — entry filters currently ignore portfolio-level gamma/vega correlation, so two strategies can stack into the same risk pocket. Connect to `SpyderF11_GreeksAggregator` and reject entries above a configurable net-vega band. |
| MED-05 | `SpyderP_PortfolioMgmt/SpyderP01_PortfolioManager.py` (L1998), `SpyderP02_AllocationOptimizer.py` (L2039), `SpyderP03_CorrelationAnalyzer.py` (L612) | — | Verified, new | Module-level `_global_*` singletons initialised via `set_*()` without a `threading.Lock`. Concurrent initialisation from the A-Series bootstrap and the Y-Series `StrategyPilotAgent` can race, yielding two different singleton instances. Use a `threading.Lock` or switch to `functools.lru_cache(maxsize=1)` on a factory. |

### 28.4 Low / Opportunities

| # | Area | Status | Description |
|---|------|--------|-------------|
| LOW-01 | Optional-dependency handling | Verified, opportunity | `SpyderM06_HMMRegimeDetector` and `SpyderF22_MLPrediction` use graceful `warnings.warn()` fallbacks when `hmmlearn` / `torch` are missing. Standardise this pattern across all optional imports (institutional libs in F/O, QuantLib in V, stable-baselines3 in L16) via a small `SpyderU_Utilities/SpyderU47_OptionalImport.py` helper. |
| LOW-02 | Silent-return `getattr` pattern | Verified, opportunity | 260+ occurrences of `getattr(obj, "x", None)`. A significant fraction masks typos (e.g. `SpyderB03_PositionTracker.py:390–391` silently falls back when `positions` is missing). Convert to explicit attribute access inside `try/except AttributeError` with logging at WARNING so regressions surface in logs. |
| LOW-03 | `MarketCondition` enum duplication | Verified, opportunity | Several D-Series and E-Series modules (D03, D26, D32, E13) redeclare their own `MarketCondition` enums. Centralise in `SpyderC00_MarketDataProtocol.py` and import elsewhere to prevent drift when new regimes are added. |
| LOW-04 | Test coverage for recently-changed modules | Verified, opportunity | 119 test files cover 473 production modules (~25% file-level). Modules touched in the last four commits (R04, E01, F10, M06, B03, C04, G05) lack dedicated protocol-compliance / integration tests. Add a T129 "recent-churn" suite that asserts protocol conformance and basic happy-path behaviour for these files and run it as a pre-commit gate. |
| LOW-05 | API Panic Mode reset condition | Verified, carry-over | In `SpyderR04_LiveEngine`, the panic counter only clears on a successful order submission. A live session that is panic-tripped but currently has no orders in flight will never auto-recover even after Tradier is healthy again. Reset on any successful broker round-trip (quote, account refresh, options chain) instead. |

### 28.5 Forward-Looking Ideas (New Capabilities)

1. **Unified Signal Ledger (new SpyderI13)** — persist every signal emitted by D/S/L/X agents with full provenance (regime, Greeks, inputs) into an append-only SQLite table. Enables post-hoc attribution, counterfactual replay, and regulator-friendly audit trails without changing strategy code. Low risk; reuses H-Series DAL.
2. **Regime-Aware Kelly Scaler** — combine `SpyderE14_KellyPositionSizer` with the consolidated `SpyderL09_UnifiedRegimeEngine` output to compute a regime-conditional fractional-Kelly multiplier. In "extreme" regimes the fraction collapses toward 0.1×; in "low vol" it can relax toward 0.5×. Ships as a new `SpyderE24_RegimeKellySizer` that delegates to both.
3. **Tradier / Massive Parity Harness (T130)** — a scheduled harness that cross-checks Tradier quotes, options chains, and Greeks against Massive for the same timestamp and alerts on divergences above a configurable threshold. Catches silent provider drift that static validators miss.
4. **Protocol Compliance CI Gate** — a Q-series script (`SpyderQ10_ValidateProtocolCompliance.py`) that imports every typed `Protocol` in C00 / E00 / F00 / Z00 and runs `isinstance()` on every registered implementor, failing CI when a method or attribute drifts. Would have caught CRIT-01 and HIGH-04 before merge.
5. **Agent Health SLO Dashboard** — extend `SpyderG32_AgentHealthDashboard` with per-agent latency histograms and error-budget burn-down, sourced from M03. Enables running the Y-Series against explicit SLOs rather than manual heartbeat watching.
6. **Secrets Provider Parity Tests** — T-series test that round-trips a sample secret through all three U46 fallback providers (env var / encrypted YAML / Vault). Today only the env path is exercised by `test_u46_secrets_manager.py`.
7. **Deterministic Paper Mode** — add a `--seed` flag to `SpyderQ93_RunPaper` that pins RNG seeds across NumPy, Python `random`, and PyTorch. Makes paper runs bitwise reproducible for regression testing of L-Series model changes.

### 28.6 Remediation Priority

1. Patch CRIT-01, CRIT-02, and CRIT-03 immediately — none of them require design work.
2. HIGH-01 (RL stubs) and HIGH-03 (regime detector consolidation) are the highest-value design-level fixes; they remove entire classes of future bugs.
3. Land Protocol Compliance CI Gate (idea #4) before any further refactors so future protocol drift is caught at merge time.
4. Everything in 28.3 and 28.4 can be bundled into a single "v3 remediation pass" PR analogous to the v2 pass in commit `08dba17`.

---

## 29. Remediation Log — v3 Pass (April 14, 2026)

Post-audit remediation pass executed same-day by Claude Opus 4.6. All 17 items from §28 were triaged: **14 resolved with code changes**, **3 reclassified as false positives** after direct inspection. Protocol-compliance CI gate (Idea #4) shipped as part of the pass. No commit yet — working tree changes only.

### 29.1 Status Summary

| Bucket | Count | IDs |
|--------|------:|-----|
| Resolved | 14 | CRIT-01, CRIT-02, CRIT-03, HIGH-02, HIGH-03, HIGH-04, MED-01, MED-03, MED-04, MED-05, LOW-01, LOW-04, LOW-05, Idea-04 |
| False positive (reclassified) | 3 | HIGH-01, MED-02, LOW-03 |
| Deferred | 1 | LOW-02 (broad 260+ occurrence refactor — out of scope for this pass) |

### 29.2 Resolved Items

| # | Files Touched | Change |
|---|---------------|--------|
| CRIT-01 | [SpyderE01_RiskManager.py](Spyder/SpyderE_Risk/SpyderE01_RiskManager.py) | Added `tradier_client` constructor param; rewrote `_request_positions()` and `_request_account_summary()` to call `tradier_client.get_positions_async()` / `get_account_balances_async()`; added `_iter_tradier_positions()` mapper; updated factories. Legacy `connect_api` kwarg retained as no-op for call-site compat. |
| CRIT-02 | [SpyderS01_DIXCalculator.py](Spyder/SpyderS_Signals/SpyderS01_DIXCalculator.py) | Wrapped `import fcntl` in `try/except ImportError`; gated all `flock()` calls behind `_HAS_FCNTL`. Windows now imports cleanly; file-lock path no-ops on non-POSIX. |
| CRIT-03 | [.env.template](.env.template) | Rewrote as 6-line alias pointer directing users to `.env.example` (single source of truth). |
| HIGH-02 | [SpyderR04_LiveEngine.py](Spyder/SpyderR_Runtime/SpyderR04_LiveEngine.py) | Added docstring block above `CLOSE_POSITIONS_ON_EMERGENCY` documenting the opt-in safety rationale (matches existing `false` default). |
| HIGH-03 | [SpyderL09_UnifiedRegimeEngine.py](Spyder/SpyderL_ML/SpyderL09_UnifiedRegimeEngine.py), [SpyderE21_HMMRegimeDetector.py](Spyder/SpyderE_Risk/SpyderE21_HMMRegimeDetector.py), [SpyderM06_HMMRegimeDetector.py](Spyder/SpyderM_Monitoring/SpyderM06_HMMRegimeDetector.py), [SpyderF10_MarketRegimeDetector.py](Spyder/SpyderF_Analysis/SpyderF10_MarketRegimeDetector.py), [SpyderV02_ModelManager.py](Spyder/SpyderV_QuantModels/SpyderV02_ModelManager.py) | Declared L09 **canonical** in its module docstring; added `DEPRECATED (2026-04-14)` notices to E21, M06, F10, and V02's `MarketRegimeDetector` class. Call-site migration tracked as follow-up. |
| HIGH-04 | [SpyderF10_MarketRegimeDetector.py](Spyder/SpyderF_Analysis/SpyderF10_MarketRegimeDetector.py) | `calculate_all_indicators()` now returns `None` instead of an empty `IndicatorSnapshot` stub, forcing callers to route to F01 `IndicatorEngine` rather than silently consuming empty data. |
| MED-01 | [SpyderB03_PositionTracker.py](Spyder/SpyderB_Broker/SpyderB03_PositionTracker.py) | `_handle_orphaned_position()` now captures both `pos` and `pos_qty` inside the `_position_lock` block; side determination uses the locked snapshot. Fill-race window closed. |
| MED-03 | [SpyderD31_StrategyOrchestrator.py](Spyder/SpyderD_Strategies/SpyderD31_StrategyOrchestrator.py) | Rewrote `_on_strategy_signal()` from pass-through stub to a real `validate_signal()` gate: resolves `RiskManager`, calls `validate_signal(signal)`, handles dict/obj/bool result shapes, publishes `RISK_ALERT` on rejection. E01's `validate_signal()` is now actually exercised. |
| MED-04 | [SpyderF09_EntryFilters.py](Spyder/SpyderF_Analysis/SpyderF09_EntryFilters.py) | `_check_correlation_filters` extended with portfolio-level Greek projection: reads `portfolio_greeks` + `candidate_greeks` from `entry_params`, checks `max_portfolio_{delta,gamma,vega}` limits, emits `FilterCheck` rejections above band. |
| MED-05 | [SpyderP01_PortfolioManager.py](Spyder/SpyderP_PortfolioMgmt/SpyderP01_PortfolioManager.py), [SpyderP02_AllocationOptimizer.py](Spyder/SpyderP_PortfolioMgmt/SpyderP02_AllocationOptimizer.py), [SpyderP03_CorrelationAnalyzer.py](Spyder/SpyderP_PortfolioMgmt/SpyderP03_CorrelationAnalyzer.py) | Module-level `_global_*` singletons now guarded by a `threading.RLock`. `get/set/reset` helpers acquire the lock. Closes A-series / Y-series bootstrap race. |
| LOW-01 | [SpyderU47_OptionalImport.py](Spyder/SpyderU_Utilities/SpyderU47_OptionalImport.py) *(new)* | Shipped `optional_import()` helper with `OptionalImport` dataclass: truthy on success, loud `ImportError` on attribute access when missing, `required_on=` platform-mandatory escape hatch, `warn_once()` for graceful-degradation logging. Not yet adopted by call sites — follow-up migration. |
| LOW-04 | [SpyderT129_ProtocolCompliance.py](Spyder/SpyderT_Testing/SpyderT129_ProtocolCompliance.py) *(new)* | 4 TestCase classes covering the exact v5/v3 regression classes: E01 exposes `validate_signal` and factory accepts `tradier_client`; F10 returns `None` (not stub); C04 `get_current_condition()` returns a valid `MarketCondition` (catches `.UNKNOWN` AttributeError); F00 protocol exports `calculate_all_indicators`. |
| LOW-05 | [SpyderR04_LiveEngine.py](Spyder/SpyderR_Runtime/SpyderR04_LiveEngine.py) | Moved `reset_api_error_count()` above the fill-status check in `_execute_order_internal` so any successful broker round-trip clears the API Panic Mode counter, not just filled orders. |
| Idea-04 | [SpyderQ10_ProtocolComplianceGate.py](Spyder/SpyderQ_Scripts/SpyderQ10_ProtocolComplianceGate.py) *(new)* | CI entry point: loads T129 via `unittest.TestLoader().loadTestsFromModule`, exits 0 on pass / 1 on failure / 2 on harness error. Intended as a pre-merge gate. Invoke with `python -m Spyder.SpyderQ_Scripts.SpyderQ10_ProtocolComplianceGate`. |

### 29.3 False Positives (Reclassified)

| # | Evidence |
|---|----------|
| HIGH-01 | The eight `NotImplementedError` methods in [SpyderL16_OptionsAdjustmentRL.py](Spyder/SpyderL_ML/SpyderL16_OptionsAdjustmentRL.py) L306–477 are an **abstract-base-class contract**, fully overridden by `IronCondorEnvironment` (L489+) and sibling strategy environments. Direct inspection confirms overrides at L492, 522, 552, 584, 611, 632, 638, 656. They are not runtime stubs; instantiating a concrete subclass never hits the `raise`. No action required. |
| MED-02 | Neither `SpyderDIXCalculator.__init__` (L169) nor `DIXCalculator.__init__` (L724) calls `_fetch_market_caps()`. The `time.sleep(YFINANCE_DELAY)` at L415 and `time.sleep(BATCH_DELAY)` at L417 live **inside** `_fetch_market_caps`, which is only invoked from explicit `calculate_dix()` calls — not from the synchronous init path. A01 boot is not blocked. |
| LOW-03 | The `MarketCondition` enums in D03, D26, D32, E13 have **disjoint members** — D03 is directional (STRONGLY_BULLISH…STRONGLY_BEARISH), D26 is volatility×trend (HIGH_VOLATILITY/TRENDING_UP/WHIPSAW…), D32 is pattern-based (BREAKOUT_PENDING…), E13 is risk-tier (CALM/CRISIS…). They are semantic homonyms, not duplicates. Mechanical consolidation would force a fake superset and lose meaning. Correct remediation is a rename-for-disambiguation pass (e.g. `DirectionalRegime`, `VolatilityRegime`, `RiskTier`), deferred as design work. |

### 29.4 Deferred

**LOW-02** (silent `getattr` pattern, 260+ occurrences) — too broad for a same-day pass. Warrants a dedicated refactor with a custom AST linter rather than manual sweeps. Carried to future work.

### 29.5 Verification Status

- 13 touched files parse clean (`ast.parse` per file).
- No test suite executed yet against runtime — **recommend running `python -m Spyder.SpyderQ_Scripts.SpyderQ10_ProtocolComplianceGate` before the next trading session** to validate the T129 contract tests against the actual import graph.
- Pre-existing lint diagnostics (line length, module naming) from IDE hooks verified as not introduced by this pass.

### 29.6 Follow-Up Work

1. Migrate existing optional-import sites (M06, F22, V-series QuantLib, L16 stable-baselines3) onto `optional_import()` (LOW-01 adoption).
2. Delete deprecated regime detector duplicates after call-site migration to L09 is complete (HIGH-03 phase 2).
3. Disambiguate the four `MarketCondition` enums via rename pass (LOW-03 design).
4. LOW-02 silent-`getattr` AST linter (new Q-series script).
5. Wire T129 into pre-commit config so Q10 runs automatically.

---

*End of Spyder Codebase Review v17 (v3) — April 14, 2026 (remediation log appended same-day)*
