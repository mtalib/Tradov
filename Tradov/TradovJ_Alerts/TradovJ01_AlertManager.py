...
Module: TradovJ01_AlertManager.py
...
try:
    from Tradov.TradovU_Utilities.TradovU02_ErrorHandler import NotificationError
except ImportError:
...
try:
    from TradovL_ML.TradovL01_MLPredictor import MLPredictor
    from TradovL_ML.TradovL09_UnifiedRegimeEngine import UnifiedRegimeEngine as RegimeClassifier
    from TradovL_ML.TradovL14_RealTimePredictor import RealTimePredictor
    ML_AVAILABLE = True
except ImportError:
    ML_AVAILABLE = False
    ...
    def update_adaptive_thresholds(self, market_regime: str, volatility_level: float) -> None:
        ...
        try:
            for _threshold_name, threshold in self.adaptive_thresholds.items():
                ...
                threshold.last_updated = datetime.now(UTC)
        ...
        try:
            ...
            if prediction and probability >= self.min_confidence:
                ...
                alert = PredictiveAlert(
                    ...
                    expires_at=datetime.now(UTC) + timedelta(minutes=time_horizon * 2)
                )
