...
class ConfigManager:
    ...
    def get_config(self, name: str, decrypt: bool = True) -> Optional[Dict[str, Any]]:
        """
    ...
    def unsubscribe(self, module_id: str, config_name: Optional[str] = None) -> bool:
        """
    ...
    def _load_config_from_file(self, file_path: Path, format: ConfigFormat) -> Optional[Dict[str, Any]]:
    ...
    def _notify_subscribers(self, config_name: str, config_data: Optional[Dict[str, Any]], 
                          event_type: ConfigEvent, key_path: Optional[str] = None) -> None:
    ...
    def _cleanup_old_backups(self) -> None:
        ...
        try:
            ...
            for config_name, backup_list in self.backups.items():
                # Remove old backups
...
def create_config_manager(config_dir: Optional[Path] = None, 
...
if __name__ == "__main__":
    # Module testing code
    print("=" * 80)
    print("SPYDER I03 - Configuration Manager Test")
    print("=" * 80)
    ...
    if retrieved_config:
        print(f"Retrieved config keys: {list(retrieved_config.keys())}")
        print(f"Module name: {retrieved_config['module_name']}")
        print(f"Timeout value: {config_mgr.get_config_value('test_module', 'parameters.timeout')}")
    ...
    print(f"Updated timeout: {updated_timeout}")
    
    # Test subscription
    print("\n4. Testing Configuration Subscription...")
    
    def config_callback(notification_data):
        print(f"Configuration changed: {notification_data['config_name']} -> {notification_data['event_type']}")
    
    sub_success = config_mgr.subscribe("test_subscriber", "test_module", config_callback)
    print(f"Subscription successful: {sub_success}")
    ...
    print(f"Production config registration: {prod_success}")
    
    prod_timeout = prod_config_mgr.get_config_value("test_module", "parameters.timeout")
    print(f"Production timeout: {prod_timeout}")
    ...
    print(f"Invalid config registration (should succeed with basic validation): {invalid_success}")
    
    # Performance metrics
    print("\n8. Performance Metrics...")
    metrics = config_mgr.performance_metrics
    print(f"Config loads: {metrics['config_loads']}")
    print(f"Config saves: {metrics['config_saves']}")
    print(f"Sync operations: {metrics['sync_operations']}")
    print(f"Validation checks: {metrics['validation_checks']}")
    ...
    print("\n1. Strategy Configuration Example...")
    strategy_config = {
    ...
    print("Registered Iron Condor strategy configuration")
    
    print("\n2. Risk Manager Integration Example...")
    
    def risk_config_updated(notification_data):
        if notification_data['key_path'] and 'risk_parameters' in notification_data['key_path']:
            print(f"Risk parameter updated: {notification_data['key_path']}")
    ...
    for env in environments:
        ...
        print(f"{env.title()}: max_position_size = {env_config['risk_parameters']['max_position_size']}")
