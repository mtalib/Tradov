            ...
            while self._running:
                try:
                    if self.state == ConnectionState.CONNECTED:
                        ...
                        self.metrics.last_heartbeat = datetime.now(UTC)
                except Exception as e:
...
class SpyderSubscriber:
    ...
    def __init__(self, port: int, component_name: str, topics: list[str] | None = None):
    ...
    def _check_heartbeat_timeout(self):
        ...
        if self.metrics.last_heartbeat:
            time_since_heartbeat = (datetime.now(UTC) - self.metrics.last_heartbeat).total_seconds()
...
class SpyderRequester:
    ...
    def request(self, message: SpyderMessage, timeout: float = None) -> SpyderMessage | None:
    ...
    def _request_internal(self, message: SpyderMessage, timeout: float) -> SpyderMessage | None:
...
def example_dashboard_subscriber():
    ...
    subscriber.metrics.last_heartbeat = datetime.now(UTC) - timedelta(seconds=10)
    time.sleep(2)
...
def example_communication_hub():
    ...
    hub.create_publisher("market_data", MARKET_DATA_PORT)
    hub.create_publisher("risk_updates", RISK_MONITOR_PORT)

    hub.create_subscriber(
        "dashboard",
        MARKET_DATA_PORT,
