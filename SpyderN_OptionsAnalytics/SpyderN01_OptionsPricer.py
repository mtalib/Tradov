...
class BlackScholesPricer:
    ...
    def calculate_greeks(S: float, K: float, T: float, r: float, q: float, 
        ...
        if option_type == OptionType.CALL:
            ...
            theta = (-theta_base - r * K * np.exp(-r*T) * norm_cdf(d2) + 
        ...
        charm = -np.exp(-q*T) * norm_pdf(d1) * (2*(r-q)*T - d2*sigma*sqrt_T) / (2*T*sigma*sqrt_T)
...
class BinomialPricer:
    ...
    @staticmethod
    def price_option(S: float, K: float, T: float, r: float, q: float, sigma: float,
                     option_type: OptionType, n_steps: int = 100) -> float:
        """
    ...
    @staticmethod
    def calculate_greeks(S: float, K: float, T: float, r: float, q: float,
                        sigma: float, option_type: OptionType, n_steps: int = 100) -> GreeksResult:
        """
        ...
        if T > THETA_TIME_SHIFT:
            ...
            price_theta = BinomialPricer.price_option(S, K, T_down, r, q, sigma, option_type, n_steps)
...
class ImpliedVolatilitySolver:
    ...
    @staticmethod
    def calculate_iv(market_price: float, S: float, K: float, T: float, r: float, q: float,
                     option_type: OptionType, method: str = 'newton') -> Optional[float]:
        """
        ...
        if method == 'newton':
            return ImpliedVolatilitySolver._newton_raphson(
                market_price, S, K, T, r, q, option_type
            )
        else:
            return ImpliedVolatilitySolver._bisection(
                market_price, S, K, T, r, q, option_type
            )
    
    @staticmethod
    def _newton_raphson(target_price: float, S: float, K: float, T: float, r: float, q: float,
    ...
    @staticmethod
    def _bisection(target_price: float, S: float, K: float, T: float, r: float, q: float,
                  option_type: OptionType) -> Optional[float]:
...
class OptionsPricer:
    ...
    def price_option(self, contract: OptionContract, market_data: MarketData,
                    model: Optional[PricingModel] = None,
                    calculate_greeks: bool = True) -> OptionPrice:
        """
        ...
        try:
            ...
            if model == PricingModel.BLACK_SCHOLES:
                ...
                if calculate_greeks:
                    greeks = self.bs_pricer.calculate_greeks(S, K, T, r, q, sigma, contract.option_type)
                else:
                    greeks = None
    ...
    def calculate_implied_volatility(self, contract: OptionContract, market_price: float,
                                    market_data: MarketData) -> Optional[float]:
        """
        Calculate implied volatility from market price
    ...
    def calculate_portfolio_greeks(self, positions: List[Tuple[OptionContract, int, MarketData]]) -> GreeksResult:
        """
    ...
    def calculate_payoff_profile(self, contract: OptionContract, premium: float,
    ...
    def check_put_call_parity(self, call_price: float, put_price: float,
        ...
        right_side = spot * np.exp(-div_yield * time_to_expiry) - strike * np.exp(-rate * time_to_expiry)
    ...
    def _get_cache_key(self, contract: OptionContract, market_data: MarketData) -> str:
        ...
        return (f"{contract.symbol}_{market_data.spot_price:.2f}_"
...
def calculate_option_price(spot: float, strike: float, time_to_expiry: float,
                          volatility: float, rate: float = DEFAULT_RISK_FREE_RATE,
...
if __name__ == "__main__":
    ...
    print(f"  Status: {'✅ VALID' if abs(parity_diff) < PUT_CALL_PARITY_TOLERANCE else '❌ INVALID'}")
    ...
    print(f"  Early Exercise Premium: ${bin_price.theoretical_value - bs_price.theoretical_value:.2f}")
