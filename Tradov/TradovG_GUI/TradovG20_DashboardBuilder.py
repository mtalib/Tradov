...
# LOCAL IMPORTS
# ==============================================================================
from Tradov.TradovG_GUI.TradovG13_EnhancedWidgets import (
    COLORS,
    GreekBar,
    MarketSymbolWidget,
    SignalMonitorPanel,
...
def build_center_panel(dashboard: Any) -> QWidget:
    ...
    try:
        _flow_interval_s = float(os.getenv("TRADOV_D31_SIGNAL_FLOW_LOG_INTERVAL_S", "300"))
...
def build_right_panel(dashboard: Any) -> QWidget:
    ...
    tradovbox_widget = QWidget()
    tradovbox_widget.setStyleSheet(
        f"background-color: {COLORS['panel']}; border: 1px solid {COLORS['border']}; border-radius: 5px;",  # noqa: E501
    )
    tradovbox_grid = QGridLayout()
    tradovbox_grid.setContentsMargins(4, 4, 4, 4)
    tradovbox_grid.setHorizontalSpacing(3)
    tradovbox_grid.setVerticalSpacing(3)
    tradovbox_grid.setColumnStretch(0, 3)
    tradovbox_grid.setColumnStretch(1, 4)
    tradovbox_grid.setColumnStretch(2, 3)
    tradovbox_grid.setColumnStretch(3, 4)
    tradovbox_grid.setRowMinimumHeight(0, 30)
    tradovbox_grid.setRowMinimumHeight(1, 24)
    tradovbox_grid.setRowMinimumHeight(2, 24)

    tradovbox_grid.addWidget(dashboard._acct_lbl("ACCOUNT", cell_style), 0, 0)
    dashboard.tradovbox_acct_number_lbl = dashboard._acct_lbl("TradovBox", cell_style)
    dashboard.tradovbox_acct_number_lbl.setToolTip("In-house TradovBox paper trading account")
    tradovbox_grid.addWidget(dashboard.tradovbox_acct_number_lbl, 0, 1)

    tradovbox_mode_container = QWidget()
    tradovbox_mode_layout = QHBoxLayout(tradovbox_mode_container)
    tradovbox_mode_layout.setContentsMargins(0, 0, 0, 0)
    tradovbox_mode_layout.setSpacing(2)

    dashboard.tradovbox_paper_status_btn = QPushButton("PAPER TRADING")
    dashboard.tradovbox_paper_status_btn.setToolTip("TradovBox paper-trading status indicator")
    dashboard.tradovbox_paper_status_btn.setFixedHeight(24)
    tradovbox_mode_layout.addWidget(dashboard.tradovbox_paper_status_btn)

    dashboard.tradovbox_paper_toggle_btn = QPushButton("ENABLE PAPER")
    dashboard.tradovbox_paper_toggle_btn.setToolTip("Enable or disable TradovBox paper trading mode")
    dashboard.tradovbox_paper_toggle_btn.setFixedHeight(24)
    dashboard.tradovbox_paper_toggle_btn.clicked.connect(dashboard._on_paper_arm_toggle_clicked)
    tradovbox_mode_layout.addWidget(dashboard.tradovbox_paper_toggle_btn)

    dashboard.tradovbox_account_type_lbl = None
    tradovbox_grid.addWidget(tradovbox_mode_container, 0, 2, 1, 2)

    tradovbox_grid.addWidget(dashboard._acct_lbl("SETTLED CASH", cell_style), 1, 0)
    dashboard.tradovbox_settled_value = dashboard._acct_lbl("—", cell_style, right=True)
    tradovbox_grid.addWidget(dashboard.tradovbox_settled_value, 1, 1)
    tradovbox_grid.addWidget(dashboard._acct_lbl("BUYING POWER", cell_style), 1, 2)
    dashboard.tradovbox_buying_value = dashboard._acct_lbl("—", cell_style, right=True)
    tradovbox_grid.addWidget(dashboard.tradovbox_buying_value, 1, 3)

    tradovbox_grid.addWidget(dashboard._acct_lbl("REALIZED P&L", cell_style), 2, 0)
    dashboard.tradovbox_realized_value = dashboard._acct_lbl(
        "—",
        cell_style + f"color: {COLORS['positive']};",
        right=True,
    )
    tradovbox_grid.addWidget(dashboard.tradovbox_realized_value, 2, 1)
    tradovbox_grid.addWidget(dashboard._acct_lbl("UNREALIZED P&L", cell_style), 2, 2)
    dashboard.tradovbox_unrealized_value = dashboard._acct_lbl(
        "—",
        cell_style + f"color: {COLORS['positive']};",
        right=True,
    )
    tradovbox_grid.addWidget(dashboard.tradovbox_unrealized_value, 2, 3)

    tradovbox_widget.setLayout(tradovbox_grid)
    layout.addWidget(tradovbox_widget)
    dashboard._update_mode_buttons()
...
def create_chart_hidden_controls_panel(dashboard: Any) -> None:
    ...
    veto_scope_label = QLabel(
        "TradovX16 Veto  +  TradovY03 Trade Veto  +  TradovY05 Consumption Veto"
    )
...
def build_toolbar(dashboard: Any) -> QWidget:
    ...
    Extracted from TradovG05_TradingDashboard.create_toolbar.
    ...
    try:
        _flow_interval_s = float(_os.getenv("TRADOV_D31_SIGNAL_FLOW_LOG_INTERVAL_S", "300"))
    except (TypeError, ValueError):
        _flow_interval_s = 300.0
    if _flow_interval_s <= 0:
        _flow_interval_s = 300.0
