...
Purpose: Safe Migration from Current Setup to IB Gateway
...
Module Description:
    ...
    and IBC settings while preserving custom settings and creating
    comprehensive backups. IB Gateway 10.39 specific migration features have been removed.
"""
...
# Version mappings
VERSION_MIGRATIONS = {
    "TWS_MAJOR_VRSN": {"old": ["1012", "1040"], "new": "1037"},
    "IB_GATEWAY_DIR": {
        "old": ["ibgateway/1012", "ibgateway/1040"],
        "new": "ibgateway/1037",
    },
    "IB_GATEWAY_VERSION": {"old": ["10.12", "10.40"], "new": "10.37"},
}
...
# Created: 2025-08-26
# Updated: {timestamp}
# Version: IB Gateway 10.37 (10.39 features removed)

# ===============================================================================
# IB GATEWAY CORE SETTINGS - DOWNGRADED TO 10.37
# ===============================================================================
export TWS_MAJOR_VRSN="1037"
export IB_GATEWAY_VERSION="10.37"
export IBC_PATH="$HOME/ibc"
export IBC_INI="$HOME/ibc/config.ini"
export IB_GATEWAY_DIR="$HOME/Jts/ibgateway/1037"
export IB_GATEWAY_HOME="$HOME/Jts"

# ===============================================================================
# JAVA CONFIGURATION FOR IB GATEWAY 10.37
# ===============================================================================
# Gateway 10.37 works best with Java 8 or 11
export JAVA_HOME="/usr/lib/jvm/java-11-openjdk-amd64"
export PATH="$JAVA_HOME/bin:$PATH"

# Java VM options optimized for Gateway 10.37
...
# Connection timeout settings (optimized for 10.37)
...
# ===============================================================================
# IB GATEWAY VALIDATION FUNCTION - ENHANCED
# ===============================================================================
validate_ib_env() {{
    echo "==============================================================================="
    echo "SPYDER IB GATEWAY ENVIRONMENT CHECK"
    echo "==============================================================================="
...
# Migration check
alias ib-migrate-check='echo "Current version: $TWS_MAJOR_VRSN (should be 1037)"'

{safety_warnings}
"""

# Enhanced IBC configuration template
IBC_CONFIG_TEMPLATE = """# IBC Configuration for IB Gateway
...
class ConfigurationMigrationManager:
    """
    Manages safe migration from current IB Gateway setup.
    ...
    IB Gateway 10.39 specific migration features have been removed.
    """
    ...
    def create_backup(self) -> bool:
        ...
        try:
            ...
            metadata = {
                ...
                "target_version": "10.37",
                "files_backed_up": [
                    str(f[0]) for f in files_to_backup if f[0].exists()
                ],
            }
    ...
    def migrate_bash_ib(self) -> bool:
        """Migrate .bash_ib file to Gateway"""
        ...
        try:
            ...
            if self.dry_run:
                self._show_diff(
                    old_content if BASH_IB_PATH.exists() else "",
                    new_content,
                    ".bash_ib",
                )
                return True
            ...
            self.changes_made.append("Updated ~/.bash_ib to Gateway")
...
if [ "$IB_TRADING_MODE" = "live" ]; then
    ...
    def update_gateway_directories(self) -> bool:
        """Create necessary directories for Gateway"""
        self.logger.info("Setting up Gateway directories...")

        directories = [
            Path.home() / "Jts" / "ibgateway" / "1037",
    ...
    def validate_migration(self) -> bool:
        ...
        if BASH_IB_PATH.exists():
            content = BASH_IB_PATH.read_text()
            if 'TWS_MAJOR_VRSN="1037"' not in content:
        ...
        # Check directories
        gateway_dir = Path.home() / "Jts" / "ibgateway" / "1037"
    ...
    def run_migration(self) -> bool:
        ...
        self.logger.info("=" * 60)
        self.logger.info("IB GATEWAY MIGRATION TOOL")
        self.logger.info("=" * 60)
        ...
        if success:
            ...
            if not self.dry_run:
                self.logger.info(
                    "Run with --rollback to restore original configuration"
                )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Migrate IB Gateway configuration"
    )
    ...
    parser.add_argument(
        "--rollback", action="store_true", help="Rollback to previous configuration"
    )
    ...
    if not args.force and not args.dry_run:
        print("\n⚠️  This will migrate your IB Gateway configuration")
