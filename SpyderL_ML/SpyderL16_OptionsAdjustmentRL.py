...
from SpyderX_Agents.SpyderX06_BacktestingAgent import BacktestingAgent
...
class OptionsEnvironment(gym.Env):
    ...
    def reset(self) -> np.ndarray:
        ...
        self.data_slice = self.historical_data.iloc[start_idx:start_idx + STEPS_PER_EPISODE]
    ...
    def step(self, action: int) -> Tuple[np.ndarray, float, bool, Dict]:
        ...
        # Check if episode is done
            ...
            self.current_step >= STEPS_PER_EPISODE or
            self.current_position is None or
            self._should_force_close()
        )
    ...
    def _get_state(self) -> PositionState:
        ...
        return PositionState(
            ...
            underlying_change=(current_data['close'] - self.current_position['entry_price']) / 
...
class IronCondorEnvironment(OptionsEnvironment):
    ...
    def _calculate_pnl(self) -> float:
        ...
        if put_short <= spot <= call_short:
            ...
            time_decay = self.current_position['initial_credit'] * (1 - self.current_position['dte'] / 45)
            return time_decay
        else:
            # Calculate intrinsic value loss
            if spot < put_short:
                loss = (put_short - spot) - (put_short - self.current_position['legs']['put_long']['strike'])
            else:
                loss = (spot - call_short) - (self.current_position['legs']['call_long']['strike'] - call_short)
...
class OptionsAdjustmentRL:
    ...
    def train(self, 
              ...
              save_freq: int = 100) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if self._is_best_model(strategy, eval_results):
                ...
                self.logger.info(f"New best model for {strategy}: Sharpe={eval_results['sharpe_ratio']:.3f}")
    ...
    def _evaluate_model(self, 
        ...
        return {
            ...
            'max_drawdown': np.mean([m.get('max_drawdown', 0) for m in episode_metrics])
        }
    ...
    def _is_best_model(self, strategy: str, metrics: Dict[str, float]) -> bool:
        ...
        current_best = self.training_history[strategy][-1]['sharpe_ratio'] if self.training_history[strategy] else 0
    ...
    def get_adjustment_recommendation(self,
                                      ...
                                      market_data: Dict[str, Any]) -> AdjustmentAction:
        """
        ...
        try:
            ...
            if hasattr(model, 'policy'):
                with torch.no_grad():
                    obs_tensor = torch.tensor(state.to_array()).unsqueeze(0).to(self.device)
    ...
    def _create_state_from_position(self,
                                    position: Dict[str, Any],
                                    market_data: Dict[str, Any]) -> PositionState:
        ...
        return PositionState(
            ...
            underlying_change=(market_data['price'] - position['entry_price']) / position['entry_price'],
    ...
    def _calculate_live_greeks(self,
                               position: Dict[str, Any],
    ...
    def _create_adjustment_parameters(self,
                                      position: Dict[str, Any],
    ...
    def create_ensemble_recommendation(self,
                                       position: Dict[str, Any],
                                       market_data: Dict[str, Any],
                                       models: Optional[List[str]] = None) -> AdjustmentAction:
        """
        ...
        # Combine reasons
        all_reasons = [rec.reason for rec in recommendations if rec.action_type == best_action]
            ...
            if '_ppo' in model_name:
                ...
                if strategy in self.vec_environments:
                    self.models[model_name] = PPO.load(
                        model_file,
                        env=self.vec_environments[strategy]
                    )
...
def create_options_adjustment_rl(config: Optional[Dict[str, Any]] = None) -> OptionsAdjustmentRL:
