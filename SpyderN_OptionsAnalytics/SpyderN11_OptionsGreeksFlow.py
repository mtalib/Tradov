...
class OptionsGreeksFlowAnalyzer:
    ...
    def get_greeks_flow_profile(self) -> GreeksFlowProfile:
        ...
        total_flow = (
            gamma_flow.expected_hedging +
            vanna_flow.expected_flow +
            charm_flow.overnight_flow
        )
    ...
    def detect_greek_regime_change(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'recommendations': self._generate_regime_recommendations(changes_detected, flip_proximity)
        }
    ...
    def predict_hedging_flows(self, spot_move: float) -> Dict[str, Any]:
        ...
        if gamma_flow:
            gamma_hedging = self._calculate_gamma_hedging_flow(
                gamma_flow.net_gamma,
                spot_move,
                gamma_flow.dealer_position
            )
        else:
            gamma_hedging = 0
    ...
    def _calculate_gamma_hedging_flow(self, net_gamma: float, spot_move: float,
                                    dealer_position: DealerPositioning) -> float:
    ...
    def _calculate_overnight_charm_flow(self, net_charm: float, 
                                      charm_by_strike: Dict[float, float]) -> float:
    ...
    def _identify_key_greek_levels(self, gamma_flow: GammaFlow, 
                                 vanna_flow: VannaFlow) -> List[float]:
        ...
        # Major gamma strikes
        sorted_gamma = sorted(gamma_flow.gamma_by_strike.items(), 
        ...
        # Major vanna strikes
        sorted_vanna = sorted(vanna_flow.vanna_by_strike.items(),
                            key=lambda x: abs(x[1]), reverse=True)
    ...
    def _assess_greek_risks(self, gamma_flow: GammaFlow, vanna_flow: VannaFlow,
                          charm_flow: CharmFlow) -> Dict[str, Any]:
    ...
    def _generate_regime_recommendations(self, changes: List[Dict], 
                                       flip_proximity: Dict) -> List[str]:
    ...
    def _identify_expiry_key_strikes(self, options: List) -> List[float]:
        ...
        # Get top strikes
        sorted_strikes = sorted(strike_importance.items(), 
...
if __name__ == "__main__":
    ...
    try:
        ...
        for move in [-5, -2, 0, 2, 5]:
            ...
            print(f"  ${move:+.0f} move: ${prediction['total_hedging_flow']:+,.0f} "
