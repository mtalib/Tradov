...
Key Features:
    ...
    - PyQt6 widget for Risk Levels Configuration 5th tab
...
# ==============================================================================
import pytz
from ib_insync import IB, Contract, Order, Trade, Fill, OrderStatus
from PyQt6.QtWidgets import (QWidget, QVBoxLayout, QHBoxLayout, QLabel, 
                            ...
                            QTreeWidgetItem, QHeaderView, QLineEdit,
                            QFormLayout, QGridLayout, QLCDNumber)
...
try:
    ...
    from SpyderB_Broker.SpyderB20_IntegratedConnectivityManager import IntegratedConnectivityManager
...
class ProfitTargetConfig:
    ...
    preferred_venues: List[OrderExecutionVenue] = field(default_factory=lambda: [OrderExecutionVenue.SMART])
...
class AlgorithmicSlicingManager:
    ...
    def create_slicing_plan(self, 
                           target_profit: float,
                           risk_allocation: float,
                           algorithm: SlicingAlgorithm,
                           time_window_minutes: int) -> List[ChildOrderSpec]:
        """
        ...
        """
        try:
            self.logger.info(f"Creating slicing plan - Target: ${target_profit:,.2f}, Algorithm: {algorithm.value}")
            ...
            if algorithm == SlicingAlgorithm.TWAP:
                child_orders = self._create_twap_plan(estimated_contracts, time_window_minutes)
            elif algorithm == SlicingAlgorithm.VWAP:
                child_orders = self._create_vwap_plan(estimated_contracts, time_window_minutes)
            elif algorithm == SlicingAlgorithm.POV:
                child_orders = self._create_pov_plan(estimated_contracts, time_window_minutes)
            elif algorithm == SlicingAlgorithm.SOR:
                child_orders = self._create_sor_plan(estimated_contracts, time_window_minutes)
            elif algorithm == SlicingAlgorithm.ADAPTIVE:
                child_orders = self._create_adaptive_plan(estimated_contracts, time_window_minutes)
            else:
                child_orders = self._create_liquidity_seeking_plan(estimated_contracts, time_window_minutes)
            
            # Apply risk allocation to each child order
            for child_order in child_orders:
                child_order.risk_allocation = risk_allocation / len(child_orders)
            
            self.logger.info(f"✅ Created slicing plan with {len(child_orders)} child orders")
            return child_orders
    ...
    def _create_twap_plan(self, total_contracts: int, time_window_minutes: int) -> List[ChildOrderSpec]:
        ...
        for i in range(num_orders):
            ...
            child_order = ChildOrderSpec(
                ...
                algorithm_params={'algorithm': 'TWAP', 'time_window': time_window_minutes},
    ...
    def _create_vwap_plan(self, total_contracts: int, time_window_minutes: int) -> List[ChildOrderSpec]:
        ...
        for i, volume_weight in enumerate(volume_profile):
            ...
            execution_time = start_time + timedelta(minutes=i * (time_window_minutes // num_periods))
    ...
    def _create_adaptive_plan(self, total_contracts: int, time_window_minutes: int) -> List[ChildOrderSpec]:
    ...
    def _create_sor_plan(self, total_contracts: int, time_window_minutes: int) -> List[ChildOrderSpec]:
        """Create Smart Order Routing execution plan"""
        child_orders = []
        
        # Distribute across multiple venues
        venues = [OrderExecutionVenue.SMART, OrderExecutionVenue.CBOE, 
                 OrderExecutionVenue.PHLX, OrderExecutionVenue.BOX]
        ...
        for venue_idx, venue in enumerate(venues):
            ...
            if venue_idx == len(venues) - 1:  # Last venue gets remainder
                venue_contracts = total_contracts - (orders_per_venue * (len(venues) - 1))
            
            num_orders = max(1, venue_contracts // child_order_size)
            
            for i in range(num_orders):
                order_qty = min(child_order_size, venue_contracts - (i * child_order_size))
                if order_qty <= 0:
                    break
                
                # Stagger execution times across venues
                execution_time = start_time + timedelta(
                    seconds=venue_idx * 15 + i * (time_window_minutes * 60 // num_orders)
                )
    ...
    def _create_pov_plan(self, total_contracts: int, time_window_minutes: int) -> List[ChildOrderSpec]:
        ...
        for minute in range(time_window_minutes):
            expected_volume = expected_volume_per_minute.get(minute, 1000)  # Default 1000 contracts/minute
            ...
            child_order = ChildOrderSpec(
                ...
                time_in_force="DAY",
                venue=OrderExecutionVenue.SMART,
                algorithm_params={'algorithm': 'POV', 'participation_rate': participation_rate},
                scheduled_time=execution_time,
    ...
    def _create_liquidity_seeking_plan(self, total_contracts: int, time_window_minutes: int) -> List[ChildOrderSpec]:
        ...
        for i in range(num_orders):
            order_qty = min(adaptive_order_size, total_contracts - (i * adaptive_order_size))
            if order_qty <= 0:
                break
...
class DayProfitTargetEngine:
    ...
    async def validate_profit_target(self, target_amount: float) -> Tuple[bool, str, float]:
        """
        ...
        try:
            ...
            if target_amount < MIN_DAILY_PROFIT_TARGET:
                return False, f"Target too low. Minimum: ${MIN_DAILY_PROFIT_TARGET:,.2f}", MIN_DAILY_PROFIT_TARGET
            
            if target_amount > MAX_DAILY_PROFIT_TARGET:
                return False, f"Target too high. Maximum: ${MAX_DAILY_PROFIT_TARGET:,.2f}", MAX_DAILY_PROFIT_TARGET
            ...
            max_achievable_target = self._calculate_max_achievable_profit(account_balance, max_risk_amount)
            
            # Check if target is achievable
            if target_amount > max_achievable_target:
                return False, f"Target exceeds maximum achievable (${max_achievable_target:,.2f}) based on account balance", max_achievable_target
            
            # Check existing positions and strategy conflicts
            if self.strategy_orchestrator:
                allocated_capital = self.strategy_orchestrator.portfolio_metrics.allocated_capital
                available_capital = account_balance - allocated_capital
                
                required_capital = self._estimate_required_capital(target_amount)
                
                if required_capital > available_capital:
                    return False, f"Insufficient available capital. Required: ${required_capital:,.2f}, Available: ${available_capital:,.2f}", available_capital * 0.8
            
            # Check market conditions
            if not self._is_market_suitable_for_target(target_amount):
                return False, "Market conditions not suitable for this target", max_achievable_target * 0.5
    ...
    async def set_profit_target(self, 
                               target_amount: float,
                               slicing_algorithm: SlicingAlgorithm = SlicingAlgorithm.ADAPTIVE,
                               time_window_minutes: int = DEFAULT_TIME_WINDOW_MINUTES,
                               child_order_size: int = DEFAULT_CHILD_ORDER_SIZE) -> bool:
        """
        ...
        try:
            # Validate target
            is_valid, message, max_target = await self.validate_profit_target(target_amount)
            if not is_valid:
            ...
            self.current_progress = ProfitTargetProgress(
                ...
                progress_percentage=0.0,
                time_elapsed_minutes=0,
                estimated_completion_time=datetime.now() + timedelta(minutes=time_window_minutes),
                orders_executed=0,
                orders_pending=0,
                risk_utilization_pct=0.0,
                execution_metrics=ExecutionMetrics()
            ...
            self.logger.info(f"✅ Profit target configured: ${target_amount:,.2f} using {slicing_algorithm.value}")
            ...
            if self.error_handler:
                self.error_handler.handle_error(e, "DayProfitTargetEngine.set_profit_target")
            return False
    ...
    async def start_profit_targeting(self) -> bool:
        ...
        try:
            ...
            self.execution_thread = threading.Thread(
                target=self._execute_slicing_plan,
                args=(child_orders,),
                daemon=True
            )
            self.execution_thread.start()
            
            self.logger.info(f"✅ Profit targeting started - {len(child_orders)} orders planned")
            return True
    ...
    async def stop_profit_targeting(self, reason: str = "user_request") -> bool:
        ...
        """
        try:
            if self.status not in [ProfitTargetStatus.IN_PROGRESS, ProfitTargetStatus.ACTIVE]:
    ...
    def _execute_slicing_plan(self, child_orders: List[ChildOrderSpec]):
        ...
        try:
            ...
            sorted_orders = sorted(child_orders, 
                                 key=lambda x: (x.scheduled_time, -x.priority_score))
            ...
            for child_order in sorted_orders:
                ...
                if success:
                    ...
                    self.logger.warning(f"Failed to execute child order: {child_order.order_id}")
    ...
    def _execute_child_order(self, child_order_spec: ChildOrderSpec) -> bool:
        ...
        try:
            ...
            self.logger.info(f"📋 Child order placed: {child_order_spec.order_id} - {child_order_spec.quantity} @ {child_order_spec.venue.value}")
    ...
    def _update_progress(self):
        ...
        try:
            ...
            if self.current_config:
                progress_pct = min(100.0, (current_profit / self.current_config.target_amount) * 100)
            else:
                progress_pct = 0.0
            ...
            if self.current_config:
                self.current_progress.risk_utilization_pct = min(100.0, 
    ...
    def _calculate_max_achievable_profit(self, account_balance: float, max_risk: float) -> float:
    ...
    def _get_current_profit(self) -> float:
        ...
        if self.execution_start_time:
            ...
            if self.current_config:
                simulated_profit = (elapsed_minutes / self.current_config.time_window_minutes) * self.current_config.target_amount * 0.7
...
class MarketImpactAnalyzer:
    ...
    def analyze_impact(self, pre_price: float, post_price: float, volume: int) -> MarketImpactAnalysis:
...
class ExecutionQualityTracker:
    ...
    def record_execution(self, venue: str, fill_price: float, target_price: float, fill_time: float):
    ...
    def get_venue_performance(self, venue: str) -> Dict[str, float]:
        ...
        return {
            ...
            'fill_rate': len(slippages) / len(self.execution_history) if self.execution_history else 0.0
        }

# ==============================================================================
# PYQT6 DAY PROFIT TARGET WIDGET
# ==============================================================================

class DayProfitTargetWidget(QWidget):
    """
    PyQt6 widget for Day Profit Target configuration and monitoring.
    
    Integrates as the 5th tab in Risk Levels Configuration dialog.
    """
    
    # Qt signals
    targetConfigured = pyqtSignal(float, str)  # target_amount, algorithm
    executionStarted = pyqtSignal()
    executionStopped = pyqtSignal(str)  # reason
    progressUpdated = pyqtSignal(dict)  # progress_data
    riskAlert = pyqtSignal(dict)  # alert_data
    ...
    def create_risk_controls_section(self, layout):
        ...
        self.emergency_stop_btn.setStyleSheet("background-color: #FF5722; color: white; font-weight: bold;")
    ...
    def validate_target_input(self):
        ...
        try:
            ...
            if target_amount < MIN_DAILY_PROFIT_TARGET:
                self.validation_label.setText(f"⚠️ Too low. Minimum: ${MIN_DAILY_PROFIT_TARGET:,.2f}")
                self.validation_label.setStyleSheet("color: orange;")
                self.configure_btn.setEnabled(False)
            elif target_amount > MAX_DAILY_PROFIT_TARGET:
                self.validation_label.setText(f"⚠️ Too high. Maximum: ${MAX_DAILY_PROFIT_TARGET:,.2f}")
    ...
    def validate_target(self):
        ...
        try:
            ...
            QTimer.singleShot(2000, lambda: self._validation_complete(target_amount, True, "Validation successful", target_amount * 0.8))
            
        except ValueError:
            QMessageBox.warning(self, "Error", "Please enter a valid number")
    
    def _validation_complete(self, target_amount: float, is_valid: bool, message: str, max_achievable: float):
    ...
    def configure_target(self):
        ...
        try:
            ...
            QTimer.singleShot(1500, lambda: self._configuration_complete(target_amount, algorithm.value))
            
        except Exception as e:
            QMessageBox.critical(self, "Configuration Error", f"Failed to configure target: {str(e)}")
    ...
    def _configuration_complete(self, target_amount: float, algorithm: str):
        ...
        self.validation_label.setText(f"✅ Target configured: ${target_amount:,.2f} using {algorithm.upper()}")
    ...
    def start_execution(self):
        """Start profit targeting execution"""
        reply = QMessageBox.question(self, "Start Execution",
    ...
    def stop_execution(self):
        """Stop execution"""
        reply = QMessageBox.question(self, "Stop Execution",
    ...
    def _update_simulated_progress(self):
        ...
        try:
            ...
            orders_executed = int(self.simulation_progress * 0.5)  # Simulate order count
    ...
    def _execution_complete(self, reason: str):
        ...
        if reason == "target_achieved":
            self.validation_label.setText("🎯 TARGET ACHIEVED! Execution completed successfully")
            self.validation_label.setStyleSheet("color: green; font-weight: bold;")
            QMessageBox.information(self, "Success", "🎯 Profit target achieved successfully!")
        else:
    ...
    def _update_analytics_chart(self):
        ...
        try:
            ...
            ax.plot(times, profits, 'b-', linewidth=2)
    ...
    def on_progress_update(self, progress: ProfitTargetProgress):
        ...
        self.time_elapsed_label.setText(f"Time Elapsed: {progress.time_elapsed_minutes} minutes")
        self.orders_executed_label.setText(f"Orders Executed: {progress.orders_executed}")
...
def create_day_profit_target_engine(ib_client: Optional[IB] = None,
                                   strategy_orchestrator: Optional[StrategyOrchestrator] = None,
                                   connectivity_manager: Optional[IntegratedConnectivityManager] = None) -> DayProfitTargetEngine:
...
def create_day_profit_target_widget(profit_engine: Optional[DayProfitTargetEngine] = None) -> DayProfitTargetWidget:
...
def main():
    ...
    try:
        ...
        print(f"  Supported Algorithms: {', '.join([a.value for a in SlicingAlgorithm])}")
