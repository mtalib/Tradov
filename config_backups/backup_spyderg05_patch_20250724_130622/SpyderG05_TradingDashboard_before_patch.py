...
Purpose: Advanced monitoring dashboard with real-time visualization and expanded Signal Monitor
Author: Mohamed Talib
Date Created: 2025-07-05 
Last Updated: 2025-07-21 Time: 23:30:56  

Description:
    This module provides a comprehensive monitoring dashboard for the SPYDER
    autonomous trading system. Features include:
    - System health monitoring with traffic light indicators
    - P&L tracking and performance metrics (6-column optimized table)
    - Risk monitoring with Greeks display
    - SPY 5-minute candlestick chart with technical indicators
    - Autonomous AI activity logs with timestamps
    - Market data watchlist with custom indicators
    - Active positions management
    - Expanded Signal monitoring panel with 10 indicators in 2x5 grid
    - Custom metric dialogs (GEX, DEX, OGL, DIX, BSWAN)
    - Professional dark theme with optimized spacing
...
class VIXMonitorDialog(MonitorDialog):
    ...
    def __init__(self, parent=None):
        ...
        self.regime_value.setStyleSheet(f"font-size: 16px; color: {COLORS['negative']}; font-weight: normal;")
        ...
        self.chart_area.setPlainText("Chart would display VIX term structure over time\n" +
...
class RSIConfluenceDialog(MonitorDialog):
    ...
    def __init__(self, parent=None):
        ...
        # Confluence Signal
        signal_frame = QFrame()
        signal_frame.setStyleSheet(f"background-color: {COLORS['panel']}; border: 1px solid {COLORS['border']}; padding: 10px;")
        ...
        self.signal_text.setStyleSheet(f"color: {COLORS['warning']}; font-size: 14px; font-weight: normal;")
...
class DivergenceDialog(MonitorDialog):
    ...
    def __init__(self, parent=None):
        ...
        # $TICK vs SPY
        tick_frame = QFrame()
        tick_frame.setStyleSheet(f"background-color: {COLORS['panel']}; border: 1px solid {COLORS['border']}; padding: 8px;")
        ...
        # MACD vs Price
        macd_frame = QFrame()
        macd_frame.setStyleSheet(f"background-color: {COLORS['panel']}; border: 1px solid {COLORS['border']}; padding: 8px;")
        ...
        # Volume vs Price
        vol_frame = QFrame()
        vol_frame.setStyleSheet(f"background-color: {COLORS['panel']}; border: 1px solid {COLORS['border']}; padding: 8px;")
        ...
        self.recommendation.setPlainText("Mixed divergence signals suggest caution.\nWait for confluence before taking positions.")
...
class AIDecisionDialog(MonitorDialog):
    ...
    def __init__(self, parent=None):
        ...
        tertiary_group.setStyleSheet(f"""
            QGroupBox {{ 
                font-weight: normal; 
                color: {COLORS['cyan']}; 
                padding-top: 15px;
        ...
        decision_frame.setStyleSheet(f"""
...
class RiskTriggersDialog(MonitorDialog):
    ...
    def __init__(self, parent=None):
        ...
        # SKEW Alert
        skew_frame = QFrame()
        skew_frame.setStyleSheet(f"background-color: {COLORS['panel']}; border: 1px solid {COLORS['warning']}; padding: 8px;")
        ...
        # $TRIN Status
        trin_frame = QFrame()
        trin_frame.setStyleSheet(f"background-color: {COLORS['panel']}; border: 1px solid {COLORS['positive']}; padding: 8px;")
        ...
        bb_frame = QFrame()
        bb_frame.setStyleSheet(f"background-color: {COLORS['panel']}; border: 1px solid {COLORS['cyan']}; padding: 8px;")
        ...
        # VWAP Deviation
        vwap_frame = QFrame()
        vwap_frame.setStyleSheet(f"background-color: {COLORS['panel']}; border: 1px solid {COLORS['neutral']}; padding: 8px;")
        ...
        overall_frame.setStyleSheet(f"background-color: {COLORS['warning']}; padding: 10px; border-radius: 5px; margin-top: 10px;")
...
class SpyderTestDashboard(QMainWindow):
    ...
    def update_chart(self):
        ...
        ax.axhline(y=pivot, color='#FFFF00', linewidth=1.5, linestyle='-', alpha=0.7, label='Pivot', zorder=1)  # Bright yellow
        ax.axhline(y=r1, color='#00FF41', linewidth=1.5, linestyle='-', alpha=0.6, label='R1', zorder=1)  # Straight line
        ax.axhline(y=r2, color='#00FF41', linewidth=1.5, linestyle='-', alpha=0.6, label='R2', zorder=1)  # Straight line
        ax.axhline(y=r3, color='#00FF41', linewidth=1.5, linestyle='-', alpha=0.6, label='R3', zorder=1)  # Straight line
        ax.axhline(y=s1, color='#FF1744', linewidth=1.5, linestyle='-', alpha=0.6, label='S1', zorder=1)  # Straight line
        ax.axhline(y=s2, color='#FF1744', linewidth=1.5, linestyle='-', alpha=0.6, label='S2', zorder=1)  # Straight line
        ax.axhline(y=s3, color='#FF1744', linewidth=1.5, linestyle='-', alpha=0.6, label='S3', zorder=1)  # Straight line
        ...
        ax.plot(ma_x, ma_y, color='#00B8D4', linewidth=1.5, alpha=0.8, label='MA(20)', zorder=2)  # Normal weight
        
        # Plot VWAP (behind candlesticks) - CHANGED TO BRIGHT PURPLE
        ax.plot(range(len(vwap)), vwap, color='#BF00FF', linewidth=1.5, alpha=0.9, label='VWAP', zorder=2)  # Bright purple
        ...
        ax.text(len(dates), pivot, f' P: {pivot:.2f}', color='#FFFF00', fontsize=9, va='center')  # Bright yellow, no bold
    ...
    def update_automation_status(self):
        ...
        if vix_value > 20:
            ...
            relevant_messages = [msg for msg in automation_messages if 'volatility' in msg.lower() or 'risk' in msg.lower()]
        elif vix_value < 15:
            # Low volatility messages
            relevant_messages = [msg for msg in automation_messages if 'premium' in msg.lower() or 'IC' in msg]
        else:
