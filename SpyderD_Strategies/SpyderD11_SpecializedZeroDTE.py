...
from SpyderU_Utilities.SpyderU07_Constants import (
    ...
    ZERO_DTE_PROFIT_TARGET, ZERO_DTE_STOP_LOSS
)
...
class SpecializedZeroDTEStrategy(BaseStrategy):
    ...
    def generate_signals(self, market_data: pd.DataFrame) -> List[TradingSignal]:
        ...
        try:
            ...
            # Generate setup
            setup = self._create_zero_dte_setup(
                selected_strategy, market_data, market_conditions
            )
    ...
    def _identify_support_resistance(self, market_data: pd.DataFrame) -> Dict[str, float]:
        ...
        try:
            ...
            prev_high = market_data['high'].iloc[-390:-330].max() if len(market_data) > 390 else high
    ...
    def _select_zero_dte_strategy(self, market_bias: MarketBias, 
                                 conditions: Dict[str, Any]) -> ZeroDTEStrategy:
    ...
    def _create_zero_dte_setup(self, strategy: ZeroDTEStrategy,
    ...
    def _setup_iron_butterfly(self, current_price: float, 
                             conditions: Dict[str, Any]) -> Optional[Dict]:
        ...
        try:
            ...
            expected_move = current_price * conditions.get('realized_vol', 0.15) / 100 * np.sqrt(1/252)
    ...
    def _setup_iron_condor(self, current_price: float,
                          conditions: Dict[str, Any]) -> Optional[Dict]:
    ...
    def _setup_credit_spread(self, current_price: float,
                           conditions: Dict[str, Any]) -> Optional[Dict]:
    ...
    def _setup_broken_wing_butterfly(self, current_price: float,
                                   conditions: Dict[str, Any]) -> Optional[Dict]:
    ...
    def _setup_short_strangle(self, current_price: float,
                            conditions: Dict[str, Any]) -> Optional[Dict]:
    ...
    def _calculate_win_probability(self, setup: Dict, conditions: Dict[str, Any]) -> float:
        ...
        try:
            ...
            if 'breakeven_lower' in setup and 'breakeven_upper' in setup:
                ...
                lower_z = (setup['breakeven_lower'] - current_price) / (current_price * vol * np.sqrt(time_fraction/252))
                upper_z = (setup['breakeven_upper'] - current_price) / (current_price * vol * np.sqrt(time_fraction/252))
                
                prob_profit = norm.cdf(upper_z) - norm.cdf(lower_z)
            
            elif 'breakeven' in setup:
                # One-sided breakeven
                z_score = (setup['breakeven'] - current_price) / (current_price * vol * np.sqrt(time_fraction/252))
    ...
    def _create_entry_signal(self, setup: ZeroDTESetup, 
                           conditions: Dict[str, Any]) -> TradingSignal:
    ...
    def _calculate_position_metrics(self, position: ZeroDTEPosition,
                                  market_data: pd.DataFrame) -> ZeroDTEMetrics:
        ...
        try:
            ...
            if hasattr(position.setup, 'breakeven_lower') and hasattr(position.setup, 'breakeven_upper'):
    ...
    def _calculate_position_greeks(self, position: ZeroDTEPosition,
                                 current_price: float,
    ...
    def _calculate_position_value(self, position: ZeroDTEPosition,
        ...
        if hasattr(setup, 'breakeven_lower') and hasattr(setup, 'breakeven_upper'):
            if setup.breakeven_lower <= current_price <= setup.breakeven_upper:
                ...
                distance_from_center = abs(current_price - (setup.breakeven_lower + setup.breakeven_upper) / 2)
                profit_ratio = 1 - (distance_from_center / ((setup.breakeven_upper - setup.breakeven_lower) / 2))
    ...
    def _update_win_probability(self, position: ZeroDTEPosition,
                              current_price: float,
    ...
    def _create_exit_signal(self, position: ZeroDTEPosition,
                          reason: str,
                          metrics: ZeroDTEMetrics) -> TradingSignal:
        ...
        self.logger.info(f"Exit 0DTE position {position.position_id}: {reason}, P&L: ${metrics.pnl:.2f}")
    ...
    def get_position_summary(self) -> List[Dict[str, Any]]:
        ...
        for position_id, position in self.active_positions.items():
            summary = {
                ...
                'time_to_expiry': (position.setup.expiration_time - datetime.now(self.eastern_tz)).seconds / 3600
            }
...
def test_specialized_zero_dte():
    ...
    dates = pd.date_range(
        start=current_time.replace(hour=9, minute=30),
        end=current_time,
        freq='5min'
    )
    ...
    print(f"Win Rate: {summary['strategy_stats']['winning_trades'] / max(1, summary['strategy_stats']['total_trades']) * 100:.1f}%")
