...
Change Log:
    2026-03-27:
        ...
        - Key is derived from TRADOV_ENCRYPTION_KEY env var or auto-generated
    2026-01-16:
...
def _get_or_create_key() -> bytes:
    ...
    If TRADOV_ENCRYPTION_KEY is set it must be a valid url-safe base64
    ...
    """
    env_key = os.environ.get("TRADOV_ENCRYPTION_KEY")
    if env_key:
        return env_key.encode() if isinstance(env_key, str) else env_key
    logger.warning(
        "TRADOV_ENCRYPTION_KEY not set — generating ephemeral key. "
...
class EncryptionManager:
    ...
    module-level key derived from ``TRADOV_ENCRYPTION_KEY`` is used.
    """
