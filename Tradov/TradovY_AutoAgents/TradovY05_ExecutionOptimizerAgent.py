...
Spyder Version: 1.0
Module: TradovY05_ExecutionOptimizerAgent.py
Group: Y (AutoAgents)
...
Description:
    ...
    Wraps TradovX07_ExecutionStrategyAgent and interfaces with
    TradovB40_TradierClient for actual order placement.
...
try:
    from Tradov.TradovX_Agents.TradovX07_ExecutionStrategyAgent import (
        TradovX07_ExecutionStrategyAgent,
    )
    X07_AVAILABLE = True
except ImportError:
    X07_AVAILABLE = False
...
try:
    from Tradov.TradovB_Broker.TradovB02_OrderManager import (
        OrderManager as TradovB02_OrderManager,
    )
...
# ==============================================================================
class TradovY05_ExecutionOptimizerAgent(BaseAutoAgent):
    ...
    def __init__(self, enable_veto_consumption: bool = True, **kwargs: Any):
        ...
        if X07_AVAILABLE:
            try:
                self._x07_agent = TradovX07_ExecutionStrategyAgent()
        ...
        if TRADIER_AVAILABLE:
            try:
                self._tradier = TradovB40_TradierClient()
            except Exception as e:
        ...
        if ORDER_MGR_AVAILABLE:
            try:
                self._order_mgr = TradovB02_OrderManager()
            except Exception as e:
...
def create_execution_optimizer_agent(
    **kwargs: Any,
    ...
    return TradovY05_ExecutionOptimizerAgent(**kwargs)
