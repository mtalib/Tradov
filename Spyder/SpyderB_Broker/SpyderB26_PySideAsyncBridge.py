...
try:
    ...
    class QObject:
        """Fallback QObject when PySide6 not available"""

        pass

    class Signal:
        """Fallback Signal when PySide6 not available"""
    ...
    class QTimer:
        """Fallback QTimer when PySide6 not available"""
...
try:
    ...
    # Create fallback classes
    class IB:
        """Fallback IB when ib_async not available"""

        pass

    class Contract:
        """Fallback Contract when ib_async not available"""

        pass

    class Order:
        """Fallback Order when ib_async not available"""

        pass

    class MarketOrder:
        """Fallback MarketOrder when ib_async not available"""

        pass

    class LimitOrder:
        """Fallback LimitOrder when ib_async not available"""

        pass

    class util:
        """Fallback util when ib_async not available"""
...
class AsyncIBGatewayBridge(QObject):
    ...
    def __init__(self, paper_trading: bool = True):
        ...
        self.logger.info(
            f"✅ AsyncIBGatewayBridge initialized for {'PAPER' if paper_trading else 'LIVE'} trading"
        )
    ...
    async def _schedule_reconnect(self):
        ...
        self.logger.info(
            f"⏳ Scheduling reconnection attempt {self.reconnect_attempts}/{MAX_RECONNECT_ATTEMPTS} in {RECONNECT_DELAY}s"
        )
    ...
    async def unsubscribe_market_data(self, req_id: int) -> bool:
        ...
        try:
            if req_id in self.subscriptions:
                ...
                self.logger.info(
                    f"✅ Unsubscribed from market data for request {req_id}"
                )
                return True
            return False
    ...
    async def test_connection(self) -> bool:
        ...
        try:
            ...
            server_time = self.ib.reqCurrentTime()
            self.logger.info(
                f"✅ Connection test successful. Server time: {server_time}"
            )
            return True
