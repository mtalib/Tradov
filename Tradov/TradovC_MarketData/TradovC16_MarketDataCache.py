...
Module Description:
    ...
    Note: TradovH03_MarketDataCache provides a simpler in-memory-only cache
...
from Tradov.TradovA_Core.TradovA05_EventManager import EventManager, EventType, Event
from Tradov.TradovH_Storage.TradovH03_MarketDataCache import (
...
DEFAULT_CACHE_CONFIG = {
    ...
    'persistence': {
        'enabled': True,
        'db_path': 'tradov_market_cache.db',
        'retention_days': 30,
        'compression': True
    },
    'redis': {
...
class MarketDataCache:
    ...
    def invalidate(self, symbol: str | None = None):
        ...
        if self.redis_client:
            if symbol:
                self.redis_client.delete(f"tradov:market:{symbol}")
            else:
                # Clear all market data keys
                for key in self.redis_client.scan_iter("tradov:market:*"):
...
if __name__ == "__main__":
    ...
    df = cache.get_range('TRAD', start_time, end_time)
    if not df.empty:
        pass
