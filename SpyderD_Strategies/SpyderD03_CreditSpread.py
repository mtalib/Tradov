...
class CreditSpreadStrategy(BaseStrategy):
    ...
    def _calculate_spread_metrics(self, spread_type: SpreadType,
                                 ...
                                 current_price: float, dte: int) -> Optional[Dict[str, Any]]:
...
if __name__ == "__main__":
    ...
    print(f"Market Condition: {strategy.market_condition.name}")
    print(f"Volatility Rank: {strategy.volatility_rank:.2f}")
    print(f"Signals Generated: {len(signals)}")
    
    for signal in signals:
        spread_data = signal.metadata.get('spread_data', {})
        print(f"\nSignal Type: {spread_data.get('spread_type', 'Unknown')}")
        print(f"Strength: {signal.strength.name}")
        print(f"Premium: ${spread_data.get('credit', 0):.2f}")
        print(f"Max Loss: ${spread_data.get('max_loss', 0):.2f}")
        print(f"Probability of Profit: {spread_data.get('probability_profit', 0):.2%}")
        print(f"Breakeven: ${spread_data.get('breakeven', 0):.2f}")
    
    # Get strategy summary
    summary = strategy.get_strategy_summary()
    print(f"\nStrategy Summary:")
    print(f"Active Bull Puts: {summary['active_positions']['bull_puts']}")
    print(f"Active Bear Calls: {summary['active_positions']['bear_calls']}")
    print(f"Total Credit: ${summary['exposure']['total_credit']:.2f}")
    print(f"Total Risk: ${summary['exposure']['total_risk']:.2f}")
    
    # Stop strategy
    strategy.stop()
    
    print("\nCreditSpreadStrategy test completed!")