...
Usage:
    from Tradov.TradovU_Utilities.TradovU42_StrategyCircuitBreaker import (
    ...
    if scb.is_allowed("iron_condor_TRAD"):
        # … submit orders …
        scb.record_success("iron_condor_TRAD")
    else:
        logger.warning("iron_condor_TRAD circuit is open – skipping")

    # On failure
    scb.record_failure("iron_condor_TRAD", reason="order rejected", pnl_impact=-120.0)

    # Track P&L separately (e.g. from fill callbacks)
    scb.record_pnl("iron_condor_TRAD", pnl=-45.0)

    # Human-readable dashboard
    print(scb.get_status_report())

Integration Points:
    - TradovD_Strategies / TradovD31_StrategyOrchestrator: call is_allowed()
      before dispatching strategy signals.
    - TradovE_Risk / TradovE01_RiskManager: call record_pnl() from P&L
      callbacks so loss-based tripping works without needing an explicit
      failure event.
    - TradovG_GUI / TradovG05_TradingDashboard: call get_all_states() to
      render per-strategy circuit status in the dashboard.
    - TradovI_Integration / TradovI09_DiagnosticsEngine_HealthChecks:
...
class StrategyCircuitBreaker:
    """Per-strategy circuit breaker for the Tradov trading system.
