...
Series: TradovA_Core
Module: TradovA06_MasterController.py
Purpose: TRADOV - Autonomous Options Trading System
...
try:
    from Tradov.TradovU_Utilities.TradovU03_DateTimeUtils import now_et as _now_et
except ImportError:
    try:
...
# Logging is configured by TradovA01_Main (the application entry point).
...
class MasterController:
    """
    Master control system for Tradov autonomous trading
    """

    def __init__(self, config_path: str = "config/tradov_config.yaml"):
    ...
    def _load_configuration(self, config_path: str) -> SystemConfig:
        ...
        return SystemConfig(
            ...
            database=config_data.get("database", {"type": "sqlite", "path": "data/tradov.db"}),
    ...
    def _get_default_autonomous_session(self) -> dict[str, Any]:
        """Default autonomous session controls for TRAD options."""
        return {
    ...
    def _get_default_config(self) -> dict[str, Any]:
        ...
        return {
            ...
            "max_positions": 10,
            "risk_limits": {"max_var": 0.10, "max_drawdown": 0.20, "max_concentration": 0.25},
            "database": {"type": "sqlite", "path": "data/tradov.db"},
            ...
            "autonomous_session": {
                "primary_start_et": os.environ.get("TRADOV_SESSION_PRIMARY_START_ET", "09:30"),
                "primary_end_et": os.environ.get("TRADOV_SESSION_PRIMARY_END_ET", "16:15"),
                "first_entry_not_before_et": os.environ.get("TRADOV_FIRST_ENTRY_NOT_BEFORE_ET", "09:45"),
                "zero_dte_no_new_risk_cutoff_et": os.environ.get("TRADOV_ZERO_DTE_NO_NEW_RISK_CUTOFF_ET", "14:30"),
                "broker_cutoff_et": os.environ.get("TRADOV_BROKER_CUTOFF_ET", "16:00"),
                "broker_cutoff_buffer_minutes": int(os.environ.get("TRADOV_BROKER_CUTOFF_BUFFER_MINUTES", "10")),
                "pin_risk_monitor_end_et": os.environ.get("TRADOV_PIN_RISK_MONITOR_END_ET", "17:30"),
                "fail_closed_if_cutoff_unknown_live": _env_bool("TRADOV_FAIL_CLOSED_IF_CUTOFF_UNKNOWN_LIVE", True),
            },
    ...
    def _initialize_module_registry(self):
        ...
        module_definitions = [
            ...
            ("B01_TradovClient", "B", "Tradov Client", ["B05_ConnectionManager"], 3),
            # Market Data (Priority 4)
            ("C01_DataFeed", "C", "Data Feed", ["B01_TradovClient"], 4),
    ...
    def start_system(self) -> bool:
        """Start the entire Tradov system"""

        try:
            logger.info("=" * 80)
            logger.info("TRADOV SYSTEM STARTUP INITIATED")
    ...
    def _get_startup_sequence(self) -> list[StartupSequence]:
        ...
        return [
            ...
            StartupSequence(
                ...
                modules=["B05_ConnectionManager", "B16_GatewayIntegration", "B01_TradovClient"],
    ...
    def _initialize_component(self, module_id: str) -> Any | None:
        ...
        if module_id == "H02_DatabaseManager":
            ...
            db_path = _Path(self.config.database.get("path", "data/tradov.db"))
            return _load(
                "TradovH_Storage.TradovH02_DatabaseManager",
                "DatabaseManager",
                db_path=db_path,
            )
        ...
        if module_id == "U02_ErrorHandler":
            return _load("Tradov.TradovU_Utilities.TradovU02_ErrorHandler", "TradovErrorHandler")

        if module_id == "I06_AgentMessageBus":
            return _load("TradovI_Integration.TradovI06_AgentMessageBus", "AgentMessageBus")

        if module_id == "L18_EnhancedMLIntegration":
            return _load("TradovL_ML.TradovL18_EnhancedMLIntegration", "EnhancedMLEngine")
        ...
        if module_id == "X16_MetaCoordinator":
            ...
            return _load("TradovX_Agents.TradovX16_MetaCoordinator", "MetaCoordinator")

        # ── Broker client — requires env-var credentials ──────────────────────
        if module_id in ("B40_TradierClient", "B01_TradovClient"):
            try:
        ...
        if module_id == "J05_TelegramBot":
            ...
            try:
                ...
                if event_mgr is None or not isinstance(event_mgr, EventManager):
                    from TradovA_Core.TradovA05_EventManager import EventManager as _EM
                    event_mgr = _EM()
                bot = TelegramBot(
                    bot_token=bot_token,
                    chat_id=chat_id,
        ...
        if module_id == "Y10_AgentScheduler":
            try:
                from TradovY_AutoAgents.TradovY10_AgentScheduler import AgentScheduler
                from TradovY_AutoAgents.TradovY01_MarketSenseAgent import TradovY01_MarketSenseAgent
                from TradovY_AutoAgents.TradovY02_StrategyPilotAgent import TradovY02_StrategyPilotAgent  # noqa: E501
                from TradovY_AutoAgents.TradovY03_RiskSentinelAgent import TradovY03_RiskSentinelAgent  # noqa: E501
                from TradovY_AutoAgents.TradovY04_AlphaLearnerAgent import TradovY04_AlphaLearnerAgent  # noqa: E501
                from TradovY_AutoAgents.TradovY05_ExecutionOptimizerAgent import TradovY05_ExecutionOptimizerAgent  # noqa: E501
                from TradovY_AutoAgents.TradovY06_NewsSentinelAgent import TradovY06_NewsSentinelAgent  # noqa: E501
                from TradovY_AutoAgents.TradovY07_TradeJournalAgent import TradovY07_TradeJournalAgent  # noqa: E501
                from TradovY_AutoAgents.TradovY08_MetaOrchestratorAgent import TradovY08_MetaOrchestratorAgent  # noqa: E501
                from TradovY_AutoAgents.TradovY09_CodeReviewerAgent import TradovY09_CodeReviewerAgent  # noqa: E501
                ...
                scheduler.register(TradovY09_CodeReviewerAgent)
        ...
        if module_id == "L09_RegimeClassifier":
            try:
                from TradovL_ML.TradovL09_UnifiedRegimeEngine import UnifiedRegimeEngine
        ...
        if module_id == "D31_StrategyOrchestrator":
            try:
                from TradovD_Strategies.TradovD31_StrategyOrchestrator import StrategyOrchestrator
                ...
                try:
                    from TradovA_Core.TradovA05_EventManager import get_event_manager as _gem
                    _event_mgr = _gem()
                except Exception:
                    _event_mgr = None
        ...
        if module_id == "A04_Scheduler":
            try:
                from TradovA_Core.TradovA04_Scheduler import Scheduler
                try:
                    from TradovA_Core.TradovA05_EventManager import get_event_manager as _gem_a04
        ...
        if module_id == "R04_LiveEngine":
            try:
                from TradovR_Runtime.TradovR04_LiveEngine import LiveEngine, LiveTradingConfig
                from TradovE_Risk.TradovE01_RiskManager import get_risk_manager as _get_rm
                _account_id = os.environ.get("TRADIER_ACCOUNT_ID", "")
                _live_cfg = LiveTradingConfig(account_id=_account_id)
                _broker = self.components.get("B01_TradovClient")
                _risk_mgr = _get_rm()
                try:
                    from TradovA_Core.TradovA05_EventManager import get_event_manager as _gem_r04
        ...
        if module_id == "E01_RiskManager":
            try:
                from TradovE_Risk.TradovE01_RiskManager import get_risk_manager
        ...
        if module_id == "E19_UnifiedRiskCoordinator":
            try:
                from TradovE_Risk.TradovE19_UnifiedRiskCoordinator import UnifiedRiskCoordinator
    ...
    def _get_shutdown_sequence(self) -> list[dict[str, Any]]:
        ...
        return [
            ...
            {"name": "Broker Connection", "modules": ["B01_TradovClient", "B05_ConnectionManager"]},
    ...
    def _check_broker_connection(self) -> bool:
        ...
        return "B40_TradierClient" in self.components or "B01_TradovClient" in self.components
...
if __name__ == "__main__":
    """
    Main entry point for Tradov Autonomous Trading System
    """
