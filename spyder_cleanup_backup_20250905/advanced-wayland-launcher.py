...
def setup_wayland_environment():
    ...
    # Store original DISPLAY if it exists, then set dummy for imports
    # This prevents the KeyError during mouseinfo import
    original_display = os.environ.get("DISPLAY")
    os.environ["DISPLAY"] = ":99"  # Dummy display for import only
...
def main():
    ...
    # Set up environment first and get original display
    original_display = setup_wayland_environment()
    ...
    try:
        ...
        dashboard_path = os.path.join(
            os.path.dirname(__file__), "SpyderR_Runtime", "SpyderR05_LiveDashboard.py"
        )
