...
class RiskMetricsCalculator:
    ...
    def calculate_metrics(self, 
                        ...
                        period_end: Optional[datetime] = None) -> RiskMetrics:
        """
    ...
    def calculate_rolling_metrics(self,
        ...
        for i in range(0, len(returns) - window_size + 1, step_size):
            ...
            # Store results
            timestamps.append(datetime.now() - timedelta(days=len(returns) - i - window_size))
    ...
    def analyze_drawdowns(self, equity_curve: List[float]) -> DrawdownAnalysis:
        ...
        worst_drawdowns = sorted(
            [(dd, start, end) for (start, end, _, _), dd in zip(drawdowns, all_dd_values)],
            key=lambda x: x[0],
            reverse=True
        )[:5]  # Top 5 worst
    ...
    def _calculate_drawdown_duration(self, equity_curve: List[float]) -> int:
        ...
        peak_idx = equity_curve.index(peak_value)
...
if __name__ == "__main__":
    print("="*80)
    print("SPYDER E06 - Risk Metrics Calculator Test")
    print("="*80)
    ...
    print("="*50)
    print(f"Period: {metrics.num_periods} days")
    print(f"\nRETURN METRICS:")
    print(f"  Total Return: {metrics.total_return:.2%}")
    print(f"  Annualized Return: {metrics.annualized_return:.2%}")
    print(f"  Average Daily Return: {metrics.average_return:.4%}")
    ...
    print(f"Max DD Duration: {dd_analysis.max_drawdown_duration} days")
    print(f"Current Drawdown: {dd_analysis.current_drawdown:.2%}")
    print(f"Underwater Periods: {dd_analysis.underwater_periods}")
    print(f"Average Drawdown: {dd_analysis.average_drawdown:.2%}")
    
    if dd_analysis.worst_drawdowns:
        print(f"\nWorst Drawdowns:")
        for i, (dd, start, end) in enumerate(dd_analysis.worst_drawdowns[:3]):
            print(f"  {i+1}. {dd:.2%} (days {start}-{end})")
    ...
    if rolling.sharpe_ratios:
        print(f"Latest Sharpe: {rolling.sharpe_ratios[-1]:.3f}")
        print(f"Average Sharpe: {np.mean(rolling.sharpe_ratios):.3f}")
        print(f"Sharpe Volatility: {np.std(rolling.sharpe_ratios):.3f}")
    ...
    print("\n✅ Risk Metrics Calculator test completed successfully!")