...
**Changelog:** v5 refreshes the architecture snapshot to reflect the latest pair-exit UX, scanner-state wording, and the current repo-aligned pair/stat-arb workflow.
...
Tradov is a production-grade, multi-agent stock/ETF and pair-trading platform that combines two trading paradigms into a single system:

- **Directional Equity/ETF Trading** — the primary legacy directional surface that trades stocks and ETFs based on regime detection, volatility analysis, and market-context signals.
...
Historical modules remain in the tree for context, but they are not part of the active execution path.
...
| X02 SentimentAnalyst | Sentiment Analyst | Social media and flow sentiment |
...
| **PCA-IV** | S14 | Volatility-surface shape via PCA |
...
Historical strategy modules remain in the repository for reference, but the active deployment is pair/stat-arb focused and the D31 strategy registry is retired from the live execution path.
...
- `TradovQ94_PairResearchWorkflow.py` for pair-specific research
- `TradovQ95_PairResearchLauncher.py` as the pair-research operator entry point
...
- `TradovT408_Q94_PairResearchWorkflow.py` and `TradovT409_Q95_PairResearchLauncher.py` for pair research coverage
...
Run the research workflows from the repo root:
...
Add `--register-model` to either launcher if you want to register the fitted artifact with the existing Tradov model registry.
...
| **Pair Trading** | Pair positions, spread charts, scanner results, plus `Cost` and `Funds Held by Broker` fields for open pair rows and risk summaries |
...
- Emits candidate economics metadata for the GUI, including estimated `Cost` and `Funds Held by Broker`
...
### Qlib-Style Pair Research

Qlib does not replace Tradov's pair engine. It can still improve pair trading by
making research repeatable and retraining disciplined:

- **Pair selection research**: score candidate pairs with walk-forward tests
- **Hedge-ratio experiments**: compare Kalman, rolling OLS, and other variants
- **Spread-regime models**: detect when a pair is mean-reverting vs broken
- **Artifact tracking**: store experiment outputs and promote only validated pairs
- **Drift handling**: retrain pair filters when cointegration or spread behavior degrades

The live pair stack stays Tradov-native:

- D51 PairScanner
- D52 CointegrationEngine
- D53 OUProcessFitter
- D54 KalmanHedgeRatio
- D42 PairTradingStrategy
- E26 PairRiskManager
- B02/B03 execution and position tracking
...
- Exposes pair cost basis and broker-held funds for dashboard presentation and worst-case loss visibility
...
### Layer 3b: Concurrent Live-Trade Cap (E01)

At any given time, the system should keep the number of live trades small enough to remain observable and controllable. The recommended default is:

- **3 total live trades max**
- **2 pair trades max**
- **1 directional trade max**

This keeps the pair book active without letting it crowd out directional exposure, while still leaving room for clean monitoring and risk intervention.
...
| `TRADOV_MAX_PAIR_TRADES` | `2` | Max concurrent pair trades |
