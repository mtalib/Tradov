...
class TradingHours:
    ...
    def is_early_close_day(self, check_date: Optional[date] = None) -> bool:
        ...
        if check_date.month == 7 and check_date.day == 3:
            if self.is_trading_day(check_date) and not self.is_trading_day(check_date + timedelta(days=1)):
                return True
        
        return False
    
    def get_market_hours(self, check_date: Optional[date] = None) -> Tuple[Optional[datetime], Optional[datetime]]:
        """
...
class DateTimeUtils:
    ...
    @staticmethod
    def get_trading_days_between(start_date: date, end_date: date, 
    ...
    @staticmethod
    def count_trading_days(start_date: date, end_date: date, 
        ...
        """
        return len(DateTimeUtils.get_trading_days_between(
    ...
    @staticmethod
    def get_option_expiry_dates(start_date: Optional[date] = None, 
                               weeks_ahead: int = 8) -> List[date]:
        """
    ...
    @staticmethod
    def format_option_symbol(underlying: str, expiry: date, 
...
def is_market_open(check_time: Optional[datetime] = None, extended_hours: bool = False) -> bool:
    ...
    if extended_hours:
        return (trading_hours.is_regular_hours(check_time) or 
                trading_hours.is_extended_hours(check_time))
    else:
    ...
    while current <= end:
        if trading_hours.is_trading_day(current):
            ...
            schedule_data.append({
                ...
                'day_quality': DateTimeUtils.get_day_quality_score() if current == date.today() else None
            })
        else:
...
def get_trading_hours(check_date: Optional[date] = None) -> Dict[str, Any]:
    ...
    hours_info = {
        ...
        'day_quality_score': DateTimeUtils.get_day_quality_score() if check_date == date.today() else None
    }
...
def get_market_hours(check_date: Optional[date] = None) -> Tuple[Optional[datetime], Optional[datetime]]:
    """
...
class TradingCalendar:
        ...
        for year in [current_year, current_year + 1]:
            for month in fomc_months:
                ...
                try:
                    ...
                    fomc_date = datetime.combine(third_wednesday, time(14, 0))  # 2 PM typical announcement
    ...
    def is_options_expiration_week(check_date: Optional[date] = None) -> bool:
        ...
        try:
            third_friday = DateTimeUtils.get_monthly_option_expiry(check_date.year, check_date.month)
    ...
    @staticmethod
    def get_upcoming_events(from_date: Optional[datetime] = None, days_ahead: int = 30) -> List[Dict[str, Any]]:
        """
...
if __name__ == "__main__":
    ...
    print(schedule[['date', 'day', 'trading_day', 'optimal_entry', 'exit_time', 'day_quality']].to_string())
    ...
    for event in events:
        print(f"  {event['date'].strftime('%Y-%m-%d')}: {event['description']} ({event['impact']} impact)")
