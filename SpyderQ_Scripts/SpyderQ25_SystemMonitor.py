...
class SpyderSystemMonitor:
    ...
    def _format_text_report(self, data: Dict) -> str:
        ...
        lines.append(f"  Memory: {metrics['memory_used_gb']:.1f}/{metrics['memory_total_gb']:.1f} GB ({metrics['memory_percent']:.1f}%)")
        lines.append(f"  Disk: {metrics['disk_usage_percent']:.1f}%")
        lines.append(f"  Network: ↑{metrics['network_sent_mb']:.1f} MB ↓{metrics['network_recv_mb']:.1f} MB")
        lines.append("")
        ...
        for name, info in data['components'].items():
            ...
            if info['pid']:
                lines.append(f"      PID: {info['pid']} | CPU: {info['cpu_percent']:.1f}% | Mem: {info['memory_mb']:.1f} MB | Up: {info['uptime']}")
            if info['error']:
        ...
        for name, spec in self.component_specs.items():
            if spec.get('critical', False):
                ...
                if health.status == ComponentStatus.STOPPED:
                    ...
                    self.alerts.append(f"Critical component '{name}' has errors: {health.last_error}")
