# Spyder Trading System — Codebase Review v17
**Date:** April 14, 2026
**Scope:** Full codebase inventory — series, modules, lines of code, and descriptions
...
Spyder is an autonomous algorithmic trading system targeting SPY (S&P 500 ETF) options. The system uses the **Tradier API** for order execution and **Massive** (formerly Polygon.io) as the primary real-time market data provider. It is structured as 25 distinct Python packages (series A–Z, excluding W), each handling a well-defined domain.
...
| Total lines of code | **425,683** |
| Production LOC | **331,098** |
| Test LOC | **94,585** |
| Series (packages) | **25** |
...
- **Market Data:** Massive SDK (primary), Databento shim (legacy/fallback)
- **GUI:** PySide6 (Qt6)
...
|--------|---------|------:|----:|----------------|
| A | `SpyderA_Core` | 8 | 9,710 | System orchestration, main entry point, configuration |
| B | `SpyderB_Broker` | 8 | 10,247 | Tradier API integration, order management, execution |
| C | `SpyderC_MarketData` | 27 | 29,364 | Real-time and historical market data, validation, routing |
| D | `SpyderD_Strategies` | 32 | 35,313 | Strategy implementations (Iron Condor, Credit Spread, Zero-DTE, etc.) |
| E | `SpyderE_Risk` | 25 | 28,275 | Risk management, position sizing, circuit breakers, drawdown |
| F | `SpyderF_Analysis` | 22 | 22,320 | Technical indicators, price action, volatility regime, ML prediction |
| G | `SpyderG_GUI` | 20 | 17,823 | PySide6 interface, dashboards, charts, widgets |
| H | `SpyderH_Storage` | 7 | 4,974 | SQLite persistence, data access layer, trade journal |
| I | `SpyderI_Integration` | 13 | 9,167 | Event routing, agent message bus, diagnostics, module registry |
...
| T | `SpyderT_Testing` | 122 | 94,585 | Full pytest test suite covering all series |
...
| `SpyderA02_TradingEngine.py` | 1,815 | Core trading loop. Drives the main cycle: polls market data, runs analysis, dispatches signals to strategies, validates through risk, submits orders, and updates portfolio state. |
...
**Package:** `SpyderC_MarketData` | **27 files** | **29,364 LOC**

The C-Series provides all market data ingestion, normalisation, validation, caching, and distribution. The primary live-data provider is Massive (C27); C26 is a lightweight shim preserved for backward compatibility and testing. C29 provides a provider-agnostic routing layer.

| Module | LOC | Description |
|--------|----:|-------------|
| `SpyderC00_MarketDataProtocol.py` | 910 | Abstract typed Protocol classes defining the interface contract (methods, return types) that all market data providers must satisfy. |
| `SpyderC01_DataFeed.py` | 1,551 | Primary real-time tick data feed. Normalises incoming raw ticks, validates completeness, applies subscriptions, and fans out to all registered consumers. |
| `SpyderC02_HistoricalData.py` | 940 | Historical OHLCV data retrieval, local caching to SQLite, and aggregation across timeframes (1m, 5m, 15m, 1h, daily). |
| `SpyderC03_OptionChain.py` | 1,083 | Options chain snapshot management. Maintains strike/expiry matrices for SPY, applies DTE and delta filters, and provides sorted strike ladders. |
...
| `SpyderC26_DatabentoClient.py` | 193 | Backward-compatible Databento shim. Provides the historical C26 interface using lightweight local mocks; retained for rollback and test compatibility. |
...
| `SpyderC29_DataProviderRouter.py` | 275 | Provider-agnostic data routing layer. Reads `ACTIVE_DATA_PROVIDER` env var and returns the appropriate client instance, decoupling strategy code from data sources. |
| `SpyderC30_OrderFlowAnalyzer.py` | 1,784 | Real-time order flow analysis: trade delta, cumulative delta, buy/sell imbalance, absorption signals, and institutional block detection. |
...
| `__init__.py` | 247 | Package init exporting primary feed, Massive client, router, and validator. |
...
| `SpyderE01_RiskManager.py` | 1,140 | Core risk manager entry point. Routes all pre-trade and post-trade risk checks to the appropriate E-Series sub-modules and aggregates verdicts. |
...
| `SpyderM07_MigrationMonitor.py` | 372 | Monitors data migration jobs (e.g., Databento → Massive) for completeness, validation, and row-count reconciliation. |
...
**Package:** `SpyderT_Testing` | **122 files** | **94,585 LOC**
...
| `SpyderT42_Integration_Test.py` | 350 | Live integration tests requiring sandbox credentials (skipped in CI). |
| `SpyderT43_OrderManager_Test.py` | 805 | Order manager tests: lifecycle state machine, partial fills, cancellations. |
| `SpyderT44_DatabentoClient_Test.py` | 305 | Databento client shim tests: interface compatibility and mock responses. |
...
*End of Spyder Codebase Review v17 — April 14, 2026*
