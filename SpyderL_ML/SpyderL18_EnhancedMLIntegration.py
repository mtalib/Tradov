...
class EnhancedMLEngine:
    ...
    def classify_regime(self, features: FeatureSet) -> MLPrediction:
        ...
        if hasattr(self.models['regime_classifier'], 'predict_proba'):
            ...
            regime_dist = {str(cls): float(prob) for cls, prob in zip(regime_classes, regime_probs)}
        else:
            regime = "NORMAL"
            confidence = 0.5
...
if __name__ == "__main__":
    ...
    print(f"Price Prediction: {price_pred.prediction:.2f}, Confidence: {price_pred.confidence:.2%}")
    
    vol_pred = ml_engine.predict_volatility(sample_features)
    print(f"Volatility Prediction: {vol_pred.prediction:.4f}, Confidence: {vol_pred.confidence:.2%}")
    
    regime_pred = ml_engine.classify_regime(sample_features)
    print(f"Regime: {regime_pred.prediction}, Confidence: {regime_pred.confidence:.2%}")
    ...
    print(f"Selected Strategies: {strategies.prediction}")
