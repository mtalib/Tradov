...
Module: TradovC04_MarketInternals.py
...
# LOCAL IMPORTS
# ==============================================================================
from Tradov.TradovA_Core.TradovA05_EventManager import Event, EventBus, EventType
...
INTERNAL_SYMBOLS = {
    ...
    # RVOL is computed locally from TRAD volume — no fetch symbol
}
...
# TRAD is fetched alongside internals solely to compute RVOL (Relative Volume).
# Its last/volume fields are NOT stored in internals_data — handled separately.
_TRAD_RVOL_SYMBOL = "TRAD"
...
class MarketInternalsSnapshot:
    ...
    rvol: float = 1.0   # TRAD Relative Volume ratio (1.0 = normal)
...
"""
        ...
        # TRAD average_volume from Tradier quote (20-day trailing average)
        self._spy_avg_volume: float = 0.0
        # TRAD cumulative volume at the start of today's session (pre-market clip)
    ...
    def _fetch_from_tradier(self) -> None:
        ...
        ($TICK, $ADD, $VIX, XLK, XLF, $TNX) plus TRAD (for RVOL) and
        pushes the values into the internals_data store via update_internal().
        """
        try:
            # Include TRAD alongside internals for RVOL computation
            symbols = list(TRADIER_FETCHABLE_SYMBOLS.keys()) + [_TRAD_RVOL_SYMBOL]
            ...
            for quote in raw:
                ...
                # --- TRAD: extract volume fields for RVOL, do NOT store as internal ---
                if tradier_symbol == _TRAD_RVOL_SYMBOL:
    ...
    def _update_spy_rvol_state(self, quote: dict) -> None:
        """Extract TRAD volume fields from a Tradier quote dict and update RVOL state.
    ...
    def _compute_rvol(self, current_volume: float) -> float:
        """Compute TRAD Relative Volume ratio.
