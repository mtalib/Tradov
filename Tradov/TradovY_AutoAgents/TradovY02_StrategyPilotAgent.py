...
Tradov Version: 1.0
Module: TradovY02_StrategyPilotAgent.py
Group: Y (AutoAgents)
...
Description:
    ...
    (TradovS) and market regime updates (from Y01). Uses a local LLM to
    validate signal quality, select optimal strategies for the current regime,
    and dynamically tune strategy parameters.

    Wraps TradovX03_StrategyDirectorAgent with autonomous scheduling and
...
try:
    from Tradov.TradovX_Agents.TradovX03_StrategyDirectorAgent import (
        TradovX03_StrategyDirectorAgent,
    )
    X03_AVAILABLE = True
except ImportError:
    X03_AVAILABLE = False
...
try:
    from Tradov.TradovD_Strategies import StrategyRegistry  # noqa: F401
...
# ==============================================================================
class TradovY02_StrategyPilotAgent(BaseAutoAgent):
    ...
    def __init__(self, **kwargs: Any):
        ...
        if X03_AVAILABLE:
            try:
                self._x03_agent = TradovX03_StrategyDirectorAgent()
    ...
    def _generate_daily_strategy_plan(self) -> None:
        ...
        self.publish(AgentOutput(
            ...
            output_type="report",
            topic="strategy.plan",
            payload={"plan": plan, "date": datetime.now(UTC).strftime("%Y-%m-%d")},
            confidence=0.7,
            reasoning=plan,
            priority="NORMAL",
    ...
    def _eod_positioning_analysis(self) -> None:
        ...
        prompt = (
            ...
            f"{sum(1 for v in self._validated_signals if v.approved and v.timestamp.date() == datetime.now(UTC).date())}\n"  # noqa: E501
...
# FACTORY
# ==============================================================================
def create_strategy_pilot_agent(**kwargs: Any) -> TradovY02_StrategyPilotAgent:
    """Factory function for creating the StrategyPilot agent."""
    return TradovY02_StrategyPilotAgent(**kwargs)
