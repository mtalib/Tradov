...
Module: TradovU03_DateTimeUtils.py
...
INDEX_OPTIONS_CLOSE = time(16, 15)  # 4:15 PM ET (SPX, TRAD)
...
class TradingHours:
    ...
    def is_options_trading_hours(self, dt: datetime | None = None, symbol: str = "TRAD") -> bool:
        """
        Check if currently in options trading hours.

        Args:
            dt: Datetime to check (None for current time)
            symbol: Symbol to check (TRAD has special hours)
        ...
        # Special handling for index options (TRAD, SPX)
        if symbol in ["TRAD", "SPX", "QQQ", "IWM"]:
...
def is_options_trading_time(check_time: datetime | None = None, symbol: str = "TRAD") -> bool:
    """
    Check if options are currently trading.

    Args:
        check_time: Time to check (None for current time)
        symbol: Option symbol (TRAD has special hours)
...
TRADTradingCalendar = TradingCalendar
