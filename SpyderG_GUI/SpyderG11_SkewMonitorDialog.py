...
from PyQt6.QtWidgets import (
    ...
    QGroupBox, QProgressBar, QTextEdit, QFrame, QGridLayout,
    QComboBox, QSpinBox, QTableWidget, QTableWidgetItem,
    QHeaderView, QSplitter, QWidget, QTabWidget, QSlider,
    QCheckBox, QButtonGroup, QRadioButton
)
from PyQt6.QtCore import Qt, QTimer, pyqtSignal, QThread, pyqtSlot, QDateTime
from PyQt6.QtGui import QColor, QPalette, QFont, QPixmap, QPainter, QBrush, QIcon
...
class SkewMonitorDialog(QDialog):
    ...
    def create_charts_tab(self) -> QWidget:
        ...
        if PYQTGRAPH_AVAILABLE:
            ...
            # Add reference lines
            self.skew_chart.addLine(y=SKEW_EXTREME_HIGH, pen=pg.mkPen('r', width=1, style=Qt.PenStyle.DashLine))
            self.skew_chart.addLine(y=SKEW_HIGH, pen=pg.mkPen('orange', width=1, style=Qt.PenStyle.DashLine))
            self.skew_chart.addLine(y=SKEW_HISTORICAL_MEAN, pen=pg.mkPen('yellow', width=2))
            self.skew_chart.addLine(y=SKEW_LOW, pen=pg.mkPen('g', width=1, style=Qt.PenStyle.DashLine))
    ...
    def create_strategies_tab(self) -> QWidget:
        ...
        self.strategies_table.setHorizontalHeaderLabels(["Strategy", "Rating", "Risk/Reward", "Reason"])
    ...
    def update_overview_display(self, data: SkewData):
        ...
        self.regime_label.setStyleSheet(f"background-color: {self.get_regime_color(data.regime)}; color: white; padding: 5px; border-radius: 3px;")
        self.regime_description.setText(self.get_regime_description(data.regime))
        
        # Update signal
        self.signal_label.setText(data.signal.replace('_', ' '))
        self.signal_label.setStyleSheet(f"background-color: {self.get_signal_color(data.signal)}; color: white; padding: 5px; border-radius: 3px;")
    ...
    def get_strategy_recommendations(self, skew_value: float) -> List[Dict]:
        ...
        if skew_value >= SKEW_EXTREME_HIGH:
            return [
                {"name": "Sell Put Spreads", "rating": "★★★★★", "risk_reward": "High/High", "reason": "Expensive put premiums"},
                {"name": "Buy Call Spreads", "rating": "★★★★", "risk_reward": "Med/High", "reason": "Relatively cheap calls"},
                {"name": "Jade Lizard", "rating": "★★★", "risk_reward": "Med/Med", "reason": "Collect high put premium"},
                {"name": "Iron Condor", "rating": "★★", "risk_reward": "Low/Med", "reason": "High put skew risk"},
                {"name": "Long Straddle", "rating": "★", "risk_reward": "High/Low", "reason": "Expensive volatility"}
            ]
        elif skew_value <= SKEW_LOW:
            return [
                {"name": "Buy Protective Puts", "rating": "★★★★★", "risk_reward": "Low/High", "reason": "Cheap tail protection"},
                {"name": "Long Volatility", "rating": "★★★★", "risk_reward": "Med/High", "reason": "Low risk pricing"},
                {"name": "Put Backspread", "rating": "★★★", "risk_reward": "Low/High", "reason": "Cheap downside exposure"},
                {"name": "Calendar Spreads", "rating": "★★★", "risk_reward": "Low/Med", "reason": "Vol expansion likely"},
                {"name": "Naked Puts", "rating": "★", "risk_reward": "High/Low", "reason": "Complacency risk"}
            ]
        else:
            return [
                {"name": "Iron Condor", "rating": "★★★★", "risk_reward": "Med/Med", "reason": "Balanced risk/reward"},
                {"name": "Calendar Spreads", "rating": "★★★", "risk_reward": "Low/Med", "reason": "Normal volatility"},
                {"name": "Credit Spreads", "rating": "★★★", "risk_reward": "Med/Med", "reason": "Standard premium"},
                {"name": "Butterfly", "rating": "★★★", "risk_reward": "Low/High", "reason": "Range-bound market"},
                {"name": "Diagonal Spreads", "rating": "★★", "risk_reward": "Med/Med", "reason": "Time decay play"}
            ]
    ...
    def get_market_interpretation(self, data: SkewData) -> str:
        ...
        if data.value >= SKEW_EXTREME_HIGH:
            ...
            interp += "Implications: Moderate hedging costs, balanced risk/reward in premium selling."
            
        elif data.value <= SKEW_LOW:
            interp += "MARKET COMPLACENCY\n"
            interp += "Very low tail risk pricing suggests market complacency. "
            interp += "Investors are not hedging aggressively, possibly due to recent calm periods. "
