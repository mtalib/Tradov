...
class OrderRouter:
    ...
    def submit_order(self, order: RoutableOrder) -> str:
        ...
        if self.circuit_breaker.state == CircuitBreakerState.TRIGGERED:
            if self.circuit_breaker.cooldown_until and datetime.now(UTC) < self.circuit_breaker.cooldown_until:  # noqa: E501
        ...
        self.order_lifecycles[order.order_id] = OrderLifecycle(
            ...
            events=[(RoutingEvent.RECEIVED, datetime.now(UTC), {})],
            routing_decisions=[],
            fills=[]
        )
    ...
    def _process_fill(self, order_id: str, venue: ExecutionVenue, quantity: int):
        ...
        if order_id in self.active_orders:
            ...
            if lifecycle.total_filled >= order.quantity:
                ...
                lifecycle.end_time = datetime.now(UTC)
        ...
        while not self.stop_event.is_set():
            try:
                ...
                recent_orders = [
                    ...
                    if lifecycle.start_time > datetime.now(UTC) - timedelta(seconds=1)
                ]
                ...
                if total_recent > 0:
                    recent_rejects = sum(
                        1 for lifecycle in self.order_lifecycles.values()
                        if lifecycle.start_time > datetime.now(UTC) - timedelta(seconds=10) and
    ...
    def _trigger_circuit_breaker(self):
        ...
        self.circuit_breaker.last_triggered = datetime.now(UTC)
        self.circuit_breaker.cooldown_until = datetime.now(UTC) + timedelta(seconds=CIRCUIT_BREAKER_COOLDOWN)  # noqa: E501
    ...
    def cancel_order(self, order_id: str) -> bool:
        ...
        if order_id in self.order_lifecycles:
            ...
            self.order_lifecycles[order_id].end_time = datetime.now(UTC)
