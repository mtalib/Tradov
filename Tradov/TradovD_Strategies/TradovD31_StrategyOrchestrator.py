...
Module Description:
    ...
    with the Tradov connectivity management and provides institutional-grade
...
Key Features:
    ...
    - Integration with TradovB connectivity and TradovE risk management
...
try:
    from Tradov.TradovD_Strategies.TradovD39_PutCreditSpread7 import (
...
try:
    from Tradov.TradovU_Utilities.TradovU03_DateTimeUtils import now_et as _d31_now_et
except ImportError:
    try:
...
try:
    ...
    try:
        from Tradov.TradovD_Strategies.TradovD01_BaseStrategy import BaseStrategy  # noqa: F401
        from Tradov.TradovD_Strategies.TradovD01_BaseStrategy import is_strategy_class as _is_strategy_class  # noqa: F401
    ...
    class EvolvedCreditSpreadAdapter(BaseStrategy):
        ...
        def __init__(self, name=None, event_manager=None, risk_profile=None, config=None):
            ...
            evolved_credit_spread_core = _optional_strategy(
                "Tradov.TradovD_Strategies.TradovD18_EvolvedCreditSpread",
        ...
        def _to_d18_market_data(self, market_data: pd.DataFrame) -> dict[str, Any]:
            prices = self._extract_series(market_data, ["TRAD", "close", "Close", "price", "last"])
        ...
        def _to_base_signal(self, native_signal: Any, default_symbol: str = "TRAD") -> BaseTradingSignal | None:
        ...
        def validate_signal(self, signal: Any) -> bool:
            return (
                signal is not None
                and getattr(signal, "symbol", "") == "TRAD"
    ...
    class VIXHedgingAdapter(BaseStrategy):
        ...
        def __init__(self, name=None, event_manager=None, risk_profile=None, config=None):
            ...
            vix_hedging_core = _optional_strategy(
                "Tradov.TradovD_Strategies.TradovD28_VIXHedging",
                "VIXHedgingStrategy",
            )
        ...
        def _recommendation_to_signal(
            ...
            reference_price = self._extract_last(market_data, [symbol, "TRAD", "close", "Close"], 20.0)
...
# Override via env: TRADOV_MAX_CONCURRENT_STRATEGIES, TRADOV_MAX_ACTIVE_HORIZON_BUCKETS.
...
def _record_risk_rejection_metric(strategy: str, rejection_reason: str) -> None:
    ...
    try:
        from Tradov.TradovB_Broker.TradovB15_PrometheusMetrics import record_risk_rejection
    except ImportError:
        return
...
class StrategyOrchestrator:
    ...
    def __init__(self,
        ...
        self.max_concurrent_strategies = max(
            1,
            int(os.environ.get("TRADOV_MAX_CONCURRENT_STRATEGIES", str(MAX_CONCURRENT_STRATEGIES)))
        )
        self.max_active_horizon_buckets = max(
            1,
            int(os.environ.get("TRADOV_MAX_ACTIVE_HORIZON_BUCKETS", str(MAX_ACTIVE_HORIZON_BUCKETS)))
        )
        if str(os.environ.get("TRADOV_ENABLE_ODTE_PIVOT_OVERLAY_SLOT", "")).strip().lower() in {
            "1",
            "true",
            "yes",
            "on",
            ...
            self.logger.warning(
                "TRADOV_ENABLE_ODTE_PIVOT_OVERLAY_SLOT is experimental; D31 only admits "
        ...
        if self.event_manager is None:
            ...
            if self.event_manager is None:
                try:
                    from Tradov.TradovA_Core.TradovA05_EventManager import get_event_manager as _gem2  # noqa: E501
        ...
        if os.getenv("TRADOV_ENABLE_PIVOT_MEAN_REVERSION", "").strip().lower() in {
            "1", "true", "yes", "on", "y",
        }:
        ...
        self._signal_flow_log_interval_s: float = float(
            os.getenv("TRADOV_D31_SIGNAL_FLOW_LOG_INTERVAL_S", "300")
        )
        self._signal_flow_last_log_monotonic: float = time.monotonic()
        try:
            self._pending_entry_reservation_ttl_s: float = max(
                5.0,
                float(os.getenv("TRADOV_D31_PENDING_ENTRY_RESERVATION_TTL_S", "90")),
            )
        ...
        try:
            self._pending_exit_reservation_ttl_s: float = max(
                1.0,
                float(os.getenv("TRADOV_D31_PENDING_EXIT_RESERVATION_TTL_S", "15")),
            )
        except (TypeError, ValueError):
        ...
        try:
            self._duplicate_entry_warning_interval_s: float = max(
                1.0,
                float(os.getenv("TRADOV_D31_DUPLICATE_ENTRY_WARNING_INTERVAL_S", "300")),
            )
        except (TypeError, ValueError):
        ...
        try:
            self._manual_close_reentry_embargo_s: float = max(
                1.0,
                float(os.getenv("TRADOV_D31_MANUAL_CLOSE_REENTRY_EMBARGO_S", "300")),
            )
        except (TypeError, ValueError):
        ...
        self._signal_drop_audit_enabled: bool = str(
            os.getenv("TRADOV_D31_SIGNAL_DROP_AUDIT", "1")
        ).strip().lower() not in {"0", "false", "no", "off"}
        self._signal_drop_audit_dir: str = str(
            os.getenv("TRADOV_D31_SIGNAL_DROP_AUDIT_DIR", os.path.join("logs", "decisions"))
        )
        ...
        self._agent_handoff_shadow_validation_enabled = str(
            os.getenv("TRADOV_AGENT_HANDOFF_SHADOW_VALIDATION", "1")
        ).strip().lower() not in {"0", "false", "no", "off"}
    ...
    def _seed_cache_from_disk(self) -> None:
        ...
        ``TRADOV_ENABLE_STARTUP_CACHE_SEED`` to preserve fail-closed behavior
        on cold starts by default.
        """
        from pathlib import Path as _Path
        import json as _json

        _data_dir = _Path.home() / "Projects" / "Tradov" / "market_data"
    ...
    def add_strategy(self, strategy_class: type, config: dict[str, Any],
        ...
        try:
            ...
            try:
                from Tradov.TradovR_Runtime.TradovR12_SessionSupervisor import get_session_supervisor  # noqa: E501
    ...
    def _get_open_option_symbols_for_short_leg_reconcile(self) -> set[str] | None:
        ...
        try:
            from Tradov.TradovR_Runtime.TradovR12_SessionSupervisor import get_session_supervisor
        except Exception:
            return None
    ...
    def _evaluate_strategy_short_leg_risk(self, strategy_id: str, strategy: Any) -> list[Any]:
        ...
        try:
            from Tradov.TradovE_Risk.TradovE25_DeltaBreachLegManager import DeltaBreachLegManager
        except Exception as exc:
    ...
    def _get_strategy_type(self, strategy_class: type) -> str:
        ...
        for suffix in ("Strategy", "Tradov", "D"):
    ...
    def _classify_market_regime_unified(
        ...
        if self._l09_engine is not None:
            try:
                from TradovL_ML.TradovL09_UnifiedRegimeEngine import (  # noqa: PLC0415
    ...
    def _get_d30_selector(self) -> Any | None:
        ...
        selector_cls = None
        try:
            from Tradov.TradovD_Strategies.TradovD30_RegimeGatedSelector import (  # noqa: PLC0415
            ...
            try:
                from TradovD_Strategies.TradovD30_RegimeGatedSelector import (  # noqa: PLC0415
                    RegimeGatedSelector as _Selector,
                )
    ...
    def _build_d30_consensus(self) -> Any | None:
        ...
        try:
            from Tradov.TradovL_ML.TradovL09_UnifiedRegimeEngine import MarketRegime as _L09R  # noqa: PLC0415
        except Exception:
            try:
                from TradovL_ML.TradovL09_UnifiedRegimeEngine import MarketRegime as _L09R  # noqa: PLC0415
            except Exception:
                return None
    ...
    def _apply_env_allowed_strategies_override(self) -> None:
        """Constrain lean allowlist via TRADOV_ALLOWED_STRATEGIES when provided."""
        raw_override = str(os.getenv("TRADOV_ALLOWED_STRATEGIES", "")).strip()
        if not raw_override:
            return
        ...
        if unknown_tokens:
            self.logger.warning(
                "D31 ignoring unknown TRADOV_ALLOWED_STRATEGIES entries: %s",
                ", ".join(unknown_tokens),
            )

        if not selected_bases:
            self.logger.warning(
                "D31 received TRADOV_ALLOWED_STRATEGIES override but no valid strategies were resolved"
            )
            return
        ...
        self.logger.info(
            "D31 lean allowlist constrained by TRADOV_ALLOWED_STRATEGIES: %s",
    ...
    def _put_credit_spread_7_eligible_now() -> bool:
        ...
        if str(os.getenv("TRADOV_ENABLE_PUT_CREDIT_SPREAD_7", "")).strip().lower() not in {
            "1",
            "true",
            "yes",
            "on",
    ...
    def _check_risk_limits(self) -> None:
        ...
        try:
            ...
            if self.risk_manager is None:
                try:
                    from Tradov.TradovE_Risk.TradovE01_RiskManager import get_risk_manager
    ...
    def _setup_event_subscriptions(self):
        ...
        if EventType is None:
            try:
                from Tradov.TradovA_Core.TradovA05_EventManager import EventType as _ET
                EventType = _ET
            except Exception:
                try:
                    from TradovA_Core.TradovA05_EventManager import EventType as _ET
                    EventType = _ET
                except Exception as exc:
                    self.logger.critical(
    ...
    def subscribe_agent_bus(self, bus: Any) -> None:
        ...
        Args:
            bus: AgentMessageBus instance (TradovI06).
        """
        try:
            bus.subscribe(
        ...
        if isinstance(data, dict):
            ...
            if isinstance(symbol, str):
                ...
                if isinstance(tick, dict):
                    ...
                    # {"symbol":"TRAD","price":...} instead of {"tick": {...}}.
    ...
    def _on_strategy_signal(self, event: Event):
        ...
        try:
            ...
            try:
                from TradovE_Risk.TradovE00_RiskProtocol import (  # type: ignore[no-redef]
                    RiskValidationRequest as _RVR,
                    BoundarySignalType as _BST,
                )
            except ImportError:
    ...
    def _get_entry_filter_gate(self) -> Any | None:
        ...
        try:
            ...
            try:
                from TradovA_Core.TradovA03_Configuration import get_config_manager  # type: ignore[no-redef]
    ...
    def _get_eco_calendar(self) -> Any | None:
        ...
        try:
            from Tradov.TradovS_Signals.TradovS18_EconomicCalendar import get_economic_calendar
        except ImportError:
            try:
                from TradovS_Signals.TradovS18_EconomicCalendar import get_economic_calendar  # type: ignore[no-redef]
            except ImportError:
    ...
    def _overlay_slot_flag_enabled() -> bool:
        ...
        return str(os.environ.get("TRADOV_ENABLE_ODTE_PIVOT_OVERLAY_SLOT", "")).strip().lower() in {
            "1",
            "true",
            "yes",
            "on",
    ...
    def _paper_calendar_spread_routing_flag_enabled() -> bool:
        ...
        return str(os.environ.get("TRADOV_ENABLE_PAPER_CALENDAR_SPREAD_ROUTING", "")).strip().lower() in {
            "1",
            "true",
            "yes",
            "on",
    ...
    def _build_multileg_market_analysis(
        ...
        try:
            from Tradov.TradovD_Strategies.TradovD32_MultiLegStrategyCoordinator import MultiLegMarketAnalyzer
        except Exception as exc:
    ...
    def _build_paper_iron_condor_structure(
        ...
        try:
            from Tradov.TradovD_Strategies.TradovD32_MultiLegStrategyCoordinator import (
    ...
    def _build_paper_iron_condor_leg_orders(
        ...
        try:
            from Tradov.TradovU_Utilities.TradovU03_DateTimeUtils import DateTimeUtils
        except Exception as exc:
    ...
    def set_live_engine(self, engine: Any) -> None:
        ...
        Args:
            engine: A TradovR04_LiveEngine (or compatible) instance that exposes
    ...
    def set_order_manager(self, manager: Any) -> None:
        ...
        Args:
            manager: A ``TradovB02_OrderManager`` (or compatible) instance that
    ...
    def set_vix_analyzer(self, analyzer: Any) -> None:
        ...
        Args:
            analyzer: A ``TradovC10_VIXAnalyzer`` (or compatible) instance that
    ...
    def set_risk_manager(self, manager: Any) -> None:
        ...
        Args:
            manager: A ``TradovE01_RiskManager`` (or compatible) instance that
    ...
    def _dispatch_approved_signal(self, signal: Any) -> None:
        ...
        try:
            ...
            if (
                ...
                and str(symbol_option_details.get("underlying") or "").strip().upper() == "TRAD"
            ):
                self._clear_pending_entry_reservation(symbol, strategy_id)
                detail = f"symbol={symbol};policy=spxw_only;blocked_underlying=TRAD"
                self.logger.warning(
...
_IS_TEST_MODE = str(os.environ.get("TRADOV_TEST_MODE", "")).lower() == "true"
...
class StrategyOrchestratorDashboard(_DASHBOARD_BASE):
    ...
    def setup_ui(self):
        ...
        self.setWindowTitle("TRADOV - Strategy Orchestrator Dashboard")
    ...
    def create_header_section(self, layout):
        ...
        # Title
        title_label = QLabel("🎯 TRADOV Strategy Orchestrator")
        title_font = QFont()
...
def main():
    ...
    logging.info("🎯 TRADOV D12 - Strategy Orchestrator")
    logging.info("=" * 60)
