...
IB_GATEWAY_PORTS = {"paper": 7497, "live": 7496, "gateway": 4001, "tws": 4002}
...
class MonitoringUtilities:
    ...
    def _get_services_status(self) -> Dict[str, Dict[str, Any]]:
        ...
        services["watchdog"] = self._check_python_service(
            "SpyderQ24_ProductionWatchdog.py"
        )
        services["system_monitor"] = self._check_python_service(
    ...
    def check_ib_connection(self) -> IBConnectionStatus:
        ...
        if ib_async and status.port_open:
            try:
                ...
                status.api_connected = loop.run_until_complete(
                    self._test_ib_api_connection()
                )
                loop.close()
            except Exception as e:
    ...
    async def _test_ib_api_connection(self) -> bool:
        ...
        ib = IB()
        try:
            # Try configured ports first, then fallback to common ports
            config_hosts_ports = []
            if "gateway" in IB_CONFIG:
                if "paper" in IB_CONFIG["gateway"]:
                    config_hosts_ports.append(
                        (
                            IB_CONFIG["gateway"]["paper"]["host"],
                            IB_CONFIG["gateway"]["paper"]["port"],
                        )
                    )
                if "live" in IB_CONFIG["gateway"]:
                    config_hosts_ports.append(
                        (
                            IB_CONFIG["gateway"]["live"]["host"],
                            IB_CONFIG["gateway"]["live"]["port"],
                        )
                    )

            # Add fallback localhost ports
            fallback_ports = [("127.0.0.1", port) for port in [7497, 7496, 4001, 4002]]

            # Combine configured and fallback
            all_hosts_ports = config_hosts_ports + fallback_ports

            for host, port in all_hosts_ports:
                try:
                    await ib.connectAsync(host, port, clientId=999)
                    self.logger.info(f"Successfully connected to IB on {host}:{port}")
                    ib.disconnect()
                    return True
                except Exception:
                    continue
    ...
    def _create_alert(
        self, level: AlertLevel, message: str, details: Any = None
    ) -> None:
        """Create an alert."""
        alert = {
            "timestamp": datetime.now().isoformat(),
...
def main():
    ...
    if args.action == "status":
        ...
        if args.json:
            ...
            print(
                f"  Memory: {resources['memory']['percent']:.1f}% ({resources['memory']['used_gb']:.1f}/{resources['memory']['total_gb']:.1f} GB)"
            )
            print(
                f"  Disk: {resources['disk']['percent']:.1f}% ({resources['disk']['free_gb']:.1f} GB free)"
            )
            print()
        ...
        if args.json:
            ...
            if resources["cpu"]["load_avg"]:
                print(
                    f"  Load Average: {', '.join(map(str, resources['cpu']['load_avg']))}"
                )
