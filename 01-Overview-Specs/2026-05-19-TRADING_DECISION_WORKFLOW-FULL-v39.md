# Trading Decision Workflow (Full) — v39
...
Source note: this saved `v39` snapshot records the second post-v37 overlay implementation slice. Current code now goes beyond the partial signal-path posture captured in v38: E00/E01 still expose a typed `OverlayPretradeVerdict` / `validate_overlay_slot()` API, and D31 now also defaults PivotMeanReversion to the `ultra_short` horizon bucket and admits a narrow third-slot registration exception in `add_strategy()` once the baseline two-slot cap is already full. The baseline two-slot model remains authoritative for every non-overlay strategy, and overlay audit-schema/runtime-disable/paper-verification work is still outstanding. The post-open runtime audit captured in v38 remains historical to the pre-v39 running process: no armed paper session or day-of decision log existed at that audit boundary.
