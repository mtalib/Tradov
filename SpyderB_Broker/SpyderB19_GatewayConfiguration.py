...
# API Ports
PORTS = {"paper": 4002, "live": 4001, "gateway_controller": 4003}  # IBC controller port
...
class GatewayConfigurationManager:
    ...
    def _create_directories(self):
        ...
        dirs = [Path(self.config.log_dir), Path(self.config.ibc_path), IB_GATEWAY_DIR]
    ...
    def validate_installation(self) -> tuple[bool, List[str]]:
        ...
        if self.config.use_xvfb:
            try:
                result = subprocess.run(
                    ["which", "xvfb-run"], capture_output=True, timeout=5
                )
                if result.returncode != 0:
                    errors.append(
                        "Xvfb not installed (required for headless operation)"
                    )
            except Exception as e:
    ...
    def generate_xvfb_command(self) -> str:
        ...
        return f"xvfb-run {XVFB_CONFIG['options']} --server-num=1"
...
def migrate_from_1037():
    ...
    subprocess.run(
        [
            "sed",
            "-i",
            's/TWS_MAJOR_VRSN="1037"/TWS_MAJOR_VRSN="1039"/g',
