...
Module: TradovC19_AfterHoursDataManager.py
...
Module Description:
    ...
    known good prices. Integrates with enhanced TradovClient for optimal data types.
...
from Tradov.TradovU_Utilities.TradovU22_ETTimeDisplay import SimpleETDisplay as ETTimeDisplay
...
CLOSING_PRICES_FILE = os.path.join(tempfile.gettempdir(), "tradov_closing_prices.json")
AFTER_HOURS_CACHE_FILE = os.path.join(tempfile.gettempdir(), "tradov_after_hours_cache.json")
...
class AfterHoursDataManager:
    ...
    reliable price data. Works with the enhanced TradovClient to use optimal
    data types for after-hours scenarios.

    Attributes:
        logger: Module logger instance
        error_handler: Error handling instance
        client: Enhanced TradovClient instance
    ...
    Example:
        >>> manager = AfterHoursDataManager(tradov_client)
        >>> closing_data = manager.get_closing_snapshot('TRAD')
        >>> after_hours = manager.get_after_hours_data('TRAD')
        >>> is_available = manager.is_closing_data_available('TRAD')
    """

    def __init__(self, client: TradovClient | None = None):
        """
        Initialize after-hours data manager.

        Args:
            client: Optional TradovClient instance
        """
    ...
    def start(self) -> bool:
        ...
        try:
            with self._lock:
                ...
                self.logger.info("Current session: %s", self.current_session.value)
            ...
            self.logger.error("Failed to start after-hours manager: %s", e, exc_info=True)
            return False
    ...
    def stop(self) -> None:
        ...
        try:
            ...
            self.logger.error("Error stopping after-hours manager: %s", e, exc_info=True)
        ...
        try:
            ...
            if req_id > 0:
                ...
                if ticker and ticker.last and ticker.last > MIN_PRICE_THRESHOLD:
                    ...
                    self.logger.warning("No valid closing data received for %s", symbol)
        ...
        for i in range(0, len(symbols), max_concurrent):
            ...
            for symbol in batch:
                try:
                    ...
                    self.logger.error("Error requesting %s: %s", symbol, e, exc_info=True)
    ...
    def get_portfolio_closing_values(self, symbols: list[str]) -> dict[str, float]:
        ...
        for symbol in symbols:
            ...
            if snapshot and snapshot.data_freshness != DataFreshness.EXPIRED:
                ...
                self.logger.warning("No valid closing price for %s", symbol)
                values[symbol] = 0.0
    ...
    def _get_current_session(self) -> AfterHoursSession:
        ...
        try:
            ...
            self.logger.error("Error determining session: %s", e, exc_info=True)
            return AfterHoursSession.CLOSED
    ...
    def _setup_after_hours_feeds(self) -> None:
        ...
        try:
            ...
            self.logger.error("Error setting up after-hours feeds: %s", e, exc_info=True)
    ...
    def _prepare_closing_data(self) -> None:
        ...
        try:
            ...
            if expired_symbols:
                self.logger.warning("Expired closing data for symbols: %s", expired_symbols)

        except Exception as e:
            self.logger.error("Error preparing closing data: %s", e, exc_info=True)
    ...
    def _stop_after_hours_feeds(self) -> None:
        ...
        try:
            ...
            self.logger.error("Error stopping after-hours feeds: %s", e, exc_info=True)
    ...
    def _load_closing_prices(self) -> None:
        ...
        try:
            ...
            self.logger.error("Error loading closing prices: %s", e, exc_info=True)
    ...
    def _save_closing_prices(self) -> None:
        ...
        try:
            ...
            self.logger.info("Saved %s closing price snapshots", len(data))

        except Exception as e:
            self.logger.error("Error saving closing prices: %s", e, exc_info=True)

# ==============================================================================
# MODULE FUNCTIONS
# ==============================================================================
def get_after_hours_manager(client: TradovClient | None = None) -> AfterHoursDataManager:
    """
...
def get_shared_after_hours_manager(client: TradovClient | None = None) -> AfterHoursDataManager:
    """
