...
DEFAULT_CONFIG = {
    "mode": "paper",
    "ib_gateway": {"host": "127.0.0.1", "port": 7497, "client_id": 1},  # Paper trading port
    "database": {"path": "spyder_trading.db"},
    "logging": {"level": "INFO", "file": "spyder.log"},
    "risk": {"max_position_size": 10000, "max_daily_loss": 5000, "max_open_positions": 20},
}
...
/____/_/     /_/ /_____/  /_____/|_| \\_\\
    ...
    def _check_ib_gateway(self) -> bool:
        ...
        try:
            result = sock.connect_ex(
                (self.config["ib_gateway"]["host"], self.config["ib_gateway"]["port"])
            )
            return result == 0
    ...
    def _start_single_module(self, module_name: str) -> bool:
        ...
        return self.orchestrator._load_module(module_name) and self.orchestrator._start_module(
            module_name
        )
...
Start specific module:
    ...
    # Add arguments
    parser.add_argument(
        "--mode", choices=["live", "paper", "backtest"], default="paper", help="Trading mode"
    )

    parser.add_argument("--config", type=str, help="Path to configuration file")

    parser.add_argument(
        "--gui", action="store_true", default=True, help="Launch with GUI (default)"
    )
