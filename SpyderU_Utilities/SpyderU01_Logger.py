            ...
            with self._lock:
                if hasattr(record, 'performance_metrics'):
                    ...
                    if (len(self.metrics_buffer) >= 100 or 
                        (datetime.now() - self.last_flush).seconds >= PERFORMANCE_LOG_INTERVAL):
                        self._flush_metrics()
        except Exception:
...
class SpyderLogger:
    ...
    def archive_old_logs(self) -> None:
        ...
        for log_dir in [LOG_BASE_DIR, TRADE_LOG_DIR, PERFORMANCE_LOG_DIR, ERROR_LOG_DIR]:
...
def get_logger(name: str = __name__, level: str = 'INFO', log_file: Optional[str] = None) -> SpyderLogger:
    """
    Get or create a logger instance.
    ...
    # Note: The original function had some issues, keeping it simple
