...
class OptionContract:
    ...
    def validate(self) -> bool:
        ...
        return (self.underlying_price > 0 and
                self.strike_price > 0 and
                self.time_to_expiry >= 0 and
                self.risk_free_rate >= 0 and
                self.dividend_yield >= 0 and
                self.volatility > 0)
...
class PricingParameters:
    ...
    def validate(self) -> bool:
        """Validate parameters."""
        return (self.binomial_steps > 0 and
                self.monte_carlo_sims > 100 and
                self.monte_carlo_paths > 10 and
                self.lsm_basis_functions > 0)
...
class SpyderPricingEngine:
    ...
    async def price_option(self,
                          ...
                          parameters: PricingParameters = None) -> PricingResult:
        """
    ...
    async def price_portfolio(self,
                            contracts: List[OptionContract],
                            parameters: PricingParameters = None) -> List[PricingResult]:
        """
        ...
        if params.parallel_processing and len(contracts) > 1:
            ...
            for contract in contracts:
                task = asyncio.create_task(
                    self.price_option(contract, params)
                )
                tasks.append(task)
    ...
    def _select_pricing_model(self,
                            contract: OptionContract,
                            parameters: PricingParameters) -> PricingModel:
    ...
    async def _calculate_price_and_greeks(self,
                                        contract: OptionContract,
                                        parameters: PricingParameters,
                                        model: PricingModel) -> PricingResult:
    ...
    async def _black_scholes_pricing(self,
                                   contract: OptionContract,
                                   parameters: PricingParameters) -> PricingResult:
        """Black-Scholes analytical pricing (European only)."""
        S, K, T = contract.underlying_price, contract.strike_price, contract.time_to_expiry
        r, q, sigma = contract.risk_free_rate, contract.dividend_yield, contract.volatility
        ...
        if contract.exercise_style == ExerciseStyle.AMERICAN:
            result.warnings.append("Black-Scholes used for American option - no early exercise value")
        
        return result
    
    async def _barone_adesi_pricing(self,
                                  contract: OptionContract,
                                  parameters: PricingParameters) -> PricingResult:
        ...
        if T < 1/365:  # Less than 1 day
            ...
            greeks = await self._calculate_numerical_greeks(contract, PricingModel.BARONE_ADESI_WHALEY)
            
            return PricingResult(
                theoretical_price=price,
                greeks=greeks,
                model_used=PricingModel.BARONE_ADESI_WHALEY,
                early_exercise_premium=price - _black_scholes_price(S, K, T, r, q, sigma, is_call),
                convergence_achieved=True
            )
    ...
    def _barone_adesi_approximation(self, S, K, T, r, q, sigma, is_call):
        ...
        # Calculate beta
        beta_plus = 0.5 - b / (sigma**2) + np.sqrt((b / (sigma**2) - 0.5)**2 + 2 * r / (sigma**2))
        beta_minus = 0.5 - b / (sigma**2) - np.sqrt((b / (sigma**2) - 0.5)**2 + 2 * r / (sigma**2))
        ...
        if is_call:
            ...
            if S >= S_star:
                ...
                return european_price + (S / S_star)**beta_plus * (S_star - K - european_price)
        else:
            S_star = K * beta_minus / (beta_minus - 1)
            if S <= S_star:
                return K - S
            else:
                return european_price + (S / S_star)**beta_minus * (K - S_star - european_price)
    
    async def _binomial_tree_pricing(self,
                                   contract: OptionContract,
                                   parameters: PricingParameters) -> PricingResult:
        ...
        if is_american:
            european_price = _binomial_tree_core(S, K, T, r, q, sigma, n, is_call, False)
            early_exercise_premium = price - european_price
        else:
            early_exercise_premium = 0.0
        
        # Greeks calculation
        greeks = await self._calculate_numerical_greeks(contract, PricingModel.BINOMIAL_TREE)
    ...
    async def _monte_carlo_lsm_pricing(self,
                                     contract: OptionContract,
                                     parameters: PricingParameters) -> PricingResult:
                ...
                if np.sum(itm) > 0:
                    ...
                    if len(X) > 3:
                        ...
                        option_values[itm] = np.where(
                            exercise, payoffs[itm, t], option_values[itm] * np.exp(-r * dt)
                        )
        ...
        early_exercise_premium = price - european_price if contract.exercise_style == ExerciseStyle.AMERICAN else 0.0
        
        # Greeks calculation
        greeks = await self._calculate_numerical_greeks(contract, PricingModel.MONTE_CARLO_LSM)
        ...
        return PricingResult(
            ...
            metadata={
                ...
                'time_steps': n_steps,
                'standard_error': np.std(option_values) / np.sqrt(n_sims) if 'option_values' in locals() else 0
            }
        )
    ...
    async def _calculate_black_scholes_greeks(self, contract: OptionContract) -> Greeks:
        ...
        vega = S * np.exp(-q * T) * _norm_pdf(d1) * np.sqrt(T) / 100  # Per 1% vol change
        
        if is_call:
            theta = (-(S * _norm_pdf(d1) * sigma * np.exp(-q * T)) / (2 * np.sqrt(T)) -
                    r * K * np.exp(-r * T) * _norm_cdf(d2) +
                    q * S * np.exp(-q * T) * _norm_cdf(d1)) / 365  # Per day
        ...
        # Cross derivatives
        vanna = -np.exp(-q * T) * _norm_pdf(d1) * d2 / sigma / 100  # Delta/vol sensitivity
        
        volga = S * np.exp(-q * T) * _norm_pdf(d1) * np.sqrt(T) * d1 * d2 / (sigma * 100)  # Vega/vol
        
        charm = q * np.exp(-q * T) * _norm_cdf(d1 if is_call else -d1) - np.exp(-q * T) * _norm_pdf(d1) * (
            2 * (r - q) * T - d2 * sigma * np.sqrt(T)
        ) / (2 * T * sigma * np.sqrt(T)) if is_call else -charm  # Delta/time
        
        veta = -S * np.exp(-q * T) * _norm_pdf(d1) * np.sqrt(T) * (
    ...
    async def _calculate_numerical_greeks(self,
                                        contract: OptionContract,
                                        model: PricingModel) -> Greeks:
        """Calculate Greeks using finite differences."""
        # Base price
        base_price_result = await self._calculate_price_and_greeks_simple(contract, model)
        ...
        price_up = (await self._calculate_price_and_greeks_simple(contract_up, model)).theoretical_price
        price_down = (await self._calculate_price_and_greeks_simple(contract_down, model)).theoretical_price
        
        delta = (price_up - price_down) / (2 * 0.01 * contract.underlying_price)
        
        # Gamma calculation
        gamma = (price_up - 2 * base_price + price_down) / ((0.01 * contract.underlying_price) ** 2)
        
        # Vega calculation
        contract_vol_up = self._bump_contract(contract, 'volatility', 0.01)
        price_vol_up = (await self._calculate_price_and_greeks_simple(contract_vol_up, model)).theoretical_price
        vega = (price_vol_up - base_price) / 0.01
        
        # Theta calculation  
        contract_time_down = self._bump_contract(contract, 'time_to_expiry', -1/365)
        price_time_down = (await self._calculate_price_and_greeks_simple(contract_time_down, model)).theoretical_price
        theta = price_time_down - base_price  # Already per day
        
        # Rho calculation
        contract_rate_up = self._bump_contract(contract, 'risk_free_rate', 0.01)
        price_rate_up = (await self._calculate_price_and_greeks_simple(contract_rate_up, model)).theoretical_price
        ...
        price_up_vol_up = (await self._calculate_price_and_greeks_simple(contract_up_vol_up, model)).theoretical_price
        
        delta_vol_up = (price_up_vol_up - price_vol_up) / (0.01 * contract.underlying_price)
        vanna = (delta_vol_up - delta) / 0.01
        
        contract_vol_up2 = self._bump_contract(contract, 'volatility', 0.02)
        price_vol_up2 = (await self._calculate_price_and_greeks_simple(contract_vol_up2, model)).theoretical_price
    ...
    def _bump_contract(self,
                      contract: OptionContract,
                      parameter: str,
                      bump_amount: float) -> OptionContract:
    ...
    async def _calculate_price_and_greeks_simple(self,
                                               contract: OptionContract,
                                               model: PricingModel) -> PricingResult:
        ...
        if model == PricingModel.BLACK_SCHOLES:
            ...
            is_american = contract.exercise_style == ExerciseStyle.AMERICAN
            price = _binomial_tree_core(S, K, T, r, q, sigma, 50, is_call, is_american)  # Fewer steps for speed
        else:
            price = _black_scholes_price(S, K, T, r, q, sigma, is_call)
        
        return PricingResult(
            theoretical_price=price,
            greeks=Greeks(delta=0, gamma=0, vega=0, theta=0, rho=0, vanna=0, volga=0, charm=0, veta=0),
            model_used=model
        )
    ...
    def _generate_cache_key(self,
                          contract: OptionContract,
                          parameters: PricingParameters) -> str:
    ...
    def _update_model_performance(self,
        ...
        if success and calculation_time > 0:
            ...
            perf.avg_calculation_time = (
                (perf.avg_calculation_time * (perf.total_calculations - 1) + calculation_time) /
        ...
        for model, perf in self.model_performance.items():
            if perf.total_calculations > 0:
                summary['models'][model.value] = {
                    ...
                    'success_rate': 1 - (perf.convergence_failures / perf.total_calculations),
...
class ModelSelector:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.time_threshold_accurate = config.get('time_threshold_accurate', 30/365)  # 1 month
...
# FACTORY FUNCTIONS
# ==============================================================================
def create_pricing_engine(config: Dict[str, Any] = None,
                         data_manager: MultiClientDataManager = None) -> SpyderPricingEngine:
...
async def main():
    ...
    print("Model                    Price      Delta    Gamma    Vega     Theta    Time(ms)")
    print("-" * 80)
    ...
    for model in models_to_test:
        try:
            ...
            print(f"{model.value:<20} ${result.theoretical_price:>7.4f} "
    ...
    for i, contract in enumerate(test_contracts):
        try:
            ...
            print(f"   Early Exercise Premium: ${result.early_exercise_premium or 0:.4f}")
    ...
    try:
        ...
        portfolio_value = sum(result.theoretical_price * 100 for result in results)  # 100 contracts each
    ...
    try:
        result = await pricing_engine.price_option(
            greeks_contract,
            PricingParameters(model=PricingModel.BLACK_SCHOLES)
        )
        
        greeks = result.greeks
        
        print(f"   Option: {greeks_contract.option_type.value.upper()} "
    ...
    try:
        ...
        if performance['models']:
            ...
            for model_name, stats in performance['models'].items():
                print(f"     {model_name:<20}: {stats['calculations']:>3} calls, "
