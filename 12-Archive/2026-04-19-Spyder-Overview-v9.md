# Spyder Trading System — Codebase Review v19 (v9)
**Date:** April 19, 2026
**Scope:** Current-state inventory of the Spyder codebase
**Prepared by:** Claude Sonnet 4.6
**Supersedes:** 2026-04-18-Spyder-Overview-v8.md

---

## Change Summary (v8 → v9)

This revision incorporates four specifications delivered on April 18–19, 2026 on the `refactor/g05-widget-extraction` branch. All changes relate to the live-trading pipeline: a formal broker `Protocol`, a full paper-broker implementation, a fill-reconciliation worker, a position-exit monitor, a session lifecycle supervisor, and a CI gate enforcing the new protocol contract.

- **S-15 — BrokerProtocol (`SpyderB21_BrokerProtocol.py`, NEW, 99 LOC):** Defined `@runtime_checkable class BrokerProtocol(Protocol)` with five required methods: `place_order`, `get_order`, `cancel_order`, `get_positions`, and `get_account_balances`. Added `is_broker_compliant(broker) -> bool` helper using `isinstance` with the `@runtime_checkable` decorator. Python 3.7 fallback provided via `typing_extensions`. B-Series grows from 9 to 10 files.

- **S-15 — R04 broker routing simplified (`SpyderR04_LiveEngine.py`, MODIFIED, 1,517 → 1,589 LOC):** `_broker_submit()` now routes exclusively through `place_order` (per the BrokerProtocol standard), removing the legacy `B02 submit_order` branch that had been added as a gap-fix adapter. Both `B40 TradierClient` (live) and `R15 PaperBroker` (paper) satisfy `BrokerProtocol`, so mode-switching is now a construction-time decision with no branching in the submit path.

- **S-07 — PaperBroker (`SpyderR15_PaperBroker.py`, NEW, 298 LOC):** Implements `BrokerProtocol` for paper/backtesting mode. `place_order(**kwargs)` returns `{"order": {"id": "PAPER-XXXXXX", "status": "pending"}}` immediately. A configurable `fill_delay_s` daemon thread transitions the order status to `"filled"` with a `fill_price` (sourced from live `MARKET_DATA` events when an `event_manager` is supplied, or from the order's stated price as fallback). Implements `get_order`, `cancel_order`, `get_positions`, and `get_account_balances`. Lifecycle: `start()` / `stop()` / `is_running` satisfy the `SessionSupervisor._components` interface.

- **S-07 — FillReconciler (`SpyderR13_FillReconciler.py`, NEW, 379 LOC):** Background fill-reconciliation worker. Once an order is accepted, R04 calls `track(order_id, tradier_order_id, order_type)`. A daemon thread polls `broker.get_order()` at a configurable cadence (`poll_cadence_market` default 2 s, `poll_cadence_limit` default 5 s) and emits the appropriate `EventType` once a terminal state is observed: `ORDER_FILLED`, `ORDER_PARTIALLY_FILLED`, `ORDER_CANCELLED`, `ORDER_EXPIRED`, or `ORDER_REJECTED`. Soft-imports Prometheus metrics (skipped silently if unavailable). Maximum of 8 consecutive errors before an order is dropped from tracking.

- **S-14 — ExitMonitor (`SpyderR14_ExitMonitor.py`, NEW, 314 LOC):** Periodic daemon sweep of all open positions in `P01 PortfolioManager`. On each 1-second sweep: (1) resolves the owning strategy from a `strategy_map`; (2) calls `strategy.check_exit(position) → ExitDecision | None`; if `ExitDecision.CLOSE`, emits `EventType.STRATEGY_SIGNAL` with `{"action": "close"}`; if no owning strategy found, emits `EventType.RISK_VIOLATION` with `{"type": "ORPHAN_POSITION"}` (de-duped per `strategy_id` via `_orphan_alerted: set`). Provides `register_strategy()` / `unregister_strategy()` and a `create_exit_monitor(...)` factory function.

- **S-07 — SessionSupervisor (`SpyderR12_SessionSupervisor.py`, NEW, 467 LOC):** Single lifecycle owner for all backend components in a live or paper session. Starts components in dependency order: `EventManager → DataFeed → DataFreshnessMonitor → FillReconciler → RiskManager → LiveEngine → StrategyOrchestrator`. On startup failure, performs a clean rollback by stopping all already-started components in reverse order. `stop(flatten=False)` tears down in reverse order; `flatten=True` attempts position flattening before the broker is torn down. `Q14 MainLauncher` is now a thin argument-parser and signal-handler that delegates all backend lifecycle to this class. R-Series grows from 8 to 12 files.

- **S-16 — CI Gate 5 (`SpyderQ10_ProtocolComplianceGate.py`, MODIFIED, 61 → 311 LOC):** Gate 5 added: `check_broker_protocol_compliance()` uses `importlib` to import `SpyderB40_TradierClient` and `SpyderR15_PaperBroker`, then verifies each class has all five `BrokerProtocol` method names (`place_order`, `get_order`, `cancel_order`, `get_positions`, `get_account_balances`) via `getattr(cls, m, None)`. No live connections are established. All five gates must pass for CI exit code 0.
...
| Metric | Count |
|--------|------:|
| Total Python files | **491** |
| Production files (A–Z, excluding T) | **366** |
| Test files (T-series) | **125** |
| Total lines of code | **~432,455** |
| Production LOC | **~337,498** |
| Test LOC | **94,957** |
| Series (packages) | **25** |
...
- **Market Data:** Tradier (runtime and execution), Massive SDK (supported market-data path / fallback), Playwright (TradingView scraping for NYSE breadth: TICK, TRIN, ADD, NYMO)
- **GUI:** PySide6 (Qt6)
...
- **Protocol boundaries:** C00 (market data), E00 (risk), F00 (analysis), Z00 (broker), and **B21 (broker runtime)** define typed `Protocol` interfaces enforced at runtime via the T129 protocol-compliance suite and the Q10 CI gate.
- **BrokerProtocol (v9):** `SpyderB21_BrokerProtocol.py` defines `@runtime_checkable BrokerProtocol(Protocol)` with five required methods. Both `B40 TradierClient` (live) and `R15 PaperBroker` (paper) satisfy this protocol. R04 `_broker_submit()` routes exclusively through `place_order`, making live/paper mode-switching a construction-time decision with no branching in the submit path.
...
- **Session lifecycle (v9):** `SpyderR12_SessionSupervisor` is the single lifecycle owner for live and paper trading sessions. `R13 FillReconciler` polls for fill confirmations and emits `ORDER_FILLED` events. `R14 ExitMonitor` sweeps open positions and emits `STRATEGY_SIGNAL {"action": "close"}` or `RISK_VIOLATION {"type": "ORPHAN_POSITION"}` as appropriate. `R15 PaperBroker` satisfies `BrokerProtocol` for paper mode.
- **Q10 CI gate:** Five gates are active in `SpyderQ10_ProtocolComplianceGate`: (1) no `random.*` / `np.random.*` in non-allowed production paths, (2) T129 protocol-compliance suite passes, (3) no `datetime.utcnow()` in production code, (4) no bare `datetime.now()` (without `timezone.utc`) in production code, **(5) B40 TradierClient and R15 PaperBroker both expose all five BrokerProtocol methods**.
...
| B | `SpyderB_Broker` | 10 | 10,537 | Tradier API integration, order management, execution, broker protocol |
...
| Q | `SpyderQ_Scripts` | 22 | 9,610 | Launchers, validators, utilities, fine-tuning scripts, CI gates (5 gates) |
| R | `SpyderR_Runtime` | 12 | ~10,130 | Paper engine, live engine, session supervisor, fill reconciler, exit monitor, paper broker, backtest harness, production deployment |
...
| `SpyderA05_EventManager.py` | 1,205 | Central synchronous event bus. Publishes and dispatches typed system-wide events (market open, signal triggered, risk breach, order fill) to registered handlers. Contains `MARKET_DATA_TICK` and `CUSTOM_METRIC_UPDATE` in the `EventType` enum. |
...
**Package:** `SpyderB_Broker` | **10 files** | **10,537 LOC**

The B-Series handles all broker-facing concerns: authentication, order lifecycle, position tracking, account information, Prometheus metric exposure for the broker layer, and the structural `BrokerProtocol` that all broker implementations must satisfy.
...
| `SpyderB21_BrokerProtocol.py` | 99 | **Structural broker Protocol (v9).** Defines `@runtime_checkable class BrokerProtocol(Protocol)` with five required methods: `place_order`, `get_order`, `cancel_order`, `get_positions`, `get_account_balances`. All return plain `dict` so callers require no broker-specific imports at runtime. Provides `is_broker_compliant(broker) -> bool` using `isinstance` with the `@runtime_checkable` decorator. Python 3.7 fallback via `typing_extensions`. Both `B40 TradierClient` (live) and `R15 PaperBroker` (paper) conform to this protocol; compliance is enforced by Q10 CI Gate 5. |
...
| `SpyderC01_DataFeed.py` | 1,068 | Primary real-time tick data feed. Normalises incoming raw ticks, validates completeness, applies subscriptions, and fans out to all registered consumers. Publishes `EventType.MARKET_DATA` so market data reaches D31 strategy instances. |
...
| `SpyderD01_BaseStrategy.py` | 1,086 | Abstract base class for all strategies. Defines the interface: `generate_signal()`, `enter_position()`, `exit_position()`, `size_position()`, and lifecycle hooks. `_process_signal()` emits both the local `SIGNAL_GENERATED` and A05's `EventType.STRATEGY_SIGNAL` so signals reliably reach D31's subscriber. Strategies must also implement `check_exit(position: _PositionView) -> ExitDecision | None` consumed by R14 ExitMonitor. |
...
| `SpyderD25_UnifiedCreditSpreadEngine.py` | 1,525 | Unified engine consolidating all credit spread variants under a single scoring and selection framework for consistent risk-adjusted strike choice. Integrates the S08 Pivot Mean-Reversion signal as a directional override. |
...
| `SpyderD31_StrategyOrchestrator.py` | 2,589 | Strategy orchestrator. Coordinates concurrent strategy execution, manages capital budgeting, and arbitrates signal conflicts. All emitted signals are routed through E01 `validate_signal()`. `set_live_engine(engine)` stores a reference to R04; approved signals forwarded via `_dispatch_approved_signal()`. Market data fan-out calls `process_market_data()` on every active strategy instance, triggering `generate_signals()`. |
...
| `SpyderE01_RiskManager.py` | 1,330 | Core risk manager entry point. Routes all pre-trade and post-trade risk checks to the appropriate E-Series sub-modules and aggregates verdicts. Exposes `validate_signal()` as the authoritative pre-submit gate and `check_daily_limits()` for daily loss enforcement. |
...
| `SpyderE13_DayProfitTarget.py` | 2,239 | Daily profit target management. Scales back risk exposure once the daily target is met; implements graduated risk reduction tiers. |
...
| `SpyderF01_Indicators.py` | 871 | Core technical indicator library: RSI, MACD, Bollinger Bands, ATR, EMA, SMA, Stochastic, and Williams %R with configurable periods. Canonical indicator engine. |
...
| `SpyderG05_TradingDashboard.py` | ~3,904 | Main trading dashboard. Integrates positions, P&L, Greeks exposure, open orders, market internals, agent status, and signal feeds in a unified tabbed view with live data refresh. **Market Overview panel (v8):** shows VIX9D and HYG in place of retired NAAIM and AABULL rows. **OPTIONS ANALYTICS panel (v8):** new left-panel group displaying live IVR, ATM IV, and VRP sourced from S07. **NYMO forwarding (v8):** `_on_custom_metrics_updated` includes NYMO in the snapshot forwarded to G17. **Toolbar extraction (v8):** `create_toolbar` reduced to `return build_toolbar(self)`. |
...
| `SpyderG13_EnhancedWidgets.py` | 1,676 | Enhanced UI widget collection extracted from G05. Houses `TrafficLightButton`, `SignalMonitorPanel`, `MarketSymbolWidget`, `GreekBar`, shared colour constants, and typed dataclasses. |
...
| `SpyderG17_MarketInternalsWidget.py` | ~720 | Live market internals monitor dialog. Embeds 1-day mini-charts and configurable alert sliders for NYSE $TICK, $ADD, $TRIN, and $NYMO. Receives data via `S07.breadth_updated` signal. |
| `SpyderG18_MarketDataWorker.py` | ~955 | QObject-based market data worker. Fetches Tradier quotes, account balances, options-chain CPC, VIX9D, HYG, and 5-minute SPY chart bars. Module-level `_get_cached_chain(client)` prevents redundant chain API calls. |
| `SpyderG19_ChartIndicators.py` | 166 | Pure-function chart indicator computation. Calculates OHLCV overlay values and returns a `ChartIndicators` dataclass. No Qt dependency. |
| `SpyderG20_DashboardBuilder.py` | ~1,255 | UI builder helpers. Contains `build_left_panel()`, `build_center_panel()`, `build_right_panel()`, OPTIONS ANALYTICS group, and `build_toolbar(dashboard)`. |
| `SpyderG21_DashboardSignalHandlers.py` | 155 | Qt signal connection helpers extracted from G05. Wires dashboard Qt signals to their handler slots. |
| `SpyderG22_TradeAuditDialog.py` | 348 | Closed-trade audit dialog for paper-engine spreads. PMR-aware: displays pivot columns with audit tooltip; CSV export includes pivot context. |
| `SpyderG29_ChartWidgetPlotly.py` | 856 | Plotly-based interactive chart widget embedded via `QWebEngineView`. |
| `SpyderG30_PlotlyDataBridge.py` | 555 | Converts internal OHLCV and indicator structures into Plotly-compatible JSON trace format. |
| `SpyderG31_PlotlyTemplates.py` | 747 | Plotly chart templates and dark-mode themes with branded colour palettes and layout presets. |
| `SpyderG32_AgentHealthDashboard.py` | 302 | Real-time agent health dashboard panel. Displays heartbeat status, last-activity timestamps, error rates, and task completion rates for all X/Y agents. |
...
| `__init__.py` | 329 | Package init exporting the application manager, main window, and trading dashboard. |
...
| `SpyderK02_DailyTradingReport.py` | 1,568 | Comprehensive daily P&L report. Includes trade log, strategy breakdown, Greeks exposure at close, commissions, slippage, and next-day outlook. All timestamps are timezone-aware. |
...
| `SpyderL09_UnifiedRegimeEngine.py` | 2,200 | **Canonical regime engine.** Unified ML regime classification combining Hidden Markov Models, k-means clustering, and ensemble voting to output a consensus regime label. Single source of truth for market regime signals across the entire Spyder system. |
...
| `SpyderL16_OptionsAdjustmentRL.py` | 1,984 | Reinforcement learning agent for adaptive options position adjustments. Defines abstract environment base class; trained with PPO via stable-baselines3. |
...
| `SpyderM04_TradingMetrics.py` | 1,125 | Trading metrics collector. Captures real-time P&L, fill count, order rejection rate, execution latency histograms, and Prometheus metric exports. Includes `spyder_risk_rejections_total{strategy, rejection_reason}` counter. |
...
| `SpyderM08_HealthEndpoint.py` | 342 | Lightweight HTTP health-check endpoint. Exposes `/health` (liveness) and `/metrics` (Prometheus scrape target). |
...
| Module | LOC | Description |
|--------|----:|-------------|
| `SpyderP01_PortfolioManager.py` | 2,183 | Top-level portfolio manager. Tracks all open positions, computes portfolio-level Greeks, P&L attribution, and provides the authoritative position state. Global singleton accessors guarded by `threading.RLock`. Exposes `portfolio_positions` dict consumed by R14 ExitMonitor. |
| `SpyderP02_AllocationOptimizer.py` | 2,221 | Capital allocation optimiser. Implements mean-variance, risk-parity, and Black-Litterman optimisation; computes efficient frontier and target weights. |
| `SpyderP03_CorrelationAnalyzer.py` | 730 | Cross-strategy and cross-position correlation monitoring. Generates correlation matrix reports and flags concentration risk from correlated bets. |
| `SpyderP04_CapitalAllocator.py` | 1,582 | Dynamic capital allocation engine. Adjusts per-strategy capital budgets based on recent performance, Sharpe, and drawdown metrics. Regime sourced from `SpyderL09_UnifiedRegimeEngine`. |
...
**Package:** `SpyderQ_Scripts` | **22 files** | **9,610 LOC**

The Q-Series contains all operational scripts: system launchers, validators, diagnostic utilities, production watchdog, GUI testing tools, CI gates (now five), and AI fine-tuning scripts.
...
| `SpyderQ10_ProtocolComplianceGate.py` | 311 | **CI gate runner with five gates (v9).** Gate 1: no `random.*` / `np.random.*` in non-allowed production paths. Gate 2: T129 protocol-compliance suite passes. Gate 3: no `datetime.utcnow()` in production code. Gate 4: no bare `datetime.now()` (without `timezone.utc`) in production code. **Gate 5 (v9):** `check_broker_protocol_compliance()` uses `importlib` to import `B40 TradierClient` and `R15 PaperBroker`, then verifies each class exposes all five `BrokerProtocol` methods (`place_order`, `get_order`, `cancel_order`, `get_positions`, `get_account_balances`) via `getattr(cls, m, None)`. No live connections established. Exits 0 on all-pass, 1 on failure. Run with `python -m Spyder.SpyderQ_Scripts.SpyderQ10_ProtocolComplianceGate`. |
| `SpyderQ14_MainLauncher.py` | 526 | Main system launcher. Presents mode selection (live / paper / backtest) and bootstraps the appropriate engine and dashboard. Delegates all backend session lifecycle to `R12 SessionSupervisor`. |
...
**Package:** `SpyderR_Runtime` | **12 files** | **~10,130 LOC**

The R-Series contains the execution engines for paper trading, live trading, and production deployment management. Four new modules added in v9 formalise the session lifecycle, fill reconciliation, position exit monitoring, and a protocol-compliant paper broker. Each engine mode shares the same strategy/risk interfaces via `BrokerProtocol`.
...
| `SpyderR04_LiveEngine.py` | 1,589 | Live trading engine. Routes validated strategy signals to B40 Tradier client (or R15 PaperBroker in paper mode) via `_broker_submit(order)`. **BrokerProtocol routing (v9):** `_broker_submit()` now routes exclusively through `place_order` per the BrokerProtocol standard, removing the legacy B02 `submit_order` branch; both live and paper brokers satisfy the protocol, so mode-switching is construction-time only. `_wait_for_execution` uses a `concurrent.futures.Future` set by the fill callback. `CLOSE_POSITIONS_ON_EMERGENCY` defaults to `false`. API Panic Mode counter resets on any successful broker round-trip. |
...
| `SpyderR08_PaperTradingQtWorker.py` | 2,041 | QObject paper-trading worker driving the G05 dashboard's live paper session. Polls Tradier quotes, manages SPY price history, opens and closes credit spreads via D25. PMR producer when `SPYDER_PIVOT_MR_ENABLED=1`. |
...
| `SpyderR12_SessionSupervisor.py` | 467 | **Session lifecycle supervisor (v9).** Single lifecycle owner for all backend components in a live or paper trading session. Starts components in dependency order: `EventManager → DataFeed → DataFreshnessMonitor → FillReconciler → RiskManager → LiveEngine → StrategyOrchestrator`. If any component fails to start, performs a clean rollback (stops already-started components in reverse order). `stop(flatten=False)` tears down in reverse; `flatten=True` attempts position flattening before broker teardown. `Q14 MainLauncher` delegates all backend lifecycle to this class. |
| `SpyderR13_FillReconciler.py` | 379 | **Background fill reconciliation worker (v9).** Once an order is accepted, R04 calls `track(order_id, tradier_order_id, order_type)`. A daemon thread polls `broker.get_order()` at a configurable cadence (`poll_cadence_market` default 2 s, `poll_cadence_limit` default 5 s) and emits `EventType.ORDER_FILLED`, `ORDER_PARTIALLY_FILLED`, `ORDER_CANCELLED`, `ORDER_EXPIRED`, or `ORDER_REJECTED` when a terminal state is observed. Soft-imports Prometheus metrics (`fill_detection_latency_ms` histogram, `reconciler_poll_total` counter, `reconciler_fill_miss_total` counter). Drops an order from tracking after `MAX_CONSECUTIVE_ERRORS = 8` poll failures. |
| `SpyderR14_ExitMonitor.py` | 314 | **Periodic position exit monitor (v9).** Daemon thread sweeps all open positions in `P01 PortfolioManager.portfolio_positions` every `sweep_interval_s` (default 1 s). For each position: (1) resolves the owning strategy from `strategy_map` by `strategy_id`; (2) calls `strategy.check_exit(position: _PositionView) → ExitDecision | None`; `ExitDecision.CLOSE` emits `EventType.STRATEGY_SIGNAL` with `{"action": "close", "signal_id": "exit-<symbol>-<uuid>", ...}`; no owning strategy emits `EventType.RISK_VIOLATION` with `{"type": "ORPHAN_POSITION"}` (de-duped per `strategy_id` via `_orphan_alerted: set`). Public API: `start()`, `stop()`, `register_strategy(id, strategy)`, `unregister_strategy(id)`. Factory: `create_exit_monitor(portfolio_manager, strategy_map, event_manager, sweep_interval_s)`. |
| `SpyderR15_PaperBroker.py` | 298 | **Protocol-compliant paper broker (v9).** Implements `BrokerProtocol` for paper/backtesting mode. `place_order(**kwargs)` returns `{"order": {"id": "PAPER-XXXXXX", "status": "pending"}}` immediately; a daemon fill thread transitions to `"filled"` with `fill_price` after `fill_delay_s` (default 0.10 s; set to 0 for instant fills in tests). Fill price sourced from live `MARKET_DATA` events when `event_manager` is supplied, or from the order's stated price / 1.0 fallback. Implements `get_order`, `cancel_order`, `get_positions`, `get_account_balances`. Starting paper account balance configurable (default $100,000). Lifecycle: `start()` / `stop()` / `is_running` satisfy the `SessionSupervisor._components` duck-type interface. |
| `__init__.py` | 81 | Package init exporting the paper engine, live engine, session supervisor, and paper broker. |
...
| Module | LOC | Description |
|--------|----:|-------------|
| `SpyderS01_DIXCalculator.py` | 760 | DIX (Dark Index) calculation. Computes the ratio of dark pool volume to total volume as a proxy for institutional accumulation or distribution. Uses platform-aware `fcntl` file locking on POSIX with a clean no-op fallback. |
...
| `SpyderS07_CustomMetricsOrchestrator.py` | 1,376 | Orchestrates all custom metrics (DIX, GEX, SKEW, Black Swan, TradingView breadth, options analytics) into a unified real-time signal feed. Emits `breadth_updated` signal with TICK/TRIN/ADD/NYMO snapshot. Options analytics (v8): IVR, ATM IV, HV20, VRP computed and published each cycle. NYMO relay: `_update_tv_breadth_metrics` relays `snap.get("nymo", float("nan"))`. |
| `SpyderS08_PivotMeanReversionSignal.py` | 329 | Pivot Mean-Reversion (PMR) signal engine. Computes classic pivot levels from prior-session OHLC and scores fade-resistance / fade-support setups. Returns a typed `PivotMRSignal` with `direction`, `score` (0–100), `confidence`, and full `reasons` / `penalties` audit lists. |
...
| `SpyderS09_FREDClient.py` | 314 | FRED (Federal Reserve Economic Data) API client. Fetches macro and yield curve data for strategy conditioning. |
| `SpyderS10_SentimentScraper.py` | 413 | Weekly sentiment data scrapers. Collects AAII individual investor survey data for contrarian positioning signals. |
| `SpyderS11_TradingViewInternals.py` | ~395 | NYSE market breadth internals via Playwright headless Chromium scraping of public TradingView symbol pages. Retrieves $TICK, $ADD, $TRIN.NY, and $NYMO with majority-vote breadth regime classification. |
...
| `SpyderT120_S07OptionsAnalytics.py` | 516 | S07 options analytics test suite (30 tests — all passing). Six test classes covering IVR, ATM IV, HV20, VRP computation, client caching, metric data structures, and `_format_metrics` output. |
...
| `SpyderT129_ProtocolCompliance.py` | 92 | Runtime protocol-compliance suite. Verifies E01 exposes `validate_signal()` and accepts `tradier_client`; F10 returns `None` from `calculate_all_indicators`; C04 `get_current_condition()` returns a valid `MarketCondition` member; F00 `AnalyticsProviderProtocol` exports `calculate_all_indicators`. `RegimeCanonicalWiringTest`: asserts D30 imports `UnifiedRegimeEngine` with L09 preceding E21, and Y01 declares `L09_AVAILABLE`. Executed by Q10 CI gate. |
| `SpyderT131_PivotMRSignalTests.py` | 301 | S08 Pivot Mean-Reversion signal test suite (24 tests, all passing). Six test classes covering no-fire conditions, fade-resistance/support fire paths, bonuses, penalties, fire-gate threshold, level selection, and bidirectional choice arbitration. |
...
| `SpyderT42_Integration_Test.py` | 346 | Live integration tests requiring sandbox credentials (skipped in CI when credentials are unavailable). |
| `SpyderT43_OrderManager_Test.py` | 805 | Order manager tests: lifecycle state machine, partial fills, cancellations. |
| `SpyderT44_DatabentoClient_Test.py` | 305 | Massive client tests: data classes, connection state, stream lifecycle, and mock responses. |
...
| `test_g13_signal_monitor_styles.py` | ~100 | Regression guard for G13 widget stylesheets. |
...
| `SpyderU03_DateTimeUtils.py` | 1,841 | Comprehensive date/time utilities. Market hours detection, session classification (pre/regular/post), timezone conversion, and trading day arithmetic. Provides `now_utc()` and `now_et()` helpers for timezone-aware timestamps. |
...
| `SpyderU47_OptionalImport.py` | 131 | Canonical helper for optional imports. Provides `optional_import(name, *, purpose, required_on)` returning an `OptionalImport` wrapper; truthy on success, raises a loud `ImportError` on attribute access when missing. |
...
| `SpyderV02_ModelManager.py` | 1,046 | Quant model lifecycle manager. Handles model registration, parameter storage, calibration scheduling, and performance tracking. Contains a legacy internal `MarketRegimeDetector` class; canonical regime signal from `SpyderL09_UnifiedRegimeEngine`. |
...
| `SpyderV09_IVEngine.py` | 973 | **Canonical IV engine.** Synchronous BSM pricing, IV solver (Newton-Raphson with bisection fallback), and full Greeks computation. Designated single source of truth for IV calculations. |
...
| `SpyderY01_MarketSenseAgent.py` | 524 | Continuously monitors market conditions. Emits typed regime-change and anomaly events. Resolves regime through `SpyderL09_UnifiedRegimeEngine` (canonical), falling back to E21 HMM when L09 is unavailable. |
...
*End of Spyder Codebase Review v19 (v9) — April 19, 2026*
