...
class CreditSpreadPosition:
    ...
    market_regime_at_entry: Optional[str]
...
class SpreadConstructionEngine:
    ...
    def construct_optimal_spread(self, market_env: MarketEnvironment,
                                spread_type: CreditSpreadType = CreditSpreadType.AUTO_SELECT) -> Optional[CreditSpreadParameters]:
    ...
    def _select_optimal_spread_type(self, market_env: MarketEnvironment) -> CreditSpreadType:
        ...
        try:
            ...
            trend_strength = abs(market_env.trend_strength)
...
class UnifiedCreditSpreadEngine:
    ...
    # ==========================================================================
    async def analyze_spread_opportunity(self, market_data: pd.DataFrame) -> Optional[CreditSpreadParameters]:
        """
    ...
    async def execute_credit_spread(self, spread_params: CreditSpreadParameters) -> Optional[str]:
        """
    ...
    def _update_position_metrics(self, position: CreditSpreadPosition, current_price: float):
        ...
        try:
            ...
            moneyness_short = position.spread_parameters.short_strike / current_price
...
if __name__ == "__main__":
    ...
    print("=" * 80)
    print("SPYDER D25 - UNIFIED CREDIT SPREAD ENGINE DEMONSTRATION")
    print("=" * 80)
    ...
    print(f"\n✅ Unified Credit Spread Engine initialized")
    ...
    print(f"   Price Trend: +${trend[-1]:.2f} over period")
    print(f"   Current VIX: {market_data['vix'].iloc[-1]:.1f}")
    ...
    print(f"   Market Bias: {market_env.market_bias.value}")
    print(f"   Trend Strength: {market_env.trend_strength:.2f}")
    print(f"   Implied Volatility: {market_env.implied_volatility:.1%}")
    print(f"   Volume Ratio: {market_env.volume_ratio:.2f}")
    
    if market_env.support_levels:
        print(f"   Support Levels: {[f'${s:.2f}' for s in market_env.support_levels[-3:]]}")
    if market_env.resistance_levels:
        print(f"   Resistance Levels: {[f'${r:.2f}' for r in market_env.resistance_levels[:3]]}")
    ...
    if spread_opportunity:
        print(f"\n✅ CREDIT SPREAD OPPORTUNITY IDENTIFIED:")
        print("-" * 60)
        print(f"   Spread Type: {spread_opportunity.spread_type.value.upper()}")
        print(f"   Spread Width: ${spread_opportunity.spread_width:.2f}")
        print(f"   Target Credit: ${spread_opportunity.target_credit:.2f}")
        print(f"   Max Loss: ${spread_opportunity.max_loss:.2f}")
        print(f"   Short Strike: ${spread_opportunity.short_strike:.2f}")
        print(f"   Long Strike: ${spread_opportunity.long_strike:.2f}")
        print(f"   Breakeven: ${spread_opportunity.breakeven:.2f}")
        print(f"   Probability of Profit: {spread_opportunity.probability_profit:.1%}")
        print(f"   Short Delta: {spread_opportunity.short_delta:.3f}")
        print(f"   Long Delta: {spread_opportunity.long_delta:.3f}")
        print(f"   Expiration: {spread_opportunity.expiration_date.strftime('%Y-%m-%d')}")
        
        # Test spread execution
        print(f"\n🚀 Executing credit spread...")
        ...
        if position_id:
            print(f"   ✅ Spread executed successfully: Position {position_id[:8]}...")
            
            # Simulate some price movement and test position management
            print(f"\n📈 Simulating price movement and position management...")
                ...
                for i, move in enumerate(price_moves):
                    ...
                    if actions:
                        for action in actions:
                            print(f"   Day {i+1}: {action['action']} - {action['reason']}")
        ...
        print(f"\n❌ No suitable credit spread opportunity found")
        print(f"   Market conditions may not be favorable for spreads")
    
    # Show final engine status
    print(f"\n📊 FINAL ENGINE STATUS:")
    final_status = engine.get_engine_status()
    print(f"   Active Positions: {final_status['active_positions']}")
    print(f"   Bull Put Spreads: {final_status['position_breakdown']['bull_put_spreads']}")
    print(f"   Bear Call Spreads: {final_status['position_breakdown']['bear_call_spreads']}")
    ...
    print(f"   Bull Put Trades: {pm['bull_put_stats']['count']}")
    print(f"   Bear Call Trades: {pm['bear_call_stats']['count']}")
    ...
    print(f"\n{('='*80)}")
    print("D-SERIES CREDIT SPREAD CONSOLIDATION SUCCESS!")
    print("✅ 4-module overlap eliminated (D03, D06, D07, D18)")
    print("✅ Intelligent spread selection and construction")  
    print("✅ Unified position management and risk controls")
    print("✅ Advanced market analysis and regime integration")
