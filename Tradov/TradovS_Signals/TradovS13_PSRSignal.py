...
Caching:
    Daily OHLCV data cached to ~/.tradov/psr_cache/ with 4-hour TTL.

Update Cadence:
    Daily close — PSR moves on weekly/monthly timescales.  No intraday refresh
    needed.  Integrated with TradovS07_CustomMetricsOrchestrator.

Integration:
    - TradovD30_RegimeGatedSelector: uses signal_level to gate strategies
    - TradovF10_MarketRegimeDetector: adds macro context to regime composite
    - TradovS07_CustomMetricsOrchestrator: exposes psr to MetricSnapshot
    - TradovG05_TradingDashboard: clickable PSR widget with detail dialog
"""
...
try:
    ...
    _error_handler = TradovErrorHandler()
except Exception:
...
# Cache
_CACHE_DIR = Path("~/.tradov/psr_cache").expanduser()
CACHE_TTL_HOURS = 4
...
class _TradierMarketClient:
    ...
    TradovB40_TradierClient (which handles order management only).
    """
...
class PSRSignal:
    """
    Pawn Shop Ratio — macro liquidity-exhaustion regime overlay for Tradov.
    ...
    Integration with S07 orchestrator:
        ...
        with MetricSnapshot expansion in TradovS07.
    """
