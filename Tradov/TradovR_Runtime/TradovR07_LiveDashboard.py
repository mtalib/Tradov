...
Series: TradovR_Runtime [Application Name] [Series Letter] [Series Name]
Module: TradovR07_LiveDashboard.py [Application Name][Series Letter] [Module Number]_[Purpose].py
...
Module Description:
    Professional runtime launcher for the enhanced TradovG05 Trading Dashboard.
...
INTEGRATION:
    • Works seamlessly with enhanced TradovG05_TradingDashboard
...
NOTE: Broker integration uses Tradier API (TradovB40_TradierClient).
      Market data via Tradier (TradovB40_TradierClient).
...
# LOCAL IMPORTS
# ==============================================================================
# Add Tradov directory to path
TRADOV_HOME = Path(__file__).parent.parent
sys.path.insert(0, str(TRADOV_HOME))

# Import the enhanced dashboard (our new G05)
from Tradov.TradovG_GUI.TradovG05_TradingDashboard import TradovTradingDashboard  # noqa: E402
import logging  # noqa: E402

# ==============================================================================
# CONSTANTS
# ==============================================================================
REAL_DATA_FILE = Path.home() / "Projects/Tradov/market_data/live_data.json"
...
class SystemChecker(QObject):
    ...
    def _check_market_data_availability(self) -> dict:
        ...
        try:
            if REAL_DATA_FILE.exists():
                ...
                data_available = (datetime.now(UTC) - last_update).seconds < 3600
    ...
    def _check_dashboard_components(self) -> dict:
        ...
        try:
            ...
            from TradovG_GUI.TradovG05_TradingDashboard import TradovTradingDashboard  # noqa: F401
            return {
                'status': 'OK',
                'dashboard_class': 'TradovG05_TradingDashboard',
                'available': True
            }
...
class TradovSplashScreen(QSplashScreen):
    ...
    def __init__(self):
        ...
        painter.drawText(20, 350, "TradovR07_LiveDashboard - Enhanced Launcher")
...
class TradovLiveDashboardLauncher:
    """Professional launcher for the enhanced Tradov Live Dashboard."""
    ...
    def launch(self):
        """Launch the enhanced Tradov dashboard with professional startup."""
        try:
            ...
            # Show splash screen
            self.splash = TradovSplashScreen()
            self.splash.show()
            ...
            self._show_error("Startup Error", f"Failed to launch Tradov dashboard: {str(e)}")
            return 1
    ...
    def _show_startup_summary(self, results: dict):
        ...
        if errors or warnings:
            ...
            QMessageBox.information(None, "Tradov Startup Summary", "\n".join(message_parts))

    def _launch_dashboard(self):
        """Launch the main enhanced trading dashboard."""
        try:
            # Create and show the enhanced dashboard
            self.dashboard = TradovTradingDashboard()
    ...
    def _log_startup_info(self):
        ...
        try:
            # Add startup logs
            self.dashboard.add_system_log("🚀 Tradov R07 Live Dashboard Launcher")
            self.dashboard.add_system_log(f"Startup completed: {datetime.now(UTC).strftime('%H:%M:%S')}")  # noqa: E501
...
def launch_tradov_dashboard():
    """
    Convenience function to launch Tradov dashboard.

    Returns:
        int: Exit code (0 = success, 1 = error)
    """
    launcher = TradovLiveDashboardLauncher()
...
def check_system_only():
    ...
    logging.info("=" * 60)
    logging.info("TRADOV SYSTEM CHECK (R07 Live Dashboard)")
    logging.info("=" * 60)
    ...
    logging.info("\n" + "=" * 60)
    logging.info("System check complete. Use launch_tradov_dashboard() to start.")
...
def main():
    ...
    logging.info("=" * 60)
    logging.info("TRADOV R07 LIVE DASHBOARD LAUNCHER")
    logging.info("=" * 60)
    logging.info("Enhanced launcher for TradovG05 Trading Dashboard")
    ...
    return launch_tradov_dashboard()
