...
class AccountManager:
    ...
    def check_trading_allowed(self, account_id: Optional[str] = None,
                            order_value: float = 0.0) -> Tuple[bool, str]:
        """
        ...
        if abs(account.risk_metrics.daily_loss_percent) >= self.risk_limits['max_daily_loss']:
            return False, f"Daily loss limit exceeded: {account.risk_metrics.daily_loss_percent:.1%}"
    ...
    def _assess_account_risk(self, account_id: str):
        ...
        try:
            ...
            if balance.excess_liquidity > 0:
                risk.margin_usage_percent = (
                    balance.full_maint_margin / 
            ...
            if len(self.daily_balances) > 0:
                ...
                if start_balance > 0:
                    risk.daily_loss_percent = (
                        (balance.net_liquidation - start_balance) / start_balance
                    )
            ...
            if len(self.daily_balances) >= 20:
                ...
                if len(returns) > 0:
                    ...
                    risk.expected_shortfall = returns[returns <= np.percentile(returns, 5)].mean() * balance.net_liquidation
    ...
    def _calculate_account_performance(self, account_id: str):
        ...
        try:
            ...
            if weekly_mask.any():
                metrics.weekly_return = (balance.net_liquidation / balances[weekly_mask].iloc[0]) - 1
            
            # Monthly return
            month_ago = now - timedelta(days=30)
            monthly_mask = timestamps > month_ago
            if monthly_mask.any():
                metrics.monthly_return = (balance.net_liquidation / balances[monthly_mask].iloc[0]) - 1
            ...
            if len(returns) >= 20:
                ...
                if metrics.daily_volatility > 0:
                    metrics.sharpe_ratio = (returns.mean() / metrics.daily_volatility) * np.sqrt(252)
    ...
    def get_balance_history(self, account_id: Optional[str] = None,
    ...
    def _start_background_threads(self):
        ...
        self._update_thread = threading.Thread(
            target=self._account_update_loop,
            name="AccountUpdate",
            daemon=True
        )
        ...
        self._history_thread = threading.Thread(
            target=self._balance_history_loop,
            name="BalanceHistory",
            daemon=True
        )
        ...
        self._risk_thread = threading.Thread(
            target=self._risk_assessment_loop,
            name="RiskAssessment",
            daemon=True
        )
        ...
        self._performance_thread = threading.Thread(
            target=self._performance_calc_loop,
...
def create_account_manager(spyder_client: SpyderClient,
                          event_manager: Optional[EventManager] = None,
                          config: Optional[Dict[str, Any]] = None) -> AccountManager:
    """
