...
Series: SpyderE_Risk
Module: SpyderE21_HMMRegimeDetector.py
...
class HMMTrainingResult:
    ...
    transition_matrix: Optional[np.ndarray] = None  # State transition probabilities
    stationary_distribution: Optional[Any] = None  # Stationary distribution
...
class RegimePrediction:
    ...
    expected_duration: Optional[float]  # Expected days in current regime
...
class HMMModelMetrics:
    ...
    bic: float
    aic: float
    transition_matrix: Optional[np.ndarray]  # State transition probabilities
    stationary_distribution: Optional[Any]  # Stationary distribution
    prediction_accuracy: Optional[float]  # Backtest accuracy
...
class HMMRegimeDetector:
    ...
    def __init__(self, n_states: int = HMM_N_STATES, 
        ...
        self.training_result: Optional[HMMTrainingResult] = None
        ...
        # Performance metrics
        self.model_metrics: Optional[HMMModelMetrics] = None
    ...
    def initialize(self, 
                 ...
                 vix_data: Optional[pd.DataFrame] = None) -> bool:
        """
    ...
    def _prepare_features(self, 
                     ...
                     vix_data: Optional[pd.DataFrame]) -> np.ndarray:
        """
    ...
    def _calculate_expected_duration(self, regime: MarketRegime) -> Optional[float]:
        """
    ...
    def get_model_metrics(self) -> Optional[HMMModelMetrics]:
        """
...
def create_sample_data(n_periods: int = 252, 
    ...
    # Create DataFrame
    returns_df = pd.DataFrame(returns, index=dates, columns=['SPY'])
...
if __name__ == "__main__":
    print("=" * 70)
    print("🔬 SPYDER HMM REGIME DETECTOR")
    print("=" * 70)
    ...
    # Train model
    print("\n3. Training HMM model...")
    if detector.training_result is not None and detector.training_result.converged:
        print(f"   ✅ Model converged")
        print(f"   Log-Likelihood: {detector.training_result.log_likelihood:.2f}")
        print(f"   BIC: {detector.training_result.bic:.2f}")
        print(f"   States: {detector.training_result.n_states}")
    else:
        print("   ⚠️  Model did not converge or not trained")
    
    # Get model metrics
    if detector.model_metrics is not None:
        print("\n4. Model Metrics:")
        print(f"   Model Type: {detector.model_metrics.model_type}")
        print(f"   States: {detector.model_metrics.n_states}")
        print(f"   Parameters: {detector.model_metrics.n_params}")
        print(f"   Log-Likelihood: {detector.model_metrics.log_likelihood:.2f}")
        print(f"   Persistence: {detector.model_metrics.persistence:.2f}")
    ...
    for i, pred in enumerate(predictions[-5:]):
        print(f"   Day {i+1}: {pred.current_regime.value.upper()} "
              f"(confidence: {pred.confidence:.2%}, "
              f"strategy: {pred.recommended_strategy})")
    ...
    print("\n" + "=" * 70)
    print("✅ HMM Regime Detector Test Completed Successfully")
    print("=" * 70)
