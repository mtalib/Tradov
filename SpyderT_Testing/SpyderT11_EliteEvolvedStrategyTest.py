...
def _test_elite_options_pricing(libs, elite_strategy: Dict[str, Any]) -> bool:
    ...
    try:
        ...
        if short_pricing and long_pricing:
            return _analyze_elite_credit_spread(short_pricing, long_pricing, 
                                              short_strike, long_strike, 
                                              current_price, elite_strategy)
        else:
...
def _analyze_elite_credit_spread(short_pricing, long_pricing, short_strike: float, 
                               long_strike: float, current_price: float, 
                               elite_strategy: Dict[str, Any]) -> bool:
...
def _calculate_elite_quality_score(net_credit: float, net_delta: float, 
                                 net_theta: float, max_loss: float, 
                                 fitness: float) -> float:
...
def _count_criteria_met(metrics) -> int:
    ...
    criteria_met = 0
    if metrics.sharpe_ratio > 1.5: criteria_met += 1
    if metrics.max_drawdown > -0.08: criteria_met += 1
    if metrics.sortino_ratio > 2.0: criteria_met += 1
    if metrics.calmar_ratio > 1.5: criteria_met += 1
    if metrics.volatility < 0.12: criteria_met += 1
    return criteria_met

def _display_final_assessment(institutional_score: float, criteria_met: int, 
