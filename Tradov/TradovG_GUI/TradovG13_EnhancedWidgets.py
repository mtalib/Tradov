...
Module Description:
    ...
    the Tradov visual theme and provide superior user experience.
"""
...
# LOCAL IMPORTS
# ==============================================================================
from Tradov.TradovG_GUI.TradovG06_DashboardData import (  # noqa: E402
...
from Tradov.TradovU_Utilities.TradovU24_StyleManager import TradovColors, get_style_manager  # noqa: E402

logger = logging.getLogger(__name__)

try:
    from Tradov.TradovG_GUI.TradovG12_SignalInfoDialog import SignalInfoDialog
...
_TOOLTIP_THEME_MARKER = "/* tradov-tooltip-theme */"
...
# ==============================================================================
class TradovStrikeRangeSlider(QWidget):
    """
    ...
    def setup_ui(self):
        ...
        title_label.setStyleSheet(f"color: {TradovColors.TEXT}; font-weight: normal;")
        ...
        self.min_label.setStyleSheet(f"color: {TradovColors.POSITIVE}; font-weight: normal;")

        self.max_label = QLabel(f"${self.current_max:.2f}")
        self.max_label.setAlignment(Qt.AlignmentFlag.AlignRight)
        self.max_label.setStyleSheet(f"color: {TradovColors.NEGATIVE}; font-weight: normal;")
...
# ==============================================================================
class TradovTradingInput(QWidget):
    """
    ...
    def setup_ui(self):
        ...
        if self.label_text:
            ...
            self.label.setStyleSheet(f"color: {TradovColors.TEXT}; font-weight: normal;")
        ...
        if self.validation_rules["prefix"]:
            ...
            prefix_label.setStyleSheet(f"color: {TradovColors.INFO};")
        ...
        if self.validation_rules["suffix"]:
            ...
            suffix_label.setStyleSheet(f"color: {TradovColors.INFO};")
        ...
        self.error_label.setStyleSheet(f"color: {TradovColors.NEGATIVE}; font-size: 8pt;")
...
# ==============================================================================
class TradovSearchableCombo(QWidget):
    """
...
# ==============================================================================
class TradovCollapsibleGroup(QWidget):
    """
...
class TradovTradingTooltip:
    ...
    def show_option_tooltip(widget: QWidget, option_data: dict[str, Any]):
        ...
        # Format option data
        tooltip_html = f"""
        <div style="background-color: {TradovColors.PANEL}; color: {TradovColors.TEXT}; padding: 10px; border: 1px solid {TradovColors.BORDER};">
            <h3 style="color: {TradovColors.INFO};">{option_data.get('symbol', 'N/A')}</h3>
            <table style="border-collapse: collapse; width: 100%;">
                <tr><td>Strike:</td><td style="color: {TradovColors.NEUTRAL};">${option_data.get('strike', 0):.2f}</td></tr>
                <tr><td>Bid:</td><td style="color: {TradovColors.POSITIVE};">${option_data.get('bid', 0):.2f}</td></tr>
                <tr><td>Ask:</td><td style="color: {TradovColors.NEGATIVE};">${option_data.get('ask', 0):.2f}</td></tr>
    ...
    def show_trade_tooltip(widget: QWidget, trade_data: dict[str, Any]):
        ...
        pnl_color = TradovColors.POSITIVE if pnl >= 0 else TradovColors.NEGATIVE

        tooltip_html = f"""
        <div style="background-color: {TradovColors.PANEL}; color: {TradovColors.TEXT}; padding: 10px; border: 1px solid {TradovColors.BORDER};">
            <h3 style="color: {TradovColors.INFO};">Trade Details</h3>
            <table style="border-collapse: collapse; width: 100%;">
                <tr><td>Symbol:</td><td>{trade_data.get('symbol', 'N/A')}</td></tr>
                <tr><td>Side:</td><td style="color: {TradovColors.POSITIVE if trade_data.get('side') == 'BUY' else TradovColors.NEGATIVE};">{trade_data.get('side', 'N/A')}</td></tr>
...
# WIDGET FACTORY
# ==============================================================================
class TradovWidgetFactory:
    """Factory class for creating enhanced Tradov widgets."""

    @staticmethod
    def create_strike_range_slider(min_strike: float, max_strike: float) -> TradovStrikeRangeSlider:
        """Create a strike range slider widget."""
        return TradovStrikeRangeSlider(min_strike, max_strike)

    @staticmethod
    def create_trading_input(input_type: str, label: str = "") -> TradovTradingInput:
        """Create a trading input widget."""
        return TradovTradingInput(input_type, label)

    @staticmethod
    def create_searchable_combo(items: list[str] = None) -> TradovSearchableCombo:
        """Create a searchable combo box widget."""
        return TradovSearchableCombo(items)

    @staticmethod
    def create_collapsible_group(title: str, expanded: bool = True) -> TradovCollapsibleGroup:
        """Create a collapsible group widget."""
        return TradovCollapsibleGroup(title, expanded)
...
class TradingMode(Enum):
    """Two trading modes available in the Tradov system.

    PAPER:    Simulated fills against live market data via TradovBox + TradovR02_PaperEngine.
    LIVE:     Real order execution through Tradier production API + TradovR04_LiveEngine.
    """
...
class ConnectionInfo:
    ...
    exist as parallel scalar attributes on TradovTradingDashboard, mutated
...
class SignalMonitorPanel(QWidget):
    ...
    def show_skew_dialog(self):
        ...
        if skew_dialog_available is None:
            try:
                from Tradov.TradovG_GUI.TradovG11_SkewMonitorDialog import (
    ...
    def show_internals_dialog(self):
        ...
        if internals_dialog_available is None:
            try:
                from Tradov.TradovG_GUI.TradovG17_MarketInternalsWidget import (
        ...
        if internals_dialog_available and MarketInternalsDialog:
            ...
            QMessageBox.information(
                ...
                "Ensure TradovG17_MarketInternalsWidget.py is present and pyqtgraph / yfinance are installed.",  # noqa: E501
            )
...
class MarketSymbolWidget(QWidget):
    ...
    def update_pmr_state(self, state: dict) -> None:
        ...
        State payload is emitted by R08 every poll. Display modes:
          * DIS    -- producer disabled (TRADOV_PIVOT_MR_ENABLED != 1)
        ...
        if not enabled:
            ...
            self.pct_label.setStyleSheet("font-size: 11px;")
            self.setToolTip(
                "Pivot MR producer disabled. Set TRADOV_PIVOT_MR_ENABLED=1 to enable."
            )
            return
...
def main():
    ...
    # Apply Tradov styling
    from Tradov.TradovU_Utilities.TradovU24_StyleManager import apply_tradov_style
    apply_tradov_style(app)

    # Create main window
    from PySide6.QtWidgets import QMainWindow, QVBoxLayout, QWidget

    window = QMainWindow()
    window.setWindowTitle("Tradov Enhanced Widgets Demo")
    ...
    if SUPERQT_AVAILABLE:
        ...
        # Strike range slider
        strike_slider = TradovWidgetFactory.create_strike_range_slider(400.0, 500.0)
        ...
        # Trading inputs
        price_input = TradovWidgetFactory.create_trading_input("price", "Entry Price")
        quantity_input = TradovWidgetFactory.create_trading_input("quantity", "Contracts")

        layout.addWidget(price_input)
        layout.addWidget(quantity_input)

        # Searchable combo
        symbols = ["TRAD241220C00450000", "TRAD241220P00450000", "TRAD241220C00455000"]
        symbol_combo = TradovWidgetFactory.create_searchable_combo(symbols)
        layout.addWidget(symbol_combo)

        # Collapsible group
        advanced_group = TradovWidgetFactory.create_collapsible_group("Advanced Options", False)
        advanced_input = TradovWidgetFactory.create_trading_input("percentage", "Max Risk %")
        ...
        fallback_label.setStyleSheet(f"color: {TradovColors.WARNING}; font-size: 14px; padding: 20px;")  # noqa: E501
