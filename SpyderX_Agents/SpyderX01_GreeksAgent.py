...
from SpyderU_Utilities.SpyderU06_MathUtils import calculate_option_greeks, black_scholes_price
from SpyderU_Utilities.SpyderU03_DateTimeUtils import get_time_to_expiration, is_market_open
...
class GreeksAgent:
    ...
    async def analyze_position_greeks(
        ...
        try:
            ...
            market_impact = await self._analyze_market_context_impact(greeks_data, context)
            ...
            analysis = AIGreeksAnalysis(
                ...
                key_insights=await self._extract_key_insights(greeks_data, risk_assessment),
                risk_explanation=await self._generate_risk_explanation(greeks_data, risk_assessment),
                market_context_impact=market_impact,
                pnl_scenarios=pnl_scenarios,
                sensitivity_analysis=sensitivity_analysis,
                probability_distributions=await self._generate_probability_distributions(greeks_data),
                hedge_recommendations=hedge_recommendations,
                optimal_hedge_ratio=await self._calculate_optimal_hedge_ratio(greeks_data),
                hedge_cost_estimate=await self._estimate_hedge_cost(hedge_recommendations),
                market_regime=context.market_regime,
                volatility_environment=self._describe_volatility_environment(context),
                correlation_risks=await self._assess_correlation_risks(greeks_data, context),
            ...
            self.error_handler.handle_ai_error(e, "GreeksAgent", "analyze_position_greeks")
    ...
    async def analyze_portfolio_greeks(
        ...
        """
        try:
            self.logger.info(f"Analyzing portfolio Greeks for {len(positions)} positions")
            ...
            portfolio_risk = await self._assess_portfolio_risk(positions, aggregated_greeks)
            ...
            return PortfolioGreeksAnalysis(
                ...
                portfolio_risk_level=self._determine_risk_level(portfolio_risk['score']),
            ...
            self.error_handler.handle_ai_error(e, "GreeksAgent", "analyze_portfolio_greeks")
            return await self._generate_fallback_portfolio_analysis(positions, portfolio_id)
    ...
    async def _initialize_and_train_models(self):
        ...
        try:
            if not HAS_SKLEARN:
                self.logger.warning("scikit-learn not available, using heuristic methods")
                return
        ...
        try:
            ...
            if X is not None and y is not None:
                ...
                self.model_metrics[model_name] = MLModelMetrics(
                    ...
                    feature_importance=dict(enumerate(getattr(model, 'feature_importances_', []))),
                    cross_validation_score=cv_scores.mean(),
                    out_of_sample_error=1 - cv_scores.mean(),
                    model_drift_score=0.0
                )
                
                self.logger.info(f"Model {model_name} trained with {n_samples} samples, "
                               f"CV score: {cv_scores.mean():.3f}")
            
        except Exception as e:
            self.logger.error(f"Synthetic training failed for {model_name}: {e}")
    
    def _generate_synthetic_training_data(self, model_name: str, n_samples: int) -> Tuple[np.ndarray, np.ndarray]:
    ...
    def _calculate_enhanced_greeks(self, greeks_data: GreeksData) -> Dict[str, float]:
        ...
        try:
            ...
            # Dollar Greeks
            enhanced_metrics['dollar_delta'] = greeks_data.delta * greeks_data.position_size * greeks_data.underlying_price
            enhanced_metrics['dollar_gamma'] = greeks_data.gamma * greeks_data.position_size * greeks_data.underlying_price * greeks_data.underlying_price / 100
            enhanced_metrics['dollar_theta'] = greeks_data.theta * greeks_data.position_size
            enhanced_metrics['dollar_vega'] = greeks_data.vega * greeks_data.position_size / 100
            
            # Risk-adjusted metrics
            enhanced_metrics['gamma_theta_ratio'] = abs(greeks_data.gamma / greeks_data.theta) if greeks_data.theta != 0 else 0
            enhanced_metrics['delta_hedging_cost'] = abs(enhanced_metrics['dollar_gamma']) * 0.5  # Simplified
            enhanced_metrics['time_decay_rate'] = abs(greeks_data.theta) / greeks_data.position_value if greeks_data.position_value > 0 else 0
            
            # Volatility sensitivity
            enhanced_metrics['vega_percentage'] = abs(greeks_data.vega / greeks_data.position_value) if greeks_data.position_value > 0 else 0
            enhanced_metrics['implied_vol_risk'] = enhanced_metrics['vega_percentage'] * 0.05  # 5% vol move
            
           
            # Time sensitivity
            enhanced_metrics['days_to_breakeven'] = abs(greeks_data.position_value / greeks_data.theta) if greeks_data.theta != 0 else float('inf')
    ...
    async def _ai_risk_assessment(self, greeks_data: GreeksData, context: MarketContext) -> Dict[str, float]:
        ...
        try:
            ...
            if HAS_SKLEARN and 'risk_predictor' in self.ml_models:
                return await self._ml_risk_assessment(greeks_data, context, risk_factors)
            else:
    ...
    def _extract_risk_factors(self, greeks_data: GreeksData, context: MarketContext) -> Dict[str, float]:
        """Extract risk factors for analysis."""
        try:
            factors = {}
            
            # Time risk
            factors['time_risk'] = min(100, (365 - greeks_data.time_to_expiration) / 365 * 100) if greeks_data.time_to_expiration > 0 else 100
            ...
            # Concentration risk
            factors['concentration_risk'] = min(100, greeks_data.position_value / 10000 * 100)
            ...
            # Volatility risk
            factors['vol_risk'] = (greeks_data.implied_volatility - 0.2) / 0.5 * 100 if greeks_data.implied_volatility > 0.2 else 0
    ...
    async def _ml_risk_assessment(self, greeks_data: GreeksData, context: MarketContext, 
        ...
        try:
            ...
            return {
                'score': np.clip(risk_score, 0, 100),
    ...
    def _heuristic_risk_assessment(self, risk_factors: Dict[str, float]) -> Dict[str, float]:
    ...
    def _extract_risk_features(self, greeks_data: GreeksData, context: MarketContext, 
                             risk_factors: Dict[str, float]) -> List[float]:
    ...
    async def _generate_natural_language_summary(
        ...
        try:
            ...
            if abs(greeks_data.delta) > 0.5:
                primary_risks.append(f"significant directional exposure (Delta: {greeks_data.delta:.2f})")
            if abs(greeks_data.gamma) > 0.05:
                primary_risks.append(f"high convexity risk (Gamma: {greeks_data.gamma:.3f})")
            if abs(greeks_data.theta) > 0.1:
                primary_risks.append(f"substantial time decay (Theta: {greeks_data.theta:.2f})")
            if abs(greeks_data.vega) > 0.2:
                primary_risks.append(f"volatility sensitivity (Vega: {greeks_data.vega:.2f})")
            ...
            if greeks_data.time_to_expiration < 30:
                ...
                time_desc = f"with {greeks_data.time_to_expiration:.0f} days to expiration"
            ...
            if primary_risks:
                summary_parts.insert(-1, f"Key concerns include {', '.join(primary_risks)}")
            
            summary_parts.append(market_desc + ", heightened vigilance is recommended." if risk_level in [RiskLevel.HIGH, RiskLevel.VERY_HIGH, RiskLevel.EXTREME] else market_desc + ", the position appears manageable.")
    ...
    async def _extract_key_insights(self, greeks_data: GreeksData, risk_assessment: Dict[str, float]) -> List[str]:
        ...
        try:
            ...
            if abs(greeks_data.delta) > 0.7:
                insights.append(f"High delta ({greeks_data.delta:.2f}) indicates strong directional bias")
            elif abs(greeks_data.delta) < 0.3:
                insights.append(f"Low delta ({greeks_data.delta:.2f}) suggests limited directional risk")
            
            # Gamma insights
            if greeks_data.gamma > 0.05:
                insights.append(f"High gamma ({greeks_data.gamma:.3f}) creates acceleration risk in delta")
            
            # Theta insights
            if greeks_data.theta < -0.1:
                insights.append(f"Significant time decay ({greeks_data.theta:.2f}) eroding position value daily")
            
            # Vega insights
            if abs(greeks_data.vega) > 0.3:
                insights.append(f"High vega ({greeks_data.vega:.2f}) makes position vulnerable to volatility changes")
            
            # Time-based insights
            if greeks_data.time_to_expiration < 7:
                insights.append("Imminent expiration dramatically increases time decay and gamma risk")
            ...
            if risk_assessment['score'] > 75:
                insights.append("Position risk exceeds recommended thresholds - consider hedging")
            elif risk_assessment['score'] < 25:
                insights.append("Position risk is well-controlled within acceptable parameters")
            
            # Market context insights
            if self.market_context.vix_level > 30:
                insights.append("Elevated VIX environment increases all Greeks volatility")
            elif self.market_context.vix_level < 15:
                insights.append("Low VIX environment may lead to sudden volatility spikes")
    ...
    async def _generate_risk_explanation(self, greeks_data: GreeksData, risk_assessment: Dict[str, float]) -> str:
        ...
        try:
            ...
            if risk_score > 80:
                explanations.append("This position carries extreme risk due to multiple overlapping risk factors.")
            elif risk_score > 60:
                explanations.append("This position has elevated risk requiring careful management.")
            elif risk_score > 40:
                explanations.append("This position has moderate risk within acceptable parameters.")
            else:
                explanations.append("This position has low risk with minimal exposure concerns.")
            
            # Greek-specific risks
            if abs(greeks_data.delta) > 0.6:
                direction = "upward" if greeks_data.delta > 0 else "downward"
                explanations.append(f"High delta exposure means significant losses on {direction} moves in {greeks_data.symbol}.")
            
            if greeks_data.gamma > 0.03:
                explanations.append("High gamma creates non-linear risk where small moves can cause large delta changes.")
            
            if greeks_data.theta < -0.05:
                daily_decay = abs(greeks_data.theta * greeks_data.position_size)
                explanations.append(f"Time decay of approximately ${daily_decay:.0f} per day erodes position value.")
            
            if abs(greeks_data.vega) > 0.25:
                vol_impact = abs(greeks_data.vega * greeks_data.position_size * 5)  # 5% vol move
                explanations.append(f"Volatility risk: 5% change in implied volatility impacts position by ~${vol_impact:.0f}.")
            
            # Time-based risks
            if greeks_data.time_to_expiration < 14:
                explanations.append("Short time to expiration amplifies all Greek risks, especially gamma and theta.")
    ...
    async def _generate_pnl_scenarios(self, greeks_data: GreeksData, context: MarketContext) -> Dict[str, float]:
        ...
        try:
            ...
            for scenario_name, move in price_moves.items():
                ...
                gamma_pnl = 0.5 * greeks_data.gamma * greeks_data.position_size * (price_change ** 2)
            ...
            for vol_scenario, vol_change in vol_changes.items():
                vega_pnl = greeks_data.vega * greeks_data.position_size * vol_change * 100
    ...
    async def _perform_sensitivity_analysis(self, greeks_data: GreeksData) -> Dict[str, Dict[str, float]]:
    ...
    async def _generate_probability_distributions(self, greeks_data: GreeksData) -> Dict[str, List[float]]:
        ...
        try:
            ...
            distributions['delta_changes'] = (greeks_data.gamma * price_changes).tolist()
        ...
        try:
            ...
            if abs(greeks_data.delta) > 0.5:
                ...
                recommendations.append({
                    ...
                    'cost_estimate': abs(hedge_shares) * greeks_data.underlying_price * 0.001,  # Simplified
                    'effectiveness': 'high',
            ...
            recommendations.sort(key=lambda x: (x['effectiveness'] == 'high', -x['cost_estimate']))
    ...
    async def _calculate_optimal_hedge_ratio(self, greeks_data: GreeksData) -> float:
        ...
        try:
            ...
            risk_tolerance = self.config.get('risk_tolerance', 0.5)  # 0 = risk-averse, 1 = risk-seeking
    ...
    async def _estimate_hedge_cost(self, hedge_recommendations: List[Dict[str, Any]]) -> float:
        """Estimate total cost of hedge recommendations."""
        try:
            total_cost = sum(rec.get('cost_estimate', 0) for rec in hedge_recommendations)
            return total_cost
    ...
    async def _analyze_market_context_impact(self, greeks_data: GreeksData, context: MarketContext) -> str:
        ...
        try:
            ...
            if context.vix_level > 30:
                impacts.append("Elevated VIX increases volatility risk and option premiums")
            ...
            if context.correlation_environment > 0.8:
                impacts.append("High correlation environment reduces diversification benefits")
            
            return ". ".join(impacts) if impacts else "Market conditions appear neutral for this position"
            
        except Exception as e:
            self.logger.error(f"Market context analysis failed: {e}")
            return "Market context analysis unavailable"
    
    async def _assess_correlation_risks(self, greeks_data: GreeksData, context: MarketContext) -> Dict[str, float]:
    ...
    def _calculate_model_confidence(self) -> float:
        ...
        try:
            ...
            confidences = [metrics.accuracy_score for metrics in self.model_metrics.values()]
    ...
    def _aggregate_portfolio_greeks(self, positions: List[GreeksData]) -> Dict[str, float]:
        ...
        try:
            ...
            if aggregated['total_value'] > 0:
                aggregated['avg_delta'] = aggregated['total_delta'] / aggregated['position_count']
                aggregated['avg_gamma'] = aggregated['total_gamma'] / aggregated['position_count']
                aggregated['avg_theta'] = aggregated['total_theta'] / aggregated['position_count']
                aggregated['avg_vega'] = aggregated['total_vega'] / aggregated['position_count']
                aggregated['avg_rho'] = aggregated['total_rho'] / aggregated['position_count']
    ...
    async def _assess_portfolio_risk(self, positions: List[GreeksData], aggregated: Dict[str, float]) -> Dict[str, float]:
        """Assess portfolio-level risk."""
        try:
            # Simplified portfolio risk assessment
            base_risk = min(100, abs(aggregated.get('total_delta', 0)) * 10 + 
    ...
    async def _calculate_diversification_score(self, positions: List[GreeksData]) -> float:
    ...
    async def _analyze_portfolio_correlations(self, positions: List[GreeksData]) -> Dict[str, float]:
    ...
    async def _generate_portfolio_insights(self, positions: List[GreeksData], 
                                         aggregated: Dict[str, float], 
                                         risk: Dict[str, float]) -> List[str]:
        ...
        try:
            ...
            if abs(total_delta) > 50:
                insights.append(f"Portfolio has significant directional bias (Total Delta: {total_delta:.0f})")
            
            total_theta = aggregated.get('total_theta', 0)
            if total_theta < -10:
                insights.append(f"Portfolio experiencing substantial time decay (${abs(total_theta):.0f}/day)")
            
            if len(positions) < 3:
                insights.append("Portfolio concentration risk - consider diversification")
                
            return insights
        except Exception:
            return ["Portfolio analysis completed"]
    
    async def _generate_rebalancing_recommendations(self, positions: List[GreeksData], 
                                                  aggregated: Dict[str, float]) -> List[Dict[str, Any]]:
    ...
    async def _perform_portfolio_stress_tests(self, positions: List[GreeksData]) -> Dict[str, float]:
    ...
    async def _find_optimal_portfolio_hedges(self, positions: List[GreeksData], 
                                           aggregated: Dict[str, float]) -> List[Dict[str, Any]]:
    ...
    async def _calculate_risk_attribution(self, positions: List[GreeksData]) -> Dict[str, float]:
        ...
        try:
            ...
            for i, pos in enumerate(positions):
                pos_risk = abs(pos.delta) * 10 + abs(pos.gamma) * 100 + abs(pos.theta) * 5
    ...
    async def _generate_portfolio_summary(self, positions: List[GreeksData], 
                                        aggregated: Dict[str, float], 
        ...
        try:
            ...
            return (f"Portfolio of {total_positions} positions with net delta of {total_delta:.0f} "
    ...
    async def _update_market_context(self):
        ...
        try:
            ...
            self.market_context.vix_level = max(10, min(60, self.market_context.vix_level))
            ...
            self.logger.debug(f"Market context updated: VIX={self.market_context.vix_level:.1f}, "
        ...
        try:
            for model_name, metrics in self.model_metrics.items():
                ...
                if len(self.prediction_accuracy_history) > 20:
                    recent_accuracy = statistics.mean(list(self.prediction_accuracy_history)[-10:])
                    ...
                    if drift_score > 0.2:  # 20% performance degradation
                        ...
                        if drift_score > 0.4:
                            self.logger.error(f"Severe model drift for {model_name} - "
                                            "consider retraining")
    ...
    async def _generate_fallback_analysis(self, greeks_data: GreeksData, mode: AnalysisMode) -> AIGreeksAnalysis:
        ...
        try:
            ...
            return AIGreeksAnalysis(
                ...
                volatility_environment=self._describe_volatility_environment(self.market_context),
                correlation_risks={},
            ...
            return AIGreeksAnalysis(
                ...
                symbol=greeks_data.symbol,
                analysis_mode=mode,
                greeks_summary={'delta': 0, 'gamma': 0, 'theta': 0, 'vega': 0, 'rho': 0},
                risk_score=50.0,
    ...
    async def _generate_fallback_portfolio_analysis(self, positions: List[GreeksData], 
                                                  portfolio_id: str) -> PortfolioGreeksAnalysis:
        ...
        try:
            ...
            return PortfolioGreeksAnalysis(
                ...
                portfolio_risk_score=0, portfolio_risk_level=RiskLevel.LOW,
                diversification_score=0, correlation_risks={}, hedge_efficiency=0,
                natural_language_summary="Portfolio analysis failed",
                portfolio_insights=[], rebalancing_recommendations=[],
                stress_test_results={}, optimal_hedges=[], risk_attribution={}
            )
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        # Perform analysis
        analysis = await agent.analyze_position_greeks(sample_greeks, AnalysisMode.DETAILED)
        ...
        for i, hedge in enumerate(analysis.hedge_recommendations[:3], 1):
            print(f"   {i}. {hedge['description']} (Cost: ${hedge['cost_estimate']:.0f})")
        
        print("\n7. Portfolio Analysis...")
        # Test portfolio analysis with multiple positions
        positions = [sample_greeks]  # Single position for demo
        portfolio_analysis = await agent.analyze_portfolio_greeks(positions, "demo_portfolio")
    ...
    try:
        ...
        print("Note: Some features require additional dependencies (scikit-learn, tensorflow)")
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
SPYDER - Automated SPY Options Trading System
...
class GreeksAgent:
    ...
    def _calculate_enhanced_greeks(self, greeks_data: GreeksData) -> Dict[str, float]:
        ...
        try:
            ...
            # Time sensitivity
            enhanced_metrics['days_to_breakeven'] = abs(greeks_data.position_value / greeks_data
