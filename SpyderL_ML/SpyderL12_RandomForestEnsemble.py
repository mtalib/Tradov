...
class SpyderRandomForestEnsemble:
    ...
    def engineer_features(self, data: pd.DataFrame, 
                         strategy_type: str = 'vanilla') -> pd.DataFrame:
        """
    ...
    async def train(self, training_data: pd.DataFrame,
                   ...
                   optimize_hyperparameters: bool = True) -> ModelPerformance:
        """
        Train Random Forest ensemble.
        Args:
            training_data: DataFrame with option data and prices
            strategy_type: Type of option strategy
            optimize_hyperparameters: Whether to run hyperparameter search
        Returns:
            Model performance metrics
        """
        ...
        if len(X) > 5000:
            ...
            self.shap_explainers[strategy_type] = shap.TreeExplainer(
                model, X[sample_idx]
            )
        else:
        ...
        logger.info(f"Training complete - RMSE: ${performance.rmse:.3f}, "
    ...
    async def _optimize_hyperparameters(self, X: np.ndarray, y: np.ndarray,
        ...
        param_dist = {
            ...
            'min_samples_split': np.arange(
                self.config.min_samples_split_range[0],
                self.config.min_samples_split_range[1],
                2
            ),
            'min_samples_leaf': np.arange(
                self.config.min_samples_leaf_range[0],
                self.config.min_samples_leaf_range[1],
                1
            ),
    ...
    async def _evaluate_model(self, model: RandomForestRegressor,
    ...
    def explain_prediction(self, option_data: pd.DataFrame,
                          strategy_type: str = 'vanilla',
    ...
    def get_feature_importance_report(self, strategy_type: str = 'vanilla',
...
async def main():
    ...
    print(f"CV Scores: {performance.cross_val_scores.mean():.3f} ± {performance.cross_val_scores.std():.3f}")
    # Feature importance
