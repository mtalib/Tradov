...
try:
    ...
    class EClient:
        """Fallback EClient when IBAPI not available"""

        def __init__(self, wrapper):
            pass

    class EWrapper:
        """Fallback EWrapper when IBAPI not available"""
...
class TestReport:
    ...
    def __post_init__(self):
        ...
        self.summary = {
            ...
            "successful": len(
                [r for r in self.results if r.status == TestStatus.SUCCESS]
            ),
            ...
            "timeouts": len(
                [r for r in self.results if r.status == TestStatus.TIMEOUT]
            ),
        }
...
class IBKRSupportTester(EClient, EWrapper):
    ...
    def nextValidId(self, orderId: OrderId):
        ...
        self.reqAccountSummary(
            reqId=1, groupName="All", tags=AccountSummaryTags.AllTags
        )
    ...
    def accountSummary(
        ...
        self._log(
            f"📈 Account data - ReqID: {reqId}, Account: {account}, Tag: {tag}, Value: {value}, Currency: {currency}"
        )
...
class SpyderClientTester:
    ...
    def run_test(self, timeout: int = TEST_TIMEOUT) -> TestResult:
        ...
        try:
            self._log(
                f"🔗 Connecting via SpyderClient to {DEFAULT_HOST}:{self.port}..."
            )
            ...
            if connection_success and self.client.is_connected():
                ...
                try:
                    ...
                    self._log(
                        f"📊 Retrieved account information: {len(result.account_data)} items"
                    )
                except Exception as e:
...
class IBKRConnectionTester:
    ...
    def run_comprehensive_test(self) -> TestReport:
        ...
        report = TestReport(test_timestamp=datetime.now(), host=DEFAULT_HOST)
    ...
    def _generate_recommendations(self, report: TestReport):
        ...
        if not successful_tests:
            ...
            if any(
                r.method == ConnectionMethod.SPYDER_CLIENT for r in successful_tests
            ):
                report.recommendations.append(
                    "Spyder client infrastructure working - use for production"
                )
            elif any(r.method == ConnectionMethod.IB_INSYNC for r in successful_tests):
                report.recommendations.append(
                    "IB-Async connection working - suitable for trading"
                )
            elif any(r.method == ConnectionMethod.RAW_IBAPI for r in successful_tests):
                report.recommendations.append(
                    "Raw IBAPI connection working - basic connectivity confirmed"
                )
        ...
        if failed_tests:
            ...
            if error_patterns:
                report.recommendations.append(
                    f"⚠️ Common errors detected: Check connectivity and authentication"
                )
