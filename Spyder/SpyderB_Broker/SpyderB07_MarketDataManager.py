...
try:
    from SpyderB_Broker.SpyderB33_APIFloodProtection import (
        ...
        APIRequest,
        APIRequestType,
        FloodProtectionAction,
    )

    HAS_FLOOD_PROTECTION = True
except ImportError:
    HAS_FLOOD_PROTECTION = False
...
class MarketDataMetrics:
    ...
    def get_health_score(self) -> float:
        ...
        active_rate = self.subscriptions_active / max(
            1, self.subscriptions_active + self.subscriptions_failed
        )
        staleness_penalty = min(
            0.5, self.stale_data_count / max(1, self.subscriptions_active)
        )

        health_score = (
            success_rate * 0.5 + active_rate * 0.3 + (1.0 - staleness_penalty) * 0.2
        ) * 100
...
class MarketDataManager:
    ...
    def __init__(self, spyder_client=None, event_manager=None):
        ...
        self.logger = (
            SpyderLogger("MarketDataManager")
            if HAS_SPYDER_LOGGER
            else SpyderLogger(__name__)
        )
        self.error_handler = (
            SpyderErrorHandler(self.logger)
            if HAS_ERROR_HANDLER
            else SpyderErrorHandler()
        )

        # Initialize flood protection
        self.flood_protection = get_flood_protection() if HAS_FLOOD_PROTECTION else None
        if self.flood_protection:
            self.logger.info("🛡️ API flood protection enabled")
        ...
        self._snapshot_history: Dict[str, deque] = defaultdict(
        ...
        self.logger.info(
            f"Available features: Pandas={HAS_PANDAS}, "
        ...
        try:
            with self._lock:
                ...
                self._metrics.stale_data_count = sum(
                    1 for snapshot in self._snapshots.values() if snapshot.is_stale()
                )
            ...
            with self._lock:
                ...
                for symbol, sub_info in self._subscriptions.items():
                    if (
                        ...
                        and (current_time - sub_info.created_at).total_seconds() > 300
                    ):
                ...
                if stale_symbols or failed_subs:
                    self.logger.debug(
                        f"Cleaned up {len(stale_symbols)} stale snapshots "
            ...
            with self._lock:
                ...
                if self.flood_protection:
                    if self.flood_protection.is_subscribed(symbol):
                        self.logger.warning(
                            f"🛡️ Prevented duplicate subscription to {symbol}"
                        )
                        return False

                    # Create API request for checking
                    ...
                    if action == FloodProtectionAction.REJECTED:
                        self.logger.warning(
                            f"🛡️ Subscription to {symbol} rejected: {reason}"
                        )
                        return False
                        ...
                        self.logger.info(
                            f"⏳ Subscription to {symbol} queued: {reason}"
                        )
                        return False
                ...
                if hasattr(self.spyder_client, "subscribe_market_data"):
                    success = self.spyder_client.subscribe_market_data(
                        symbol, self._on_market_data
                    )
                    if success:
                        ...
                        if self.flood_protection:
                            req_id = hash(symbol) % 10000  # Generate req_id
                            self.flood_protection.register_subscription(
                                symbol, req_id, APIRequestType.MARKET_DATA
                            )
    ...
    def unsubscribe(self, symbol: str) -> bool:
        ...
        try:
            with self._lock:
                ...
                # Unregister from flood protection
                if self.flood_protection:
                    self.flood_protection.unregister_subscription(symbol)
