...
HIDDEN_SYMBOLS = {
    "VIX_FUTURES": ["VX"],
    "ADDITIONAL_INTERNALS": ["ADVN-NYSE", "DECN-NYSE", "UVOL-NYSE", "DVOL-NYSE", "VOLD-NYSE", "NYHL-NYSE"],
...
CUSTOM_METRICS = {
    "CALCULATED_INTERNALLY": ["GEX", "DEX", "OGL", "DIX", "SWAN"]
}
...
class LiveIBConnectionTester:
    ...
    def initialize(self) -> bool:
        ...
        """
        try:
            self.logger.info(f"Initializing IB connection at {self.config['host']}:{self.config['port']}")
        ...
        try:
            ...
            while time.time() - start_time < timeout:
                ...
                if ticker.bid is not None or ticker.ask is not None or ticker.last is not None:
                    ...
                    self.logger.info(f"✅ {symbol}: Bid={ticker.bid}, Ask={ticker.ask}, Last={ticker.last}")
                    break
    ...
    def _test_options_expiry(self, expiry_type: str, expiry: str, spy_price: float):
            ...
            for right in ['C', 'P']:
                ...
                try:
                    ...
                    if ticker.bid is not None or ticker.ask is not None:
                        ...
                        self.logger.info(f"✅ SPY {expiry} {strike}{right}: Bid={ticker.bid}, Ask={ticker.ask}")
                    else:
        ...
        result.success_rate = (result.success_count / result.tested_count) if result.tested_count > 0 else 0.0
    ...
    def _print_results(self, report: TestReport):
        ...
        for expiry_type, result in report.options_results.items():
            print(f"{expiry_type}: {result.success_count}/{result.tested_count} ({result.success_rate:.1f}%)")
...
# MODULE FUNCTIONS
# ==============================================================================
def create_test_config(host: str = DEFAULT_HOST, 
                      port: int = DEFAULT_PORT,
                      client_id: int = DEFAULT_CLIENT_ID) -> Dict[str, Any]:
    """
