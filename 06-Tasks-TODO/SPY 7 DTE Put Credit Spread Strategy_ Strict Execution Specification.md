...
*   **Strike Selection (Deterministic Fallback Logic):** The system shall follow this strict hierarchy for selecting the short strike:
    ...
    2.  **Fallback 1 (Liquidity Check):** If the selected strike has a Bid-Ask Spread > 10% of the mid-price, move to the next nearest strike (closer to the money) that meets the liquidity threshold (Bid-Ask Spread ≤ 10%).
    3.  **Fallback 2 (Volatility/Z-score):** If no strike meets the liquidity threshold within +/- 2 strikes of the initial 10-delta target, calculate the theoretical strike using: `Strike = Current Price * (1 - (7-day IV / sqrt(365)) * 1.28)`. Select the nearest available strike to this calculated value.
