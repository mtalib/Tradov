# Trading Decision Workflow (Full) — v37
...
Source note: this saved `v37` snapshot records a separate overlay audit after v36. That audit confirmed the optional ODTE Pivot third-slot policy remains planned-only: live code still enforces the baseline two-slot model, the documented overlay flag is unsupported, and overlay-specific tests remain scaffolds. D31 now logs an explicit warning when `SPYDER_ENABLE_ODTE_PIVOT_OVERLAY_SLOT` is set so operators are not left assuming a third-slot path exists. This snapshot also adds a concrete implementation plan anchored to the current D31/E01/test boundaries so future overlay work can land against the real code path rather than the older design-only checklist.
...
- **No live third-slot overlay exception exists in current code.** The ODTE Pivot overlay contract in Sections 6.2/9.3 remains planned design only; D31 currently enforces the baseline two-slot cap unconditionally.
...
- The optional ODTE Pivot third-slot concept remains planned only. In the current code, D31 still enforces the baseline two-slot model even if an operator sets the documented overlay flag.
...
>
> **Concurrency limit:** Max 2 strategies open (one long-term/swing + one intraday/0DTE). No live third-slot overlay admission exists in the current code path; the ODTE Pivot overlay design remains planned-only.
...
### 6.2) Planned ODTE Pivot Overlay Slot (Not Yet Implemented)

As-implemented status (v37 audit): no live D31 or E01 code currently honors this contract. D31 still enforces `MAX_CONCURRENT_STRATEGIES = 2` and `MAX_ACTIVE_HORIZON_BUCKETS = 2`, `SPYDER_ENABLE_ODTE_PIVOT_OVERLAY_SLOT` is not wired into configuration or admission logic, and the overlay-specific tests are still skipped scaffolds. The items below are retained as design intent only.
...
- Planned-only overlay state: up to 3 active strategies only when the 3rd is an ODTE Pivot overlay entry that passes Section 6.2 gates. This state is not currently implemented.
...
9. Enforce baseline max 2 concurrent strategies (one long-term/swing + one intraday/0DTE, enforced per horizon bucket). The ODTE Pivot third-slot design in Section 6.2 is not currently active.
...
- Must enforce baseline max concurrent strategies = 2 (one per horizon bucket) in the current code. No implemented 3rd-slot ODTE Pivot overlay exception exists today.
...
- ODTE Pivot third-slot logic is not implemented. If `SPYDER_ENABLE_ODTE_PIVOT_OVERLAY_SLOT` is set, the current code must be treated as baseline two-slot only; v37 adds an explicit D31 warning for this unsupported flag.
...
Verification boundary (v37): grep/read audit of D30, D31, E01, configuration, and focused tests found no live flag wiring, no explicit E01 overlay-verdict API, no D31 overlay-admission fields, and only skipped scaffold tests for the overlay workstreams below.
...
### 9.4) Implementation Status and Sign-Off Matrix (v34-v37)
...
>
> **Current audit status (v37):** Every overlay workstream below remains **Planned**. The current codebase still enforces the baseline two-slot admission model; the overlay scaffolds are placeholders, not evidence of implementation.

| Workstream | Owner Module(s) | Status | Sign-Off Criteria | Target Test File(s) |
|---|---|---|---|---|
| D31 overlay admission ordering | D31 | Planned | Third slot admitted only for ODTE Pivot after baseline cap is reached and all overlay gates pass | `Spyder/SpyderT_Testing/SpyderT141_D31_EntryTrustGate.py` (extend), `Spyder/SpyderT_Testing/SpyderT219_D31_ODTEOverlaySlot.py` (scaffold exists — implement) |
| D31 normalization and duplicate overlay guard | D31 | Planned | D34 aliases normalize to `pivot_mean_reversion`; duplicate overlay entries blocked for same symbol during active/reserved state | `Spyder/SpyderT_Testing/SpyderT141_D31_EntryTrustGate.py` (extend), `Spyder/SpyderT_Testing/SpyderT193_D31_DispatchResultHardening.py` (regression) |
| E01 overlay pre-trade verdict API | E01 | Planned | Explicit allow/deny verdict with reason code and limits snapshot; fail-closed on missing inputs | `Spyder/SpyderT_Testing/SpyderT58_RiskManagementTests.py` (extend), `Spyder/SpyderT_Testing/SpyderT230_E01_OverlayPretradeVerdict.py` (rename from T220_E01 scaffold) |
| Overlay risk thresholds | E01 | Planned | Deny when projected overlay Greeks breach limits or daily risk used fraction exceeds 0.60 | `Spyder/SpyderT_Testing/SpyderT58_RiskManagementTests.py` (extend), `Spyder/SpyderT_Testing/SpyderT112_ESeries.py` (extend) |
| Execution-quality and event-window gates | D31 + E01 + F09 | Planned | Overlay blocked when spread/slippage gate fails or event window is blocked; no false allows on stale inputs | `Spyder/SpyderT_Testing/SpyderT141_D31_EntryTrustGate.py` (extend), `Spyder/SpyderT_Testing/SpyderT46_RiskManager_Test.py` (extend), `Spyder/SpyderT_Testing/SpyderT231_ODTE_ExecutionQualityEventWindow.py` (new — T221 is occupied) |
...
| End-to-end paper verification | R12 + D31 + E01 + B02/B40 | Planned | Paper session shows stable 2-slot baseline, controlled 3rd overlay admission, and correct block behavior | `Spyder/SpyderT_Testing/SpyderT55_PaperTradingHarness_Test.py` (extend), `Spyder/SpyderT_Testing/SpyderT116_RSeries.py` (extend) |
...
### 9.5) Concrete Implementation Plan Anchored to Current Code (v37)

This section converts the design-only checklist above into the lowest-churn implementation plan that matches the live codebase as of the v37 audit.
...
1. `add_strategy()` remains the owning strategy-allocation admission surface.
  ...
  - Insert the overlay exception exactly where D31 currently rejects `current_active >= self.max_concurrent_strategies` or a new horizon-bucket breach.
  - The exception must fire only when all of these are true:
    - `SPYDER_ENABLE_ODTE_PIVOT_OVERLAY_SLOT=true`
...
3. `_on_strategy_signal()` is the owning pre-dispatch risk handoff surface.
  ...
  - Only when the signal is requesting the overlay exception and the baseline cap is already full, build overlay metadata and call a new E01 overlay-verdict method before dispatch.
...
1. Keep `SpyderT143_D31_AdmissionGuardrails.py` as the current baseline regression until the feature is implemented.
  - Today it proves the flag is unsupported and baseline two-slot limits remain in force.
  - When overlay goes live, replace that assertion with the new explicit feature-on admission contract rather than dropping the regression entirely.
...
1. Add E00 typed overlay verdict objects and E01 flat risk-limit keys.
2. Implement E01 `validate_overlay_slot()` and its focused tests.
3. Wire D31 metadata extraction and overlay admission/disable hooks.
4. Extend decision-audit schema and telemetry tests.
5. Run a clean paper-session replay only after all focused tests are green and the current unsupported-flag warning is replaced with live feature logging.
...
The ODTE Pivot third-slot overlay described in v34-v36 is not implemented in the current codebase.
...
- `SPYDER_ENABLE_ODTE_PIVOT_OVERLAY_SLOT` was documented but not consumed by configuration or admission logic.
- The overlay-focused tests (`SpyderT219_D31_ODTEOverlaySlot.py`, `SpyderT220_E01_OverlayPretradeVerdict.py`, `SpyderT222_D31_OverlayAuditSchema.py`) are still skipped scaffolds.
...
No attempt was made to implement the planned overlay contract because that would require new D31 admission fields, an explicit E01 overlay-verdict API, audit schema work, and end-to-end paper verification that are all still marked Planned in Section 9.4.
