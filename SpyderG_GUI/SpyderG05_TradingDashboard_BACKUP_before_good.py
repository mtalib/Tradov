...
# Matplotlib for charting (fallback)
import matplotlib
...
try:
    ...
    print(f"⚠️ Plotly charts not available, using matplotlib fallback: {e}")
    USE_PLOTLY = False
import pandas as pd
...
class ThreadSafeMarketDataWorker(QObject):
    ...
    def _cleanup_real_data_connector(self):
        ...
        # DO NOT deleteLater on singleton - it must persist for reconnection
        self.real_data_connector = None
    ...
    def _start_real_data_connector(self):
        ...
        try:
            ...
            # DO NOT setParent - this is a singleton that must persist
            # Validate C++ object is still alive
            try:
                _ = connector.objectName()
            except RuntimeError:
                self.log_message.emit(
                    "⚠️ IBDataConnector C++ object deleted - recreating"
                )
...
class SpyderTradingDashboard(QMainWindow):
    ...
    def update_chart(self):
        ...
        if USE_PLOTLY and hasattr(self.chart_widget, "update_chart_data"):
            # Plotly chart - it handles updates internally via real-time data streams
            # The PlotlyChartWidget automatically updates when market data changes
            return
