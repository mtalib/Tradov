...
try:
    ...
    # Create placeholder classes when connection manager is not available
...
class IBTradingInterface:
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            self.logger.info(
                f"Connecting to IB API on port {port} with client ID {client_id}"
            )
        ...
        try:
            ...
            if self._event_loop and not self._event_loop.is_closed():
                future = asyncio.run_coroutine_threadsafe(
                    self._disconnect(), self._event_loop
                )
    ...
    def get_market_snapshot(self) -> MarketSnapshot:
        ...
        if self._spy_ticker:
            self._market_snapshot.spy_price = (
                float(self._spy_ticker.last) if self._spy_ticker.last else None
            )
            self._market_snapshot.spy_bid = (
                float(self._spy_ticker.bid) if self._spy_ticker.bid else None
            )
            self._market_snapshot.spy_ask = (
                float(self._spy_ticker.ask) if self._spy_ticker.ask else None
            )
            self._market_snapshot.spy_volume = (
                int(self._spy_ticker.volume) if self._spy_ticker.volume else None
            )
    ...
    def get_options_chain(
        self, expiry_date: str, strikes_around_spot: int = 10
    ) -> List[SpyderOptionContract]:
        """
    ...
    async def _get_options_chain_async(
        self, expiry_date: str, strikes_around_spot: int = 10
    ) -> List[SpyderOptionContract]:
        """
        ...
        try:
            ...
            center_idx = min(
                range(len(all_strikes)), key=lambda i: abs(all_strikes[i] - spy_price)
            )
            ...
            await self.event_emitter.emit(
                TradingEvent.OPTIONS_CHAIN_RECEIVED,
                {
                    "expiry": expiry_date,
    ...
    def subscribe_option_data(self, option_contract: SpyderOptionContract) -> bool:
        ...
        try:
            future = asyncio.run_coroutine_threadsafe(
                self._subscribe_option_data_async(option_contract), self._event_loop
            )
    ...
    async def _subscribe_option_data_async(
        self, option_contract: SpyderOptionContract
    ) -> bool:
        ...
        try:
            ...
            if not qualified_contracts:
                self.logger.error(
                    f"Could not qualify contract: {option_contract.symbol} {option_contract.expiry} {option_contract.strike}{option_contract.option_type.value}"
                )
                return False
    ...
    def place_option_order(
        ...
        try:
            future = asyncio.run_coroutine_threadsafe(
                self._place_option_order_async(
                    option_contract, action, quantity, order_type, limit_price
                ),
                self._event_loop,
            )
            return future.result(timeout=10)
    ...
    async def _place_option_order_async(
        self,
        option_contract: SpyderOptionContract,
        action: OrderAction,
        quantity: int,
        ...
        try:
            ...
            self.logger.info(
                f"📤 Placed {action.value} order for {quantity} {option_contract.symbol} {option_contract.expiry} {option_contract.strike}{option_contract.option_type.value}"
            )
    ...
    def cancel_order(self, order_id: int) -> bool:
        ...
        try:
            future = asyncio.run_coroutine_threadsafe(
                self._cancel_order_async(order_id), self._event_loop
            )
            return future.result(timeout=10)
        except Exception as e:
    ...
    async def _on_spy_ticker_update(self, ticker):
        ...
        try:
            await self.event_emitter.emit(
                TradingEvent.MARKET_DATA_RECEIVED,
                {
                    "symbol": "SPY",
        ...
        try:
            ...
            if order_id in self._active_orders:
                ...
                spyder_order.status = (
                    ...
                    if hasattr(OrderStatus, trade.orderStatus.status)
                ...
                if trade.orderStatus.status == "Filled":
                    ...
                    asyncio.create_task(
                        self.event_emitter.emit(TradingEvent.ORDER_FILLED, spyder_order)
                    )
                elif trade.orderStatus.status == "Cancelled":
                    asyncio.create_task(
                        self.event_emitter.emit(
                            TradingEvent.ORDER_CANCELLED, spyder_order
                        )
                    )
        except Exception as e:
    ...
    async def _on_position(self, position):
        ...
        try:
            if position.contract.secType == "OPT" and position.contract.symbol == "SPY":
                ...
                await self.event_emitter.emit(
                    TradingEvent.POSITION_UPDATED, spyder_position
                )
    ...
    async def _on_portfolio(self, item):
        ...
        try:
            await self.event_emitter.emit(
                TradingEvent.PORTFOLIO_UPDATED,
                {
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.info(
            f"📋 Test option contract: {test_option.symbol} {test_option.expiry} {test_option.strike}{test_option.option_type.value}"
        )
        ...
        logger.info(
            "💡 Connect to IBConnectionManager to start live trading with ib_async"
        )
