...
class GreeksAlert:
    ...
    limit_type: GreeksLimitType
    action_required: Optional[HedgingAction] = None
    timestamp: datetime = field(default_factory=datetime.now)
...
class GreeksCalculationEngine:
    ...
    def invalidate_cache(self, position_id: Optional[str] = None):
        """Invalidate cache entries."""
        if position_id:
            # Invalidate specific position
            keys_to_remove = [
                k for k in self.position_cache.keys()
...
class LEANGreeksValidator:
    ...
    def validate_greeks(
        ...
        limits: Optional[GreeksLimit] = None,
...
class GreeksAggregator:
    ...
    def get_position_greeks(self, position_id: str) -> Optional[PositionGreeks]:
...
if __name__ == "__main__":
    ...
    print(f"Total Rho: {portfolio_greeks.total_rho:.2f}")
    
    # Get hedging requirements
    print("\n=== Hedging Requirements ===")
    hedging = aggregator.get_hedging_requirements()
    for hedge_type, amount in hedging.items():
        print(f"{hedge_type}: {amount:.2f}")
    ...
    print(f"First calculation: {first_time:.4f}s")
    
    # Second calculation (cache hit)
    start = time.time()
    aggregator.update_portfolio(positions, market_data)
    second_time = time.time() - start
    print(f"Second calculation (cached): {second_time:.4f}s")
