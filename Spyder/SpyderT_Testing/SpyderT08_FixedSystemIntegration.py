...
def test_fixed_system_integration():
    ...
    try:
        ...
        from SpyderX15_StrategyGeneratorAgent import \
    ...
    try:
        ...
        from SpyderU20_InstitutionalLibraries import (
    ...
    try:
        ...
        if evolved_strategy_file.exists():
            from SpyderD16_EvolvedCreditSpread import \
...
def calculate_institutional_metrics(returns):
    ...
    downside_std = (
        np.std(downside_returns) * np.sqrt(252) if len(downside_returns) > 0 else volatility
    )
