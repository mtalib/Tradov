...
from SpyderC_MarketData.SpyderC03_OptionChain import get_option_chain
...
class MicrostructureAnalyzer:
    ...
    def update_order_book(self, symbol: str, bids: List[Tuple[float, int]], 
                         asks: List[Tuple[float, int]]) -> OrderBookSnapshot:
        """
    ...
    def detect_order_book_imbalance(self, symbol: str) -> Optional[MicrostructureEvent]:
        ...
        try:
            ...
            self.error_handler.handle_error(e, {
                'method': 'detect_order_book_imbalance',
                'symbol': symbol
            })
            return None
    ...
    # ==========================================================================
    def process_trade(self, symbol: str, price: float, size: int, 
                     side: str, exchange: str = "") -> Optional[SweepOrder]:
        """
        ...
        try:
            ...
            if size >= LARGE_ORDER_SIZE:
                ...
                recent_trades = [
                    t for t in self.sweep_candidates[symbol]
    ...
    def _is_sweep_pattern(self, symbol: str, trades: List[Dict]) -> bool:
        ...
        try:
            ...
            self.error_handler.handle_error(e, {
                'method': '_is_sweep_pattern',
                'symbol': symbol
            })
            return False
    
    def _create_sweep_order(self, symbol: str, trades: List[Dict]) -> Optional[SweepOrder]:
        ...
        try:
            ...
            exchanges = list(set(t.get('exchange', '') for t in trades if t.get('exchange')))
            time_span = max(t['timestamp'] for t in trades) - min(t['timestamp'] for t in trades)
    ...
    def detect_hidden_liquidity(self, symbol: str) -> Optional[MicrostructureEvent]:
        ...
        try:
            ...
            self.error_handler.handle_error(e, {
                'method': 'detect_hidden_liquidity',
                'symbol': symbol
            })
            return None
    ...
    # ==========================================================================
    def process_quote(self, symbol: str, bid: float, ask: float, 
                     bid_size: int, ask_size: int) -> Optional[MicrostructureEvent]:
        """
        ...
        try:
            ...
            recent_quotes = [
                q for q in self.quote_history[symbol]
            ...
            if quote_rate > QUOTE_STUFFING_THRESHOLD:
                ...
                for i in range(1, len(recent_quotes)):
                    if (recent_quotes[i]['bid'] != recent_quotes[i-1]['bid'] or
                        recent_quotes[i]['ask'] != recent_quotes[i-1]['ask']):
                        price_changes += 1
                    if (recent_quotes[i]['bid_size'] != recent_quotes[i-1]['bid_size'] or
                        recent_quotes[i]['ask_size'] != recent_quotes[i-1]['ask_size']):
                        size_changes += 1
        ...
        try:
            ...
            if symbol not in self.flow_metrics:
                self.flow_metrics[symbol] = OrderFlowMetrics(
                    ...
                    volume_imbalance=0, trade_imbalance=0,
                    large_buy_orders=0, large_sell_orders=0
                )
            ...
            if total_trades > 0:
                metrics.trade_imbalance = (metrics.buy_trades - metrics.sell_trades) / total_trades
            
        except Exception as e:
            self.error_handler.handle_error(e, {
                'method': '_update_flow_metrics',
                'symbol': symbol
            })
    ...
    def analyze_order_flow(self, symbol: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if len(trade_times) > 1:
                ...
                trade_clustering = statistics.stdev(inter_trade_times) if len(inter_trade_times) > 1 else 0
            else:
            ...
            institutional_ratio = len(large_trades) / len(recent_trades) if recent_trades else 0
    ...
    def _calculate_flow_toxicity(self, symbol: str, trades: List[Dict]) -> float:
        ...
        try:
            ...
            self.error_handler.handle_error(e, {
                'method': '_calculate_flow_toxicity',
                'symbol': symbol
            })
            return 0.0
            ...
            if flow_analysis:
                ...
                if metrics:
                    ...
                    if abs(metrics.volume_imbalance) > 0.7:
                        signal_type = (MicrostructureSignal.IMBALANCE_BUY 
                    ...
                    if flow_analysis.get('toxicity', 0) > 0.6:
                        events.append(MicrostructureEvent(
                            ...
                            details={
                                ...
                                'institutional_ratio': flow_analysis.get('institutional_ratio', 0)
                            },
    ...
    def _handle_order_book_update(self, event: Event):
        ...
        try:
            ...
            self.error_handler.handle_error(e, {
                'method': '_handle_order_book_update',
                'event': event
            })
    ...
    def _handle_quote_update(self, event: Event):
        ...
        try:
            ...
            if all([symbol, bid is not None, ask is not None, 
                   bid_size is not None, ask_size is not None]):
                self.process_quote(symbol, bid, ask, bid_size, ask_size)
                
        except Exception as e:
            self.error_handler.handle_error(e, {
                'method': '_handle_quote_update',
                'event': event
            })
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        try:
            stats = {
                ...
                'total_events_processed': sum(len(h) for h in self.quote_history.values()),
