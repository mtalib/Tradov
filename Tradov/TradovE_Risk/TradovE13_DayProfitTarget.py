...
BROKER NOTE:
    ...
    Order execution is via TradovB40_TradierClient (Tradier REST API).
    Market data is sourced from TradovB40_TradierClient (Tradier).
...
Key Features:
    ...
    - Integration with TradovD31_StrategyOrchestrator for capital coordination
...
try:
    ...
    from Tradov.TradovU_Utilities.TradovU02_ErrorHandler import (
        TradovErrorHandler,
        TradingError,  # noqa: F401
    )
    from Tradov.TradovU_Utilities.TradovU15_PerformanceMetrics import PerformanceMetrics  # noqa: F401
    from Tradov.TradovU_Utilities.TradovU10_TradingCalendar import TradingCalendar

    # Broker integration (TradovB01/B20 removed — deprecated IB modules)
    from TradovB_Broker.TradovB02_OrderManager import OrderManager  # noqa: F401

    # Strategy coordination
    from TradovD_Strategies.TradovD31_StrategyOrchestrator import StrategyOrchestrator

    # Event management
    from TradovA_Core.TradovA05_EventManager import EventManager, Event, EventType, get_event_manager  # noqa: E501, F401

    TRADOV_MODULES_AVAILABLE = True
except ImportError as e:
    logging.debug("Optional Tradov modules not available for DayProfitTarget: %s", e)
...
try:
    from Tradov.TradovU_Utilities.TradovU48_Money import Money as _Money, ZERO as _MONEY_ZERO
except ImportError:
    try:
        from Tradov.TradovU_Utilities.TradovU48_Money import (
            Money as _Money,
            ZERO as _MONEY_ZERO,
        )
    except ImportError:
...
class AlgorithmicSlicingManager:
    ...
    def _get_spy_options_contract(self) -> Contract:  # noqa: F821
        """Get TRAD options contract for current expiration"""
...
class DayProfitTargetEngine:
    ...
    def __init__(
        ...
        # Legacy broker client removed — order execution via TradovB40_TradierClient
    ...
    def _update_progress(self):
        ...
        try:
            ...
            if self.execution_start_time:
                elapsed = datetime.now(UTC) - self.execution_start_time
            ...
            self.current_progress.last_updated = datetime.now(UTC)
    ...
    def _emit_profit_target_emergency(self) -> None:
        ...
        """
        try:
            if TRADOV_MODULES_AVAILABLE and self.event_manager is not None:
    ...
    def _check_account_balance_risk(self) -> RiskAlert | None:
        ...
        try:
            ...
            if self.current_account_balance < min_balance:
                return RiskAlert(
                    alert_id=f"acct_balance_{int(datetime.now(UTC).timestamp())}",
    ...
    def _check_daily_loss_limit(self) -> RiskAlert | None:
        ...
        try:
            ...
            if current_profit < -abs(self.current_config.max_risk_amount):
                return RiskAlert(
                    alert_id=f"daily_loss_{int(datetime.now(UTC).timestamp())}",
...
class DayProfitTargetWidget(QWidget):
    ...
    def _start_simulated_progress(self):
        ...
        self.simulation_start_time = datetime.now(UTC)
    ...
    def _update_simulated_progress(self):
        ...
        try:
            ...
            elapsed = datetime.now(UTC) - self.simulation_start_time
...
def main():
    ...
    logging.info("🎯 TRADOV E13 - Day Profit Target Engine")
    logging.info("=" * 60)
