...
from SpyderB_Broker.SpyderB02_OrderManager import (OrderAction, OrderManager,
...
from SpyderB_Broker.SpyderB05_ConnectionManager import (ConnectionConfig,
                                                        ConnectionManager)
from SpyderB_Broker.SpyderB06_ContractBuilder import ContractBuilder
from SpyderB_Broker.SpyderB10_IBDataTypes import (IBContract, IBOrder,
                                                  OrderStatus, SecurityType,
                                                  create_option_contract,
                                                  create_stock_contract)
from SpyderB_Broker.SpyderB12_GatewayAutomation import (GatewayAutomation,
                                                        GatewayConfig,
                                                        TradingMode)
...
def setup_logging(level=logging.INFO):
    ...
    logging.basicConfig(
        ...
        handlers=[logging.StreamHandler(), logging.FileHandler("spyder_broker_tests.log")],
    )
...
class SpyderBrokerTestBase(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.ib_config = IBConfig(
            host=TEST_CONFIG["host"], port=TEST_CONFIG["port"], client_id=TEST_CONFIG["client_id"]
        )
        ...
        if TEST_CONFIG["use_mock"]:
            ...
            cls.spyder_client.create_stock_contract = Mock(
                side_effect=lambda symbol: Mock(
                    symbol=symbol, secType="STK", exchange="SMART", currency="USD"
                )
            )
            cls.spyder_client.create_option_contract = Mock(
                side_effect=lambda symbol, exp, strike, right: Mock(
                    symbol=symbol,
...
class TestMarketDataAvailability(SpyderBrokerTestBase):
    ...
    def setUp(self):
        ...
        super().setUp()
        self.test_results = {"successful": [], "failed": [], "errors": []}
        ...
        try:
            ...
            if not contract:
                self.test_results["errors"].append(
                    {"symbol": symbol, "category": category, "error": "Failed to create contract"}
                )
            ...
            if req_id > 0:
                ...
                if ticker and (ticker.bid or ticker.ask or ticker.last):
                    self.test_results["successful"].append(
                        {
                            "symbol": symbol,
                            "category": category,
                            "data": {"bid": ticker.bid, "ask": ticker.ask, "last": ticker.last},
                        }
                    )
                    logger.info(
                        f"  ✅ {symbol}: Bid={
                    ...
                    self.test_results["failed"].append(
                        {"symbol": symbol, "category": category, "reason": "No data received"}
                    )
                ...
                self.test_results["failed"].append(
                    {"symbol": symbol, "category": category, "reason": "Subscription failed"}
                )
                logger.warning(f"  ❌ {symbol}: Subscription failed")

        except Exception as e:
            self.test_results["errors"].append(
                {"symbol": symbol, "category": category, "error": str(e)}
            )
            logger.error(f"  ❌ {symbol}: Error - {e}")

    def _test_option_contract(
        self, symbol: str, expiration: datetime, strike: float, right: str
    ) -> bool:
    ...
    def tearDown(self):
        ...
        if (
            self.test_results["successful"]
            or self.test_results["failed"]
            or self.test_results["errors"]
        ):
    ...
    def _generate_availability_report(self):
        ...
        total_tested = (
            len(self.test_results["successful"])
...
class TestSpyderClientIntegration(SpyderBrokerTestBase):
    ...
    def test_contract_creation(self):
        ...
        # Option contract
        spy_option = self.spyder_client.create_option_contract("SPY", "20250620", 450.0, "C")
...
Examples:
    ...
    parser.add_argument(
        "--real", action="store_true", help="Use real IB connection instead of mocks"
    )
    ...
    # Run tests
    success = run_tests(specific_test=args.test, use_real_connection=args.real)
