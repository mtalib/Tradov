...
Module Description:
    ...
    Q14 (TradovQ14_MainLauncher) becomes a thin argument-parser + signal
...
class SessionSupervisor:
    ...
    def __init__(
        ...
        self._startup_profile_enabled: bool = self._is_truthy_env(
            os.getenv("TRADOV_SESSION_SUPERVISOR_PROFILE_STARTUP")
        )
    ...
    def stop(self, flatten: bool = False) -> None:
        ...
        try:
            from Tradov.TradovP_PortfolioMgmt import (
    ...
    def _start_freshness_monitor(self) -> bool:
        try:
            from Tradov.TradovE_Risk.TradovE24_DataFreshnessMonitor import create_freshness_monitor
    ...
    def _resolve_freshness_monitor_thresholds(self) -> tuple[float, float]:
        ...
        rth_threshold_raw = os.environ.get("TRADOV_DATA_FRESHNESS_RTH_THRESHOLD_S")
        try:
            rth_threshold_s = float(rth_threshold_raw) if rth_threshold_raw is not None else 3.0
        except (TypeError, ValueError):
            rth_threshold_s = 3.0

        ooh_threshold_raw = os.environ.get("TRADOV_DATA_FRESHNESS_OOH_THRESHOLD_S")
        try:
        ...
        if feed_status == "degraded" and quote_poll_interval_s is not None and quote_poll_interval_s > 0.0:
            ...
            if rth_threshold_raw is None:
                ...
                self.logger.warning(
                    "Explicit TRADOV_DATA_FRESHNESS_RTH_THRESHOLD_S=%.1fs is below degraded REST quote fallback threshold %.1fs; keeping explicit override.",
                    rth_threshold_s,
    ...
    def _start_paper_broker(self) -> bool:
        ...
        """
        try:
            from Tradov.TradovR_Runtime.TradovR15_PaperBroker import create_paper_broker
    ...
    def _start_fill_reconciler(self) -> None:
        try:
            from Tradov.TradovR_Runtime.TradovR13_FillReconciler import FillReconciler
    ...
    def _start_live_engine(self) -> bool:
        try:
            from Tradov.TradovR_Runtime.TradovR04_LiveEngine import create_live_engine
            account_id = (
    ...
    def _start_orchestrator(self) -> bool:
        try:
            ...
            if self.mode != "paper":
                try:
                    from Tradov.TradovL_ML.TradovL09_UnifiedRegimeEngine import (
            ...
            if self.mode == "paper":
                ...
                try:
                    from Tradov.TradovB_Broker.TradovB02_OrderManager import OrderManager
    ...
    def _start_liveness_monitor(self) -> None:
        ...
        try:
            from Tradov.TradovR_Runtime.TradovR05_LivenessMonitor import (
    ...
    def _start_exit_monitor(self) -> None:
        try:
            from Tradov.TradovR_Runtime.TradovR14_ExitMonitor import create_exit_monitor
            from Tradov.TradovP_PortfolioMgmt import (
    ...
    def _consume_paper_start_authorization(self) -> tuple[bool, str | None]:
        ...
        if self.dry_run:
            return True, "dry_run"
        if self._is_truthy_env(os.getenv("TRADOV_ALLOW_PAPER_SESSION_START")):
            return True, "env_override"

        authorized = bool(self._tradov_paper_start_authorized)
        self._tradov_paper_start_authorized = False
        if authorized:
    ...
    def _validate_live_only_tradier_policy(self) -> tuple[bool, str]:
        ...
        if self._is_truthy_env(os.getenv("TRADOV_ALLOW_SANDBOX_MARKET_DATA")):
            violations.append("TRADOV_ALLOW_SANDBOX_MARKET_DATA=true")
...
def authorize_paper_session_start(supervisor: "SessionSupervisor | None") -> None:
    ...
    if supervisor is None:
        return
    supervisor._tradov_paper_start_authorized = True
