...
Purpose: PCA proxy custom metrics for SPY regime monitoring

Author: Tradov Dev
Year Created: 2026
Last Updated: 2026-05-10 Time: 00:00:00

Description:
    Provides two custom-metric style PCA signals:
    - PCA-PROXY: a real sector-proxy eigenfactor signal built from rolling
      daily returns of SPY sector ETFs.
        - PCA-IV: a live eigenfactor built from persisted SPY IV-surface features,
...
class PCASignalEngine:
    ...
    def get_iv_placeholder_snapshot(self) -> PCAMetricSnapshot:
        ...
        if stored_snapshots > 0:
            message = (
                "History seeding is active for the future SPY IV-surface PCA factor model."
            )
        else:
    ...
    def record_iv_surface_snapshot(self, snapshot: dict[str, Any]) -> dict[str, Any]:
        """Persist a compact SPY surface snapshot for future PCA-IV training."""
    ...
    def _bootstrap_iv_surface_history_from_scalar_history(self) -> None:
        """Bootstrap PCA-IV history from scalar SPY IV history when empty."""
        if self._iv_surface_history_count > 0:
            return
    ...
    def _build_scalar_iv_bootstrap_rows(self) -> list[dict[str, Any]]:
        """Convert scalar SPY IV history into approximate PCA-IV feature rows."""
        ...
        feature_frame = pd.DataFrame(
            {
                ...
                "feature_curve_position_group": change_accel,
        ...
        for timestamp, row in feature_frame.iterrows():
            ...
            feature_curve_position_group = float(row["feature_curve_position_group"])
            ...
            bootstrap_rows.append(
                {
                    ...
                    "feature_curve_position_group": feature_curve_position_group,
    ...
    _FEATURE_COLUMN_ALIASES: dict[str, str] = {
        "feature_curve_butterfly": "feature_curve_position_group",
    }
        ...
        with self._iv_surface_history_path.open("r", encoding="utf-8") as handle:
            for line in handle:
                ...
                for column in PCA_IV_FEATURE_COLUMNS:
                    ...
                    if value is None:
                        for alias, canonical in self._FEATURE_COLUMN_ALIASES.items():
                            if canonical == column:
                                value = payload.get(alias)
                                break
                    row[column] = self._coerce_float(value)
                rows_by_ts[snapshot_ts] = row
    ...
    def _normalize_iv_surface_snapshot(self, snapshot: dict[str, Any]) -> dict[str, Any]:
        ...
        feature_curve_position_group = None
        ...
        if (
            ...
            feature_curve_position_group = atm_iv_0dte - (2.0 * atm_iv_7dte) + atm_iv_30dte
