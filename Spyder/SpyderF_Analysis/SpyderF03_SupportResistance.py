...
class SupportResistanceAnalyzer:
    ...
    def analyze(self, data: pd.DataFrame, current_volatility: Optional[float] = None) -> SupportResistanceAnalysis:
        """
    ...
    def get_cluster_threshold(self, volatility: Optional[float] = None) -> float:
        """
...
if __name__ == "__main__":
    ...
    for volatility in [0.05, 0.15, 0.25, 0.35]:
        ...
        print(f"Cluster threshold: {threshold:.4f}")
        ...
        if analysis.support_levels:
            for level in analysis.support_levels[:3]:
                print(f"  ${level.price:.2f} - {level.strength_category} "
        ...
        print(f"Key levels: {[f'${p:.2f}' for p in analysis.key_levels]}")
        print()
    
    # Test level updates
    print("=== Level Touch Updates ===")
    current_price = analysis.current_price
    
    # Simulate price touching a level
    if analysis.support_levels:
        test_level = analysis.support_levels[0]
        print(f"Testing touches near ${test_level.price:.2f}")
        print(f"Before: {test_level.touches} touches, {test_level.strength_category}")
        
        # Update touches
        for _ in range(5):
            analyzer.update_level_touches(test_level.price + 0.05, [test_level])
        
        print(f"After: {test_level.touches} touches, {test_level.strength_category}")
