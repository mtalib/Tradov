...
Module: TradovB40_TradierClient.py
...
from Tradov.TradovU_Utilities.TradovU40_RateLimiter import rate_limit
from Tradov.TradovU_Utilities.TradovU41_CircuitBreaker import tradier_breaker
...
class OptionLeg:
    ...
    Attributes:
        option_symbol: OCC-format option symbol (e.g., TRAD260220C00550000).
...
class RateLimitInfo:
    ...
    surfaces that data so Tradov's rate limiter (TradovU40) can adapt
...
def build_option_symbol(
    ...
    Example:
        >>> build_option_symbol("TRAD", "2026-02-20", "C", 550.00)
        'TRAD260220C00550000'
        >>> build_option_symbol("TRAD", "260220", "P", 545.50)
        'TRAD260220P00545500'

    Args:
        underlying: Underlying ticker (e.g., "TRAD").
    ...
    # but do not force a TRAD-specific grid onto SPX/SPXW index options.
    etf_underlyings = {"TRAD", "QQQ", "IWM", "DIA"}
...
def parse_option_symbol(symbol: str) -> dict[str, Any]:
    ...
    Example:
        >>> parse_option_symbol("TRAD260220C00550000")
        {'underlying': 'TRAD', 'expiration': '260220', 'expiration_date': '2026-02-20',
         'option_type': 'C', 'strike': 550.0, 'symbol': 'TRAD260220C00550000'}
...
class TradierClient:
    ...
    def __init__(
        ...
        self._balances_stale_ttl_seconds: int = max(
            30,
            int(os.environ.get("TRADOV_BALANCE_STALE_TTL_SECONDS", "180")),
        )
    ...
    def close(self) -> None:
        ...
        try:
            ...
            logger.debug("TradierClient session close failed", exc_info=True)
    ...
    def get_rate_limit_info(self) -> RateLimitInfo | None:
        ...
        (TradovU40_RateLimiter, dashboard widgets) can query this to adapt
    ...
    def add_to_watchlist(self, watchlist_id: str, symbols: list[str]) -> dict[str, Any]:
        ...
        Example:
            >>> import client.add_to_watchlist("default", ["TRAD", "QQQ"])
        """
        if not symbols:
    ...
    def place_order(
        ...
        Args:
            symbol: Security symbol (e.g., "TRAD")
    ...
    def close_position(
        ...
        # OCC option symbols are 21 characters (e.g. TRAD260220C00550000);
    ...
    def preview_order(
        ...
        and margin impact.  No order is submitted.  Use this in TradovE_Risk
        ...
        Example:
            ...
            ...     "TRAD", OrderSide.BUY, 10, OrderType.LIMIT, limit_price=560.00
            ... )
    ...
    def preview_multileg_order(
        ...
        Example:
            >>> preview = client.preview_multileg_order("TRAD", legs, "credit", price=2.00)
        """
        if not legs:
    ...
    def get_quotes(self, symbols: list[str]) -> dict[str, Any]:
        ...
        Args:
            symbols: List of symbols (e.g., ["TRAD", "QQQ"])

        Returns:
            Quote data

        Example:
            >>> quotes = client.get_quotes(["TRAD"])
            >>> spy_quote = quotes["quotes"]["quote"]
            >>> print(f"TRAD: ${spy_quote['last']}")
        """
    ...
    def get_option_chain(self, symbol: str, expiration: str) -> dict[str, Any]:
        ...
        Args:
            symbol: Underlying symbol (e.g., "TRAD")
    ...
    def get_option_strikes(
        ...
        Example:
            >>> strikes = client.get_option_strikes("TRAD", "2026-03-20")
    ...
    def get_option_chain_with_greeks(
        ...
        Example:
            >>> greeks = client.get_option_chain_with_greeks("TRAD", "2026-03-20")
            >>> for g in greeks:
            ...     if abs(g.delta) > 0.3:
        ...
        # TRAD chains with greeks can be 180+ contracts — allow 60 s to avoid
    ...
    def find_options_by_delta(
        ...
        Example:
            ...
            ...     "TRAD", "2026-03-20", target_delta=0.16,
            ...     option_type="put"
            ... )
        """
    ...
    def get_market_calendar(
        ...
        Used by TradovA04_Scheduler and TradovU10_TradingCalendar.
    ...
    def get_time_sales(
        ...
        Example:
            >>> ts = client.get_time_sales("TRAD", interval="1min", start="2026-03-16 09:30")
        """
    ...
    def search_symbols(self, query: str, indexes: bool = False) -> dict[str, Any]:
        ...
        Args:
            query: Search string (e.g., "Apple" or "TRAD").
    ...
    def get_historical_quotes(
        ...
        Example:
            >>> history = client.get_historical_quotes("TRAD", interval="daily",
    ...
    def place_multileg_order(
        ...
        Example:
            ...
            >>> legs = [
            ...     OptionLeg(
            ...         option_symbol=build_option_symbol("TRAD", "2026-03-20", "P", 540),
            ...         side=OrderSide.SELL_TO_OPEN,
            ...         quantity=1,
            ...     ),
            ...     OptionLeg(
            ...         option_symbol=build_option_symbol("TRAD", "2026-03-20", "P", 535),
            ...         side=OrderSide.BUY_TO_OPEN,
            ...         quantity=1,
            ...     ),
            ... ]
            >>> order = client.place_multileg_order(
            ...     "TRAD", legs, order_type="credit", price=1.50
            ... )

            >>> # Iron Condor (4 legs)
            >>> legs = [
            ...     OptionLeg(build_option_symbol("TRAD", "2026-03-20", "P", 535),
            ...               OrderSide.BUY_TO_OPEN, 1),
            ...     OptionLeg(build_option_symbol("TRAD", "2026-03-20", "P", 540),
            ...               OrderSide.SELL_TO_OPEN, 1),
            ...     OptionLeg(build_option_symbol("TRAD", "2026-03-20", "C", 570),
            ...               OrderSide.SELL_TO_OPEN, 1),
            ...     OptionLeg(build_option_symbol("TRAD", "2026-03-20", "C", 575),
            ...               OrderSide.BUY_TO_OPEN, 1),
            ... ]
            >>> order = client.place_multileg_order(
            ...     "TRAD", legs, order_type="credit", price=2.00
            ... )
        """
        if not legs:
            raise ValueError("At least one OptionLeg is required")
        ...
        if not tag:
            import uuid
            tag = f"tradov-multileg-{uuid.uuid4().hex[:16]}"
        payload["tag"] = tag
    ...
    def place_oco_order(
        ...
        Example:
            >>> legs = [
            ...     OptionLeg("TRAD260320C00550000", OrderSide.SELL_TO_CLOSE, 1),
            ...     OptionLeg("TRAD260320C00560000", OrderSide.BUY_TO_CLOSE, 1),
            ... ]
            >>> order = client.place_oco_order("TRAD", legs)
        """
        if len(legs) < 2:
    ...
    def place_oto_order(
        ...
        Example:
            >>> legs = [
            ...     OptionLeg("TRAD260320P00500000", OrderSide.BUY_TO_OPEN, 1),
            ...     OptionLeg("TRAD260320P00495000", OrderSide.SELL_TO_CLOSE, 1),
            ... ]
            >>> order = client.place_oto_order("TRAD", legs)
        """
        if len(legs) < 2:
    ...
    def place_otoco_order(
        ...
        Example:
            >>> legs = [
            ...     OptionLeg("TRAD260320C00550000", OrderSide.BUY_TO_OPEN, 1),   # entry
            ...     OptionLeg("TRAD260320C00550000", OrderSide.SELL_TO_CLOSE, 1), # profit target
            ...     OptionLeg("TRAD260320C00550000", OrderSide.SELL_TO_CLOSE, 1), # stop
            ... ]
            >>> order = client.place_otoco_order("TRAD", legs)
        """
        if len(legs) < 3:
    ...
    async def place_order_async(
        ...
        Args:
            ...
            stop_price: Stop price (required for stop orders)
            order_class: Security class (equity, option)
            tag: Optional Tradier idempotency tag (e.g. ``"tradov-<order_id>"``).
    ...
    async def get_quotes_async(self, symbols: list[str]) -> dict[str, Any]:
        ...
        Example:
            >>> quotes = await client.get_quotes_async(["TRAD", "QQQ"])
        """
        loop = asyncio.get_running_loop()
...
class TradierMarketStream:
    ...
    *existing* connection — no reconnect required (see :meth:`update_symbols`).
    ...
    Example::
        ...
        client = create_tradier_client_from_env()
        stream = TradierMarketStream(client, symbols=["TRAD", "QQQ"])
    ...
    def update_symbols(self, symbols: list[str]) -> None:
        ...
        Example::
            ...
            stream.update_symbols(["TRAD", "QQQ", "IWM"])
        """
