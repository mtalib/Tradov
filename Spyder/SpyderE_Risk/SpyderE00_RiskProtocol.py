...
Series: SpyderE_Risk
Module: SpyderE00_RiskProtocol.py
...
Concrete satisfiers (no inheritance required):
    RiskManager (SpyderE01) already satisfies RiskManagerProtocol structurally.
    BaseStrategy (SpyderD01) already satisfies StrategyStateProvider structurally.
...
class BoundarySignalType(Enum):
    ...
    coupled to the SignalType enum defined inside SpyderD01_BaseStrategy.
    """
...
class RiskValidationRequest:
    ...
    Attributes:
        symbol:       Ticker symbol (e.g., "SPY" or an OCC options symbol).
...
class RiskManagerProtocol(Protocol):
    ...
    inheriting from it.  SpyderE01.RiskManager already satisfies it
    structurally.
...
class StrategyStateProvider(Protocol):
    ...
    other D-Series class.  SpyderD01.BaseStrategy already satisfies it
    structurally.
