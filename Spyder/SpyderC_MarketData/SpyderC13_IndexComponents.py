...
from Spyder.SpyderU_Utilities.SpyderU07_Constants import TimeFrame, MARKET_SECTORS
...
class IndexComponentAnalyzer:
    ...
    def _get_spy_performance(self, period: int) -> Optional[float]:
    ...
    def _check_ad_divergence(self, ad_slope: float, spy_trend: float) -> Optional[str]:
...
# MODULE FUNCTIONS
# ==============================================================================
def create_index_analyzer(config: Optional[Dict] = None) -> IndexComponentAnalyzer:
    """
...
if __name__ == "__main__":
    ...
    try:
        ...
        internals = analyzer.get_market_internals()
        print("\n📊 Market Internals:")
        print(f"  Breadth Signal: {internals.breadth.signal.name}")
        print(f"  Market Regime: {internals.market_regime.name}")
        print(f"  Rotation: {internals.rotation_signal.value}")
        print(f"  Strength Score: {internals.strength_score:.1f}/100")
        ...
        print(f"  % Above MA50: {breadth.percent_above_ma50:.1%}")
        print(f"  New Highs/Lows: {breadth.new_highs}/{breadth.new_lows}")
        
        # Get sector analysis
        print(f"\n🏭 Top Sectors:")
        for sector in internals.sectors[:3]:
            print(f"  {sector.sector}: {sector.performance_1d:+.2f}% (1d), "
                  f"{sector.performance_1m:+.2f}% (1m)")
            
        # Get A/D line analysis
        ad_analysis = analyzer.get_advance_decline_analysis()
        if 'current_value' in ad_analysis:
            print(f"\n📉 A/D Line Analysis:")
            print(f"  Current: {ad_analysis['current_value']:.0f}")
            print(f"  Trend: {ad_analysis['trend_direction']}")
            print(f"  Signal: {ad_analysis['signal']}")
            
        # Get rotation analysis
        rotation = analyzer.get_sector_rotation_analysis()
        print(f"\n🔄 Sector Rotation:")
        print(f"  Pattern: {rotation['rotation_pattern']}")
        print(f"  Risk Appetite: {rotation['risk_assessment']}")
        print(f"  Strongest: {', '.join(rotation['strongest_sectors'])}")
        
        time.sleep(5)
        
    finally:
        analyzer.cleanup()
        print("\n✅ Index component analyzer test completed")
