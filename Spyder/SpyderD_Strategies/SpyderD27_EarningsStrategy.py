...
class EarningsStrategyHandler:
    ...
    def get_upcoming_earnings(
        ...
        try:
            ...
            logger.error("Earnings fetch failed: %s", e, exc_info=True)
            return []
    ...
    def _fetch_earnings_calendar(
        ...
        try:
            ...
            logger.error("Earnings calendar fetch error: %s", e, exc_info=True)
            return []
    ...
    def calculate_expected_move(
        ...
        try:
            ...
            logger.error("Expected move calculation failed: %s", e, exc_info=True)
            return self._empty_expected_move(symbol)
    ...
    def _get_current_iv(self, symbol: str) -> float:
        ...
        if symbol in self._iv_cache:
            ...
            if (datetime.now(UTC) - cache_time).seconds < 300:
                return iv
        ...
        try:
            ...
            self._iv_cache[symbol] = (iv, datetime.now(UTC))
            return iv
    ...
    def _get_expiry_after_earnings(self, earnings_date: date) -> date:
        ...
        if self._data_provider is not None:
            try:
                ...
                logger.warning("_get_expiry_after_earnings: Tradier error: %s", e, exc_info=True)
