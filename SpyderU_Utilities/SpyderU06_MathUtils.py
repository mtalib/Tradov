...
def calculate_probability_touch(current_price: float, target_price: float,
                              volatility: float, days_to_expiry: float) -> float:
    """
...
def calculate_probability_profit(breakeven_price: float, current_price: float,
                               volatility: float, days_to_expiry: float,
                               is_bullish: bool = True) -> float:
    """
...
# ==============================================================================
def find_root(func: Callable[[float], float], a: float, b: float,
             tol: float = CONVERGENCE_THRESHOLD,
             max_iter: int = MAX_ITERATIONS) -> Optional[float]:
    """
...
def minimize_scalar(func: Callable[[float], float], bounds: Tuple[float, float],
                   tol: float = CONVERGENCE_THRESHOLD) -> Tuple[Optional[float], Optional[float]]:
    """
...
def calculate_position_size(account_value: float, risk_percent: float,
                          stop_loss_points: float, contract_multiplier: float = 1.0) -> int:
    """
...
def cubic_spline_interpolation(x_points: List[float], y_points: List[float],
                             x_new: Union[float, List[float]]) -> Union[float, List[float]]:
    """
...
# ARRAY OPERATIONS
# ==============================================================================
def rolling_window(data: List[float], window_size: int,
                  func: Callable[[List[float]], float]) -> List[float]:
    """
...
class MathUtils:
    ...
    @staticmethod
    def calculate_sharpe_ratio(returns: list, risk_free_rate: float = 0.02, periods_per_year: int = TRADING_DAYS_PER_YEAR) -> float:
        """Calculate Sharpe ratio."""
        return calculate_sharpe_ratio(returns, risk_free_rate, periods_per_year)
    
    @staticmethod
    def calculate_sortino_ratio(returns: list, risk_free_rate: float = 0.02, target_return: float = 0.0, periods_per_year: int = TRADING_DAYS_PER_YEAR) -> float:
        """Calculate Sortino ratio."""
