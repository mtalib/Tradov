...
Series: TradovL_ML
Module: TradovL01_MLPredictor.py
...
try:
    from Tradov.TradovC_MarketData.TradovC01_DataFeed import get_data_feed_manager
except ImportError:
    def get_data_feed_manager():
        return None  # type: ignore

try:
    from Tradov.TradovF_Analysis.TradovF01_Indicators import get_indicators
except ImportError:
    def get_indicators():
        return None  # type: ignore

try:
    from Tradov.TradovH_Storage.TradovH02_DatabaseManager import get_database_manager
except ImportError:
    def get_database_manager():
        return None  # type: ignore

FEATURE_DIR = Path.home() / ".tradov" / "models" / "features"
CHECKPOINT_DIR = Path.home() / ".tradov" / "models" / "checkpoints"
...
class MLPredictor:
    ...
    def _create_directories(self) -> None:
        ...
        directories = [
            Path.home() / ".tradov" / "models",
            FEATURE_DIR,
            CHECKPOINT_DIR,
            Path.home() / ".tradov" / "models" / "saved",
            Path.home() / ".tradov" / "models" / "reports",
        ]
    ...
    def _save_model(self, model_name: str) -> None:
        ...
        try:
            ...
            # Save model
            model_path = Path.home() / ".tradov" / "models" / "saved" / f"{model_name}_model.pkl"
            ...
            if model_name in self.scalers:
                scaler_path = (
                    Path.home() / ".tradov" / "models" / "saved" / f"{model_name}_scaler.pkl"
                )
                joblib.dump(self.scalers[model_name], scaler_path)

            # Save config
            config_path = Path.home() / ".tradov" / "models" / "saved" / f"{model_name}_config.json"
            config_dict = {
    ...
    def _load_models(self) -> None:
        ...
        saved_dir = Path.home() / ".tradov" / "models" / "saved"
