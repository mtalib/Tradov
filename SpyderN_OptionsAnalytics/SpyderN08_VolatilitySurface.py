...
from SpyderN_OptionsAnalytics.SpyderN01_VolatilitySmile import VolatilitySmileAnalyzer
...
class VolatilitySurfaceAnalyzer:
    ...
    def __init__(self, symbol: str = "SPY"):
        ...
        # Surface storage
        self.current_surface: Optional[VolatilitySurface] = None
        self.surface_history: List[VolatilitySurface] = []
        self.local_vol_surface: Optional[LocalVolatility] = None
    ...
    def calculate_local_volatility(self, 
                                 surface: Optional[VolatilitySurface] = None) -> LocalVolatility:
        """
    ...
    def plot_smile_term_structure(self, 
                                 surface: Optional[VolatilitySurface] = None) -> plt.Figure:
        ...
        for pct, color in zip(strike_percentages, colors):
...
if __name__ == "__main__":
    ...
    print("="*60)
    print("SPYDER - Volatility Surface Analyzer Test")
    print("="*60)
    ...
    print(f"  Fit Quality (R²): {metrics['fit_quality']:.3f}")
    print(f"  Smoothness: {metrics['smoothness']:.3f}")
    print(f"  Arbitrage Free: {metrics['arbitrage_free']}")
    print(f"  Points Used: {metrics['num_points']}")
    print(f"  Calibration Time: {metrics['calibration_time']:.2f}s")
    print(f"  Moneyness Range: {metrics['moneyness_range'][0]:.2f} - {metrics['moneyness_range'][1]:.2f}")
    print(f"  Time Range: {metrics['time_range_days'][0]:.0f} - {metrics['time_range_days'][1]:.0f} days")
    ...
    if anomalies:
        ...
        for anomaly in anomalies[:3]:  # Show first 3
            ...
            print(f"    Location: Moneyness={anomaly.location[0]:.2f}, Time={anomaly.location[1]*365:.0f}d")
            if anomaly.suggested_trade:
                print(f"    Trade: {anomaly.suggested_trade}")
    else:
    ...
    for strike in test_strikes:
        ...
        print(f"  Strike {strike}: IV = {vol:.1%}")
        
    # Calculate local volatility
    print("\nCalculating local volatility surface...")
    local_vol = analyzer.calculate_local_volatility(surface)
    print(f"  Local vol grid shape: {local_vol.local_vol_grid.shape}")
    print(f"  Local vol range: {local_vol.local_vol_grid.min():.1%} - {local_vol.local_vol_grid.max():.1%}")
    ...
    print("  ✓ Smile term structure plot created")
    ...
    print(f"  ✓ Exported DataFrame with {len(df_export)} rows")
    
    print("\n✅ Volatility Surface Analyzer test completed successfully!")
