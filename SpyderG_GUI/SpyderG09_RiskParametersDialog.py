...
from PyQt6.QtWidgets import (
    ...
    QLabel, QPushButton, QSpinBox, QDoubleSpinBox, QCheckBox, QRadioButton,
...
class RiskParametersDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        self.setWindowTitle("SPYDER - Risk Levels Configuration")
        self.setModal(True)
    ...
    def setup_connections(self):
        ...
        controls = [
            self.risk_per_trade_spin, self.max_daily_loss_spin, self.max_contracts_spin,
            self.max_positions_spin, self.max_buying_power_spin, self.allow_0dte_check,
            self.max_delta_spin, self.max_gamma_spin, self.max_theta_spin, self.max_vega_spin,
            self.iv_scaling_check, self.iv_threshold_spin, self.vix_scaling_check, 
            self.vix_threshold_spin, self.high_vol_reduction, self.low_vol_increase,
            self.trend_reduction, self.zero_dte_enabled, self.zero_dte_reduction,
            self.expiration_week_check, self.expiration_reduction, self.earnings_check,
            self.earnings_blackout, self.fomc_check, self.fomc_reduction,
            self.monitoring_enabled, self.update_frequency, self.delta_alert_spin,
            self.pnl_alert_spin
        ]
    ...
    def get_parameters(self) -> Dict:
        ...
        strategy_settings = {}
        for i in range(self.strategy_table.rowCount()):
            strategy_name = self.strategy_table.item(i, 0).text().lower().replace(" ", "_").replace("/", "_")
            radio_button = self.strategy_table.cellWidget(i, 1)
            risk_spin = self.strategy_table.cellWidget(i, 2)
            contracts_spin = self.strategy_table.cellWidget(i, 3)
        ...
        if filename:
            try:
                ...
                QMessageBox.information(
                    ...
                    f"Risk levels imported from:\n{filename}"
                )
            except Exception as e:
                QMessageBox.critical(
                    self, "Import Error",
                    f"Error importing levels:\n{str(e)}"
                )
                
    def export_settings(self):
        """Export settings to JSON file"""
        filename, _ = QFileDialog.getSaveFileName(
            self, "Export Risk Levels",
            f"spyder_risk_levels_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json",
            "JSON Files (*.json);;All Files (*)"
        )
        ...
        if filename:
            try:
                ...
                QMessageBox.information(
                    ...
                    f"Risk levels exported to:\n{filename}"
                )
            except Exception as e:
                QMessageBox.critical(
                    self, "Export Error",
                    f"Error exporting levels:\n{str(e)}"
                )
