...
Here is the complete, consolidated Master Specification document designed specifically to be ingested by a coding agent  

You can copy and paste this directly into a `.md` file (e.g., `v1.0-Lean-Architecture-Spec.md`) and feed it to your agent as its primary context prompt for the upcoming refactor.
...
1.  **Data Gate (`F09`):** Is the option chain quote < 1500ms old? Are bid/ask spreads within limits? *(If no -> Halt).*
2.  **Regime Gate (`L09` -> `D30`):** Which of the 6 regimes are we in? Is it Regime 5 (Crisis) or 6 (Event)? *(If yes -> Halt).*
3.  **Strategy Gate (`D31`):** Which of the permitted 4 strategies matches the current regime?
4.  **Risk Gate (`E01`):** Does this trade violate max concurrent strategies (2) or max portfolio Greeks? *(If yes -> Reject).*
5.  **Execution (`B02`):** Send limit order. Log execution telemetry (latency, slippage).
