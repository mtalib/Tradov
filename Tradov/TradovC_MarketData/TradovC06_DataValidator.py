...
Module: TradovC06_DataValidator.py
...
_TRADING_WATCH_SYMBOLS: frozenset[str] = frozenset({"TRAD", "$TICK", "$ADD"})
...
class DataValidator:
    ...
    def __init__(self, config: dict | None = None):
        ...
        self.logger = TradovLogger.get_logger("DataValidator")
...
if __name__ == "__main__":
    ...
    if validator.initialize():
        ...
        # Test quality check
        quality = validator.get_data_quality("TRAD")

        # Get symbol metrics
        metrics = validator.get_symbol_metrics("TRAD")
        if metrics:
            pass
