...
# -*- coding: utf-8 -*-
"""
SPYDER - Remote TWS Connection Adapter (Fixed Version)
=====================================================
...
class RemoteTWSAdapter:
    ...
    def __init__(
        ...
        self.ib = IB_CLASS() if IB_ASYNC_AVAILABLE and IB_CLASS else None
    ...
    async def connect_async(self) -> bool:
        ...
        try:
            ...
            if not self.ib and IB_ASYNC_AVAILABLE and IB_CLASS:
                self.ib = IB_CLASS()
            ...
            if self.ib and hasattr(self.ib, "connectAsync"):
            ...
            if self.ib and hasattr(self.ib, "isConnected") and self.ib.isConnected():
                ...
                try:
                    accounts = []
                    if self.ib and hasattr(self.ib, "managedAccounts"):
    ...
    async def disconnect_async(self) -> bool:
        ...
        try:
            if self.ib and hasattr(self.ib, "isConnected") and self.ib.isConnected():
                if hasattr(self.ib, "disconnectAsync"):
            ...
            if self.connection_start_time:
                ...
                self.network_metrics.connection_uptime_seconds += uptime.total_seconds()
    ...
    def _trigger_callbacks(self, callbacks: List[object], *args, **kwargs):
    ...
    def add_connection_callback(self, callback: object):
        """Add callback for connection events"""
        self.connection_callbacks.append(callback)

    def add_disconnection_callback(self, callback: object):
        """Add callback for disconnection events"""
        self.disconnection_callbacks.append(callback)

    def add_error_callback(self, callback: object):
...
class RemoteTWSConnectionManager:
    """
    Enhanced connection manager that provides a unified interface
    for remote TWS connections.
    """
    ...
    def __init__(
        ...
        self.remote_adapter = RemoteTWSAdapter(remote_config, logger)
        self.connection_type = "remote_tws"
        self.ib = None
        self.state = RemoteConnectionState.DISCONNECTED
    ...
    def get_connection_info(self) -> dict:
        ...
        base_info["network_metrics"] = (
            self.remote_adapter.get_network_metrics().__dict__
        )
        base_info["performance_stats"] = self.remote_adapter.get_performance_stats()
...
def load_remote_tws_config(config_path: Optional[str] = None) -> RemoteTWSConfig:
    ...
    try:
        ...
        if isinstance(IB_CONFIG, dict) and "gateway" in IB_CONFIG:
            gateway_config = IB_CONFIG["gateway"]
            if isinstance(gateway_config, dict):
                paper_config = gateway_config.get("paper", {})
                live_config = gateway_config.get("live", {})

                windows_ip = str(paper_config.get("host", "192.168.1.100"))
                paper_port = int(paper_config.get("port", 7497))
                live_port = int(live_config.get("port", 7496))
                client_id = int(paper_config.get("clientId", 1))
            else:
                raise ValueError("Invalid gateway config structure")
        else:
            raise ValueError("Invalid IB_CONFIG structure")

        # Create configuration with safe kwargs
        remote_config = (
            REMOTE_CONNECTION_CONFIG
            if isinstance(REMOTE_CONNECTION_CONFIG, dict)
            else {}
        )
        config = RemoteTWSConfig(
