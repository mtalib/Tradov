...
class MaxPainCalculator:
    ...
    def calculate_max_pain(
        ...
        if use_cache and cache_key in self._cache:
            ...
            if (datetime.now(UTC) - cache_time).seconds < self.cache_ttl:
    ...
    def _get_nearest_expiry(self, symbol: str) -> date:
        """Get nearest options expiration from market data provider."""
...
def create_max_pain_calculator_from_env() -> 'MaxPainCalculator':
    """Create MaxPainCalculator using the configured OptionsDataProvider."""
    data_provider = None
