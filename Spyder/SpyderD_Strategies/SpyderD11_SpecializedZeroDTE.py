...
class SpecializedZeroDTEStrategy(BaseStrategy):
    ...
    def _create_exit_signal(
        ...
        signal = TradingSignal(
            ...
            metadata={
                ...
                "hold_time": (datetime.now(UTC) - position.setup.entry_time).seconds / 60,  # minutes
            },
        )
    ...
    def get_daily_summary(self) -> dict[str, Any]:
        ...
        return {
            ...
            "fed_day": self._is_fed_day(datetime.now(UTC)),
            "economic_events": self.economic_calendar.get(datetime.now(UTC).date(), []),
        }
    ...
    def should_exit_position(self, position: dict, current_data: dict) -> bool:
        """Delegate to intraday stop-loss / profit-target checks."""
        pnl_pct = current_data.get('pnl_pct', 0.0)
        stop_loss = self.config.get('stop_loss_pct', -0.50)
