...
Change Log:
        ...
        - Replaced legacy broker (TradovB01_TradovClient / TradovC07_MarketDataHub)
...
from Tradov.TradovA_Core.TradovA05_EventManager import EventManager, EventType, Event, get_event_manager  # noqa: E501
...
DEFAULT_FEED_CONFIG = {
    ...
    'error_threshold': 10,
    'custom_metrics': {
        'gex': {'enabled': True, 'module': 'TradovC15_GEXDEXCalculator'},
        'dix': {'enabled': True, 'module': 'TradovS01_DIXCalculator'},
        'swan': {'enabled': True, 'module': 'TradovS06_BlackSwanDataCollector'}
    }
}
...
class NullProvider(MarketDataProvider):
    ...
    The live market data is delivered by ``TradovB40_TradierClient``
...
def create_provider(
    ...
    is delivered by ``TradovB40_TradierClient``, not via a streaming feed.
...
class DataFeedManager:
    ...
    Usage::
        ...
        from TradovC_MarketData.TradovC01_DataFeed import DataFeedManager

        feed = DataFeedManager()
        feed.subscribe("TRAD", my_callback)
        feed.start()
    """
    ...
    def __init__(
        ...
        self._quote_poll_interval_s: float = max(
            1.0,
            float(os.environ.get("TRADOV_FEED_QUOTE_POLL_INTERVAL_S", "1.0")),
        )
        self._quote_poll_interval_offhours_s: float = max(
            self._quote_poll_interval_s,
            float(os.environ.get("TRADOV_FEED_OFFHOURS_QUOTE_POLL_INTERVAL_S", "20.0")),
        )
...
def create_data_feed(
    ...
    Args:
        symbols: Optional list of symbols to subscribe to (e.g. ``["TRAD",
    ...
    """
    from Tradov.TradovA_Core.TradovA05_EventManager import get_event_manager as _gem
