...
Series: SpyderO_TechnicalIndicators     
Module: SpyderO01_CoreTechnicalIndicators.py
...
if __name__ == "__main__":
    print("=" * 80)
    print("SPYDER O01 - CORE TECHNICAL INDICATORS DEMONSTRATION")
    print("=" * 80)
    ...
    print(f"MACD Signal: {macd_data['signal'][-1]:.4f}")
    print(f"MACD Histogram: {macd_data['histogram'][-1]:.4f}")
    ...
    print(f"BB Middle: ${bb_data['middle'][-1]:.2f}")
    print(f"BB Lower: ${bb_data['lower'][-1]:.2f}")
    
    # ATR
    atr_values = indicators.atr(high, low, prices)
    print(f"ATR(14): ${atr_values[-1]:.2f}")
    
    # ADX
    adx_data = indicators.adx(high, low, prices)
    print(f"ADX: {adx_data['adx'][-1]:.2f}")
    print(f"DI+: {adx_data['di_plus'][-1]:.2f}")
    print(f"DI-: {adx_data['di_minus'][-1]:.2f}")
    ...
    for category, signals in all_signals.items():
        ...
        for signal in signals:
            print(f"  {signal.indicator_name}: {signal.signal_type.value} "
    ...
    print(f"Net Bias: {summary['net_bias'].upper()}")
    print(f"Signal Strength: {summary['signal_strength'].upper()}")
    ...
    print(f"Detected {len(sr_levels)} key levels:")
    
    for level in sr_levels[-5:]:  # Show last 5 levels
        print(f"  {level.level_type.title()}: ${level.price_level:.2f} "
    ...
    if pivots:
        ...
        print(f"  R1: ${pivots['r1']:.2f}  R2: ${pivots['r2']:.2f}  R3: ${pivots['r3']:.2f}")
    ...
    print(f"VWAP: ${vwap_values[-1]:.2f}")
    print(f"MFI: {mfi_values[-1]:.2f}")
    
    print(f"\n{'='*80}")
    print("CORE TECHNICAL INDICATORS SYSTEM READY!")
    print("✓ Pure Python implementation - no TA-Lib dependency")
    print("✓ Comprehensive indicator suite with signal generation")
    print("✓ Options-specific calculations and regime awareness")
    print("✓ Support/resistance detection and volume analysis")
    print("✓ Multi-timeframe signal synthesis capability")
    print(f"{'='*80}")
