...
try:
    ...
    from SpyderB_Broker.SpyderB02_OrderManager import Order, OrderStatus
...
class OrderRouter:
    ...
    def _is_venue_suitable(self, venue: ExecutionVenue, order: RoutableOrder) -> bool:
        ...
        if order.time_in_force == "IOC" and VENUE_CHARACTERISTICS[venue]['latency_ms'] > 10:
            return False
    ...
    def get_order_status(self, order_id: str) -> Optional[OrderLifecycle]:
    ...
    def get_venue_performance(self, venue: ExecutionVenue) -> Optional[VenuePerformance]:
...
if __name__ == "__main__":
    ...
    for order in test_orders:
        ...
        print(f"Submitted: {order_id} - {order.routing_strategy.value} ({order.priority.name})")
    ...
    for order in test_orders:
        ...
        if lifecycle:
            ...
            print(f"  Routing Decisions: {len(lifecycle.routing_decisions)}")
            if lifecycle.routing_decisions:
                for decision in lifecycle.routing_decisions:
                    print(f"    → {decision.venue.value}: {decision.quantity} shares")
            print(f"  Fills: {len(lifecycle.fills)}")
            print(f"  Total Filled: {lifecycle.total_filled}")
            if lifecycle.avg_fill_price > 0:
                print(f"  Avg Fill Price: ${lifecycle.avg_fill_price:.2f}")
    ...
    print("\n  Queue Depths:")
    for priority, depth in stats['queue_depths'].items():
        print(f"    {priority}: {depth}")
    
    # Venue statistics
    print("\n🏢 Venue Statistics:")
    venue_stats = router.get_venue_statistics()
    for venue, vstats in venue_stats.items():
        if vstats['total_orders'] > 0:
            print(f"  {venue}:")
            print(f"    Fill Rate: {vstats['fill_rate']:.1%}")
            print(f"    Available: {vstats['available']}")
    
    # Shutdown
    router.shutdown()
    print("\n✅ Order router test completed")
