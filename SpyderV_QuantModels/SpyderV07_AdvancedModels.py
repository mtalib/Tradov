...
try:
    from SpyderB_DataBroker.SpyderB08_MultiClientDataManager import MultiClientDataManager
except ImportError:
    MultiClientDataManager = None

try:
    from SpyderV_QuantModels.SpyderV06_VolatilityEngine import SpyderVolatilityEngine, VolatilityModel
except ImportError:
...
def _merton_likelihood(params, returns, dt):
    ...
    for i in range(n):
        ...
        for k in range(max_jumps + 1):
            ...
            poisson_prob = np.exp(-lambda_jump * dt) * (lambda_jump * dt) ** k / np.math.factorial(k)
            ...
            normal_density = np.exp(-0.5 * ((r - mean_return) / std_return) ** 2) / (std_return * np.sqrt(2 * np.pi))
...
class SpyderAdvancedModelsEngine:
    ...
    def __init__(self, 
        ...
        self.jump_threshold = self.config.get('jump_threshold', JUMP_DETECTION_THRESHOLD)
        ...
        self.calibration_frequency_hours = self.config.get('calibration_frequency_hours', MAX_CALIBRATION_AGE_HOURS)
        
        # Threading
        self.executor = ThreadPoolExecutor(max_workers=2, thread_name_prefix="AdvancedModels")
        
        self.logger.info("✅ SpyderAdvancedModelsEngine initialized")
        self.logger.info("⚠️ Regime switching functions removed - now handled by L09_UnifiedRegimeEngine")
    ...
    async def analyze_market_conditions(self,
                                      include_jump_analysis: bool = True,
                                      include_crisis_assessment: bool = True) -> AdvancedModelResults:
        """
        ...
        try:
            ...
            if len(self.return_history) < self.min_data_points:
                self.logger.warning(f"Insufficient data for analysis: {len(self.return_history)} < {self.min_data_points}")
            ...
            results = AdvancedModelResults(
                ...
                crisis_assessment=await self._assess_crisis_probability() if include_crisis_assessment else None,
                recent_jumps=await self._analyze_recent_jumps() if include_jump_analysis else [],
        ...
        try:
            ...
            with self._data_lock:
                ...
                if timestamps:
                    ...
                    start_time = self.timestamp_history[-1] if self.timestamp_history else datetime.now()
                    new_timestamps = [start_time + timedelta(minutes=i) for i in range(1, len(prices) + 1)]
            ...
            self.logger.debug(f"Updated market data: {len(prices)} new prices, {len(self.return_history)} total returns")
    ...
    async def calibrate_merton_model(self, force_recalibration: bool = False) -> bool:
        ...
        try:
            ...
            if len(self.return_history) < self.min_data_points:
                self.logger.warning(f"Insufficient data for Merton calibration: {len(self.return_history)}")
                return False
            ...
            if result.success:
                ...
                self.logger.info(f"   λ={lambda_jump:.3f}, μ_J={mu_jump:.3f}, σ_J={sigma_jump:.3f}")
        ...
        try:
            ...
            for i, is_jump in enumerate(jump_indicators):
                if is_jump and i < len(recent_returns):
                    ...
                    significance = abs(jump_return) / std_return if std_return > 0 else 0
                    
                    # Create jump event
                    jump_event = JumpEvent(
                        timestamp=datetime.now() - timedelta(minutes=len(recent_returns)-i),
                    ...
                    self.logger.info(f"🚨 Jump detected: {jump_type.value}, size={jump_return:.4f}")
    ...
    async def _analyze_recent_jumps(self) -> List[JumpEvent]:
        ...
        try:
            ...
            recent_jumps = [jump for jump in self.jump_history if jump.timestamp >= cutoff_time]
    ...
    async def _assess_crisis_probability(self) -> CrisisAssessment:
        ...
        try:
            ...
            if len(self.return_history) >= 30:
                ...
                downside_volatility = np.std(recent_returns[recent_returns < 0]) * np.sqrt(252)
                ...
                stress_indicators = {
                    ...
                    'large_jumps_count': len([j for j in recent_jumps if j.significance_level > CRISIS_JUMP_THRESHOLD])
                }
            else:
                stress_indicators = {
                    'volatility_30d': 0.15,
            ...
            recommendations = self._generate_crisis_recommendations(crisis_level, stress_indicators)
    ...
    def _calculate_crisis_probability(self, stress_indicators: Dict[str, float]) -> float:
        ...
        try:
            ...
            crisis_prob = base_prob * vol_factor * jump_factor * large_jump_factor * dd_factor
    ...
    async def _forecast_volatility(self) -> Dict[str, float]:
        ...
        try:
            ...
            return {
                ...
                'model_confidence': self.model_performance.get('calibration_success', False)
            }
    ...
    async def _generate_strategy_recommendations(self) -> List[str]:
        ...
        try:
            if self.merton_params is None:
                return ["Model not calibrated - cannot provide strategy recommendations"]
            
            # Jump-based recommendations
            if self.merton_params.lambda_jump > 0.5:
                recommendations.append("High jump frequency detected - consider straddle/strangle strategies")
                recommendations.append("Avoid short volatility strategies due to jump risk")
            elif self.merton_params.lambda_jump < 0.1:
                recommendations.append("Low jump frequency - volatility selling strategies may be viable")
            
            # Jump size recommendations
            if abs(self.merton_params.mu_jump) > 0.05:
                if self.merton_params.mu_jump < 0:
                    recommendations.append("Negative jump bias detected - consider protective puts")
                else:
                    recommendations.append("Positive jump bias detected - consider call strategies")
            
            # Volatility level recommendations  
            if self.merton_params.sigma > 0.25:
                recommendations.append("High volatility environment - consider premium selling strategies")
                recommendations.append("Use wider profit targets due to increased price swings")
            elif self.merton_params.sigma < 0.12:
                recommendations.append("Low volatility environment - consider volatility buying strategies")
            
            # Recent jump activity
            recent_jumps = len([j for j in self.jump_history 
                              if j.timestamp >= datetime.now() - timedelta(days=7)])
            if recent_jumps > 3:
                recommendations.append("Recent jump activity elevated - increase position monitoring")
    ...
    async def _check_and_recalibrate(self):
        ...
        if not self._is_calibration_current() or self.validation_status != ModelValidationStatus.CALIBRATED:
    ...
    def _generate_synthetic_data(self, n_points: int = 500):
        ...
        try:
            ...
            self.price_history.extend(prices[1:])  # Skip initial price if it's duplicate
        ...
        try:
            return {
                ...
                'jump_analysis': {
                    ...
                    'recent_jumps_7d': len([j for j in self.jump_history 
                                          if j.timestamp >= datetime.now() - timedelta(days=7)]),
                    'jump_detection_threshold': self.jump_threshold
                },
                'crisis_analysis': {
                    'current_crisis_level': self.crisis_history[-1].crisis_level.value if self.crisis_history else 'unknown',
                    'crisis_probability': self.crisis_history[-1].crisis_probability if self.crisis_history else 0.0,
                    'assessments_count': len(self.crisis_history)
                },
                'model_parameters': {
                    'merton': {
                        'mu': self.merton_params.mu if self.merton_params else None,
                        'sigma': self.merton_params.sigma if self.merton_params else None,
                ...
                'calibration_errors': self.calibration_errors[-5:] if self.calibration_errors else []
            }
        except Exception as e:
            return {'error': str(e)}
...
async def main():
    ...
    print("   ⚠️ REGIME SWITCHING FUNCTIONS REMOVED - now handled by L09_UnifiedRegimeEngine")
    ...
    try:
        ...
        jump_returns = [0.001, 0.002, -0.08, 0.001, 0.002, 0.06, -0.001]  # Contains jumps
        ...
        for i, jump in enumerate(recent_jumps[:3]):  # Show top 3
            print(f"   Jump {i+1}: {jump.jump_type.value}, size={jump.jump_size:.4f}, "
    ...
    try:
        ...
        print(f"   Crisis Level: {results.crisis_assessment.crisis_level.value if results.crisis_assessment else 'N/A'}")
        
        if results.volatility_forecast and 'total_forecast_volatility' in results.volatility_forecast:
    ...
    if 'error' not in status:
        print(f"   Merton Model: {'✅ Calibrated' if status['models_calibrated']['merton'] else '❌ Not calibrated'}")
        print(f"   Data Quality: {status['data_status']['data_quality']}")
        print(f"   Total Jumps: {status['jump_analysis']['total_jumps_detected']}")
        print(f"   Current Crisis Level: {status['crisis_analysis']['current_crisis_level'].upper()}")
