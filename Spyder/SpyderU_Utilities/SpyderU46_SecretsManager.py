...
Module Description:
    ...
    Resolution priority (highest to lowest):
        ...
        2. Environment variables  (SPYDER_SECRET_<KEY> or plain <KEY>)
        3. Encrypted YAML file    (~/.spyder/secrets.yaml, Fernet-encrypted)
        4. Plain YAML file        (~/.spyder/secrets.yaml, plaintext fallback)
...
Key Features:
    ...
    • Fernet-encrypted YAML file via SpyderU04_Encryption
...
Dependencies:
    ...
    • PyYAML (standard Spyder dep) — for YAML persistence
    • SpyderU04_Encryption — Fernet encryption (graceful degradation to
      plaintext if unavailable)
    • SpyderU01_Logger (graceful fallback to stdlib logging)

Environment Variables:
    SPYDER_SECRETS_FILE    — override default ~/.spyder/secrets.yaml
    VAULT_ADDR             — HashiCorp Vault address (e.g. http://127.0.0.1:8200)
    VAULT_TOKEN            — Vault token
    VAULT_SECRET_PATH      — Vault KV-v2 path (default: spyder/data)
...
try:
    ...
    _log.warning(
        "SpyderU04_Encryption unavailable — secrets YAML will be stored as plaintext"
    )

# ==============================================================================
# CONSTANTS
# ==============================================================================
_DEFAULT_SECRETS_FILE = Path.home() / ".spyder" / "secrets.yaml"
_ENV_PREFIX = "SPYDER_SECRET_"
_VAULT_ADDR = os.getenv("VAULT_ADDR", "")
_VAULT_TOKEN = os.getenv("VAULT_TOKEN", "")
_VAULT_PATH = os.getenv("VAULT_SECRET_PATH", "spyder/data")
...
class SecretsManager:
    ...
    def get_all(self) -> dict[str, str]:
        ...
        # Add any SPYDER_SECRET_* env vars
    ...
    # ------------------------------------------------------------------
    # Convenience helpers for common Spyder secrets
    # ------------------------------------------------------------------
    ...
    def slack_webhook_url(self) -> str | None:
        return self.get("SPYDER_SLACK_WEBHOOK_URL")

    @property
    def teams_webhook_url(self) -> str | None:
        return self.get("SPYDER_TEAMS_WEBHOOK_URL")

    @property
    def discord_webhook_url(self) -> str | None:
        return self.get("SPYDER_DISCORD_WEBHOOK_URL")
