...
>
> **Concurrency limit:** Max 2 strategies open (1 long-term/swing + 1 intraday/0DTE)
...
- The concurrency limit is 2: one long-term/swing slot and one intraday/0DTE slot. Both must belong to different horizon buckets.
...
- Are REGIME / BIAS / STRATEGY STANCE / STRATEGY GATE set up?
...
### Are REGIME / BIAS / STANCE / GATE being set correctly?

Yes, but with an important distinction:

- Execution gates use D31 + F09 + E01 runtime checks.
- Dashboard pills are computed in G05 from S07 metrics (with sticky/fallback logic), not directly from D31 execution verdicts.

Implication:

- A green-looking dashboard can coexist with blocked execution if entry trust, regime-policy, or risk gates fail.
- BIAS is informational and does not directly gate dispatch.
...
2. Display-vs-execution regime source split
  - G05 regime pills are not a direct mirror of D31 gate outcomes.
  - Treat D31 decision logs as source of truth for execution eligibility.
...
| 2 | Add D31 drop-reason badge to dashboard | Medium |
| 3 | Run first weekday paper-session with both fixes active, confirm at least one approved-and-dispatched trade | **Required** |
