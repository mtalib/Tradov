...
Tradov Version: 1.0
Module: TradovS03_BlackSwanIndicator.py
Group: S (Signals)
...
Calculation Methodology:
    The overall Black Swan score is a weighted sum of four independent components:
        ...
        Market Internals    20 %     TRAD price momentum (currently simplified)
...
When Calculations Are Triggered:
    ...
    - Via scheduler: TradovS04_BlackSwanScheduler invokes this module at the
...
Data Sources:
    Primary:  yfinance — ^VIX, ^VIX9D, ^VXN, ^RVX (volatility)
                         HYG, LQD, TLT (credit)
                         TRAD, QQQ, IWM, DX-Y.NYB (market / liquidity)
...
MARKET_SYMBOLS = {"spy": "TRAD", "qqq": "QQQ", "iwm": "IWM", "dxy": "DX-Y.NYB"}
...
class BlackSwanIndicator:
    ...
    def _simulate_quote(self, symbol: str) -> float:
        ...
        base_prices = {
            ...
            "TRAD": 450.0 + np.random.normal(0, 5),
    ...
    def _calculate_internals_score(self, data: dict, market_internals_override: dict | None = None) -> ComponentScore:  # noqa: E501
        ...
        (QQQ and IWM relative weakness vs TRAD) as a broad market-health indicator.
        """
        ...
        # ── Cross-asset breadth: QQQ and IWM vs TRAD ───────────────────────────
        # If both QQQ and IWM are significantly below TRAD (size-cap divergence),
        ...
        if spy and qqq and iwm:
            ...
            # (QQQ ~0.98×TRAD, IWM ~0.40×TRAD at typical market levels)
            qqq_norm = qqq / (spy * 0.98) if spy else 1.0
            iwm_norm = iwm / (spy * 0.40) if spy else 1.0
            breadth = (qqq_norm + iwm_norm) / 2.0  # 1.0 = in-line with TRAD
            if breadth < 0.93:
