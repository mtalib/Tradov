...
class SpyderDIXScheduler:
    ...
    def run_scheduled_calculation(self) -> Optional[CalculationResult]:
        ...
        try:
            ...
            self.logger.info(f"DIX calculation completed successfully in "
    ...
    def _morning_check(self) -> None:
        ...
        if dix_data:
            self.logger.info(f"Morning DIX: {dix_data['dix_percentage']:.2f}% "
...
"""
        ...
        # Get report path
        report_path = f"/home/ubuntu/spyder_dix_charts/dix_analysis_report_{result.timestamp.strftime('%Y%m%d')}.md"
